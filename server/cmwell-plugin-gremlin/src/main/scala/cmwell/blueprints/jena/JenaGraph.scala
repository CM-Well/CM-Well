...
class JenaGraph(model: Model = ModelFactory.createDefaultModel) extends Graph {
  ...
  override def getVertex(id: Object) = {
    Seq(
      model.listSubjects.filter(_.id == id.toString),
      ...
      case None =>
        throw new QueryException(s"Vertex [$id] not present in Graph")
    }
  }
  ...
  override def getEdges(key: String, value: Object): Iterable[Edge] =
    throw new UnsupportedOperationException(
      "getEdges by key,value is not supported"
    )
  override def getVertices(key: String, value: Object): Iterable[Vertex] =
    throw new UnsupportedOperationException(
      "getVertices by key,value is not supported"
    )

  override def addEdge(id: Object,
                       outVertex: Vertex,
                       inVertex: Vertex,
                       label: String) =
    throw new UnsupportedOperationException(
