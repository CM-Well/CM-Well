...
trait IRWCassSpec extends AsyncFlatSpec with Matchers with IRWServiceTest {
  ...
  "bulk write and read" should "be successful" in {
    ...
    val data = Vector.tabulate(10) { i =>
      ObjectInfoton("/cmt/cm/bulk-test/objinfo_" + i,
                    "dc_test",
                    None,
    ...
    irw.writeSeqAsync(data).flatMap { _ =>
      ...
      for {
        ...
        withClue(s"number of infotons retrieved by uuids ${us
          .mkString("[", ",", "]")} must equal the total number of written infotons [${data.size}]") {
          us.size should equal(data.size)
        }
        withClue(s"number of infotons retrieved by paths ${ps
          .mkString("[", ",", "]")} must equal the total number of written infotons [${data.size}]") {
          ps.size should equal(data.size)
        }
        withClue(s"number of infotons retrieved by uuids ${uj.mkString("[", ",", "]")} and junk ${junks
          .mkString("[", ",", "]")} must equal the total number of written infotons [${data.size}]") {
          uj.size should equal(data.size)
        }
        withClue(s"number of infotons retrieved by uuids ${pj.mkString("[", ",", "]")} and junk ${junks
          .mkString("[", ",", "]")} must equal the total number of written infotons [${data.size}]") {
  ...
  "file write and read" should "be successful" in {
    ...
    val fileInfo = FileInfoton("/irw/command-test/file_1",
                               "dc_test",
                               Some(666L),
                               Map("name" -> Set[FieldValue](FString("gal"), FString("yoav"))),
                               fileContent)
          ...
          i match {
            case FileInfoton(_, _, _, _, _, c, _) =>
              c match {
                case Some(cc) =>
                  withClue(
                    new String(cc.data.get) + "-------------------------------------" + new String(fileContent.data.get)
                  ) {
  ...
  "object write and read" should "be successful" in {
    val objInfo = ObjectInfoton("/irw/command-test/objinfo_1",
                                "dc_test",
                                None,
                                Map("name" -> Set[FieldValue](FString("gal"), FString("yoav"))))
    ...
    val objInfo_v2 = ObjectInfoton("/irw/command-test/objinfo_1",
                                   "dc_test",
                                   None,
  ...
  "object write and update indexTime" should "be successful" in {
    import scala.concurrent.duration._
    val objInfo =
      ObjectInfoton("/irw/command-test/JohnSmith", "dc_test", None, Map("status" -> Set[FieldValue](FString("R.I.P"))))
