    ...
    def getFromCas(lvl: ConsistencyLevel, isARetry: Boolean = false): Future[Box[Infoton]] = {
      ...
      resultedInfotonFuture
        ...
        .flatMap {
          ...
          case io @ FullBox(i) =>
            if (isARetry) {
              logger.warn(san"The uuid $uuid is only available in QUORUM")
              if (i.uuid != uuid)
                logger.error(
                  san"The infoton [${i.systemFields.path}] retrieved with different uuid [${i.uuid}] from requested uuid [$uuid]"
                )
            }
  ...
  def readPathUUIDA(path: String, level: ConsistencyLevel = ONE, retry: Boolean = false)(
    ...
    executeAsync(stmt, s"'${getLastInPath.getQueryString}'.bind($path)")
      .flatMap((extractLast _).andThen {
        ...
        case os =>
          if (os.isDefined && retry) logger.warn(san"The path $path is only available in QUORUM")
    ...
    def executeRetry(statement: Statement, retry: Int = 4, timeToWait: Long = 1000): ResultSet = {
      ...
      while (count != retry) {
        try {
          ...
          case e: Throwable =>
            count += 1
            logger.error(san"in executeRetry $statement retry $count", e)
