...
class IRWServiceNativeImpl(storageDao: Dao,
  ...
  lazy val setInfotonData: PreparedStatement =
    storageDao.getSession.prepare("INSERT INTO infoton (uuid,fields) VALUES (?,?)")
  lazy val setDc: PreparedStatement =
    storageDao.getSession.prepare("UPDATE infoton SET fields['$dc'] = ? where uuid = ?")
  lazy val setIndexTime: PreparedStatement =
    storageDao.getSession.prepare("UPDATE infoton SET fields['$indexTime'] = ? where uuid = ?")
  ...
  lazy val setHistory: PreparedStatement =
    storageDao.getSession.prepare("UPDATE path SET history[?] = ? where path = ?")
  lazy val setPathLastAndHistory: PreparedStatement =
    storageDao.getSession.prepare("INSERT INTO path (path,history,last) VALUES (?,?,?)")
...
//  @deprecated("use `executeRetryAsync` instead","1.4.x")
  ...
  private def executeRetryAsync(statement: Statement,
                                retries: Int = 4,
                                timeToWaitMs: Long = 1000): Future[ResultSet] = {
