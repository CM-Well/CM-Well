...
trait FTSServiceESTest extends FTSMixin { this: Suite =>
  ...
  override protected def beforeAll() {
    ...
    val testOnlyTemplate =
      Source.fromURL(this.getClass.getResource("/test_indices_template_override.json")).getLines.reduceLeft(_ + _)
    ftsServiceES.client
      .admin()
      .indices()
...
trait FTSServiceEsSpec extends FlatSpec with Matchers /*with ElasticSearchTestNode with FTSServiceESTest */ {
  this: FTSMixin =>
  ...
  "indexing existing infoton" should "store it in current index" in {
    ...
    val m: Map[String, Set[FieldValue]] =
      Map("name" -> Set(FString("yehuda"), FString("moshe")), "family" -> Set(FString("smith")))
    val updatedInfotonToIndex =
      ObjectInfoton("/fts-test/objinfo1/a/b/c", "dc_test", Some(System.currentTimeMillis()), lastModified, m)
    Await.result(ftsService.index(updatedInfotonToIndex, Some(infotonToIndex)), timeout)
    refreshAll()
    val result =
      Await.result(ftsService.search(None,
  ...
  val bulkInfotons = Vector.empty ++ (for (i <- 1 to 500)
    yield
      ObjectInfoton("/fts-test/bulk/info" + i,
                    "dc_test",
  ...
  "bulk indexing infotons" should "store them in current index" in {
    ...
    refreshAll()
    val result =
      Await.result(ftsService.search(Some(PathFilter("/fts-test/bulk", true)), None, None, DefaultPaginationParams),
                   timeout)
  ...
  "bulk indexing existing infotons" should "store their previous version in history index and current version in current index" in {
    ...
    Await
      .result(ftsService.search(Some(PathFilter("/fts-test/bulk", true)), None, None, DefaultPaginationParams), timeout)
      .total should equal(500)
    Await
      .result(ftsService.search(pathFilter = Some(PathFilter("/fts-test/bulk", true)),
                                None,
                                None,
                                DefaultPaginationParams,
                                withHistory = true),
  ...
  "deleting infoton" should "remove it from current index" in {
    val infotonToDelete = ObjectInfoton("/fts-test/infoToDel",
                                        "dc_test",
                                        Some(System.currentTimeMillis()),
  ...
  "purging infoton" should "permanently delete infoton with given UUID from history index" in {
    ...
    refreshAll()
    val result = Await.result(ftsService.search(None,
  ...
  "listChildren" should "return a list of given infoton's current version children" in {
    ...
    val infotonToList2 = ObjectInfoton("/fts-test/infotons/infotonToList2",
                                       "dc_test",
                                       Some(System.currentTimeMillis()),
                                       Map("city" -> Set[FieldValue](FString("Or-Yehuda"), FString("Modiin"))))
    val infotonToList3 = LinkInfoton(path = "/fts-test/infotons/infotonToList3",
                                     dc = "dc_test",
    ...
    refreshAll()
    Await
      .result(ftsService.listChildren("/fts-test/infotons", 0, 20, false, ftsService.defaultPartition), timeout)
      .infotons
  ...
  "search API" should "find infotons using path filter with descendants sorted by lastModified (desc)" in {
    ...
    val response =
      Await.result(ftsService.search(Some(PathFilter("/fts-test/search", true)), None, None, DefaultPaginationParams),
                   timeout)
  ...
  it should "find infotons using PathFilter without descendants" in {
    Await
      .result(ftsService.search(Some(PathFilter("/fts-test/search", false)), None, None, DefaultPaginationParams),
              timeout)
      .infotons
      .length should equal(3)
  }
  ...
  it should "find infotons using FieldFilter" in {
    Await
      .result(ftsService.search(None, Some(FieldFilter(Must, Contains, "car", "mazda")), None, DefaultPaginationParams),
              timeout)
      .infotons
      .length should equal(1)
    Await
      .result(
        ftsService.search(Some(PathFilter("/fts-test/search", true)),
    ...
    Await
      .result(
        ftsService.search(
          ...
          None,
          Some(
            DatesFilter(Some(isoDateFormatter.parseDateTime("2013-01-01T10:00:00Z")),
  ...
  it should "use exact value when sorting on string field" in {
    ...
    val i1 = ObjectInfoton(path = "/fts-test2/sort-by/1",
                           indexTime = None,
                           dc = "dc_test",
                           fields = Map("car" -> Set[FieldValue](FString("Mitsubishi Outlander"))))

    val i2 = ObjectInfoton(path = "/fts-test2/sort-by/2",
                           indexTime = None,
                           dc = "dc_test",
...
//    val infotons = Vector.empty ++ (for(i <- 1 to 500) yield (ObjectInfoton("/fts-test/scroll/info" + i,"dc_test", None, Map("name" + i -> Set[FieldValue](FString("value" + i), FString("another value" + i)))), None))
    ...
    val startScrollResult =
      Await.result(ftsService.startScroll(pathFilter = Some(PathFilter("/fts-test/scroll", false)),
                                          None,
                                          None,
