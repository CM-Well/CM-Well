...
sealed trait FTSMixin extends BeforeAndAfterAll { this: Suite =>
  ...
  def getUUID(uuid: String, isCurrent: Boolean = true) = ftsService match {
    case es: FTSServiceNew =>
      es.client
        .prepareGet("cm_well_p0_0", "infoclone", uuid)
        .execute()
        .actionGet()
...
trait FTSServiceESTest extends FTSMixin { this: Suite =>
  ...
  override protected def beforeAll() {
    ...
    val indicesTemplate = Source
      .fromURL(this.getClass.getResource("/indices_template.json"))
      .getLines
      .reduceLeft(_ + _)
    ftsServiceES.client
      .admin()
    ...
    val testOnlyTemplate = Source
      .fromURL(
        this.getClass.getResource("/test_indices_template_override.json")
      )
      .getLines
      .reduceLeft(_ + _)
    ftsServiceES.client
      .admin()
      .indices()
      .preparePutTemplate("test_indices_template")
      .setSource(testOnlyTemplate)
      .execute()
      .actionGet()
...
//    ftsServiceNew.client.admin().indices().delete(new DeleteIndexRequest("_all"))
    ...
    ftsServiceNew.client
      .admin()
      .indices()
      .preparePutTemplate("indices_template_new")
      .setSource(indicesTemplate)
      .execute()
      .actionGet()
...
trait FTSServiceEsSpec
    extends FlatSpec
    with Matchers /*with ElasticSearchTestNode with FTSServiceESTest */ {
  this: FTSMixin =>

  System.setProperty("dataCenter.id", "dc_test")
  val timeout = FiniteDuration(10, SECONDS)

  val m: Map[String, Set[FieldValue]] = Map(
    "name" -> Set(FString("yehuda"), FString("moshe"))
  )
  val infotonToIndex =
    ObjectInfoton("/fts-test/objinfo1/a/b/c",
                  "dc_test",
                  Some(System.currentTimeMillis()),
                  m)
  "indexing new infoton" should "store it in current index" in {
    Await.result(
      ftsService.index(infotonToIndex, None, ftsService.defaultPartition),
      timeout
    )
    refreshAll()
  ...
  "indexing existing infoton" should "store it in current index" in {
    ...
    val m: Map[String, Set[FieldValue]] =
      Map("name" -> Set(FString("yehuda"), FString("moshe")),
          "family" -> Set(FString("smith")))
    val updatedInfotonToIndex = ObjectInfoton("/fts-test/objinfo1/a/b/c",
                                              "dc_test",
                                              Some(System.currentTimeMillis()),
                                              lastModified,
                                              m)
    Await.result(ftsService.index(updatedInfotonToIndex, Some(infotonToIndex)),
                 timeout)
    refreshAll()
    val result = Await.result(
      ftsService.search(
        None,
        Some(
          FieldFilter(Must, Equals, "system.path", "/fts-test/objinfo1/a/b/c")
        ),
        None,
  ...
  it should "store its previous version in the history index" in {
    ...
    withClue(
      s"${res.getIndex}, ${res.getSource}, ${res.getType}, ${res.getVersion}, ${res.isSourceEmpty}"
    ) {
      res.isExists should equal(true)
    }
  }

  val bulkInfotons = Vector.empty ++ (for (i <- 1 to 500)
    yield
      ObjectInfoton(
        ...
        Some(System.currentTimeMillis()),
        Map(
          "name" + i -> Set[FieldValue](FString("value" + i),
                                        FString("another value" + i))
        )
      ))
  ...
  "bulk indexing infotons" should "store them in current index" in {
    ...
    Await.result(
      ftsService.bulkIndex(bulkInfotons, Nil, ftsService.defaultPartition),
      timeout
    )
    refreshAll()
    val result =
      Await.result(ftsService.search(Some(PathFilter("/fts-test/bulk", true)),
                                     None,
                                     None,
                                     DefaultPaginationParams),
                   timeout)
  ...
  "bulk indexing existing infotons" should "store their previous version in history index and current version in current index" in {
    val updatedBulkInfotons = Vector.empty ++ (for (i <- 1 to 500)
      yield
        ObjectInfoton(
          "/fts-test/bulk/info" + i,
          "dc_test",
          Some(System.currentTimeMillis()),
          Map(
            "name" + i -> Set[FieldValue](FString("moshe" + i),
                                          FString("shirat" + i))
          )
        ))
    Await.result(ftsService.bulkIndex(updatedBulkInfotons, bulkInfotons),
                 timeout)
    refreshAll()

    Await
      .result(ftsService.search(Some(PathFilter("/fts-test/bulk", true)),
                                None,
                                None,
                                DefaultPaginationParams),
              timeout)
    ...
    Await
      .result(ftsService.search(pathFilter =
  ...
  "deleting infoton" should "remove it from current index" in {
    ...
    val deletedInfoton =
      DeletedInfoton("/fts-test/infoToDel",
                     "dc_test",
                     Some(System.currentTimeMillis()))
  ...
  it should "move it to history index and add tombstone" in {
    ...
    resultWithHistory.infotons.exists(_.isInstanceOf[DeletedInfoton]) should equal(
      true
    )
  }
  ...
  "purging infoton" should "permanently delete infoton with given UUID from history index" in {
    val infotonToPurge =
      ObjectInfoton("/fts-test/infoToPurge",
                    "dc_test",
                    Some(System.currentTimeMillis()))
    Await.result(ftsService.index(infotonToPurge, None), timeout)
    val updatedInfotonToPurge =
      ObjectInfoton("/fts-test/infoToPurge",
                    "dc_test",
                    Some(System.currentTimeMillis()))
    Await.result(ftsService.index(updatedInfotonToPurge, Some(infotonToPurge)),
                 timeout)
    refreshAll()
    val result = Await.result(
      ftsService.search(
  ...
  "purgeAll infoton" should "permanently delete all infoton's versions with given path from all indices" in {
    ...
    val updatedInfotonToPurgeAll =
      ObjectInfoton("/fts-test/infoToPurgeAll", "dc_test")
    Await.result(
      ftsService.index(updatedInfotonToPurgeAll, Some(infotonToPurgeAll)),
      timeout
    )
    refreshAll()
    Await
      .result(
        ftsService.search(
          None,
          Some(
            FieldFilter(Must, Equals, "system.path", infotonToPurgeAll.path)
          ),
          None,
          DefaultPaginationParams,
          SortParam.empty,
    ...
    Await.result(
      ftsService
        .purgeAll(infotonToPurgeAll.path, true, ftsService.defaultPartition),
      timeout
    )
    refreshAll()
    val f =
    ...
    f.onSuccess {
      case FTSSearchResponse(total, offset, length, infotons, None) =>
        logger.debug(
          s"before failing: total: $total, offset: $offset, length: $length and infotons:\n${infotons
            .map(_.path)
            .mkString("\t", "\n\t", "\n")} "
        )
  ...
  "listChildren" should "return a list of given infoton's current version children" in {
    val infotonToList1 = ObjectInfoton("/fts-test/infotons/infotonToList1",
                                       "dc_test",
                                       Some(System.currentTimeMillis()))
    val infotonToList2 =
      ObjectInfoton(
    ...
    val infotonToList3 =
      ...
      ).copy(indexTime = Some(System.currentTimeMillis()))
    ...
    Await
      .result(
        ftsService.listChildren("/fts-test/infotons",
                                0,
                                20,
                                false,
                                ftsService.defaultPartition),
        timeout
      )
      .infotons
  ...
  "search API" should "find infotons using path filter with descendants sorted by lastModified (desc)" in {
    ...
    val objectInfotonToSearch2 = ObjectInfoton(
      ...
      Some(System.currentTimeMillis()),
      isoDateFormatter.parseDateTime(
        "2013-01-01T10:01:00Z"
      ),
      Map("os" -> Set[FieldValue](FString("osx")),
    ...
    val updatedObjectInfotonToSearch = ObjectInfoton(
      ...
      Map(
        ...
        "food" -> Set[FieldValue](FString("Sushi"),
                                  FString("Falafel"),
                                  FString("Malabi")),
        "copyright" -> Set[FieldValue](
          FString("Cm well team ©")
    ...
    Await.result(
      ftsService
        .index(updatedObjectInfotonToSearch, Some(objectInfotonToSearch)),
      timeout
    )
    ...
    val response =
      Await.result(ftsService.search(Some(PathFilter("/fts-test/search", true)),
                                     None,
                                     None,
                                     DefaultPaginationParams),
                   timeout)
    response.infotons.length should equal(4)
    response.infotons.head.path should equal(
      "/fts-test/search/linkInfotonToSearch"
    )
    response.infotons.last.path should equal(
      "/fts-test/search/objectInfotonToSearch2"
    )
  ...
  it should "find infotons using PathFilter without descendants" in {
    Await
      .result(ftsService.search(Some(PathFilter("/fts-test/search", false)),
                                None,
                                None,
                                DefaultPaginationParams),
              timeout)
  ...
  it should "find infotons using FieldFilter" in {
    Await
      .result(
        ftsService.search(None,
                          Some(FieldFilter(Must, Contains, "car", "mazda")),
                          None,
                          DefaultPaginationParams),
        timeout
      )
      ...
      .result(
        ftsService.search(
          ...
          Some(
            MultiFieldFilter(
              Must,
              Seq(FieldFilter(Must, Contains, "copyright", "well"),
                  FieldFilter(Should, Equals, "since", "2009"))
            )
          ),
          None,
          DefaultPaginationParams
      ...
      .result(
        ftsService.search(
          ...
          Some(
            MultiFieldFilter(
              Must,
              Seq(SingleFieldFilter(Should, Contains, "car", None),
                  SingleFieldFilter(Should, Contains, "ver", None))
            )
          ),
          None,
          DefaultPaginationParams
    ...
    Await
      .result(
        ftsService.search(
          ...
          Some(
            DatesFilter(
              Some(isoDateFormatter.parseDateTime("2013-01-01T10:00:00Z")),
              Some(isoDateFormatter.parseDateTime("2013-01-03T10:00:00Z"))
            )
          ),
          DefaultPaginationParams
        ),
      ...
      .result(
        ftsService.search(
          ...
          datesFilter = Some(
            DatesFilter(
              Some(isoDateFormatter.parseDateTime("2013-01-01T10:00:00Z")),
              Some(isoDateFormatter.parseDateTime("2013-01-06T10:00:00Z"))
            )
          ),
          DefaultPaginationParams
        ),
    ...
    Await
      .result(
        ftsService.search(
          ...
          paginationParams = PaginationParams(0, 2)
        ),
        timeout
      )
      .infotons
    ...
    Await
      .result(
        ftsService.search(
          ...
          paginationParams = PaginationParams(2, 1)
        ),
        timeout
      )
      .infotons
  ...
  it should "include history versions when turning on the 'withHistory' flag" in {
    Await
      .result(ftsService.search(None,
                                fieldsFilter = Some(
                                  FieldFilter(Must, Contains, "car", "mazda")
                                ),
                                None,
                                DefaultPaginationParams,
                                withHistory = true),
  ...
  it should "use exact value when sorting on string field" in {
    ...
    val i1 = ObjectInfoton(
      path = "/fts-test2/sort-by/1",
      indexTime = None,
      dc = "dc_test",
    ...
    val i2 =
      ObjectInfoton(path = "/fts-test2/sort-by/2",
                    indexTime = None,
                    dc = "dc_test",
                    fields =
                      Map("car" -> Set[FieldValue](FString("Subaru Impreza"))))
    ...
    val response = Await.result(
      ftsService.search(
        pathFilter = Some(PathFilter("/fts-test2/sort-by", true)),
        fieldsFilter = Some(SingleFieldFilter(Must, Equals, "car", None)),
        None,
        DefaultPaginationParams,
        sortParams = FieldSortParams(List(("car" -> Desc))),
        debugInfo = true
      ),
      timeout
    )
...
//    val infotons = Vector.empty ++ (for(i <- 1 to 500) yield (ObjectInfoton("/fts-test/scroll/info" + i,"dc_test", None, Map("name" + i -> Set[FieldValue](FString("value" + i), FString("another value" + i)))), None))
    ...
    refreshAll()
    val startScrollResult =
      Await.result(
        ftsService.startScroll(pathFilter =
                                 Some(PathFilter("/fts-test/scroll", false)),
                               None,
                               None,
                               paginationParams = PaginationParams(0, 60)),
        timeout
      )
    ...
    var count = 0
    var scrollResult =
      Await.result(ftsService.scroll(startScrollResult.scrollId), timeout)
    while (scrollResult.infotons.nonEmpty) {
      count += scrollResult.infotons.length
      scrollResult =
        Await.result(ftsService.scroll(scrollResult.scrollId), timeout)
    }
