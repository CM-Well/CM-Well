...
class FTSServiceES private (classPathConfigFile: String, waitForGreen: Boolean)
  ...
  if (isTransportClient) {
    ...
    val esSettings =
      ImmutableSettings.settingsBuilder().loadFromClasspath(classPathConfigFile)
  ...
  def index(
    infoton: Infoton,
    previousInfoton: Option[Infoton] = None,
    ...
    val infotonToIndex =
      new String(JsonSerializer.encodeInfoton(infoton, toEs = true), "utf-8")
    ...
    currentFuture.andThen {
      ...
      case Success(v) =>
        loger.debug("successfully indexed infoton uuid: " + infoton.uuid);
        totalIndexed.addAndGet(1)
    }
    ...
    if (previousInfoton.isDefined) {
      ...
      val previousWriteFuture = injectFuture[IndexResponse](
        client
          .prepareIndex(
            partition + "_history",
            "infoclone",
            previousInfoton.get.uuid
          )
          .setSource(
            JsonSerializer.encodeInfoton(previousInfoton.get, toEs = true)
          )
          .setConsistencyLevel(WriteConsistencyLevel.ALL)
          .execute(_)
      ).map { _ =>
      ...
      val previousDeleteFuture = injectFuture[DeleteResponse](
        ...
        .andThen {
          ...
          case Success(v) =>
            loger.debug("successfully deleted infoton uuid: " + infoton.uuid)
        }
        .map { _ =>
          }
      currentFuture
        .flatMap(_ => previousWriteFuture)
        .flatMap(_ => previousDeleteFuture)
    } else {
      currentFuture.map { _ =>
        }
    }
    ...
    response.onComplete {
      ...
      case Success(response) =>
        try {
          ...
          logger.error(
            s"failed to index ${failed.size} out of $requestedToIndexSize infotons: ${failed
              .mkString(",")}"
          )
          ...
          case t: Throwable =>
            logger.error(
              s"exception while handling ES response errors:\n ${getStackTrace(t)}"
            )
        }
      case Failure(t) =>
  ...
  def bulkIndex(
    ...
    currentInfotons.foreach { current =>
      val indexSuffix =
        if (current.isInstanceOf[DeletedInfoton]) "_history" else "_current"
      bulkRequest.add(
        client
    ...
    previousInfotons.foreach { previous =>
      ...
      bulkRequest.add(
        client
          .prepareDelete(partition + "_current", "infoclone", previous.uuid)
      )
    }
  ...
  def purge(uuid: String, partition: String = defaultPartition)(
    ...
    injectFuture[DeleteResponse](
      client.prepareDelete(partition + "_history", "infoclone", uuid).execute(_)
    ).map(x => true)
  }
  ...
  def purgeAll(
    ...
    val indices = getIndicesNamesByType("history") ++ getIndicesNamesByType(
      "current"
    )
  ...
  def getIndicesNamesByType(suffix: String,
    ...
    val currentAliasRes = client.admin
      .indices()
      .prepareGetAliases(s"${partition}_${suffix}")
      .execute()
      .actionGet()
  ...
  private def fieldType(fieldName: String) = {
    fieldName match {
      ...
      case "type" | "system.parent" | "system.path" | "system.uuid" |
          "system.dc" | "system.quad" | "content.data" | "content.base64-data" |
          "content.mimeType" =>
        StringType
        ...
        case SingleFieldFilter(fieldOperator, valueOperator, name, valueOpt) =>
          if (valueOpt.isDefined) {
            ...
            val exactFieldName = fieldType(name) match {
              case StringType if (!name.startsWith("system")) =>
                s"fields.${name}.%exact"
              case _ => name
            }
...
//    }
  ...
  def aggregate(pathFilter: Option[PathFilter] = None,
    ...
    def filterToBuilder(
      ...
      implicit def fieldValueToValue(fieldValue: Field) =
        fieldValue.operator match {
          case AnalyzedField => fieldValue.value
          case NonAnalyzedField =>
            s"infoclone.fields.${fieldValue.value}.%exact"
        }
      ...
      val aggBuilder = filter match {
        ...
        case HistogramAggregationFilter(
          ...
          val eMin: java.lang.Long =
            extMin.getOrElse(null).asInstanceOf[java.lang.Long]
          val eMax: java.lang.Long =
            extMax.getOrElse(null).asInstanceOf[java.lang.Long]
          AggregationBuilders
            .histogram(name)
            .field(field)
            .interval(interval)
  ...
  def search(pathFilter: Option[PathFilter] = None,
    ...
    val indices = (partition + "_current") :: (if (withHistory)
                                                 partition + "_history" :: Nil
                                               else Nil)
        ...
        fsp.foreach {
          case (name, order) => {
            val unmapped = name match {
              ...
              case _ => {
                if (name.startsWith("system.") || name.startsWith("content."))
                  null
                else
                  name.take(2) match {
                    case "d$" => "date"
            ...
            val uname =
              if (unmapped == "string" && name != "type")
                s"fields.${name}.%exact"
              else name
            request.addSort(
              fieldSort(uname).order(order).unmappedType(unmapped)
            )
          }
        }
    }
  ...
  def thinSearch(pathFilter: Option[PathFilter] = None,
    ...
    if (debugInfo) {
      ...
      logger.debug(
        s"thinSearch debugInfo request ($oldTimestamp): ${request.toString}"
      )
    }
    ...
    resFuture
      .map { response =>
        if (debugInfo)
          logger.debug(
            s"thinSearch debugInfo response: ($oldTimestamp - ${System
  ...
  def startSuperScroll(pathFilter: Option[PathFilter] = None,
    ...
    val aliases =
      if (withHistory) List("cmwell_current", "cmwell_history")
      else List("cmwell_current")
    val ssr = client
      .admin()
      .cluster()
      .prepareSearchShards(aliases: _*)
      .setTypes("infoclone")
      .execute()
      .actionGet()
  ...
  def startScroll(pathFilter: Option[PathFilter] = None,
    ...
    if (onlyNode.isDefined) {
      request.setPreference(s"_only_node_primary:${onlyNode.map {
        case "local" => localNodeId; case n => n
      }.get}")
    }
  ...
  def startSuperMultiScroll(
    ...
    logger: Logger): Seq[Future[FTSStartScrollResponse]] = {
    ...
    val indices = indicesNames("cmwell_current") ++ (if (withHistory)
                                                       indicesNames("_history")
                                                     else Nil)
  ...
  def startMultiScroll(
    ...
    logger: Logger = loger): Seq[Future[FTSStartScrollResponse]] = {
    logger.debug(
      s"StartMultiScroll request: $pathFilter, $fieldsFilter, $datesFilter, $paginationParams, $withHistory"
    )
    if (partition != defaultPartition) {
...
//    val fields = "system.path" :: "system.uuid" :: "system.lastModified" :: Nil
  ...
  private def injectFuture[A](
    ...
    timeout: Duration = FiniteDuration(10, SECONDS)
  )(implicit executionContext: ExecutionContext) = {
    val p = Promise[A]()
...
case class CurrentInfotonToIndex(override val infoton: Infoton)
    extends InfotonToIndex(infoton)
case class PreviousInfotonToIndex(override val infoton: Infoton)
    extends InfotonToIndex(infoton)
case class DeletedInfotonToIndex(override val infoton: Infoton)
    extends InfotonToIndex(infoton)
...
object SortParam {
  ...
  def apply(sortParam: (String, FieldSortOrder)*) =
    new FieldSortParams(sortParam.toList)
...
case class FieldSortParams(fieldSortParams: List[SortParam.FieldSortParam])
    extends SortParam
...
case object Must extends FieldOperator {
  override def applyTo(softBoolean: SoftBoolean): SoftBoolean =
    softBoolean match {
...
case class SingleFieldFilter(override val fieldOperator: FieldOperator = Must,
  ...
  def filter(i: Infoton): SoftBoolean = {
    ...
    val unmangled =
      if (name.length > 2 && name.charAt(1) == '$') name.drop(2) else name
    ...
    fieldOperator match {
      case Must =>
        i.fields
          .flatMap(
            _.get(unmangled)
              .map(_.exists(fv => value.forall(v => valOp(fv, v))))
          )
      ...
      case MustNot =>
        i.fields
          .flatMap(
            _.get(unmangled)
              .map(_.forall(fv => !value.exists(v => valOp(fv, v))))
          )
...
object FieldFilter {
  def apply(fieldOperator: FieldOperator,
            valueOperator: ValueOperator,
            name: String,
            value: String) =
...
case class FTSStartScrollResponse(total: Long,
                                  ...
                                  searchQueryStr: Option[String] = None)
case class FTSScrollResponse(total: Long,
                             scrollId: String,
                             infotons: Seq[Infoton],
                             nodeId: Option[String] = None)
...
case class FTSThinInfoton(path: String,
                          uuid: String,
                          lastModified: String,
                          indexTime: Long,
                          score: Option[Float])
