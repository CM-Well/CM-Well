...
import org.elasticsearch.common.netty.util.{
  HashedWheelTimer,
  Timeout,
  TimerTask
}
...
object FTSServiceNew {
  def apply(esClasspathYaml: String) =
    new FTSServiceNew(ConfigFactory.load(), esClasspathYaml)
}

class FTSServiceNew(config: Config, esClasspathYaml: String)
    extends FTSServiceOps
    with NsSplitter
    with FTSServiceNewMBean {
  ...
  override val defaultPartition =
    config.getString("ftsService.defaultPartitionNew")
  ...
  if (isTransportClient) {
    ...
    client = new TransportClient(esSettings)
      .addTransportAddress(
        new InetSocketTransportAddress(actualTransportAddress, transportPort)
      )
    loger.info(
      s"starting es transport client [/$actualTransportAddress:$transportPort]"
    )
  } else {
    val esSettings = ImmutableSettings
      .settingsBuilder()
      .loadFromClasspath(esClasspathYaml)
      .build()
  ...
  def nodesHttpAddresses() = {
    nodesInfo.getNodes.map { node =>
      val inetAddress = node.getHttp.getAddress
        .publishAddress()
        .asInstanceOf[InetSocketTransportAddress]
        .address()
  ...
  def createIndex(
    indexName: String
  )(implicit executionContext: ExecutionContext): Future[CreateIndexResponse] =
    injectFuture[CreateIndexResponse](
      client.admin.indices().prepareCreate(indexName).execute(_)
    )

  def updateAllAlias()(
    implicit executionContext: ExecutionContext
  ): Future[IndicesAliasesResponse] =
  ...
  def search(pathFilter: Option[PathFilter],
    ...
    if (pathFilter.isEmpty && fieldsFilter.isEmpty && datesFilter.isEmpty) {
      throw new IllegalArgumentException(
        "at least one of the filters is needed in order to search"
      )
    }
  ...
  private def fieldType(fieldName: String) = {
    fieldName match {
      ...
      case "system.parent" | "system.path" | "system.kind" | "system.uuid" |
          "system.dc" | "system.quad" | "content.data" | "content.mimeType" |
          "link.to" =>
        StringType
  ...
  private def applySortToRequest(sortParams: SortParam,
                                 request: SearchRequestBuilder): Unit =
    sortParams match {
      case NullSortParam => // don't sort.
      case FieldSortParams(fsp) if fsp.isEmpty =>
        request.addSort("system.lastModified", SortOrder.DESC)
      case FieldSortParams(fsp) =>
        fsp.foreach {
          case (name, order) => {
            ...
            request.addSort(
              fieldSort(reversed).order(order).unmappedType(fType.asString)
            )
          }
        }
    }
  ...
  def fullSearch[T](
    ...
    if (debugInfo) {
      logger.info(
        s"[!$debugInfoIdentifier] thinSearch debugInfo request ($oldTimestamp): ${request.toString}"
      )
    }
    ...
    resFuture
      .map { response =>
        if (debugInfo) {
          logger.info(
            s"[!$debugInfoIdentifier] thinSearch debugInfo response: ($oldTimestamp - ${System
              .currentTimeMillis()}): ${response.toString}"
          )
        }
        (
          response,
          render(response, sortParams eq NullSortParam),
          searchQueryStr
        )
      }
      .andThen {
        case Failure(err) =>
          logger.error(s"thinSearch failed, time took: [$oldTimestamp - ${System
            .currentTimeMillis()}], request:\n${request.toString}")
      }
  }

  override def executeBulkActionRequests(
  ...
  def executeIndexRequests(indexRequests: Iterable[ESIndexRequest])(
    ...
    bulkRequest
      .request()
      .add(indexRequests.map {
        _.esAction.asInstanceOf[ActionRequest[_ <: ActionRequest[_ <: AnyRef]]]
      }.asJava)
        ...
        if (!bulkResponse.hasFailures) {
          ...
          promise.success(
            SuccessfulBulkIndexResult
              .fromSuccessful(indexRequests, bulkResponse.getItems)
          )
        } else {
          logger.debug(s"failures in es response")
          promise.success(
            SuccessfulBulkIndexResult(indexRequests, bulkResponse)
          )
        }
      case err @ Failure(exception) =>
        logger.warn(
          s"[!${err.##}] Elasticsearch rejected execution of current bulk",
          exception
        )
  ...
  def executeBulkIndexRequests(indexRequests: Iterable[ESIndexRequest],
    ...
    esResponse.onComplete {
      case Success(bulkResponse) =>
        if (!bulkResponse.hasFailures) {
          ...
          val bulkSuccessfulResult =
            SuccessfulBulkIndexResult.fromSuccessful(
              indexRequests,
              bulkResponse.getItems.filter {
                !_.isFailed
              }
            )
          val allFailures = bulkResponse.getItems.filter(_.isFailed).map {
            itemResponse =>
              (itemResponse, indexedIndexRequests(itemResponse.getItemId))
          }

          val (recoverableFailures, nonRecoverableFailures) =
            allFailures.partition {
              case (itemResponse, _) =>
                itemResponse.getFailureMessage.contains(
                  "EsRejectedExecutionException"
                )
            }

          // filter expected errors that can happen since since we're at least once and not exactly once
          val unexpectedErrors = nonRecoverableFailures.filterNot {
            case (bulkItemResponse, _) =>
              bulkItemResponse.getFailureMessage.startsWith(
          ...
          unexpectedErrors.foreach {
            case (itemResponse, esIndexRequest) =>
              val infotonPath =
                infotonPathFromActionRequest(esIndexRequest.esAction)
              logger.error(
                s"ElasticSearch non recoverable failure on doc id:${itemResponse.getId}, path: $infotonPath . due to: ${itemResponse.getFailureMessage}"
              )
          }
          recoverableFailures.foreach {
            case (itemResponse, esIndexRequest) =>
              val infotonPath =
                infotonPathFromActionRequest(esIndexRequest.esAction)
              logger.error(
                s"ElasticSearch recoverable failure on doc id:${itemResponse.getId}, path: $infotonPath . due to: ${itemResponse.getFailureMessage}"
              )
          }

          val nonRecoverableBulkIndexResults =
            SuccessfulBulkIndexResult.fromFailed(unexpectedErrors.map { _._1 })

          if (recoverableFailures.length > 0) {
            if (numOfRetries > 0) {
              logger.warn(
                s"will retry recoverable failures after waiting for $waitBetweenRetries milliseconds"
              )
              val updatedIndexRequests =
                updateIndexRequests(
                  recoverableFailures.map { _._2 },
                  new DateTime().getMillis
                )

              val reResponse =
                SimpleScheduler.scheduleFuture[SuccessfulBulkIndexResult](
                  waitBetweenRetries.milliseconds
                )({
                  executeBulkIndexRequests(
                    updatedIndexRequests,
                    numOfRetries - 1,
                    (waitBetweenRetries * 1.1).toLong
                  )
                })
              promise.completeWith(reResponse.map {
                bulkSuccessfulResult ++ nonRecoverableBulkIndexResults ++ _
              })
            } else {
              logger.error(
                s"exhausted all retries attempts. logging failures to RED_LOG and returning results "
              )
              promise.success(
                SuccessfulBulkIndexResult(indexRequests, bulkResponse)
              )
      ...
      case err @ Failure(exception) =>
        val errorId = err.##
        if (exception.getLocalizedMessage.contains(
              "EsRejectedExecutionException"
            )) {
          logger.warn(
            s"[!$errorId] Elasticsearch rejected execution of current bulk",
            exception
          )
          if (numOfRetries > 0) {
            logger.warn(
              s"[!$errorId] retrying rejected bulk after waiting for $waitBetweenRetries milliseconds"
            )
            val f =
              SimpleScheduler.scheduleFuture(waitBetweenRetries.milliseconds)({
                val updatedIndexRequests =
                  updateIndexRequests(indexRequests, new DateTime().getMillis)
                executeBulkIndexRequests(
                  updatedIndexRequests,
                  numOfRetries - 1,
                  (waitBetweenRetries * 1.1).toLong
            ...
            logger.error(
              s"[!$errorId] exhausted all retries attempts. logging failures to RED_LOG "
            )
            promise.failure(exception)
          }
        } else {
          logger.error(
            s"[!$errorId] unexpected Exception from Elasticsearch.",
            exception
          )
          // TODO log to RED_LOG
          promise.failure(exception)
  ...
  private def startShardScroll(pathFilter: Option[PathFilter] = None,
                               fieldsFilter: Option[FieldFilter] = None,
                               datesFilter: Option[DatesFilter] = None,
                               withHistory: Boolean,
                               withDeleted: Boolean,
    ...
    scrollResponseFuture.map { scrollResponse =>
      FTSStartScrollResponse(
        scrollResponse.getHits.totalHits,
  ...
  def startSuperScroll(pathFilter: Option[PathFilter],
    ...
    val ssr = client
      .admin()
      .cluster()
      .prepareSearchShards(s"${defaultPartition}_all")
      .setTypes("infoclone")
      .execute()
      .actionGet()
  ...
  def startScroll(pathFilter: Option[PathFilter],
    ...
    logger.debug(
      s"StartScroll request: $pathFilter, $fieldsFilter, $datesFilter, $paginationParams, $withHistory"
    )
    ...
    val indices =
      if (indexNames.nonEmpty) indexNames else Seq(s"${partition}_all")
    ...
    injectFuture[SearchResponse](fakeRequest.execute(_)).flatMap {
      fakeResponse =>
        ...
        val scrollResponseFuture =
          injectFuture[SearchResponse](request.execute(_))
  ...
  def startSuperMultiScroll(pathFilter: Option[PathFilter],
    ...
    logger.debug(
      s"StartMultiScroll request: $pathFilter, $fieldsFilter, $datesFilter, $paginationParams, $withHistory"
    )
    def indicesNames(indexName: String): Seq[String] = {
      val currentAliasRes = client.admin
        .indices()
  ...
  def startMultiScroll(pathFilter: Option[PathFilter],
    ...
    val indices = {
      val currentAliasRes = client.admin
        .indices()
        .prepareGetAliases(s"${partition}_all")
        .execute()
        .actionGet()
  ...
  def scroll(scrollId: String, scrollTTL: Long, nodeId: Option[String])(
    ...
    scrollResponseFuture.onComplete {
      ...
      case Success(scrollResponse) => {
        ...
        if (status >= 400)
          p.failure(new Exception(s"bad scroll response: $scrollResponse"))
        else {
          if (status != 200)
            logger.warn(
              s"scroll($scrollId, $scrollTTL, $nodeId) resulted with status[$status] != 200: $scrollResponse"
            )

          p.complete(
            Try(esResponseToInfotons(scrollResponse, includeScore = false))
              .map { infotons =>
                FTSScrollResponse(
  ...
  implicit def sortOrder2SortOrder(
  ...
  private def applyFiltersToRequest(request: SearchRequestBuilder,
    ...
    pathFilter.foreach { pf =>
      if (pf.path.equals("/")) {
        ...
        boolFilterBuilder.must(
          termFilter(
            if (pf.descendants) "parent_hierarchy" else "parent",
            pf.path
          )
        )
      }
    ...
    datesFilter.foreach {
      case DatesFilter(Some(from), Some(to)) =>
        boolFilterBuilder.must(
          rangeFilter("lastModified").from(from.getMillis).to(to.getMillis)
        )
      case DatesFilter(None, Some(to)) =>
        boolFilterBuilder.must(rangeFilter("lastModified").to(to.getMillis))
      case DatesFilter(Some(from), None) =>
        boolFilterBuilder.must(rangeFilter("lastModified").from(from.getMillis))
    ...
    def applyFieldFilter(fieldFilter: FieldFilter,
                         outerQueryBuilder: BoolQueryBuilder): Unit = {
      fieldFilter match {
        case SingleFieldFilter(
            fieldOperator,
            valueOperator,
            reversedNsTypedField,
            valueOpt
            ) =>
          val name = {
            if (reversedNsTypedField.startsWith("system.") || reversedNsTypedField
                  .startsWith("content.") || reversedNsTypedField.startsWith(
                  "link."
                ))
              reversedNsTypedField
          ...
          if (valueOpt.isDefined) {
            ...
            val exactFieldName = fieldType(fieldName) match {
              case StringType
                  if (!name.startsWith("system.") && !name.startsWith(
                    "content."
                  ) && !name
                    .startsWith("link.")) =>
                s"$name.%exact"
            ...
            fieldOperator match {
              case Must =>
                outerQueryBuilder.must(
                  filteredQuery(matchAllQuery(), existsFilter(name))
                )
              case MustNot =>
                outerQueryBuilder.must(
                  filteredQuery(matchAllQuery(), missingFilter(name))
                )
              case _ =>
                outerQueryBuilder.should(
                  filteredQuery(matchAllQuery(), existsFilter(name))
                )
            }
          }
    ...
    val query =
      (fieldsOuterQueryBuilder.hasClauses, boolFilterBuilder.hasClauses) match {
        case (true, true) =>
  ...
  def aggregate(pathFilter: Option[PathFilter],
    ...
    var counter = 0
    val filtersMap: collection.mutable.Map[String, AggregationFilter] =
    ...
    def filterToBuilder(
      ...
      implicit def fieldValueToValue(fieldValue: Field): String =
        fieldValue.operator match {
          ...
          case NonAnalyzedField =>
            s"infoclone.${reverseNsTypedField(fieldValue.value)}.%exact"
        }
      ...
      val aggBuilder = filter match {
        ...
        case HistogramAggregationFilter(
            _,
            field,
            interval,
            minDocCount,
        ...
        case SignificantTermsAggregationFilter(
            _,
            field,
            backGroundTermOpt,
            minDocCount,
          ...
          val sigTermsBuilder = AggregationBuilders
            .significantTerms(name)
            .field(field)
            .minDocCount(minDocCount)
            .size(size)
          backGroundTermOpt.foreach { backGroundTerm =>
            sigTermsBuilder.backgroundFilter(
              termFilter(backGroundTerm._1, backGroundTerm._2)
            )
          }
          sigTermsBuilder
        case CardinalityAggregationFilter(_, field, precisionThresholdOpt) =>
          val cardinalityAggBuilder =
            AggregationBuilders.cardinality(name).field(field)
      ...
      if (filter.isInstanceOf[BucketAggregationFilter]) {
        filter.asInstanceOf[BucketAggregationFilter].subFilters.foreach {
          subFilter =>
            aggBuilder
              .asInstanceOf[AggregationBuilder[
                _ <: AggregationBuilder[_ <: Any]
              ]]
          ...
          case ta: InternalTerms =>
            TermsAggregationResponse(
              filtersMap
                .get(ta.getName)
                .get
                .asInstanceOf[TermAggregationFilter],
              ta.getBuckets.asScala.map {
                b =>
                  val subAggregations: Option[AggregationsResponse] =
                    b.asInstanceOf[HasAggregations].getAggregations match {
                      case null => None
                      case subAggs =>
                        if (subAggs.asList().isEmpty) None
                        else Some(esAggsToOurAggs(subAggs))
                    }
          ...
          case sa: InternalStats =>
            StatsAggregationResponse(
              filtersMap
                .get(sa.getName)
                .get
                .asInstanceOf[StatsAggregationFilter],
              sa.getCount,
              sa.getMin,
              sa.getMax,
              sa.getAvg,
          ...
          case ca: InternalCardinality =>
            CardinalityAggregationResponse(
              filtersMap
                .get(ca.getName)
                .get
                .asInstanceOf[CardinalityAggregationFilter],
              ca.getValue
            )
          case ha: Histogram =>
            HistogramAggregationResponse(
              filtersMap
                .get(ha.getName)
                .get
                .asInstanceOf[HistogramAggregationFilter],
              ha.getBuckets.asScala.map {
                b =>
                  val subAggregations: Option[AggregationsResponse] =
                    b.asInstanceOf[HasAggregations].getAggregations match {
                      case null    => None
                  ...
                  Bucket(
                    FieldValue(b.getKeyAsNumber.longValue()),
                    b.getDocCount,
                    subAggregations
                  )
              }.toSeq
          ...
          case sta: InternalSignificantTerms =>
            ...
            SignificantTermsAggregationResponse(
              filtersMap
                .get(sta.getName)
                .get
                .asInstanceOf[SignificantTermsAggregationFilter],
              buckets.headOption
                .fold(0L)(_.getSubsetSize) /* if(!buckets.isEmpty) buckets(0).getSubsetSize else 0 */,
              buckets.map {
                b =>
                  val subAggregations: Option[AggregationsResponse] =
                    b.asInstanceOf[HasAggregations].getAggregations match {
      ...
      case Success(res) =>
        Try(esAggsToOurAggs(res.getAggregations, searchQueryStr)).recoverWith {
          case ex: Throwable =>
            Failure(
              new Exception(
                s"aggregations converting failure: $buildErrString",
                ex
              )
            )
        }
  ...
  def rInfo(path: String,
    ...
    val fakeRequest = client
      .prepareSearch(alias)
      .setTypes("infoclone")
      .addFields("system.uuid", "system.lastModified") // TODO: fix should add indexTime, so why not pull it now?

    fakeRequest.setQuery(QueryBuilders.matchQuery("path", path))

    injectFuture[SearchResponse](al => fakeRequest.execute(al)).flatMap {
      fakeResponse =>
        val relevantShards = fakeResponse.getSuccessfulShards
        ...
        val scrollResponseFuture =
          injectFuture[SearchResponse](al => request.execute(al))

        scrollResponseFuture.map { scrollResponse =>
          if (scrollResponse.getHits.totalHits == 0)
            Source.empty[Vector[(Long, String, String)]]
          else
  ...
  val bo =
    collection.breakOut[Array[SearchHit], (String, Long, String), Vector[
      (String, Long, String)
    ]]
  ...
  override def getLastIndexTimeFor(
    ...
    val filtersSeq: List[FieldFilter] = List(
      ...
      SingleFieldFilter(
        MustNot,
        Contains,
        "system.parent.parent_hierarchy",
        Some("/meta/")
      ) //NO META
    )
    applyFiltersToRequest(
      ...
      None,
      Some(
        MultiFieldFilter(Must, fieldFilters.fold(filtersSeq)(filtersSeq.::))
      ),
      None,
    ...
    injectFuture[SearchResponse](request.execute).map { sr =>
      ...
      if (hits.length < 1) None
        ...
        hits.headOption.map(
          _.field("system.indexTime").getValue.asInstanceOf[Long]
        )
      }
    }
  }
  ...
  private def tryLongThenInt[V](
    ...
    uuid: String,
    pathForLog: String
  )(implicit logger: Logger = loger): V =
    try {
      getValueAs[Long](hit, fieldName) match {
        ...
        case Failure(e) => {
          ...
          logger.trace(
            s"$fieldName not Long (outer), uuid = $uuid, path = $pathForLog",
            e
          )
  ...
  private def tryInt[V](hit: SearchHit,
                        fieldName: String,
                        f: Long => V,
                        default: V,
                        uuid: String)(implicit logger: Logger = loger): V =
    try {
  ...
  private val memoizedBreakoutForEsResponseToThinInfotons = scala.collection
    .breakOut[Array[SearchHit], FTSThinInfoton, Vector[FTSThinInfoton]]
  private def esResponseToThinInfotons(
    ...
    esResponse.getHits
      ...
      .map { hit =>
        ...
        val lastModified =
          hit.field("system.lastModified").value.asInstanceOf[String]
        val indexTime = tryLongThenInt[Long](
          hit,
          "system.indexTime",
          identity,
          -1L,
  ...
  private def esResponseToInfotons(
    esResponse: org.elasticsearch.action.search.SearchResponse,
    includeScore: Boolean
  ): Vector[Infoton] = {
    ...
    if (esResponse.getHits.hits().nonEmpty) {
      ...
      hits.map { hit =>
        ...
        val lastModified =
          new DateTime(
            hit.field("system.lastModified").getValue.asInstanceOf[String]
          )
        val id = hit.field("system.uuid").getValue.asInstanceOf[String]
        val dc = Try(hit.field("system.dc").getValue.asInstanceOf[String])
          .getOrElse(Settings.dataCenter)
        val indexTime =
          tryLongThenInt[Option[Long]](
            hit,
            "system.indexTime",
            Some.apply[Long],
            None,
        ...
        val score: Option[Map[String, Set[FieldValue]]] =
          if (includeScore)
            Some(Map("$score" -> Set(FExtra(hit.score(), sysQuad))))
          else None
        ...
        hit.field("system.kind").getValue.asInstanceOf[String] match {
          ...
          case "FileInfoton" =>
            val contentLength = tryLongThenInt[Long](
              hit,
              "content.length",
              identity,
              -1L,
            ...
            new FileInfoton(
              ...
              Some(
                FileContent(
                  hit.field("content.mimeType").getValue.asInstanceOf[String],
                  contentLength
                )
              )
            ) {
          ...
          case unknown =>
            throw new IllegalArgumentException(
              s"content returned from elasticsearch is illegal [$unknown]"
            ) // TODO change to our appropriate exception
        }
      }.toVector
    } else {
      Vector.empty
  ...
  /**
    ...
    * @return
    */
  private def updateIndexRequests(
    esIndexRequests: Iterable[ESIndexRequest],
    currentTime: Long
  ): Iterable[ESIndexRequest] = {

    def updateIndexRequest(indexRequest: IndexRequest): IndexRequest = {
      val source = indexRequest.sourceAsMap()
      val system =
        source.get("system").asInstanceOf[java.util.HashMap[String, Object]]
  ...
  private def infotonPathFromActionRequest(
    actionRequest: ActionRequest[_]
  )(implicit logger: Logger = loger) = {
    if (!actionRequest.isInstanceOf[IndexRequest] && !actionRequest
          .isInstanceOf[UpdateRequest]) {
      logger.error(
        s"got an actionRequest:$actionRequest which is non of the supported: IndexRequest or UpdateRequest"
      )
      ...
      if (timeout.isFinite()) {
        ...
        new ActionListener[A] {
          def onFailure(t: Throwable): Unit = {
            ...
            if (!p.tryFailure(t))
              logger.error(
                s"Exception from ElasticSearch (future timed out externally, response returned in [${System
          ...
          def onResponse(res: A): Unit = {
            task.cancel()
            if (p.trySuccess(res))
              logger.trace(s"Response from ElasticSearch [took ${System
                .currentTimeMillis() - timestamp}ms]:\n${res.toString}")
            else
              logger.error(
                s"Response from ElasticSearch (future timed out externally, response returned in [${System
                  .currentTimeMillis() - timestamp}ms])\n${res.toString}"
              )
          }
        }
        ...
        new ActionListener[A] {
          def onFailure(t: Throwable): Unit = {
            logger.error(
              s"Exception from ElasticSearch (no timeout, response returned in [${System
                .currentTimeMillis() - timestamp}ms])",
              t
            )
            p.failure(t)
          ...
          override def run(timeout: Timeout) = {
            promise.failure(
              new TimeoutException(
                "Operation timed out after " + after.toMillis + " millis"
              )
            )
          }
        },
  ...
  override def extractSource[T: EsSourceExtractor](uuid: String, index: String)(
    ...
    injectFuture[GetResponse](
      client.prepareGet(index, "infoclone", uuid).execute(_)
    ).map { hit =>
      implicitly[EsSourceExtractor[T]].extract(hit) -> hit.getVersion
    }
  }

  override def purgeHistory(
    path: String,
    isRecursive: Boolean,
    partition: String
  ...
  override def getMappings(withHistory: Boolean, partition: String)(
    ...
    implicit class AsLinkedHashMap[K](lhm: Option[AnyRef]) {
      def extract(k: K) = lhm match {
        case Some(m) =>
          Option(m.asInstanceOf[java.util.LinkedHashMap[K, AnyRef]].get(k))
        case None => None
      }
      def extractKeys: Set[K] =
        lhm
          .map(
            _.asInstanceOf[java.util.LinkedHashMap[K, Any]]
              .keySet()
              .asScala
              .toSet
          )
          .getOrElse(Set.empty[K])
      def extractOneValueBy[V](selector: K): Map[K, V] =
        lhm
          .map(_.asInstanceOf[java.util.LinkedHashMap[K, Any]].asScala.map {
            case (k, vs) =>
              k -> vs.asInstanceOf[java.util.LinkedHashMap[K, V]].get(selector)
          }.toMap)
          .getOrElse(Map[K, V]())
    }
    ...
    csf.map { cs =>
      ...
      cs.getMetaData.iterator.asScala
        .filter(_.index().startsWith("cm"))
        .foreach { imd =>
          val nested =
            Some(imd.mapping("infoclone").getSourceAsMap.get("properties"))
  ...
  override def getIndicesNamesByType(suffix: String,
                                     partition: String): Seq[String] =
    getIndicesNames(partition).toSeq
  ...
  override def bulkIndex(
    currentInfotons: Seq[Infoton],
    previousInfotons: Seq[Infoton],
    partition: String
  )(implicit executionContext: ExecutionContext): Future[BulkResponse] = ???

  override def purgeByUuidsAndIndexes(
  ...
  override def purge(uuid: String, partition: String)(
    implicit executionContext: ExecutionContext
  ): Future[Boolean] = {
  ...
  override def purgeByUuidsFromAllIndexes(
    uuids: Vector[String],
    partition: String
  )(implicit executionContext: ExecutionContext): Future[BulkResponse] = {
...
case class ESIndexRequest(
  esAction: ActionRequest[_ <: ActionRequest[_ <: AnyRef]],
  newIndexTime: Option[Long]
) {
...
case class SuccessfulIndexResult(uuid: String, newIndexTime: Option[Long])
    extends IndexResult
case class FailedIndexResult(uuid: String, reason: String, idx: Int)
    extends IndexResult
...
case class SuccessfulBulkIndexResult(
  override val successful: Iterable[SuccessfulIndexResult],
...
object SuccessfulBulkIndexResult {
  ...
  def apply(indexRequests: Iterable[ESIndexRequest],
            esResponse: BulkResponse): SuccessfulBulkIndexResult = {
    val indexedIndexRequests = indexRequests.toIndexedSeq
    val successfulResults = esResponse.getItems.collect {
      case esItemResponse: BulkItemResponse if !esItemResponse.isFailed =>
        SuccessfulIndexResult(
          esItemResponse.getId,
          indexedIndexRequests(esItemResponse.getItemId).newIndexTime
        )
    }
      ...
      case true =>
        esResponse.getItems.collect {
          case esItemResponse: BulkItemResponse if esItemResponse.isFailed =>
            FailedIndexResult(
              esItemResponse.getId,
              esItemResponse.getFailureMessage,
              esItemResponse.getItemId
            )
        }.toIterable
    }
  ...
  def fromSuccessful(
    ...
    successfulItemResponses: Iterable[BulkItemResponse]
  ): SuccessfulBulkIndexResult = {
    val indexedIndexRequests = indexRequests.toIndexedSeq
    SuccessfulBulkIndexResult(successfulItemResponses.map { sir =>
      SuccessfulIndexResult(
        sir.getId,
        indexedIndexRequests(sir.getItemId).newIndexTime
      )
    })
  }
...
object BulkResultWithIndexTime {
  def apply(failed: Iterable[(String, String)]) =
    new BulkResultWithIndexTime(Iterable.empty[(String, Long)], failed)
}
...
case class BulkResult(successful: Iterable[String],
                      failed: Iterable[(String, String)] =
                        Iterable.empty[(String, String)]) {
  def ++(other: BulkResult) =
    BulkResult(successful ++ other.successful, failed ++ other.failed)
}

object BulkResult {
  def apply(failed: Iterable[(String, String)]) =
    new BulkResult(Iterable.empty[String], failed)
}
