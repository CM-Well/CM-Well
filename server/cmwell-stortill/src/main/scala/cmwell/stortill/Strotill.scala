...
object Strotill {
  ...
  def apply(irw: IRWService, ftsService: FTSServiceOps): Strotill =
    new Strotill(irw, ftsService)
}
...
class Strotill(irw: IRWService, ftsService: FTSServiceOps) extends LazyLogging {
  ...
  def createActionsToFixEs(
    v: Vector[Infoton]
  ): Seq[ActionRequest[_ <: ActionRequest[_ <: AnyRef]]] = {
