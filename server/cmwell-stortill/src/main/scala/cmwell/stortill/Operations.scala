...
abstract class Operations(irw: IRWService, ftsService: FTSServiceOps) {
  ...
  def fixDc(path: String,
            dc: String,
            retries: Int = 1,
            indexTimeOpt: Option[Long] = None): Future[Boolean]
  def shutdown: Unit
}

object ProxyOperations {

  lazy val specialInconsistenciesLogger: Logger = Logger(
    LoggerFactory.getLogger("cmwell.xfix")
  )
...
class ProxyOperations private (irw: IRWService, ftsService: FTSServiceOps)
  ...
  def retry[T](task: => Future[T]) =
    cmwell.util.concurrent.retry[T](maxRetries, retryWait)(task)(global)
  ...
  override def rFix(
    ...
    retries: Int,
    parallelism: Int
  ): Future[Source[(Boolean, String), NotUsed]] = {

    type MergedByTimestamp =
      (Timestamp, Vector[(Timestamp, Uuid)], Vector[(Timestamp, Uuid, EsIndex)])

    val cassandraPathsSource: Source[(Timestamp, Uuid), NotUsed] =
      irw.historyReactive(path)
    ftsService
      .rInfo(
        path,
      ...
      .map { rEsInfo =>
        ...
        val elasticsearchPathInfo
          : Source[(Timestamp, Uuid, EsIndex), NotUsed] = {
          rEsInfo
        ...
        Source.fromGraph(GraphDSL.create() { implicit b =>
          {
            ...
            val smb = b.add(
              new SortedStreamsMergeBy[
                (Timestamp, Uuid),
                (Timestamp, Uuid, EsIndex),
                Timestamp
              ](_._1, _._1)
            )
            val ial = b.add(
              new MapInitAndLast[
                MergedByTimestamp,
                (MergedByTimestamp, Boolean)
              ](_ -> false, _ -> true)
            )
            val fix = b.add(
              Flow[(MergedByTimestamp, Boolean)]
                .mapAsyncUnordered(parallelism) {
                  case ((t, v1, v2), isLast) =>
                    fixWith(
                      ...
                      isContainsCurrent = isLast
                    ).map {
                      case (bool, "") =>
                        bool -> s"${cmwell.util.string
                          .dateStringify(new DateTime(t))} [$t]"
                      case (bool, m) =>
                        bool -> s"$m, ${cmwell.util.string
                          .dateStringify(new DateTime(t))} [$t]"
                    }
                }
            )
  ...
  override def fix(path: String,
                   retries: Int,
                   limit: Int): Future[(Boolean, String)] = {
    val cUuids: Future[Vector[(Timestamp, Uuid)]] = retry(
      irw.historyAsync(path, limit)
    )
  ...
  private def fixWith(path: String,
    ...
    val esUuids: Future[Vector[(Uuid, EsIndex)]] = rawEsUuids.flatMap {
      ...
      case esus => {
        ...
        cmwell.util.concurrent.travector(esusmap.toSeq) {
          case (uuid, Vector(uuidInSingleIndex)) =>
            Future.successful(uuidInSingleIndex)
          case (uuid, vec) => {
            val oldestIngest = vec.minBy(_._2)
            ftsService
              .purgeByUuidsAndIndexes(vec.filterNot(oldestIngest.eq))
              .map { bulkRes =>
    ...
    cUuids.flatMap { usFromC =>
      esUuids.flatMap { usFromES =>
        ...
        val foundFut: Future[Set[Either[Uuid, Infoton]]] =
          Future.traverse(all) { uuid =>
            // if o.isEmpty, o.get will throw,
            // and the retry will kickoff again
            retry(
              irw
                .readUUIDAsync(uuid, cmwell.irw.QUORUM)
                .map(o => Option(o.get))
            ).recoverWith {
                case t: Throwable => {
        ...
        foundFut.flatMap { findings =>
          ...
          val purgeNotFound = {
            ...
            val uuidFromEsButNotFound = usFromES.filter {
              case (u, _) => nFound(u)
            }
            val uuidFromCasButNotFound = usFromC.filter {
              case (_, u) => nFound(u)
            }
          ...
          val fixFoundFut: Future[(Boolean, String)] = {
            ...
            if (found.isEmpty)
              irw.purgePathOnly(path).map(_ => true -> "no UUIDs were found")
            else {
              ...
              val foundAndFixedFut =
                Future.traverse(found)(fixAndUpdateInfotonInCas)
              foundAndFixedFut.flatMap { foundAndFixed =>
                ...
                Future
                    ...
                    case (_, is) => {
                      ...
                      Future
                        .traverse(is.filterNot(_ == maxiton)) { i =>
                          log.debug(
                            s"purging  an infoton only because lastModified collision: ${ProxyOperations.jsonFormatter
                              .render(i)}"
                          )
                          ...
                          val f1 = retry(
                            ftsService.purgeByUuidsAndIndexes(
                              onlyES(i.uuid).map(i.uuid -> _)
                            )
                          ).map[Infoton] { br =>
                              if (br.hasFailures) logEsBulkResponseFailures(br);
                              i
                            }
                            .recover {
                              case e: Throwable =>
                                log.info(
                                  s"purge from es failed for uuid=${i.uuid} of path=${i.path}",
                                  e
                                ); i
                            }
                          val f2 = retry(
                            purgeFromCas(
                              i.path,
                              i.uuid,
                              onlyC.getOrElse(i.uuid, i.lastModified.getMillis)
                            )
                          ).map(_ => i).recover {
                            case e: Throwable =>
                              log
                                .info(
                                  s"purge from cas failed for uuid=${i.uuid} of path=${i.path}",
                                  e
                                ); i
                          }
                          f1.flatMap(_ => f2)
                  ...
                  .flatMap { noRepetitionsInLastModified =>
                    val writeToCasPathsFut = Future.traverse(
                      noRepetitionsInLastModified
                        .filter(i => onlyES.contains(i.uuid))
                    ) { onlyEsInfoton =>
                      val f =
                        retry(
                          irw.setPathHistory(onlyEsInfoton, cmwell.irw.QUORUM)
                        ).recover {
                          case e: Throwable =>
                            log.info(
                    ...
                    writeToCasPathsFut.flatMap { writeToCasPaths =>
                      ...
                      val uFromEsThatAreFound = usFromES.filterNot {
                        case (u, _) => nFound(u)
                      }
                      val purgeFromEsFut =
                        Future.traverse(uFromEsThatAreFound) {
                      ...
                      purgeFromEsFut.flatMap { _ =>
                        ...
                        if (isNewBg) {
                          ...
                          val actions = foundAndFixed.map(
                            i =>
                              ESIndexRequest(
                                createEsIndexActionForNewBG(
                                  i,
                                  if (i.indexName.isEmpty)
                                    "cm_well_latest"
                                  else i.indexName,
                                  i eq cur
                                ),
                          ...
                          retry(ftsService.executeBulkIndexRequests(actions))
                            .map(_ => (true, ""))
                        } else {
                          val actions = foundAndFixed.map {
                            case i if isContainsCurrent && (i eq cur) =>
                              createEsIndexAction(i, "cmwell_current_latest")
                            case i =>
                              createEsIndexAction(i, "cmwell_history_latest")
                          }
                          // todo .recover + check .hasFailures
                          retry(ftsService.executeBulkActionRequests(actions))
                            .map(_ => (true, ""))
                        }
                      }
                    }
  ...
  private def logEsBulkResponseFailures(br: BulkResponse) = {
    log.info(
      s"ES BulkResponse has failures: ${br.getItems.filter(_.isFailed).map(_.getFailureMessage).mkString(", ")}"
    )
  }
  ...
  private def purgeAndLog(path: String,
      ...
      .traverse(uuidsFromEs) {
        case (uuidInEs, esIndex) =>
          val f = retry(ftsService.extractSource(uuidInEs, esIndex)).recover {
            case e: Throwable =>
              log.error(
                s"could not retrieve ES sources for uuid=[$uuidInEs] from index=[$esIndex]",
                e
              )
    ...
    onlyC.foreach {
      case (timestamp, uuid) =>
        log.info(
          s"$uuid for $path was not found in cas.infoton, although it was found in cas.path[$timestamp]"
        )
    }
    ...
    logEsSourcesForMissingUuids.onComplete { t =>
      t.failed.foreach(
        err => log.error("logEsSourcesForMissingUuids future failed", err)
      )
      p.completeWith {
        val f1 = {
          if (uuidsFromEs.isEmpty) Future.successful(())
            ...
            retry(ftsService.purgeByUuidsAndIndexes(uuidsFromEs)).recover {
              case e: Throwable =>
                log.error(s"error occured while purging=${uuidsFromEs
                  .mkString("[", ",", "]")} for path=[$path] from ES", e)
            }
        }

        val f2 = Future.traverse(uuidsFromCas) {
          case (timestamp, uuid) =>
            retry(irw.purgeFromPathOnly(path, timestamp, cmwell.irw.QUORUM))
              .recover {
                case e: Throwable =>
                  log.error(
                    s"could not purge uuid=[$uuid] for path=[$path] from CAS.path with timestamp=[$timestamp}]",
                    e
                  )
              }
        }
  ...
  private def createEsIndexActionForNewBG(
    infoton: Infoton,
    index: String,
    isCurrent: Boolean
  ): ActionRequest[_ <: ActionRequest[_ <: AnyRef]] = {
    ...
    val serializedInfoton =
      JsonSerializerForES.encodeInfoton(infotonWithUpdatedIndexTime, isCurrent)
    Requests
      .indexRequest(index)
      .`type`("infoclone")
      .id(infoton.uuid)
  ...
  private def createEsIndexAction(infoton: Infoton, index: String) =
    Requests
      ...
      .source(
        JsonSerializer
          .encodeInfoton(infoton, omitBinaryData = true, toEs = true)
      )
      ...
      .fold(Future.successful(i))(
        j =>
          irw.writeAsyncDataOnly(j, cmwell.irw.QUORUM).recover {
            case e: Throwable =>
              log.error(
                s"could not write to cassandra the infoton with uuid=[${j.uuid}}] for path=[${j.path}}]",
                e
              )
              j
        }
  ...
  override def info(
    path: String,
    limit: Int
  ): Future[(CasInfo, EsExtendedInfo, ZStoreInfo)] = {
        ...
        case (
            ...
            Some(
              FileInfoton(
                ...
                _,
                _,
                Some(FileContent(_, _, _, Some(dp))),
                _
              )
            )
            ) =>
  ...
  override def fixDc(path: String,
                     dc: String,
                     retries: Int,
                     indexTimeOpt: Option[Long] = None): Future[Boolean] = {
    ...
    val task = cmwell.util.concurrent.retry(retries, 1.seconds) {
      s.extractLastCas(path).flatMap { infoton =>
        ...
        val (infotonWithFixedIndexTime, addIdxTInCasFut) = {
          ...
          indexTimeOpt match {
            case Some(t) if iIdxTOpt.isDefined && t == iIdxTOpt.get =>
              infoton -> dummyFut
            ...
            case None =>
              addIndexTime(infoton, Some(lmMillis), force = true) ->
                s.irwProxy
                  .addIndexTimeToUuid(infoton.uuid, lmMillis, level = QUORUM)
          }
        }
        ...
        Future.sequence(Seq(addIdxTInCasFut, addDcInCasFut)).map { _ => () =>
          {
            val esIndexAction = {
              Requests
                ...
                .source(
                  JsonSerializer
                    .encodeInfoton(infotonWithFixedIndexTimeAndDc, true, true)
                )
            }
            s.ftsProxy
              .purgeByUuids(Seq(), Some(infotonWithFixedIndexTimeAndDc.uuid))
              .flatMap {
                case br if br.hasFailures =>
                  Future.failed(new Exception("purging indexes failed"))
                case _ =>
                  s.ftsProxy
                    .executeBulkActionRequests(Seq(esIndexAction))
                    .flatMap {
                      case br if br.hasFailures =>
                        Future.failed(new Exception("re-indexing failed"))
