...
case class GridSubDiv(user : String ,
  ...
  override def startElasticsearch(hosts : GenSeq[String]): Unit = {
    command(s"cd ${instDirs.globalLocation}/cm-well/app/es/cur; ${startScript("./start-master.sh")}", ips.par.take(esMasters).intersect(hosts), false)
  ...
  override def startCassandra(hosts : GenSeq[String]): Unit = {
    ...
    for(host <- hosts) {
      for(index <- 1 to dataDirs.casDataDirs.size){
        command(s"bash -c 'sleep ${s} ; cd ${instDirs.globalLocation}/cm-well/app/cas/cur/; ${startScript(s"./start${Host.getIndexTxt(index)}.sh")}' > /dev/null 2> /dev/null &", Seq(host), false)
        s += 1
      }
    }
  }
  ...
  override def initCassandra(hosts : GenSeq[String]): Unit = {
    ...
    for(host <- hosts.drop(3)){
      command(s"bash -c 'sleep ${s} ; cd ${instDirs.globalLocation}/cm-well/app/cas/cur/; ${startScript(s"./start.sh")}' > /dev/null 2> /dev/null &", Seq(host), false)
      s += 1
    }
    ...
    for(host <- hosts){
      for(index <- 2 to dataDirs.casDataDirs.size){
        command(s"bash -c 'sleep ${s} ; cd ${instDirs.globalLocation}/cm-well/app/cas/cur/; ${startScript(s"./start$index.sh")}' > /dev/null 2> /dev/null &", Seq(host), false)
        s += 1
      }
    }
  ...
  override def initElasticsearch(hosts : GenSeq[String]): Unit = {
    command(s"cd ${instDirs.globalLocation}/cm-well/app/es/cur; ${startScript("./start-master.sh")}", hosts.take(esMasters), false)
  ...
  override def unprepareMachines(hosts: GenSeq[String]): Unit = {
    throw new Exception("Nothing was done! Please use unprepareMachines with sudoer only this way the network configuration will be changed")
  }
