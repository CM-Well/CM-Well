  ...
  def linkDataDirectories(hosts: GenSeq[String]) {
    ...
    componentDataDirs.foreach { dd =>
      for (index <- 1 to dd._2.size) {
        h.command(
          s"test -L ${h.getInstDirs.intallationDir}/data/${ResourceBuilder
            .getIndexedName(dd._1, index)} || ln -s ${dd._2(index - 1)} ${h.getInstDirs.intallationDir}/data/${ResourceBuilder
            .getIndexedName(dd._1, index)}",
          hosts,
          false
        )
      }
  ...
  def createLoggingDirectories(hosts: GenSeq[String]) {
    ...
    componentMappings.foreach { componentMapping =>
      for (index <- 1 to componentMapping._2) {
        h.command(
          s"mkdir -p ${h.getInstDirs.intallationDir}/log/${ResourceBuilder
            .getIndexedName(componentMapping._1, index)}",
          hosts,
          false
        )
      }
...
trait ConfigurableComponent {
  ...
  def createConigurationsDirectoires(hosts: GenSeq[String]) {
    ...
    componentMappings.foreach { componentMapping =>
      for (index <- 1 to componentMapping._2) {
        h.command(
          s"mkdir -p ${h.getInstDirs.intallationDir}/conf/${ResourceBuilder
            .getIndexedName(componentMapping._1, index)}",
          hosts,
          false
        )
...
abstract class ComponentProps(h: Host,
                              name: String,
                              location: String,
                              hasDate: Boolean) {
  def getName = name
  ...
  def targetFullPath: String =
    s"${h.getInstDirs.intallationDir}/$targetLocation"
  ...
  def getTarResName(path: String, location: String): String =
    h.command(
        "tar -tf " + s"$location/$path" + " | awk -F '/' '{print $1}' | head -1"
      )
      .get
      .trim
  ...
  def getZipResName(path: String, location: String): String =
    h.command(
        "unzip -l " + s"$location/$path" + " | awk '{print $4}' |  awk -F '/' '{print $1}' | head -4 | tail -1"
      )
      .get
      .trim
  ...
  def createSymbolicLink(name: String,
                         symbolicLinkName: String,
                         hosts: GenSeq[String]) = {
    h.command(
      s"ln -s $targetFullPath/$name $targetFullPath/$symbolicLinkName",
      hosts,
      false
    )
  }
  ...
  def relink(linkTo: String, hosts: GenSeq[String]) = {
    h.command(
      s"test -L $targetFullPath/${symLinkName.get} && rm $targetFullPath/${symLinkName.get}",
      hosts,
      false
    )
    h.command(
  ...
  def quickCopy(name: String, hosts: GenSeq[String]) = {
    ...
    symLinkName match {
      case Some(sln) => {
        ...
        if (isDir)
          h.command(
            s"test -d $targetFullPath/$sln/ && $archiveCommand $targetFullPath/$sln/ $targetFullPath/$name",
            hosts,
            false
          )
        else
          h.command(
            s"test -f $targetFullPath/$sln && $archiveCommand $targetFullPath/$sln $targetFullPath/$name",
            hosts,
            false
          )
      }
  ...
  def addPostfixToComponent(remoteLocation: String,
                            name: String,
                            postfix: String,
                            hosts: GenSeq[String]) = {
    h.command(
      s"mv $remoteLocation/$name $remoteLocation/$name-$postfix",
      hosts,
      false
    )
  }

  def uploadComponent(localLocation: String,
                      target: String,
    ...
    val results =
      if (h.command(
              s"if test -d $localLocation/$realName; then echo 'folder'; else echo 'file'; fi"
            )
            .get
            .equals("folder"))
        h.rsync(s"$localLocation/$realName/", s"$target/$realName", hosts)
      else h.rsync(s"$localLocation/$realName", s"$target/", hosts)

    if (results.filter(r => r.isFailure).size > 0)
      throw new Exception("Failed to upload components")
...
case class CassandraProps(h: Host)
  ...
  override val componentDataDirs: Map[String, GenSeq[String]] =
    Map(
      "cas" -> h.getDataDirs.casDataDirs,
      "ccl" -> h.getDataDirs.casCommitLogDirs
    )

  override val componentMappings: Map[String, Int] = Map(
    "cas" -> h.getDataDirs.casDataDirs.size
  )
...
case class ElasticsearchProps(h: Host)
  ...
  override val componentMappings: Map[String, Int] =
    Map("es" -> h.getDataDirs.esDataDirs.size, "es-master" -> 1)
...
case class KafkaProps(h: Host)
  ...
  override val componentDataDirs: Map[String, GenSeq[String]] = Map(
    "kafka" -> h.getDataDirs.kafkaDataDirs
  )
...
case class ZooKeeperProps(h: Host)
  ...
  override val componentDataDirs: Map[String, GenSeq[String]] = Map(
    "zookeeper" -> Seq(h.getDataDirs.zookeeperDataDir)
  )
...
case class BgProps(h: Host)
  ...
  override def getUnpackedName(packageName: String,
                               location: String): Option[String] = None
...
case class JavaProps(h: Host)
    extends ComponentProps(h, "jdk", "components-extras", false) {
  def targetLocation = "app"
  def unpackCommand: Option[String] = Some("tar -xf")
  def symLinkName: Option[String] = Some("java")
}

case class Mx4JProps(h: Host)
    extends ComponentProps(h, "mx4j", "components", false) {
...
class Deployment(h: Host) {
  ...
  val componentProps: Vector[ComponentProps] = Vector(
    ...
    DcProps(h)
          ...
          List(LogstashProps(h), KibanaProps(h))
        else
          Vector.empty[ComponentProps]) ++ (if (h.getDeployJava)
                                              Vector(JavaProps(h))
                                            else
  ...
  def createScript(module: ComponentConf) {
    ...
    if (confContent != null) {
      ...
      h.command(
        s"chmod +x ${module.scriptDir}/${confContent.fileName}",
        module.host,
        false
      )
    }
  ...
  def createConf(module: ComponentConf) {
    ...
    if (confContents != null) {
      confContents.foreach { confContent =>
        ...
        createFile(
          confContent.content,
          confContent.fileName,
          dir,
          Seq(module.host)
        )
        if (confContent.executable)
          h.command(
            s"chmod +x $dir/${confContent.fileName}",
            module.host,
            false
          )
      }
