...
abstract class ComponentConf(var host : String , var scriptDir: String , var scriptName : String, var confDir : String, var confName : String, var moduleIndex : Int) {
  val PATH = "$PATH"
  val BMSG = "bmsg; if [ $? -eq 1 ] ; then exit 13; fi"
  val CHKSTRT = s"if [ `ps aux | grep -v grep | grep -v starter | grep '${getPsIdentifier}' | wc -l` -gt 0 ] ; then exit 0; fi"
  //val BMSG = ""

  def createExportEnvStr(environmentVar: String) =
    sys.env.get(environmentVar).map(value => s"export $environmentVar='$value'")

  def genDebugStr(port : Int) = s"""if [ "$$CMWELL_DEBUG" = "true" ] ; then DEBUG_STR=`echo -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=$port`; else DEBUG_STR=''; fi"""
...
case class CassandraConf(home : String, seeds : String , clusterName : String , resourceManager : JvmMemoryAllocations , snitchType : String , ccl_dir : String , dir : String = "cas", rowCacheSize : Int, replicationFactor : Int = 3, template : String = "cassandra.yaml", listenAddress : String = "127.0.0.1", rpcAddress : String = "127.0.0.1", sName : String, index : Int , rs : RackSelector , g1 : Boolean, hostIp : String) extends ComponentConf(hostIp, s"$home/app/cas/cur",sName, s"$home/conf/$dir","cassandra.yaml", index) {
  ...
  override def mkScript: ConfFile = {
    ...
    val cmsGc = Seq("-XX:+HeapDumpOnOutOfMemoryError",
      "-XX:+UseParNewGC",
    ...
    val jvmArgs = JVMOptimizer.java7ExtraArgs ++ Seq("-ea") ++ jamm ++ Seq(
      ...
      Seq("-Djava.net.preferIPv4Stack=true",
    ...
    val args = Seq("starter", "java") ++ agentLibArgs ++ jvmArgs ++ JVMOptimizer.gcLoggingJVM(s"$home/log/" + dir + "/gc.log") ++ Seq("-cp", classpath, "org.apache.cassandra.service.CassandraDaemon")
  ...
  override def mkConfig: List[ConfFile] = {
    val confContent = ResourceBuilder.getResource(s"scripts/templates/${template}",
      Map("clustername" -> clusterName, "seeds" -> seeds, "listen_address" -> listenAddress, "rpc_address" -> rpcAddress,"ccl_dir" -> ccl_dir , "dir" -> dir, "root_dir" -> home, "endpoint_snitch" -> snitchType, "row_cache_size" -> rowCacheSize.toString))
...
//      Map("file_path" -> s"$home/log/$dir/system.log"))
    ...
    val rackConfContent = ResourceBuilder.getResource("scripts/templates/cassandra-rackdc.properties",
    ...
    val cassandraStatus = ResourceBuilder.getResource("scripts/templates/cassandra-status-viewer-template", Map("home" -> home, "host" -> hostIp))

    List(ConfFile("cassandra.yaml", confContent, false) , ConfFile("logback.xml", logBackContent) , ConfFile("cassandra-rackdc.properties", rackConfContent, false), ConfFile("cassandra-status-viewer", cassandraStatus, true), ConfFile("cassandra-cql-init-cluster", cqlInit), ConfFile("cassandra-cql-init-cluster-new", cqlInit2), ConfFile("zstore-cql-init-cluster", cqlInit3))
  }
}
...
case class ZookeeperConf(home : String, clusterName : String, servers : Seq[String] , hostIp : String) extends ComponentConf(hostIp, s"$home/app/zookeeper", "start.sh", s"$home/conf/zookeeper","zoo.conf", 1) {
  val dir = "zookeeper"
...
case class BgConf(home : String, zookeeperServers : Seq[String], clusterName: String, dataCenter :String ,hostName : String , resourceManager : JvmMemoryAllocations, sName : String, isMaster : Boolean , minMembers : Int = 1 , partition : Int = 0 , numOfPartitions : Int = 1, logLevel : String , debug : Boolean, hostIp : String, seeds : String, seedPort : Int = 9301, dir:String = "bg") extends ComponentConf(hostIp, s"$home/app/bg",sName, s"$home/conf/bg","bg.yml", 1) {
  override def mkScript: ConfFile = {
    def jvmArgs = {
      ...
      val mXmn = resourceManager.getMxmn
      val mXss = resourceManager.getMxss
      val jmx = Seq(s"-Dcom.sun.management.jmxremote.port=${PortManagers.bg.jmxPortManager.getPort(1)}",
    ...
    val args = Seq("starter", "java", "$DEBUG_STR", s"-Dcmwell.home=$home") ++ jvmArgs ++ Seq("-cp", s""" "conf:$home/app/bg/lib/*" """, "cmwell.bg.Runner")
...
case class CtrlConf(home : String, sName : String, seeds : String, clusterName : String, resourceManager : JvmMemoryAllocations, singletonStarter : Boolean, pingIp : String,minMembers : Int = 1, user : String ,logLevel : String, debug : Boolean, hostIp : String) extends ComponentConf(hostIp, s"$home/app/ctrl",sName, s"$home/conf/ctrl","ctrl.yml", 1) {
  val port = 7777
  ...
  override def mkScript: ConfFile = {
    ...
    val args = Seq("-XX:+UseG1GC",
                  ...
                  "-Duser.timezone=GMT0",
                  "-Dcom.sun.management.jmxremote.authenticate=false",
                  "-Dcom.sun.management.jmxremote.ssl=false") ++ JVMOptimizer.gcLoggingJVM(s"$home/log/ctrl/gc.log")
...
case class DcConf(home : String, sName : String, clusterName : String, resourceManager : JvmMemoryAllocations,pingIp : String,minMembers : Int = 1, logLevel : String, debug : Boolean, target : String , hostIp : String) extends ComponentConf(hostIp, s"$home/app/dc",sName, s"$home/conf/dc","dc.yml", 1) {
  val port = 7777
  override def mkScript: ConfFile = {
    val mXmx = resourceManager.getMxmx
    val mXms = resourceManager.getMxms
...
object LogstashConf {
  def genLogstashConfFile(clusterName : String, esHost : String , globalFields : Map[String, String], logslocation : String, subDivision : Int) : String = {
    ...
    def getComponentFieldsMap(component : String, logName : String, componentIndex : Int) = Map("component" -> component, "logName" -> logName, "componentIndex" -> componentIndex.toString)
    ...
    val casLogs = createComponentEntries("Cassandra","cas", subDivision ,List("gc.log.*","stdout.log","stderr.log", "system.log"))
    val esLogs = createComponentEntries("Elasticsearch", "es", subDivision ,List(s"${clusterName}_index_indexing_slowlog.log",s"${clusterName}_index_search_slowlog.log",s"${clusterName}.log", "gc.log.*","stderr.log","stdout.log"))
    val esMasterLogs = createComponentEntries("ElasticsearchMaster", "es-master", 1 ,List(s"${clusterName}_index_indexing_slowlog.log",s"${clusterName}_index_search_slowlog.log",s"${clusterName}.log", "gc.log.*","stderr.log","stdout.log"))
    val wsLogs = createComponentEntries("Ws","ws",1,List("access.log","application.log","gc.log.*","stderr.log","stdout.log"))
    val bgLogs = createComponentEntries("Bg","bg",1,List("application.log","gc.log.*","heartbeat.log","stderr.log","stdout.log"))
    val ctrlLogs = createComponentEntries("Ctrl","ctrl",1,List("application.log","stderr.log","stdout.log", "cluster_state.log","alerts.log", "gc.log.*"))
...
case class LogstashConf(clusterName : String,elasticsearchUrl : String, home : String, dir : String = "logstash", sName : String, subdivision : Int, hostIp : String) extends ComponentConf(hostIp, s"$home/app/logstash",sName, s"$home/conf/logstash","logstash.yml", 1) {
  override def mkScript: ConfFile = {
    val scriptContent =
      s"""
         |export PATH=$home/app/java/bin:$home/bin/utils:$PATH
  ...
  override def mkConfig: List[ConfFile] = {
    val confContent = LogstashConf.genLogstashConfFile(clusterName,elasticsearchUrl, Map.empty[String, String], s"$home/log", subdivision)
    List(ConfFile("logstash.conf", confContent, false))
  }
}

case class KibanaConf(hostIp : String, home : String, listenPort : String, listenAddress : String, elasticsearchUrl : String) extends ComponentConf(hostIp, s"$home/app/kibana","start.sh", s"$home/conf/kibana", "kibana.yml", 1) {
  override def mkScript: ConfFile = {
    val scriptContent =
      s"""
