...
class DataInitializer(h: Host, jwt: String, rootDigest: String, rootDigest2: String) {
  ...
  def uploadMetaDataOld(): Unit = withTry("upload meta data (old)") {
    ...
    awaitSeq(lb.sliding(chunkSize, chunkSize).map{ l =>
      ...
      httpPost(s"http://${h.ips.head}:9000/_cmd", bagOfInfotons,  NoType, TextPlain, Seq("op"->"init", "format"->"jsonw", accessLogMarker("uploadMetaDataOld")))
    }.toSeq)
  }

  def uploadNameSpaces(): Unit = withTry("upload namespaces") {
    await(httpPost(s"http://${h.ips.head}:9000/_in", ResourseFilename("meta_ns_prefixes_snapshot_infotons.nt"), NoType, TextPlain, Seq("format" -> "ntriples", accessLogMarker("uploadNameSpaces"))))
  }
  ...
  def uploadMetaData(): Unit = withTry("upload meta data") {
    ...
    awaitSeq(lb.sliding(chunkSize, chunkSize).map{ l =>
      val bagOfInfotons = """{"type" : "BagOfInfotons","infotons" : [""" + l.mkString(",") + """]}"""
      httpPost(s"http://${h.ips.head}:9000/_cmd", bagOfInfotons,  NoType, TextPlain, Seq("op"->"init", "format"->"jsonw"))
    }.toSeq)
  }

  def uploadSampleData(): Unit = withTry("upload sample data") {
    await(httpPost(s"http://${h.ips.head}:9000/_in", ResourseFilename("tr.sample.n3"), NoType, TextPlain, Seq("format" -> "n3")))
  }
  ...
  def updateKnownHosts(): Unit = withTry("update known hosts") {
    ...
    val payload = (Seq(markReplace) ++ h.ips.map(ip => s"""<cmwell://meta/sys> <cmwell://meta/nn#known-cmwell-hosts> "$ip:9000" . """)).mkString("\n")
    await(httpPost(s"http://${h.ips.head}:9000/_in", payload, NoType, TextPlain, Seq("op"->"init", "format"->"ntriples", accessLogMarker("updateKnownHosts"))))
  }
  ...
  def uploadBasicUserInfotons(host : String): Unit = withTry("upload built-in UserInfotons") {
    ...
    val denyForAnon = Seq("/meta/auth").map(p =>s"""{"id":"$p","recursive":true,"sign":"-","permissions":"rw"}""").mkString(",")
    ...
    val userInfotons = Map(
      ...
      "root" -> s"""{"digest":"$rootDigest","digest2":"$rootDigest2","paths":[${allowRoot("rw")}],"operations":["Admin","Overwrite"],"roles":[]}""",
      "pUser" -> s"""{"paths":[${allowRoot("rw")}],"operations":["Admin","Overwrite"],"roles":[]}""",
  ...
  def uploadDirectory(path : String, url : String, exclude: Set[String] = Set()): Unit = withTry(s"upload directory to $url") {
    ...
    val res = awaitSeq(filesToUpload.map(file =>
      uploadFile(file.getPath, goodUrl + file.getPath.substring(pathSize)).map(p.sideEffectInc).recover { case _ =>
        {h.info(s"Failed to upload eli3 ${file.getName}"); Thread.dumpStack()}
      }
    ))
    p.complete()
    res
  ...
  def logVersionUpgrade(host: String): Unit = withTry("log version") {
    ...
    val payload = s"""{"logType":"system-version","git_commit_version":"${BuildInfo.gitCommitVersion}","release":"${BuildInfo.release}"}"""
    await(httpPost(httpPath, payload, Obj, Json, Nil))
  }

  private def httpPost(url: String, payload: ResourseFilename, postType: PostType, contentType: ContentType, queryParameters: Seq[(String,String)]): Future[String] =
    httpPost(url, scala.io.Source.fromFile(s"resources/${payload.fileName}").map(_.toByte).toArray, postType, contentType, queryParameters)

  private def httpPost(url: String, payload: String, postType: PostType, contentType: ContentType, queryParameters: Seq[(String,String)]): Future[String] =
    httpPost(url, payload.getBytes("UTF-8"), postType, contentType, queryParameters)

  private def httpPost(url: String, payload: Array[Byte], postType: PostType, contentType: ContentType, queryParameters: Seq[(String,String)]): Future[String] = {
    import SimpleResponse.Implicits.UTF8StringHandler
    import scala.concurrent.duration._
  ...
  private def withTry(desc: String)(block: => Unit): Unit = Try(block).recover { case t =>
