...
class CommandSpec extends FlatSpec with Matchers {
  ...
  "command encode decode" should "be successful" in {
    ...
    val objInfo = ObjectInfoton("/command-test/objinfo1",
                                "dc_test",
                                None,
                                Map("name" -> Set[FieldValue](FString("gal"), FString("yoav"))))
    // create all commands case classes for serialization testing
    val linkInfo01 = LinkInfoton("/command-test/objinfo1",
                                 "dc_test",
                                 Map("name" -> Set[FieldValue](FString("gal"), FString("yoav"))),
                                 "/mark",
                                 LinkType.Forward)
    val linkInfo02 = LinkInfoton("/command-test/objinfo1",
                                 "dc_test",
                                 Map("name" -> Set[FieldValue](FString("gal"), FString("yoav"))),
                                 "/mark",
                                 LinkType.Permanent)
    val linkInfo03 = LinkInfoton("/command-test/objinfo1",
                                 "dc_test",
                                 Map("name" -> Set[FieldValue](FString("gal"), FString("yoav"))),
                                 "/mark",
                                 LinkType.Temporary)
    ...
    val cmdDeletePathAttributeValues =
      DeleteAttributesCommand("/command-test/deletePath",
                              Map("name" -> Set[FieldValue](FString("gal"), FString("yoav"))),
                              new DateTime)
    ...
    val deleteF = Map("location" -> Set[FieldValue](FString("usa")),
  ...
  "file infoton decode" should "be successful" in {
    System.setProperty("dataCenter.id", "dc_test")
    val fInfoton = FileInfoton(path = "/stam/kacha",
                               dc = "dc_test",
  ...
  "TrackingID de/serialization" should "be enabled for any SingleCommand" in {
    val objInfot = ObjectInfoton("/command-test/objinfo1",
                                 "dc_test",
                                 None,
                                 DateTime.now(DateTimeZone.UTC),
  ...
  "TrackingID de/serialization" should "be enabled for any IndexCommand" in {
    val objInfot = ObjectInfoton("/command-test/objinfo1",
                                 ...
                                 DateTime.now(DateTimeZone.UTC),
                                 Map("name" -> Set[FieldValue](FString("Neta-li"), FString("Shalev"))))
    val iCommand = IndexNewInfotonCommand(objInfot.uuid,
                                          true,
                                          objInfot.path,
                                          None,
                                          "",
