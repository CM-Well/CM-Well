...
class CommandSpec extends FlatSpec with Matchers {
  ...
  "command encode decode" should "be successful" in {
    ...
    val cmdDeletePath =
      DeletePathCommand("/command-test/deletePath", new DateTime)
    val cmdDeletePathAttributeValues =
    ...
    val deleteF =
      Map("location" -> Set[FieldValue](FString("usa")),
          "company" -> Set[FieldValue](FString("IBM"), FString("FACEBOOK")))

    val updateF = Map("location" -> Set[FieldValue](FString("israel")),
                      "company" -> Set[FieldValue](FString("GOOGLE")))

    val cmdUpdate =
      UpdatePathCommand("/command-test/update", deleteF, updateF, updateDate)

    val cmds =
      Vector(cmdWrite,
             cmdDeletePathAttributeValues,
             cmdDeletePath /*, cmdMerged1*/,
             cmdUpdate)

    val c = Vector(WriteCommand(linkInfo01),
                   WriteCommand(linkInfo02),
                   WriteCommand(linkInfo03))
    ...
    for (item <- cmds) {
      ...
      cmpCommand match {
        case WriteCommand(infoton, trackingID, prevUUID) =>
          ...
          infoton.fields.get("name").size should equal(
            objInfo.fields.get("name").size
          );
          infoton.lastModified.isEqual(objInfo.lastModified) should equal(true)
        case DeleteAttributesCommand(path,
                                     fields,
                                     lastModified,
                                     trackingID,
                                     prevUUID) =>
          path should equal(cmdDeletePathAttributeValues.path)
        case DeletePathCommand(path, lastModified, trackingID, prevUUID) =>
          path should equal(cmdDeletePath.path)
        ...
        case OverwriteCommand(_, trackingID) =>
          ??? //TODO: add tests for OverwriteCommand
      }
    }
  }
  ...
  "file infoton decode" should "be successful" in {
    System.setProperty("dataCenter.id", "dc_test")
    val fInfoton = FileInfoton(
      path = "/stam/kacha",
      dc = "dc_test",
      content = Some(FileContent("test text".getBytes("UTF-8"), "text/plain"))
    )
    val payload: Array[Byte] = CommandSerializer.encode(WriteCommand(fInfoton))
    val cmpCommand = CommandSerializer.decode(payload)
    val wc = cmpCommand.asInstanceOf[WriteCommand]
    wc.infoton.asInstanceOf[FileInfoton].content.get.mimeType should equal(
      "text/plain"
    )
  ...
  "CommandRef" should "be successfully encoded/decoded" in {
    ...
    CommandSerializer.decode(CommandSerializer.encode(commandRef)) should equal(
      commandRef
    )
  }

  "IndexNewInfotonCommand with infoton" should "be successfully encoded/decoded" in {
    val infoton = ObjectInfoton("/cmt/cm/news/1",
                                "dc",
                                None,
                                DateTime.now(DateTimeZone.UTC),
                                Map("a" -> Set[FieldValue](FString("b"))))
    val indexCommand = IndexNewInfotonCommand(infoton.uuid,
                                              true,
                                              infoton.path,
                                              Some(infoton),
                                              "")
    ...
    val decodedCommand =
      CommandSerializer.decode(payload).asInstanceOf[IndexNewInfotonCommand]
  ...
  "IndexNewInfotonCommand without infoton" should "be successfully encoded/decoded" in {
    val infoton = ObjectInfoton("/cmt/cm/news/1",
                                ...
                                Map("a" -> Set[FieldValue](FString("b"))))
    val indexCommand =
      IndexNewInfotonCommand(infoton.uuid, true, infoton.path, None, "")
    val payload = CommandSerializer.encode(indexCommand)
    val decodedCommand =
      CommandSerializer.decode(payload).asInstanceOf[IndexNewInfotonCommand]
    decodedCommand should equal(indexCommand)
  ...
  "TrackingID de/serialization" should "be enabled for any IndexCommand" in {
    ...
    val iCommand =
      IndexNewInfotonCommand(
        objInfot.uuid,
        true,
        objInfot.path,
        None,
        "",
        Seq(StatusTracking("sweet", 2), StatusTracking("kids", 1))
      )
    val eCommand = CommandSerializer.encode(iCommand)
    val dCommand =
      CommandSerializer.decode(eCommand).asInstanceOf[IndexNewInfotonCommand]
  ...
  "prevUUID de/serialization" should "be enabled for any SingleCommand" in {
    ...
    val wCommand =
      WriteCommand(objInfot, None, Some("0123456789abcdef0123456789abcdef"))
    val eCommand = CommandSerializer.encode(wCommand)
    val dCommand = CommandSerializer.decode(eCommand).asInstanceOf[WriteCommand]
    dCommand should equal(wCommand)
  }
