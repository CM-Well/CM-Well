...
object JsonSerializer extends AbstractJsonSerializer with LazyLogging {
  ...
  def decodeTrackingIDWithParser(
    jsonParser: JsonParser
  ): Option[Either[Vector[StatusTracking], String]] = {
    if (jsonParser.nextToken() == JsonToken.VALUE_STRING)
      Some(Right(jsonParser.getText()))
    else {
      assume(
        jsonParser.currentToken == JsonToken.START_ARRAY,
        s"expected value of tid is either a string or an array"
      )
  ...
  def decodeCommandWithParser(originalJson: Array[Byte],
    ...
    jsonParser.getCurrentName() match {
      case "type" =>
        logger.error("This version (0) is not supported anymore!!!"); !!!
      case "version" => {
        ...
        ver match {
          ...
          case "6" =>
            JsonSerializer6.decodeCommandWithParser(
              jsonParser,
              tidOpt,
              prevUUIDOpt
            )
  ...
  private def encodeCommandWithGenerator(command: Command,
                                         jsonGenerator: JsonGenerator): Unit = {
    jsonGenerator.writeStartObject()
    jsonGenerator.writeStringField(
      "version",
      cmwell.util.build.BuildInfo.encodingVersion
    )
    command match {
    ...
    command match {
      ...
      case DeleteAttributesCommand(
        ...
        jsonGenerator.writeStringField(
          "lastModified",
          dateFormatter.print(lastModified)
        )
      ...
      case UpdatePathCommand(
        ...
        jsonGenerator.writeStringField("path", path)
        encodeUpdateFieldsWithGenerator(
          deleteFields,
          updateFields,
          jsonGenerator
        )
  ...
  private def encodeUpdateFieldsWithGenerator(
  ...
  private def encodeFieldsWithGenerator(fields: Map[String, Set[FieldValue]],
    ...
    def fullEncodeFieldValue(fv: FieldValue, jp: JsonGenerator): Unit =
      fv match {
        case FString(str, l, q) =>
          jp.writeString(s"s${l.getOrElse("")}\n${q.getOrElse("")}\n$str")
        case FBoolean(bool, q) =>
          jp.writeString(s"b${q.getOrElse("")}\n${bool.toString.head}")
        case FReference(fr, q) => jp.writeString(s"r${q.getOrElse("")}\n$fr")
        case FDate(dateTime, q) =>
          jp.writeString(s"d${q.getOrElse("")}\n$dateTime")
        case FInt(int, q)   => jp.writeString(s"i${q.getOrElse("")}\n$int")
        case FLong(long, q) => jp.writeString(s"j${q.getOrElse("")}\n$long")
        case FBigInt(bigInt, q) =>
          jp.writeString(s"k${q.getOrElse("")}\n$bigInt")
        case FFloat(float, q) => jp.writeString(s"f${q.getOrElse("")}\n$float")
        case FDouble(double, q) =>
          jp.writeString(s"g${q.getOrElse("")}\n$double")
        case FBigDecimal(bigDecimal, q) =>
          jp.writeString(s"h${q.getOrElse("")}\n$bigDecimal")
    ...
    def escapeString(s: String): String =
      if (!s.isEmpty && s.head == '#' && !toEs) s"#$s" else s
...
//    }
    ...
    val fieldsByType = fields.flatMap {
      case (k, vs) if toEs && vs.isEmpty =>
        ((k -> vs) :: typeChars.map(typeChar => (typeChar + k) -> vs)).toMap
      case (k, vs) if toEs =>
        vs.groupBy { v =>
          s"${prefixByType(v)}$k"
        }
  ...
  private def encodeInfotonWithGenerator(infoton: Infoton,
    ...
    jsonGenerator.writeStringField(
      "lastModified",
      dateFormatter.print(infoton.lastModified)
    )
...
//      p.success(None)//(Some(idxT))
    ...
    infoton match {
      case FileInfoton(
          ...
          _,
          _,
          Some(FileContent(dataOpt, mimeType, dl, dp)),
          _
          ) =>
