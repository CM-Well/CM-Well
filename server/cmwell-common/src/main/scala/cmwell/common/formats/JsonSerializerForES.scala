...
object JsonSerializerForES
  ...
  private def encodeFieldsWithGenerator(fields: Map[String, Set[FieldValue]],
    ...
    def prefixByType(fValue: FieldValue): String =
      FieldValue.prefixByType(fValue) match {
        case 's' => ""
        case chr => s"$chr$$"
      }

    val namespaceToTypedFieldsMap =
      fields.foldLeft(Map.empty[String, Map[String, Set[FieldValue]]]) {
        case (m, (key, values)) => {
          ...
          val innerMapAddition = {
            ...
            if (values.isEmpty)
              ((fieldName -> values) :: typeChars.map(
                typeChar => (typeChar + fieldName) -> values
              )).toMap
            else
              values.groupBy { v =>
