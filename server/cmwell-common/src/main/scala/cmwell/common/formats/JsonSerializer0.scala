...
import cmwell.common.MergedInfotonCommand
 ...
 */
object JsonSerializer0 extends AbstractJsonSerializer with LazyLogging {
        ...
        jType match {
          ...
          case JsonToken.VALUE_STRING => {
            ...
            val fv: FieldValue = v.head match {
              case 'd' => FDate(v.tail)
              case 'f' => FFloat(v.tail.toFloat)
              case 'g' => FDouble(v.tail.toDouble)
              case 'h' => FBigDecimal(BigDecimal(v.tail).underlying())
              case 'i' => FInt(v.tail.toInt)
              case 'j' => FLong(v.tail.toLong)
              case 'k' => FBigInt(BigInt(v.tail).underlying())
              case 'r' => FReference(v.tail)
              case 's' => FString(v.tail)
              case 'x' => {
                val arr = v.tail.split('\n')
                val (uri,value) = (arr(1),arr.drop(2).mkString("\n"))
                FExternal(value,uri)
              }
              case _ => ???
            }
  ...
  private def decodeInfotonWithParser(jsonParser:JsonParser):Infoton = {
    ...
    val dc = SettingsHelper.dataCenter
    ...
    infotonType match {
      case "ObjectInfoton" =>
        ...
        ObjectInfoton(path, dc, None, lastModified, fields)
      case "LinkInfoton" =>
        ...
        LinkInfoton(path, dc, None, lastModified, fields, linkTo, linkType)
      case "FileInfoton" =>
        ...
        FileInfoton(path, dc, None, lastModified, fields, fileContent)
    }
  }

  def decodeCommandWithParser(jsonParser:JsonParser):Command = {
      // expecting type field
    assume(jsonParser.getCurrentToken()==JsonToken.FIELD_NAME && "type".equals(jsonParser.getCurrentName()), s"expected 'type' field name\n${jsonParser.getCurrentLocation.toString}")
    assume(jsonParser.nextToken()==JsonToken.VALUE_STRING, s"expected value for 'type' field\n${jsonParser.getCurrentLocation.toString}")
    ...
    commandType match {
      ...
      case "BulkCommand" =>
        ...
        while(jsonParser.nextToken() != JsonToken.END_ARRAY){
          assume(jsonParser.nextToken()==JsonToken.FIELD_NAME, s"expected field ${jsonParser.getCurrentLocation.toString}")
          commands += decodeCommandWithParser(jsonParser)
        }
