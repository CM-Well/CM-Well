...
object JsonSerializer6 extends AbstractJsonSerializer with LazyLogging {
  ...
  def decodeCommandWithParser(jsonParser:JsonParser, tidOpt: Option[Either[Vector[StatusTracking],String]], prevUUIDOpt: Option[String]):Command = {
    ...
    commandType match {
      case "WriteCommand" =>
        ...
        command = WriteCommand(infoton, tidOpt.flatMap(_.right.toOption), prevUUIDOpt)
      ...
      case "IndexNewInfotonCommand" =>
        ...
        assume(jsonParser.nextToken()==JsonToken.VALUE_STRING, s"expected value for 'path' field\n${jsonParser.getCurrentLocation.toString}")
        val path = jsonParser.getText
        assume(jsonParser.nextToken()==JsonToken.FIELD_NAME && "indexName".equals(jsonParser.getCurrentName()),
          s"expected 'indexName' field name\n${jsonParser.getCurrentLocation.toString}")
        assume(jsonParser.nextToken()==JsonToken.VALUE_STRING, s"expected value for 'indexName' field\n${jsonParser.getCurrentLocation.toString}")
        ...
        command = IndexNewInfotonCommand(uuid, isCurrent, path, infotonOpt, indexName, tidOpt.fold(Seq.empty[StatusTracking])(_.left.get))
      ...
      case "IndexExistingInfotonCommand" =>
        ...
        assume(jsonParser.nextToken()==JsonToken.VALUE_STRING, s"expected value for 'indexName' field\n${jsonParser.getCurrentLocation.toString}")
        val indexName = jsonParser.getText
        command = IndexExistingInfotonCommand(uuid, weight, path, indexName, tidOpt.fold(Seq.empty[StatusTracking])(_.left.get))
        assume(jsonParser.nextToken()== JsonToken.END_OBJECT, s"expected end of command object\n${jsonParser.getCurrentLocation.toString}")
      ...
      case "DeleteAttributesCommand" =>
        ...
        assume(jsonParser.nextToken()==JsonToken.VALUE_STRING, s"expected value for 'lastModified' field\n${jsonParser.getCurrentLocation.toString}")
        val lastModified = dateFormatter.parseDateTime(jsonParser.getText())
        command = DeleteAttributesCommand(path, fields, lastModified, tidOpt.flatMap(_.right.toOption), prevUUIDOpt)
        //expecting end of command object
        assume(jsonParser.nextToken()== JsonToken.END_OBJECT, s"expected end of command object\n${jsonParser.getCurrentLocation.toString}")
      ...
      case "DeletePathCommand" =>
        ...
        assume(jsonParser.nextToken()==JsonToken.VALUE_STRING, s"expected value for 'lastModified' field\n${jsonParser.getCurrentLocation.toString}")
        val lastModified = dateFormatter.parseDateTime(jsonParser.getText())
        command = DeletePathCommand(path, lastModified, tidOpt.flatMap(_.right.toOption), prevUUIDOpt)
        //expecting end of command object
        assume(jsonParser.nextToken()== JsonToken.END_OBJECT, s"expected end of command object\n${jsonParser.getCurrentLocation.toString}")
      ...
      case "UpdatePathCommand" =>
        ...
        assume(jsonParser.nextToken()==JsonToken.VALUE_STRING, s"expected value for 'lastModified' field\n${jsonParser.getCurrentLocation.toString}")
        val lastModified = dateFormatter.parseDateTime(jsonParser.getText())
        command = UpdatePathCommand(path, deleteFields, updateFields, lastModified, tidOpt.flatMap(_.right.toOption), prevUUIDOpt)
        //expecting end of command object
        assume(jsonParser.nextToken()== JsonToken.END_OBJECT, s"expected end of command object\n${jsonParser.getCurrentLocation.toString}")
