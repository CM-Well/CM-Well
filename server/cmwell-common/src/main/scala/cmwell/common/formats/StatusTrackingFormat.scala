...
object StatusTrackingFormat extends JavaTokenParsers {
  ...
  val restOfTheInputAsString = Parser { in =>
    ...
    Success(
      source.subSequence(offset, length).toString,
      in.drop(length - offset)
    )
  }
  ...
  def parseTrackingStatus(input: String): Try[StatusTracking] =
    parseAll(statusTracking, input) match {
