...
object MimeTypeIdentifier {
  ...
  def identify(bytes: Array[Byte], name: String) =
    Try(tika.detect(bytes, name)).toOption
  ...
  def isTextual(mimeType: String) =
    (mimeType.contains("text") || textualMimeTypes.exists(_.equals(mimeType)))
