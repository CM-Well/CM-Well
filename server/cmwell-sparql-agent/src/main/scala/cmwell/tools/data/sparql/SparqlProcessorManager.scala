...
object SparqlProcessorManager {
  ...
  case class Job(config: Config, path: Path, killSwitch: KillSwitch, reporter: ActorRef) {
...
class SparqlProcessorManager extends Actor with LazyLogging {
  ...
  private val stpSettings = ConfigFactory.load()
  private val hostConfigFile = stpSettings.getString("cmwell.agents.sparql-triggered-processor.host-config-file")
  private val hostUpdatesSource = stpSettings.getString("cmwell.agents.sparql-triggered-processor.host-updates-source")
  private val hostWriteOutput = stpSettings.getString("cmwell.agents.sparql-triggered-processor.host-write-output")
  private val materializedViewFormat = stpSettings.getString("cmwell.agents.sparql-triggered-processor.format")
  private val pathAgentConfigs = stpSettings.getString("cmwell.agents.sparql-triggered-processor.path-agent-configs")
  private val writeToken = stpSettings.getString("cmwell.agents.sparql-triggered-processor.write-token")
  private val initDelay = stpSettings.getDuration("cmwell.agents.sparql-triggered-processor.init-delay").toMillis.millis
  private val interval = stpSettings.getDuration("cmwell.agents.sparql-triggered-processor.config-polling-interval").toMillis.millis
  ...
  def receive = {
    ...
    GridReceives.monitoring(sender).orElse {
      case CheckConfig => getYamlConfigs().map(AnalyzeReceivedConfig.apply) pipeTo self
      case AnalyzeReceivedConfig(received) =>
        val configs = received.map { case (path, config) =>
        ...
        handleSensors(configs, activeJobs, nonActiveConfigs)
      ...
      case JobsToDisable(jobs) =>
        jobs.foreach { case (path, job) =>
          // stop jobs and store it in non-active job list
          job.killSwitch.shutdown()
          //todo: how do we know that the job stopped???
          job.reporter ! PoisonPill
          activeJobs -= path
          nonActiveConfigs += (path -> job.config)
        }
      case AddToNonActive(configs) => nonActiveConfigs ++= configs
      case NonActiveJobsToRemove(configs) => nonActiveConfigs --= configs.keySet
      case StopJob(registered, path, job) if registered =>
        job.killSwitch.shutdown()
        //todo: how do we know the job stopped???
        activeJobs -= path
        job.reporter ! PoisonPill
  ...
  /**
    ...
    */
  def handleSensors(received: Configs, currentActiveJobs: Jobs, currentNonActiveConfigs: Configs) = {
    ...
    storedJobPathsWithStatus.map { storedJobs =>
      ...
      val pathsToDisable = nonActiveStoredJobs.keySet intersect received.keySet.diff(currentActiveJobs.keySet ++ currentNonActiveConfigs.keySet)
      received.filterKeys(pathsToDisable.contains)
    }.map(AddToNonActive.apply) pipeTo self
    // disable jobs which have new active=false status
    storedJobPathsWithStatus.map {
      _.filter { case (_, isActive) => !isActive }
    }
      .map(nonActiveStoredJobs => currentActiveJobs filterKeys nonActiveStoredJobs.keySet)
      .map(JobsToDisable.apply) pipeTo self
    // start jobs which have a new 'active' status
    storedJobPathsWithStatus.map {
      _.filter { case (_, isActive) => isActive }
    }
      .map(newActiveStoredJobs => currentNonActiveConfigs filterKeys newActiveStoredJobs.keySet)
      .map(registerToStart)
    // stop and remove jobs which no longer exist
    val jobsToStopAndRemove = currentActiveJobs.filterKeys(currentActiveJobs.keySet diff received.keySet)
    registerJobsToStop(jobsToStopAndRemove)
    // remove non active jobs which are no longer exist
    self ! NonActiveJobsToRemove(currentNonActiveConfigs.filterKeys(currentNonActiveConfigs.keySet diff received.keySet))
    // find new configs (did not appear before) and register them to start
    storedJobPathsWithStatus.map { storedJobs =>
      val nonActiveStoredJobs = storedJobs.filter { case (_, isActive) => !isActive }
      val pathsToStart = received.keySet diff (currentActiveJobs.keySet ++ currentNonActiveConfigs.keySet ++ nonActiveStoredJobs.keySet)
      received.filterKeys(pathsToStart.contains)
    }.map(registerToStart)
  }
  ...
  /**
    ...
    */
  def stringifyActiveJobs(activeJobs: Jobs, currentNonActiveConfigs: Configs): Future[Iterable[Table]] = {
  ...
  def getYamlConfigs() = {
    ...
    getPathsOfConfigInfotons()
      ...
      .map {
        case Some((path, data)) => path -> extractConfigStringFromInfotonData(data)
        case None => ???
      }
