...
final case class PartialDone(completed: Int, total: Int)
    extends TrackingStatus {
...
object TrackingStatus {
  ...
  def apply(value: String): TrackingStatus = value match {
    ...
    case "Failed"                                => Failed
    case partialDoneRegex(completed, total) =>
      PartialDone(completed.toInt, total.toInt)
    case other =>
      throw new IllegalArgumentException(
        s"$other is not a valid TrackingStatus"
      )
  }
