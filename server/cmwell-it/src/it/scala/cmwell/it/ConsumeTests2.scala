...
class ConsumeTests2 extends AsyncFunSpec with Inspectors with Matchers with Helpers with fixture.NSHashesAndPrefixes with LazyLogging {
  describe("consume API") {
    ...
    val firstConsumableChunk: Future[ConsumeResults] = {
      val file = Source.fromURL(this.getClass.getResource("/consumeable-data3.nt")).mkString
    ...
    val f2 = f1.flatMap(t => Http.get(path, List("op" -> "consume", "position" -> t._2.get, "length-hint" -> "7", "without-last-modified" -> "")).map(requestHandler))
