...
import akka.cluster.ClusterEvent.{ClusterDomainEvent, MemberRemoved, UnreachableMember, MemberUp}
...
import k.grid.{GridReceives, Grid}
...
abstract class MonitorActor(var checker : Checker, hostName : String, start : Int, interval : Int, clusterListener : Boolean = false) extends Actor with LazyLogging {
...
class CassandraMonitorActor(hostName : String, start : Int, interval : Int) extends MonitorActor(CassandraChecker, hostName, start, interval) {
  override def createUpdateState(cs : ComponentState): Set[UpdateStat] =  Set(UpdateCasStat(hostName, cs))
}

class ElasticsearchMonitorActor(hostName : String, start : Int, interval : Int) extends MonitorActor(ElasticsearchChecker, hostName, start, interval) {
...
class ZookeeperMonitorActor(hostName : String, start : Int, interval : Int) extends MonitorActor(ZookeeperChecker, hostName, start, interval) {
...
class SystemMonitorActor(hostName : String, start : Int, interval : Int) extends MonitorActor(SystemChecker, hostName, start, interval) {
