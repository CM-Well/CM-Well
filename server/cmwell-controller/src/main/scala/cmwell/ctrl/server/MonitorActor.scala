...
abstract class MonitorActor(var checker: Checker,
  ...
  override def preStart(): Unit = {
    scheduleCancellable = system.scheduler.schedule(
      interval milli,
      interval milli,
      self,
      SendNotification
  ...
  override def preRestart(reason: Throwable, message: Option[Any]): Unit = {
    logger.info(
      s"preRestart of actor $self reason: $reason when processing $message"
    )
  ...
  override def receive: Receive = GridReceives.monitoring(sender).orElse {
    case SendNotification =>
      if (!busy) {
        ...
        logger
          .warn(
            s"Skipping check iteration, last result: ${checker.getLastStates(1).headOption}"
          )
      }
    case cs: ComponentState =>
      checker match {
        case rch: RestarterChecker => {
          ...
          } else if (cs.getColor == GreenStatus && restartCancellable != null)
            self ! CancelRestart
        }
        case _ => // Do nothing
      }
      ...
      logger.debug(
        s"Storing $cs in using actor $self in path ${self.path} states store."
      )
    ...
    case ScheduleRestart(d, rch) => {
      restartCancellable =
        Grid.system.scheduler.scheduleOnce(d, self, DoRestart(rch))
    }
    case DoRestart(rch) => {
      rch.doRestart
...
class DcMonitorActor(hostName: String, start: Int, interval: Int)
  ...
  override def createUpdateState(cs: ComponentState): Set[UpdateStat] =
    cs match {
      case DcStatesBag(states, ch, genTime) =>
        states.map(dcStat => UpdateDcStat(dcStat._1, dcStat._2)).toSet
    }
}
...
class SystemMonitorActor(hostName: String, start: Int, interval: Int)
  ...
  override def createUpdateState(cs: ComponentState): Set[UpdateStat] =
    Set(UpdateSystemStat(hostName, cs))
}
