...
case class AddNode(node: String) extends Task with LazyLogging {
  ...
  private def startElasticsearch(cmd: ActorSelection,
    ...
    Grid.system.scheduler.scheduleOnce(60.seconds) {
      (HealthActor.ref ? GetElasticsearchDetailedStatus)
        .mapTo[ElasticsearchGridStatus]
        .map { f =>
          f.getStatesMap.get(node) match {
            case Some(s) =>
              if (s.getColor == GreenStatus || s.getColor == YellowStatus)
                prom.success(Unit)
  ...
  private def startCassandra(cmd: ActorSelection, prom: Promise[Unit]): Unit = {
    ...
    cmd ! StartCassandra
    Grid.system.scheduler.scheduleOnce(60.seconds) {
      (HealthActor.ref ? GetCassandraDetailedStatus)
        .mapTo[CassandraGridStatus]
        .map { f =>
          f.getStatesMap.get(node) match {
            case Some(s) =>
