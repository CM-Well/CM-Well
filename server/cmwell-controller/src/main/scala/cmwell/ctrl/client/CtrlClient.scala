...
object CtrlClient extends LazyLogging {
  ...
  def getHealthControl(joinToHost: String, retries: Int = 10): Unit = {
    val fa = Grid
      .selectActor("HealthActor-router", GridJvm(joinToHost, Jvms.DC))
      .resolveOne(15.seconds)
  ...
  def getCassandraDetailedStatus: Future[Map[String, CassandraState]] = {
    (healthActor ? GetCassandraDetailedStatus)
      .mapTo[Map[String, CassandraState]]
  }

  def getElasticsearchDetailedStatus
    : Future[Map[String, ElasticsearchState]] = {
    (healthActor ? GetElasticsearchDetailedStatus)
      .mapTo[Map[String, ElasticsearchState]]
  }
