...
trait ComponentGridState {
  protected var states: Map[String, ComponentState] =
    Map.empty[String, ComponentState]
  def name: String
  ...
  def update(host: String, cs: ComponentState) {
    ...
    afterUpdate
    raiseEvents(host)(previousState, currentState)
      .foreach(state => HealthActor.ref ! state)
  ...
  def getCurrentState(host: String,
                      previousState: ComponentState,
                      receivedState: ComponentState): ComponentState =
    receivedState

  def raiseEvents(
    ip: String
  ): PartialFunction[(ComponentState, ComponentState), Set[ComponentEvent]]
...
class WebGridStatus extends ComponentGridState with AlertReporter {
  ...
  private[this] def updateWsColor() {
    ...
    if (greens < colors.size) {
      logger.warn(
        s"updateWsColor: HC detects some non-greens ws, states = $states"
      )
    }
  ...
  def checkRestart(ip: String): Unit = {
    ...
    if (c >= webFailCount) {
      ...
      logger.warn(
        s"The Web server on node $ip malfunctioned $webFailCount times and therefore was restarted."
      )
      alert(
        s"The Web server on node $ip malfunctioned $webFailCount times and therefore was restarted.",
        YELLOW,
...
class DcGridStatus extends ComponentGridState with LazyLogging {
  ...
  override def raiseEvents(
    ...
    case (s1: DcCouldNotGetDcStatus, s2: DcCouldNotGetDcStatus) =>
      ...
      if (s1.getColor == GreenStatus && s2.getColor == YellowStatus)
        Set(DcCouldNotGetDcEvent(s2.dcId, s2.checkerHost))
    ...
    case (s1: DcNotSyncing, s2: DcNotSyncing)
        if (s1.getColor == YellowStatus && s2.getColor == RedStatus) =>
      logger.debug(
        s"DcGridStatus: case (s1 : DcNotSyncing, s2 : DcNotSyncing) if (s1.getColor == YellowStatus && s2.getColor == RedStatus )"
      )
      logger.debug(
        s"DcGridStatus: s1.getColor=${s1.getColor}, s2.getColor=${s2.getColor}"
      )
    ...
    case (s1: DcState, s2: DcSyncing) if s1.getColor == GreenStatus =>
      Set.empty[ComponentEvent]
...
class ZkGridStatus extends ComponentGridState with LazyLogging {
  ...
  override def getColor: StatusColor = {
    ...
    if (totalMachines < 3) {
      ...
      if (numOfOk == 3) GreenStatus
      else if (numOfOk == 2) YellowStatus
      else RedStatus
    }
  }
  ...
  override def raiseEvents(
    ...
    case (s1: ZookeeperReadOnly, s2: ZookeeperReadOnly) =>
      Set.empty[ComponentEvent]
    case (s1: ZookeeperNotOk, s2: ZookeeperNotOk) => Set.empty[ComponentEvent]
    case (s1: ZookeeperNotRunning, s2: ZookeeperNotRunning) =>
      Set.empty[ComponentEvent]
    case (s1: ZookeeperSeedNotRunning, s2: ZookeeperSeedNotRunning) =>
      Set.empty[ComponentEvent]
...
class CassandraGridStatus extends ComponentGridState {
  ...
  override def raiseEvents(
    ...
    case (_, s2: CassandraOk) =>
      ...
      partition._1.keySet
        .map(ip => CassandraNodeNormalEvent(ip)) ++ partition._2.keySet
        .map(ip => CassandraNodeDownEvent(ip))
...
class ElasticsearchGridStatus extends ComponentGridState with LazyLogging {
  ...
  def normalizeMasters() = { // AKA taking the horses back to the dovecote
    val runningMasters = {
      val esStates =
        states.map(x => (x._1, x._2.asInstanceOf[ElasticsearchState]))
    ...
    seedNodesThatShouldStartMaster.foreach(
      CommandActor.select(_) ! StartElasticsearchMaster
    )
    nodesThatShouldStopMaster.foreach(
      node =>
        delayedTask(15.seconds) {
          CommandActor.select(node) ! StopElasticsearchMaster
      }
    )
    ...
    if (seedNodesThatShouldStartMaster.size + nodesThatShouldStopMaster.size + replacementNewMasters.size == 0)
      logger.debug(
        "Normalizing ES Masters: Nothing to do (all ES-Masters are in the right places)"
      )
    else
      logger.info(
  ...
  override def raiseEvents(
    ...
    case (s1: ReportTimeout, s2: ReportTimeout) => Set.empty[ComponentEvent]
    case (s1: ElasticsearchGreen, s2: ElasticsearchGreen) =>
      Set.empty[ComponentEvent]
    case (s1: ElasticsearchYellow, s2: ElasticsearchYellow) =>
      Set.empty[ComponentEvent]
    case (s1: ElasticsearchRed, s2: ElasticsearchRed) =>
      Set.empty[ComponentEvent]
    case (s1: ElasticsearchBadCode, s2: ElasticsearchBadCode) =>
      Set.empty[ComponentEvent]
    case (s1: ElasticsearchDown, s2: ElasticsearchDown) =>
      Set.empty[ComponentEvent]
    ...
    case (_, s2: ElasticsearchBadCode) =>
      Set(ElasticsearchNodeBadCodeEvent(ip, s2.code))
