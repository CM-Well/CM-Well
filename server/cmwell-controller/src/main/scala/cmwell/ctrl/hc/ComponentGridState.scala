...
trait ComponentGridState {
  ...
  def getCurrentState(host : String, previousState : ComponentState, receivedState : ComponentState) : ComponentState = receivedState
...
class WebGridStatus extends ComponentGridState with AlertReporter {
  ...
  def checkRestart(ip : String): Unit = {
    ...
    if(c >= webFailCount) {
      ...
      alert(s"The Web server on node $ip malfunctioned $webFailCount times and therefore was restarted.", YELLOW, Some(ip), Some("Web"), Some(ip))
    } else {
...
class DcGridStatus extends ComponentGridState with LazyLogging {
  ...
  override def raiseEvents(ip: String): PartialFunction[(ComponentState, ComponentState), Set[ComponentEvent]] = {
    ...
    case (s1 : DcCouldNotGetDcStatus, s2 : DcCouldNotGetDcStatus) =>
      logger.debug(s"DcGridStatus: s1.getColor=${s1.getColor}(${s1.errorCounter}), s2.getColor=${s2.getColor}(${s2.errorCounter})")
      if(s1.getColor == GreenStatus && s2.getColor == YellowStatus) Set(DcCouldNotGetDcEvent(s2.dcId, s2.checkerHost))
      else if(s1.getColor == YellowStatus && s2.getColor == RedStatus) Set(DcCouldNotGetDcLongEvent(s2.dcId, s2.checkerHost))
      else Set.empty[ComponentEvent]

    case (s1 : DcNotSyncing, s2 : DcNotSyncing) if (s1.getColor == YellowStatus && s2.getColor == RedStatus ) =>
      logger.debug(s"DcGridStatus: case (s1 : DcNotSyncing, s2 : DcNotSyncing) if (s1.getColor == YellowStatus && s2.getColor == RedStatus )")
    ...
    case (s1 : DcNotSyncing, s2 : DcNotSyncing) if (s1.getColor == GreenStatus && s2.getColor == YellowStatus ) =>
      logger.debug(s"DcGridStatus: case (s1 : DcNotSyncing, s2 : DcNotSyncing) if (s1.getColor == GreenStatus && s2.getColor == YellowStatus )")
      logger.debug(s"DcGridStatus: s1.getColor=${s1.getColor}, s2.getColor=${s2.getColor}")
...
class CassandraGridStatus extends ComponentGridState  {
  ...
  override def raiseEvents(ip: String): PartialFunction[(ComponentState, ComponentState), Set[ComponentEvent]] = {
    ...
    case (s1 : CassandraOk, s2 : CassandraOk) =>
      ...
      (partitionNow._1.keySet -- partitionPrev._1.keySet).map(ip => CassandraNodeNormalEvent(ip)) ++ (partitionNow._2.keySet -- partitionPrev._2.keySet).map(ip => CassandraNodeDownEvent(ip))
    case (_, s2 : CassandraOk) =>
      val partition = s2.m.partition(item => item._2 == "UN")
      partition._1.keySet.map(ip => CassandraNodeNormalEvent(ip)) ++ partition._2.keySet.map(ip => CassandraNodeDownEvent(ip))
...
class ElasticsearchGridStatus extends ComponentGridState with LazyLogging {
  ...
  def normalizeMasters() = { // AKA taking the horses back to the dovecote
    ...
    nodesThatShouldStopMaster.foreach(node => delayedTask(15.seconds){ CommandActor.select(node) ! StopElasticsearchMaster })
    ...
    if(seedNodesThatShouldStartMaster.size + nodesThatShouldStopMaster.size + replacementNewMasters.size == 0)
      ...
      logger.info(s"Normalizing ES Masters: seedNodesThatShouldStartMaster[${p(seedNodesThatShouldStartMaster)}], " +
