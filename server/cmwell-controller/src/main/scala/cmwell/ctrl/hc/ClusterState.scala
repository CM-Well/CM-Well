...
object ClusterState extends LazyLogging with AlertReporter {
  ...
  def newState(cs: ClusterEvent) = {
    ...
    clusterLogger.info(
      s"current state: $currentState, event: $cs, new state: $newState"
    )
...
case class DownNodesDetected(nodes: Set[String])
...
//        ElasticsearchController.stopElasticsearchRebalancing
        logger.info(
          s"Starting grace time scheduler of $downNodesGraceTimeMinutes minutes"
        )
        ClusterState.esResumeRebalancingTask = Grid.system.scheduler
          .scheduleOnce(downNodesGraceTimeMinutes minutes) {
...
case class NodesJoinedDetected(nodes: Set[String])
    extends ClusterEvent
    with LazyLogging {
