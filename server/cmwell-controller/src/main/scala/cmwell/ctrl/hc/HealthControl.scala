...
object HealthControl {
  ...
  lazy val services = Map(
    "CassandraMonitorActor" -> ServiceInitInfo(
      classOf[CassandraMonitorActor],
      None,
      Grid.hostName,
      0,
      hcSampleInterval
    ),
    "DcMonitorActor" -> ServiceInitInfo(
      classOf[DcMonitorActor],
      None,
      Grid.hostName,
      0,
      hcSampleInterval
  ...
  def init: Unit = {
      ...
      Try(
        Await.result(
          cmwell.util.concurrent
            .retry(10, 5.seconds) { HealthActor.ref ? NodeJoinRequest(host) }
            .mapTo[JoinResponse],
          30.seconds
        )
      ) match {
    ...
    Grid.create(
      classOf[BgHealthMonitorActor],
      "BgHealthMonitorActor",
      Grid.hostName,
      0,
      bgSampleInterval
    )
    Grid.create(
      classOf[ElasticsearchMonitorActor],
      "ElasticsearchMonitorActor",
      Grid.hostName,
      0,
      hcSampleInterval
    )
    Grid.create(
      classOf[SystemMonitorActor],
      "SystemMonitorActor",
      Grid.hostName,
      0,
      hcSampleInterval
    ...
    Grid.create(
      classOf[ZookeeperMonitorActor],
      "ZookeeperMonitorActor",
      Grid.hostName,
      0,
      hcSampleInterval
    )
