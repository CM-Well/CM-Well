...
case class GcStats(hostName: String,
                   pid: Int,
                   roles: Set[String],
                   timeInGc: Long,
                   amountOfGc: Long,
...
class HealthActor extends Actor with LazyLogging with AlertReporter {
  ...
  override def preStart(): Unit = {
    ...
    system.scheduler.schedule(
      ...
      30 seconds,
      self,
      CheckElasticsearchMasterStatus
    )
  ...
  private[this] var gcStats: Map[(String, Int), (Set[String], Long, Long)] =
    Map.empty
  private[this] var heakupLatencyStats
    : Map[(String, Int), (Set[String], Long)] = Map.empty

  private[this] var hostNames: Map[String, String] = Map.empty

  private[this] val cancellableScheduler =
  ...
  private def getCasDetailed = {
    val detailedRes = hostIps.map { hip =>
      casStat.getState match {
        case CassandraOk(m, _, _) =>
          (hip._1, CassandraOk(m.filter(stats => hip._2.contains(stats._1))))
...
//          }
      ...
      case GcStats(hostName, pid, roles, timeInGc, amountOfGc, gcInterval) =>
        ...
        gcStats =
          gcStats.updated((hostName, pid), (roles, timeInGc, amountOfGc))
      ...
      case HeakupLatency(
        ...
        log.debug(s"GotHeakupLatencyStats $hostName $pid ${roles
          .toSeq(1)} $p25 $p50 $p75 $p90 $p99 $p99_5 $max")
        heakupLatencyStats =
          heakupLatencyStats.updated((hostName, pid), (roles, max))
      case GetActiveNodes => sender ! ActiveNodes(hostIps.keySet)
      case GetClusterStatus =>
        sender ! ClusterStatus(
          Grid.availableMachines,
          casStat.getState,
          esStat.getStatesMap.headOption
            .getOrElse("" -> ElasticsearchDown())
            ._2,
          ...
          esStat.getStatesMap
            .filter(_._2.asInstanceOf[ElasticsearchState].hasMaster)
            .keySet
        )
      ...
      case GetWebStatus =>
        sender ! (wsStat.getStatesMap.map(t => (t._1, t._2)), wsStat.getColor)
      case GetBgStatus =>
        sender ! (bgStat.getStatesMap.map(t => (t._1, t._2)), bgStat.getColor)
      ...
      case UpdateHostName(ip, hostName) =>
        hostNames = hostNames.updated(ip, hostName)
      ...
      case t: Task => {
        t match {
          case an @ AddNode(node) =>
            ...
            val ar =
              Grid.create(
                ...
                name = TaskExecutorActor.name + "AddNode" + System
                  .currentTimeMillis()
                  .toString
              )
          ...
          case cn @ ClearNode(node) =>
            ...
            val ar = Grid.createSomewhere(
              white = included,
              black = Set.empty[GridJvm],
              clazz = classOf[TaskExecutorActor],
              name = TaskExecutorActor.name + "ClearNode" + System
                .currentTimeMillis()
                .toString
            )
      ...
      case CheckForWsCrisis =>
        ...
        val (greens, notGreens) =
          wsStat.getStatesMap.partition(_._2.getColor == GreenStatus)
        if (greens.size <= 2 && notGreens.size > 0) {
          logger.warn(
            s"web services are not responding, restarting ${notGreens.map(_._1).mkString(", ")}."
          )
          notGreens
            .foreach(comp => CommandActor.select(comp._1) ! RestartWebserver)
...
object ZookeeperUtils {
  ...
  def zkServers =
    using(
      scala.io.Source
        .fromFile(s"${Config.cmwellHome}/conf/zookeeper/zoo.cfg")
    ) { source =>
      source
        .getLines()
        .filter(_.startsWith("server."))
        .map(_.dropWhile(_ != '=').takeWhile(_ != ':').substring(1))
        .toSet
    }
}
