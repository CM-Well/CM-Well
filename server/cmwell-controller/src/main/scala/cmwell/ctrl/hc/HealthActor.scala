...
case class HeakupLatency(hostName : String, pid : Int, roles : Set[String], p25: Long, p50: Long, p75: Long, p90: Long, p99: Long, p99_5: Long, max: Long)
...
class HealthActor extends Actor with LazyLogging with AlertReporter{
  ...
  override def receive: Receive =
    ...
    case CheckIdles =>
      ...
      kafkaStat.getStatesMap.foreach {
        kafS =>
...
//          }
    ...
    case GetClusterStatus => sender ! ClusterStatus(Grid.availableMachines,
                                                    casStat.getState,
    ...
    case NodeJoinRequest(node) =>
      log.info(s"Join request from: $node , current state: ${ClusterState.getCurrentState} , down nodes: ${DData.getDownNodes}")
    ...
    case t : Task => {
      t match {
        case an@AddNode(node) =>
          ...
          val ar = Grid.create(clazz = classOf[TaskExecutorActor], name = TaskExecutorActor.name + "AddNode" + System.currentTimeMillis().toString)
        ...
        case cn@ClearNode(node) =>
          ...
          val ar = Grid.createSomewhere(white = included,
