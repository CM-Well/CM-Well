...
//  }
    ...
    def json: String = {
      ...
      Json
        .obj(
          ...
          "trams" -> Json.obj("alert" -> Json.toJson(fields))
        )
        .toString
    }
  }
  ...
  def alert(msg: String,
    ...
    logger.info(
      s"""CM-WELL-ALERT alert_status=$color severity=${color.severity} alert_message='$msg' EventSource=ctrl${if (host.isDefined)
        ...
        ""}${if (eventKey.isDefined) s" EventKey=${eventKey.get}" else ""}${if (node.isDefined)
        s" Node=${node.get}"
      else ""}"""
    )
  ...
  def alert(clusterEvent: ClusterEvent) {
    clusterEvent match {
      case NodesJoinedDetected(nodes) =>
        alert(s"The nodes ${nodes.mkString(",")} joined the cluster.", GREEN)
      case DownNodesDetected(nodes) =>
        alert(s"The nodes ${nodes.mkString(",")} are down.", RED)
  ...
  def alert(host: String, joinResponse: JoinResponse) {
    joinResponse match {
      case JoinOk =>
        alert(s"$host was trying to join and was accepted.", GREEN, Some(host))
      ...
      case JoinShutdown =>
        alert(
          s"$host was trying to return to the cluster but was rejected because to much time passed.",
          YELLOW,
          Some(host)
        )
    }
  ...
  def alert(ce: ComponentEvent, hostName: Option[String]) {
    ce match {
      case WebNormalEvent(ip) =>
        alert(s"Web is normal at $ip", GREEN, Some(ip), Some("Web"), hostName)
      case WebBadCodeEvent(ip, code) =>
        alert(
          s"Web is returning code $code at $ip",
          YELLOW,
          Some(ip),
          Some("Web"),
          hostName
        )
      case WebDownEvent(ip) =>
        alert(s"Web is down at $ip", RED, Some(ip), Some("Web"), hostName)

      case BgOkEvent(ip) =>
        alert(s"Bg is normal at $ip", GREEN, Some(ip), Some("Bg"), hostName)
      case BgNotOkEvent(ip) =>
        alert(
          s"Bg is not ok (stuck?) at $ip",
          RED,
          Some(ip),
          Some("Bg"),
          hostName
      ...
      case CassandraNodeUnavailable(ip) =>
        alert(
          s"Cassandra is unavailable at $ip",
          YELLOW,
          getHostNameFromIp(ip),
          Some("Cassandra"),
          hostName
      ...
      case ElasticsearchNodeGreenEvent(ip) =>
        alert(
          s"Elasticsearch is green at $ip",
          GREEN,
          Some(ip),
          Some("Elasticsearch"),
          hostName
      ...
      case ElasticsearchNodeYellowEvent(ip) =>
        alert(
          s"Elasticsearch is yellow at $ip",
          YELLOW,
          Some(ip),
          Some("Elasticsearch"),
          hostName
      ...
      case ElasticsearchNodeBadCodeEvent(ip, code) =>
        alert(
          s"Elasticsearch is returning code $code at $ip",
          YELLOW,
          Some(ip),
          Some("Elasticsearch"),
          hostName
      ...
      case DcNormalEvent(dc, checker) =>
        alert(
          s"Syncing data from data center $dc.",
          GREEN,
          Some(checker),
          Some("DataCenter")
        )
      case DcNotSyncingEvent(dc, checker) =>
        alert(
          s"Couldn't sync from data center $dc.",
          YELLOW,
          Some(checker),
          Some("DataCenter")
        )
      case DcLostConnectionEvent(dc, checker) =>
        alert(
          s"Couldn't sync from data center $dc for very long time.",
          RED,
          Some(checker),
          Some("DataCenter")
        )
      case DcCouldNotGetDcEvent(dc, checker) =>
        alert(
          s"Couldn't get meta data from the data center $dc.",
          YELLOW,
          Some(checker),
          Some("DataCenter")
        )
      case DcCouldNotGetDcLongEvent(dc, checker) =>
        alert(
          s"Couldn't get meta data from the data center $dc for long time.",
          RED,
          Some(checker),
          Some("DataCenter")
        )
      case DcCouldNotGetDataEvent(checker) =>
        alert(
          s"Couldn't get local data center meta data.",
          RED,
          Some(checker),
          Some("DataCenter")
        )
      ...
      case ComponentChangedColorEvent(component, color) =>
        alert(
          s"$component changed to $color",
          AlertColor.fromStatusColor(color),
          eventKey = Some(component)
        )
