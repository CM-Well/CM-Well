...
//  }
  ...
  case class Alert(msg : String, color : AlertColor = GREEN, host : Option[String] = None, eventKey : Option[String] = None, node : Option[String] = None) {
    def json : String = {
      val fields = Map(
        "message" -> msg,
  ...
  def alert(host : String, joinResponse : JoinResponse) {
    joinResponse match {
      ...
      case JoinBootComponents => alert(s"$host was trying to return to the cluster and was accepted. It will start cm-well components on this machine.", GREEN, Some(host))
      case JoinShutdown => alert(s"$host was trying to return to the cluster but was rejected because to much time passed.", YELLOW, Some(host))
    }
  }
  ...
  def alert(ce : ComponentEvent, hostName : Option[String]) {
    ce match {
      ...
      case CassandraNodeUnavailable(ip) => alert(s"Cassandra is unavailable at $ip", YELLOW, getHostNameFromIp(ip), Some("Cassandra"), hostName)

      case ElasticsearchNodeGreenEvent(ip) => alert(s"Elasticsearch is green at $ip", GREEN, Some(ip), Some("Elasticsearch"), hostName)
      case ElasticsearchNodeYellowEvent(ip) => alert(s"Elasticsearch is yellow at $ip", YELLOW, Some(ip), Some("Elasticsearch"), hostName)
      case ElasticsearchNodeRedEvent(ip) => alert(s"Elasticsearch is red at $ip", RED, Some(ip), Some("Elasticsearch"), hostName)
      case ElasticsearchNodeBadCodeEvent(ip, code) => alert(s"Elasticsearch is returning code $code at $ip", YELLOW, Some(ip), Some("Elasticsearch"), hostName)
      case ElasticsearchNodeDownEvent(ip) => alert(s"Elasticsearch is down at $ip", RED, Some(ip), Some("Elasticsearch"), hostName)

      case DcNormalEvent(dc, checker) => alert(s"Syncing data from data center $dc.", GREEN, Some(checker), Some("DataCenter"))
      case DcNotSyncingEvent(dc, checker) => alert(s"Couldn't sync from data center $dc.", YELLOW, Some(checker), Some("DataCenter"))
      case DcLostConnectionEvent(dc, checker) => alert(s"Couldn't sync from data center $dc for very long time.", RED, Some(checker), Some("DataCenter"))
      case DcCouldNotGetDcEvent(dc, checker) => alert(s"Couldn't get meta data from the data center $dc.", YELLOW, Some(checker), Some("DataCenter"))
      case DcCouldNotGetDcLongEvent(dc, checker) => alert(s"Couldn't get meta data from the data center $dc for long time.", RED, Some(checker), Some("DataCenter"))
      case DcCouldNotGetDataEvent(checker) => alert(s"Couldn't get local data center meta data.", RED, Some(checker), Some("DataCenter"))

      case ComponentChangedColorEvent(component, color) => alert(s"$component changed to $color", AlertColor.fromStatusColor(color), eventKey = Some(component))
