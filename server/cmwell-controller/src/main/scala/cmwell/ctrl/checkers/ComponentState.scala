...
case class WebOk(responseTime: Int,
...
case class WebBadCode(code: Int,
                      responseTime: Int,
                      genTime: Long = ComponentState.currentTimestamp)
    extends WebState {
...
case class WebDown(genTime: Long = ComponentState.currentTimestamp)
    extends WebState {
  ...
  override def getColor: StatusColor = {
    if (rackMapping.isEmpty) {
      ...
      val rackMap = rackMapping.map { rack =>
        val rackStat =
          if (rack._2
                .map(innerIp => m.getOrElse(innerIp, "DN"))
                .toVector
                .filterNot(_ == "UN")
                .size > 0) "DN"
          else "UN"
...
case class CassandraDown(genTime: Long = ComponentState.currentTimestamp)
    extends CassandraState {
  override def getColor: StatusColor = RedStatus
}
...
case class ElasticsearchBadCode(code: Int,
                                hasMaster: Boolean = false,
                                genTime: Long = ComponentState.currentTimestamp)
    extends ElasticsearchState {
  override def getColor: StatusColor = RedStatus
...
case class ElasticsearchDown(hasMaster: Boolean = false,
                             genTime: Long = ComponentState.currentTimestamp)
    extends ElasticsearchState {
  override def getColor: StatusColor = RedStatus
}
...
case class DcSyncing(dcId: String,
                     dcDiff: DcDiff,
                     checkerHost: String,
                     genTime: Long = ComponentState.currentTimestamp)
    extends DcState {
...
case class DcCouldNotGetDcStatus(
  dcId: String,
  dcDiff: DcDiff,
  errorCounter: Int,
  checkerHost: String,
...
case class ZookeeperOk(genTime: Long = ComponentState.currentTimestamp)
    extends ZookeeperState {
  override def getColor: StatusColor = GreenStatus
}

case class ZookeeperReadOnly(genTime: Long = ComponentState.currentTimestamp)
    extends ZookeeperState {
  override def getColor: StatusColor = YellowStatus
}

case class ZookeeperNotRunning(genTime: Long = ComponentState.currentTimestamp)
    extends ZookeeperState {
  override def getColor: StatusColor = GreenStatus
}

case class ZookeeperSeedNotRunning(
  genTime: Long = ComponentState.currentTimestamp
) extends ZookeeperState {
  override def getColor: StatusColor = RedStatus
}

case class ZookeeperNotOk(genTime: Long = ComponentState.currentTimestamp)
    extends ZookeeperState {
  override def getColor: StatusColor = RedStatus
}
...
case class KafkaOk(genTime: Long = ComponentState.currentTimestamp)
    extends KafkaState {
  override def getColor: StatusColor = GreenStatus
}

case class KafkaNotOk(genTime: Long = ComponentState.currentTimestamp)
    extends KafkaState {
  override def getColor: StatusColor = RedStatus
}
...
case class BgOk(genTime: Long = ComponentState.currentTimestamp)
    extends BgState {
  override def getColor: StatusColor = GreenStatus
}

case class BgNotOk(genTime: Long = ComponentState.currentTimestamp)
    extends BgState {
  override def getColor: StatusColor = RedStatus
}
...
case class ReportTimeout(genTime: Long = ComponentState.currentTimestamp)
