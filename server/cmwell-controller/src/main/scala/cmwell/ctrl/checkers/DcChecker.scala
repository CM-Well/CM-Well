...
case class ActiveDcSync(id: String,
                        qp: Option[String],
                        wh: Boolean,
                        host: String)
...
case class DcDiff(infotonDiff: InfotonDiff,
                  indextimeDiff: IndextimeDiff,
                  remoteHost: String,
                  id: String)
...
object DcChecker extends Checker with RestarterChecker with LazyLogging {
  ...
  private def getLastIndexTime(host: String,
                               dc: String,
                               qp: Option[String],
                               withHistory: Boolean): Future[Long] = {
    val qpPart = qp.fold("?")(qp => s"?qp=$qp&")
    val whPart = if (withHistory) "with-history&" else ""
    Http
      .get(s"http://$host/proc/dc/$dc$qpPart${whPart}format=json&with-data")
      .map { r =>
        val json: JsValue = Json.parse(r.payload)
  ...
  override def doRestart: Unit = {
    logger.info(
      s"Restarting DataCenterSyncManager since it failed for ${restartAfter.toString}"
    )
  ...
  override def check: Future[ComponentState] = {
    ...
    val result = getActiveDcSyncs(host).flatMap { activeDcSync =>
      val futures = activeDcSync.map { dc =>
        ...
        val remoteLastIndexTimeF =
          getLastIndexTime(remoteHost, dc.id, dc.qp, dc.wh).recover {
            case err: Throwable => -1L
          }
        val localLastIndexTimeF =
          getLastIndexTime(host, dc.id, dc.qp, dc.wh).recover {
            case err: Throwable => -1L
          }
        val aggregated = for {
          ...
          DcDiff(
            InfotonDiff(
              0L /*localNumberOfInfotons*/,
              0L /*remoteNumberOfInfotons*/
            ),
        ...
        aggregated
          .map { dcDiff =>
            if (dcDiff.indextimeDiff.remote != -1L)
                ...
                case Some(sb: DcStatesBag) =>
                  ...
                  val newState = lastState match {
                    case Some(DcSyncing(dcId, _dcDiff, ch, _)) =>
                      if (dcDiff.indextimeDiff.isEqual)
                    ...
                    case Some(DcNotSyncing(dcId, _dcDiff, nsc, ch, _)) =>
                      if (dcDiff.indextimeDiff.isEqual)
                        ...
                        DcNotSyncing(id, dcDiff, 1 + nsc, Config.listenAddress)
              ...
              logger.debug(
                "DcChecker: Couldn't retrieve remote last index time."
              )
              getLastStates(1).headOption match {
                case Some(sb: DcStatesBag) =>
