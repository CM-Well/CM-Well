...
For example, in the case of Organization Authority entities, the entity's PermID serves as the "key" for retrieving the corresponding infoton. So for example, to get the infoton for PermID 5035948006, the URI is:

http://cm-well-host/permid.org/1-5035948006

Let’s take a look at the URI's components:

-	**http://cm-well-host.com** – the CM-Well hostname.  
...
-	**1-** By convention, all PermIDs within CM-Well start with a 1- prefix to indicate that this identifier is a PermID.  There are many other prefixes, for example:
	* **2-** A generated ID native to CM-Well, used to link dependent infotons to a main infoton.
	* **3-** The rest of the ID is a Novus Document GUID.
-	**5035948006** – The PermID of the entity we're retrieving.  


> See https://thehub.thomsonreuters.com/docs/DOC-901167 for a complete list of CM-Well domain prefixes.

Viewing the CM-Well URI shown above in a web browser displays this page:
   
<img src="./_Images/CM-Well-Company-Page.png" height=550 width=650>

As you can see in the image above, there are two different sections in the data that is returned:

* **System** - metadata about the infoton. These are "administrative details" that CM-Well itself produces.
* **Object** - the infoton's own data (which in the case of an organization, is derived from the Organization Authority).
...
Click http://cm-well-ppe.int.thomsonreuters.com/permid.org/1-5035948006?format=turtle  to see the same results as above, but now rendered as a text file in RDF’s Turtle format.  The results will look like this (truncated for brevity): 
...
> Note that in this format you still receive the infoton's system metadata, shown with the **sys** prefix in the example above.
...
The results of the query in the previous section are a list of links to the infotons that match the query, that looks like this:

<img src="./_Images/InfotonLinks.png" height=550 width=650>
...
http://cm-well-ppe.int.thomsonreuters.com/permid.org?op=search&qp=CommonName.mdaas:Marriott%20Ownership%20Resorts&with-data
...
<img src="./_Images/Results-with-data.png" height=500 width=950>
