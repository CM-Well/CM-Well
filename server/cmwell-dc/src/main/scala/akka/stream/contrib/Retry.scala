...
object Retry {
  ...
  /**
    ...
    * break down a "heavy" element which failed into multiple "thin" elements, that may succeed indevidually.
    * Since it's very easy to inflate elements in retry cycle, there's also a limit parameter,
  ...
  private[akka] class RetryCoordinator[I, S, O](retryWith: S => Option[(I, S)]) extends GraphStage[BidiShape[(I, S), (Try[O], S), (Try[O], S), (I, S)]] {
    val in1 = Inlet[(I, S)]("Rertry.ext.in")
    val out1 = Outlet[(Try[O], S)]("Rertry.ext.out")
    val in2 = Inlet[(Try[O], S)]("Rertry.int.in")
    val out2 = Outlet[(I, S)]("Rertry.int.out")
