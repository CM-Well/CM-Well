...
object TsvRetriever extends LazyLogging {
  ...
  case class TsvFlowState(tsvRetrieveInput: TsvRetrieveInput, retriesLeft: Int, lastException: Option[Throwable], consumeState: ConsumeState)

  type TsvRetrieveInput = String
  type TsvRetrieveOutput = TsvFlowOutput
  type TsvRetrieveState = TsvFlowState

  def apply(dcInfo: DcInfo, decider: Decider)(implicit mat: Materializer, system: ActorSystem): Source[List[InfotonData], (KillSwitch, Future[Seq[Option[String]]])] = {
    SourceGen.unfoldFlowWith(Future.successful(dcInfo.positionKey.get), retrieveTsvsWithRetryAndLastPositionKey(dcInfo, decider)) {
      ...
      case Success(TsvFlowOutput(tsvs, nextPositionKey, isNoContent)) => {
        logger.info(s"Data Center ID ${dcInfo.id} from ${dcInfo.location}. Got 204 no content. Will close the stream. It will be opened on the next scheduled check.")
      ...
      case Failure(ex) => {
        logger.error(s"Data Center ID ${dcInfo.id}: Retrieve of TSVs from ${dcInfo.location} failed. Completing the stream (current got TSVs should be ok unless another exception is caught later). The exception is:", ex)
        None
      }
    }
  }

  def retrieveTsvsWithRetryAndLastPositionKey(dcInfo: DcInfo, decider: Decider)(implicit mat: Materializer, system: ActorSystem): Flow[Future[String], Try[TsvRetrieveOutput], (KillSwitch, Future[Seq[Option[String]]])] = {
    ...
    val positionKeySink = Flow.fromFunction[Try[TsvRetrieveOutput], Option[String]] {
  ...
  def retrieveTsvsWithRetry(dcInfo: DcInfo, decider: Decider)(implicit mat: Materializer, system: ActorSystem): Flow[Future[String], Try[TsvRetrieveOutput], KillSwitch] =
    Flow[Future[String]]
      .mapAsync(1)(identity)
      .viaMat(KillSwitches.single)(Keep.right)
      .map(positionKey => Future.successful(positionKey) -> TsvFlowState(positionKey, Settings.initialTsvRetryCount, None, ConsumeState(BulkConsume, System.currentTimeMillis)))
    ...
    Flow[(Future[TsvRetrieveInput], TsvRetrieveState)]
      ...
      .flatMapConcat {
          ...
          entity.dataBytes
            ...
            .map { data =>
              ...
              if (state.retriesLeft < Settings.initialTsvRetryCount) {
                ...
                yellowlog.info(s"TSV (bulk)consume succeeded only after $consumeCount (bulk)consumes. token: ${state.tsvRetrieveInput}.")
              }
        ...
        case (res@Success(HttpResponse(s, h, entity, _)), state) => {
          ...
          val ex = RetrieveTsvBadResponseException(s"Retrieve TSVs using ${state.consumeState.op} failed. Data center ID ${dcInfo.id}, using remote location ${dcInfo.location}.", bodyFut, e)
        ...
        var infotonsGot: Long = 0;
      {
        case output@(Success(TsvFlowOutput(tsvs, nextPositionKey, _)), state) =>
          ...
          logger.info(s"Data Center ID ${dcInfo.id}: Got TSVs stream source. The next position key to consume is $nextPositionKey. Got ${tsvs.size} TSVs using ${state.consumeState.op}. Total TSVs got $infotonsGot. Read rate: ${rate.formatted("%.2f")} TSVs/second")
  ...
  private def retryDecider(dataCenterId: String, location: String)(implicit mat: Materializer, system: ActorSystem) = (state: TsvRetrieveState) => state match {
