        ...
        .flatMapConcat {
          case (Success(response), state) if response.status.isSuccess() => {
            response.entity.dataBytes
              .via(
                Framing
                  .delimiter(endln, maximumFrameLength = maxStatementLength)
              )
              .fold(
                RetrieveTotals(
                  state._1
        ...
        case (ingestSeq, RetrieveStateStatus(retriesLeft, ex)) =>
          if (ingestSeq.size == 1 && retriesLeft == 0) {
            ...
            Some(
              List(
                akka.pattern
                  .after(delay, sys.scheduler)(Future.successful(ingestSeq)) -> ingestState
              )
            )
            ...
            Some(ingestSeq.map { infotonMetaAndData =>
              ...
              val delay =
                Settings.maxSingleRetrieveRetryDelay.div(
                  Settings.initialSingleRetrieveRetryCount
                )
