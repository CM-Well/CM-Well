...
object InfotonRetriever extends LazyLogging {
  ...
  val breakOut2 = scala.collection
    .breakOut[RetrieveInput, (Future[RetrieveInput], RetrieveState), List[(Future[RetrieveInput], RetrieveState)]]

  //The path will be unique for each bulk infoton got into the flow
  def apply(dcInfo: DcInfo, decider: Decider)(
    implicit sys: ActorSystem,
    mat: Materializer
  ): Flow[Seq[InfotonData], scala.collection.immutable.Seq[InfotonData], NotUsed] = {
        ...
        .flatMapConcat {
          case (Success(response), state) if response.status.isSuccess() => {
            response.entity.dataBytes
              ...
              .fold(
                RetrieveTotals(state._1.map(im => (im.meta.path, (new ByteStringBuilder, None))).toMap,
                ...
                parsed.fold {
                  ...
                  totals.parsed.get(p.path) match {
                    ...
                    case Some((builder, _)) => {
                      ...
                      val parsedTotals =
                        p.indexTime.fold(totals.parsed)(_ => totals.parsed + (p.path -> (builder, p.indexTime)))
          ...
          case (res @ Success(HttpResponse(s, h, entity, _)), state) => {
            ...
            Source.single(Failure[RetrieveOutput](ex),
    ...
    response match {
      case (response @ Success(res), state @ (input, status), body) => {
        ...
        val uuidsWithBadIndexTime = res.collect {
          case (id, casIndexTime) if casIndexTime.isDefined && id.meta.indexTime != casIndexTime.get => id
        }
        if (missingUuids.isEmpty && uuidsWithBadIndexTime.isEmpty) {
          status.lastException.foreach { e =>
            val bulkCount =
              if (input.size > 1) Settings.initialBulkRetrieveRetryCount - status.retriesLeft + 1
              else Settings.initialBulkRetrieveRetryCount + 1
            val singleCount =
              if (input.size > 1) 0 else Settings.initialSingleRetrieveRetryCount - status.retriesLeft + 1
            yellowlog.info(
