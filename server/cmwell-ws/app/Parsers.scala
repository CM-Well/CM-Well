...
package util {
  ...
  trait PrefixRequirement {
    @inline final protected[this] def prefixRequirement(
      requirement: Boolean,
      message: => String
    ): Unit = {
      if (!requirement)
  ...
  class RegexAndFieldNameParser
      extends JavaTokenParsers
      with LazyLogging
      with PrefixRequirement {

    protected[this] val fieldRegex = """[\w+\-*.$]+""".r

    val namespaceUri: Parser[String] =
      """^(([^:/?#$]+):)?(//([^$/?#]*))?([^$?#]*)(\?([^$#]*))?(#([^$]*))?""".r
  ...
  (                                                                          1.  start a group to capture protocol + optional domain
   (cmwell:/)                                                                2.  if protocol is `cmwell:` no need to specify domain
    ...
    val cmwellUri
      : Parser[String] = cmwellUriPrefix ~> cmwellUriType ~ "#" ~ ncName ^? {
      case "sys" ~ _ ~ localName => s"system.$localName"
      case "nn" ~ _ ~ localName  => localName
    }

    val uri: Parser[Either[UnresolvedFieldKey, DirectFieldKey]] = cmwellUri ^^ {
      s =>
        Right(NnFieldKey(s))
    } | namespaceUri ^^ { u =>
    ...
    val fullPredicateURI
      : Parser[Either[UnresolvedFieldKey, DirectFieldKey]] = "$" ~> uri <~ "$"

    val fieldParser
      : Parser[Either[UnresolvedFieldKey, DirectFieldKey]] = fullPredicateURI | (fieldRegex ^^ {
      s =>
        s.lastIndexOf('.') match {
          ...
          case i =>
            s.splitAt(i) match {
              case (first, dotLast)
                  if dotLast.nonEmpty && isSystemProperty(
                    first -> dotLast.tail
                  ) =>
                Right(NnFieldKey(s))
              case (first, dotLast)
              ...
              case (first, dotLast) =>
                dotLast.tail match {
                  ...
                  case t if t.head == '$' =>
                    Right(HashedFieldKey(first, t.tail))
  ...
  object AggregationsFiltersParser extends RegexAndFieldNameParser {
    ...
    def aggregationsParser: Parser[List[RawAggregationFilter]] =
      repsep(aggregationParser, "~")
    ...
    def subAggregationsParser: Parser[Seq[RawAggregationFilter]] =
      ("<" ~ aggregationsParser ~ ">").? ^^ {
    ...
    def rawFieldParser: Parser[RawField[FieldValeOperator]] =
      ",field" ~> ("::" | ":") ~ fieldParser ^^ {
    ...
    def backgroundTermParser: Parser[(String, String)] =
      ",backgroundTerm:" ~ sLiteral ~ "*" ~ sLiteral ^^ {
        case _ ~ key ~ _ ~ value => (key, value)
      }

    def precisionThresholdParser: Parser[Long] =
      ",precisionThreshold:" ~ pNumber ^^ {
        case _ ~ pt => pt.toLong
      }
    ...
    def statsAggregationParser: Parser[RawStatsAggregationFilter] =
      "type:stats" ~ nameParser.? ~ rawFieldParser ^^ {
    ...
    def sigTermsAggregationParser
      : Parser[RawSignificantTermsAggregationFilter] =
      "type:sig" ~ nameParser.? ~ rawFieldParser ~
        ...
        case _ ~ nameOpt ~ field ~ backgroundTermOpt ~ minDocCountOpt ~ sizeOpt ~ subAggs =>
          ...
          val minDocCount = minDocCountOpt.getOrElse(0)
          val size = sizeOpt.getOrElse(10)
          RawSignificantTermsAggregationFilter(
            name,
            field,
            backgroundTermOpt,
            minDocCount,
    ...
    def parseAggregationParams(
      apString: Option[String]
    ): Try[List[RawAggregationFilter]] = {
      if (apString.isDefined) {
        parseAll(aggregationsParser, apString.get) match {
          case Success(aggregationFilters, _) =>
            scala.util.Success(aggregationFilters)
        ...
        scala.util.Failure(
          new IllegalArgumentException(
            "'ap' parameter is required for aggregation operation"
          )
        )
      }
    }
  ...
  object RTSQueryPredicate extends RegexParsers {
    ...
    private def values: Parser[Seq[String]] =
      value ^^ (Seq(_)) | "[" ~> repsep(value, ",") <~ "]"

    private def single: Parser[(String, Seq[String])] =
      (field ~ ":" ~ values) ^^ {
        case f ~ ":" ~ v => f -> v
      }

    private def multi: Parser[Map[String, Seq[String]]] =
      repsep(single, ",") ^^ (_.toMap)
    ...
    private def rule: Parser[(String) => Rule] = opt("*" | "-") ~ multi ^^ {
      ...
      case Some("*") ~ m =>
        _ =>
          MatchFilter(
            new MatchMap(m.mapValues(_.map(FieldValue.parseString).toSet))
          )
      case Some("-") ~ m =>
        (path: String) =>
          PMFilter(
  ...
  class BaseExpandParser extends FieldFilterParser {
    ...
    protected val filter
      : Parser[RawFieldFilter] = "[" ~> fieldFilters <~ "]" ^? ({
...
//      case Nil => RawEmptyFieldFilter // TODO: empty can mean infoton existence (no ghost skips).
      ...
      xs.headOption.fold("expansion filters must not be empty")(
        _ => s"unknown error for $xs"
    ))

    protected val uriPattern
      : Parser[NsPattern] = "$" ~> namespaceUri <~ "$" ^? {
      case uri if uri.length > 1 && uri.takeRight(2).matches("(#|/|;)[*]") =>
        NsUriPattern(uri.init)
    }

    protected val nsWildcardPattern
      : Parser[NsPattern] = uriPattern | ("[*][.]".r ~> fieldRegex ^^ {
    ...
    protected def levelExpansionTautology(
      filter: Option[RawFieldFilter]
    ): LevelExpansion =
      LevelExpansion(List(FilteredField(JokerPattern, filter)))

    protected def filteredField: Parser[FilteredField[FieldPattern]] =
      fieldPattern ~ filter.? ^^ {
  ...
  object ExpandGraphParser extends BaseExpandParser {
    ...
    private def fields: Parser[LevelExpansion] =
      filteredFields ^^ LevelExpansion.apply
    ...
    def getLevelsExpansionFunctions(
      xgInput: String
    ): Try[List[LevelExpansion]] = {
  ...
  object PathGraphExpansionParser extends BaseExpandParser {
    ...
    private def expandUp: Parser[ExpandUp] =
      "<" ~> filteredFieldsExact ^^ ExpandUp.apply

    private def expandIn: Parser[ExpandIn] =
      ">" ~> filteredFields ^^ ExpandIn.apply

    private def path: Parser[PathExpansion] =
      rep(expandIn | expandUp) ^^ PathExpansion.apply

    private def paths: Parser[PathsExpansion] =
      repsep(path, "|") ^^ PathsExpansion.apply

    def getPathsExpansionFunctions(ygInput: String): Try[PathsExpansion] =
      getPathExpansions(ygInput, "yg")

    def getGQPs(gqpInput: String): Try[PathsExpansion] =
      getPathExpansions(gqpInput, "gqp")

    def getPathExpansions(input: String, api: String): Try[PathsExpansion] = {
      if (input.isEmpty)
        scala.util.Failure(new IllegalArgumentException(s"$api empty input"))
      else {
        parseAll(paths, input) match {
          ...
          case NoSuccess(msg, _) =>
            scala.util.Failure(
              new IllegalArgumentException(
                s"error: $msg, accured while parsing: $input"
              )
            )
        }
      }
...
//    def literalParser: Parser[String] = """(?:[\p{L}\p{Sc}\(\)_0-9/.@ &:\-\\']+)""".r
    ...
    def valueParser: Parser[Option[String]] =
      (dollarsEscaped | unescapedValue).?

    def singleFieldFilters: Parser[List[RawFieldFilter]] =
      rep1sep(singleFieldFilter, ",")

    def singleFieldFilter: Parser[RawSingleFieldFilter] =
      fieldOperator ~ fieldParser ~ valueOperator ~ valueParser ^^ {
  ...
  class FieldFilterParser extends BaseFieldFilterParser {
    ...
    def multiFieldFilter: Parser[RawMultiFieldFilter] =
      fieldOperator ~ "[" ~ fieldFilters <~ "]" ^^ {
        case fo ~ _ ~ ffs => RawMultiFieldFilter(fo, ffs)
      }

    def rawFieldFilter: Parser[RawFieldFilter] =
      multiFieldFilter | singleFieldFilter
    ...
    def parseQueryParams(qpString: String): Try[RawFieldFilter] = {
      ...
      def mapQuads(fieldFilter: RawFieldFilter): RawFieldFilter = {
        fieldFilter match {
          case RawMultiFieldFilter(fo, filters) =>
            RawMultiFieldFilter(fo, filters.map(mapQuads))
          case RawSingleFieldFilter(
              fo,
              vo,
              Right(NnFieldKey("system.quad")),
              Some(quad)
      ...
      if (qpString.isEmpty)
        UFailure(new IllegalArgumentException("qp param must not be empty"))
      else
  ...
  object SortedIteratorIdParser extends FieldFilterParser {
    ...
    def extractRawKeyAsFieldName(rff: RawFieldFilter): Try[FieldFilter] =
      rff match {
        case RawMultiFieldFilter(fo, rffs) =>
          Try
            .traverse(rffs)(extractRawKeyAsFieldName)
            .map(MultiFieldFilter(fo, _))
        case RawSingleFieldFilter(fo, vo, Right(dfk), v) =>
          USuccess(SingleFieldFilter(fo, vo, dfk.internalKey, v))
        ...
        case RawSingleFieldFilter(fo, vo, Left(rfk), v) =>
          UFailure {
            new IllegalArgumentException(
              s"supplied fields must be direct. ${rfk.externalKey} needs to be resolved."
            )
          }
      }

    def parseSortedIteratorId(base64EncodedId: String) = {
      if (base64EncodedId.isEmpty)
        scala.util.Failure(
          new IllegalArgumentException("position cannot be empty")
        )
      else
        Try {
  ...
  object SortedIteratorIdFormatter {
    ...
    def formatSortedIteratorId(
      sortedIteratorState: SortedIteratorState
    ): String = {
  ...
  object SortByParser extends RegexAndFieldNameParser {
    ...
    def fieldSortParam: Parser[RawSortParam.RawFieldSortParam] =
      fieldSortOperator ~ fieldParser ^^ {
        case order ~ fieldKey => fieldKey -> order
      }

    def fieldSortParams: Parser[RawSortParam] =
      rep1sep(fieldSortParam, ",") ^^ RawFieldSortParam.apply

    def parseFieldSortParams(fsParams: String): Try[RawSortParam] = {

      if (fsParams.isEmpty || fsParams.equalsIgnoreCase("system.score"))
        USuccess(RawNullSortParam)
      else
        ...
        case _: Throwable =>
          try {
            ...
            case _: Throwable =>
              try {
                val dateTime =
                  shortDateFormatter.parseDateTime(dateStr).toMutableDateTime
                dateType match {
