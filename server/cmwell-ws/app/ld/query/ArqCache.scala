...
object ArqCache { // one day this will be a distributed cache ... // todo zStore all the caches! Actually, think twice, because serialization. Thrift...?
  ...
  private lazy val searchesCache: LoadingCache[String, Seq[Quad]] =
    CacheBuilder
      ...
      .build(new CacheLoader[String, Seq[Quad]] {
...
class ArqCache(crudServiceFS: CRUDServiceFS) extends LazyLogging {
  ...
  def putSearchResults(key: String, value: Seq[Quad]): Unit =
    searchesCache.put(key, value)
  def getSearchResults(key: String): Seq[Quad] =
    Try(searchesCache.get(key)).toOption.getOrElse(Seq())
  ...
  private lazy val infotonsCache: LoadingCache[String, Option[Infoton]] =
    CacheBuilder
      ...
      .build(new CacheLoader[String, Option[Infoton]] {
        override def load(key: String) =
          Await
            .result(crudServiceFS.getInfoton(key, None, None), 9.seconds) match {
  ...
  private lazy val identifiersCache: LoadingCache[String, String] =
    CacheBuilder
      ...
      .build(new CacheLoader[String, String] {
        override def load(key: String) = translateAndFetch(Uri(key)).hash
      })
  ...
  private lazy val namespacesCache: LoadingCache[String, NsResult] =
    CacheBuilder
      ...
      .maximumSize(1024)
      .expireAfterAccess(5, TimeUnit.MINUTES)
      .build(new CacheLoader[String, NsResult] {
        override def load(key: String) = translateAndFetch(Hash(key))
      })
  ...
  private def translateAndFetch(input: Input): NsResult = {
    ...
    Await.result(
      crudServiceFS.getInfoton(s"/meta/ns/$hash", None, None),
      9.seconds
    ) match {
      case Some(Everything(i)) if knownUri.isEmpty => {
        NsResult(
          extractFieldValue(i, "url"),
          hash,
          extractFieldValue(i, "prefix")
        )
      }
      case Some(Everything(i))
          if extractFieldValue(i, "url") == knownUri.get => {
      ...
      case _ if knownUri.isDefined => {
        ...
        val infotonsWithThatUrl =
          Await.result(searchFutureRes, 9.seconds).infotons
        (infotonsWithThatUrl.length: @switch) match {
          case 0 =>
            logger.debug(s"[arq] ns for $hash was not found")
            throw new NamespaceException(
              s"Namespace ${knownUri.get} does not exist"
            )
          case 1 =>
