...
object JenaArqExtensions {
  def get(jenaArqExtensionsUtils: JenaArqExtensionsUtils) =
    new JenaArqExtensions(jenaArqExtensionsUtils)
}

class JenaArqExtensions private (jenaArqExtensionsUtils: JenaArqExtensionsUtils)
    extends LazyLogging {
  // init extension1
  val globalArqContext = ARQ.getContext
  val originalStageGenerator =
    globalArqContext.get(ARQ.stageGenerator).asInstanceOf[StageGenerator]
  StageBuilder.setGenerator(
    globalArqContext,
    new SortingAndMappingStageGenerator(
      jenaArqExtensionsUtils,
      Option(originalStageGenerator)
    )
  )
...
/**
  ...
  */
class SortingAndMappingStageGenerator(
  jenaArqExtensionsUtils: JenaArqExtensionsUtils,
  original: Option[StageGenerator] = None
) extends StageGenerator
    with LazyLogging {
  override def execute(basicPattern: BasicPattern,
                       queryIterator: QueryIterator,
                       ec: JenaExecutionContext): QueryIterator = {

    ec.getActiveGraph match {
      case graph: CmWellGraph =>
        logger.debug(
          s"[arq][FLOW] execute was invoked with ${basicPattern.getList.length} triplePatterns"
        )
        ...
        if (needToOptimize)
          graph.dsg
            .logVerboseMsg(
              "Plan",
              s"Optimizing ${basicPattern.getList.length} statements..."
            )

        val mappedTriplePatterns = Try(basicPattern.getList.map { trpl =>
          val internalReprPredicate =
            jenaArqExtensionsUtils.predicateToInnerRepr(trpl.getPredicate)
          ...
          case Failure(e) =>
            e match {
              case err: NamespaceException =>
                graph.dsg.msgs += "Error" -> err.toString; List[Triple]()
              case _ =>
                graph.dsg.msgs += "Error" -> e.getMessage; List[Triple]()
            }
        }

        val all = if (needToOptimize) {
          val (varSubAndConstObj, others) = mappedTriplePatterns.partition(
            stmt => stmt.getSubject.isVariable && stmt.getObject.isConcrete
          )
          val bySubject = varSubAndConstObj.groupBy(_.getSubject.getName)
          val squashed = bySubject.map {
            case (_, gTriples) =>
              JenaArqExtensionsUtils.squashBySubject(gTriples)
          }
          val markedOriginal = bySubject.flatMap {
            case (_, trpls) if trpls.length == 1 =>
              Seq.empty[Triple] // no need to cache non-squashed triple - no mangling needed
            case (subName, trpls) =>
              trpls.map(
                JenaArqExtensionsUtils.mangleSubjectVariableNameIntoPredicate
              )
          }
          val markedConstSub =
            others.map(
              JenaArqExtensionsUtils
                .mangleSubjectVariableNameIntoPredicate("", _)
            )
        ...
        val sorted =
          if (needToOptimize)
            jenaArqExtensionsUtils.sortTriplePatternsByAmount(all)(graph)
          else all
        ...
        if (graph.dsg.config.explainOnly)
          graph.dsg.logMsg(
            ...
            sorted
              .map { t =>
                Try {
                  val noarmalizedPred = NodeFactory.createURI(
                    JenaArqExtensionsUtils
                      .unmanglePredicate(t.getPredicate)
                      ._2
                      .getURI
                      .replace(
                        JenaArqExtensionsUtils.cmwellInternalUriPrefix,
                        ""
                      )
                      .replace(
                        JenaArqExtensionsUtils.engineInternalUriPrefix,
                        "Cache += "
                      )
                  )
        ...
        val modifiedQueryIterator =
          if (graph.dsg.config.explainOnly)
            QueryIterRoot.create(ec) // empty iterator
          else
            sorted.foldLeft(queryIterator) { (qI, p) =>
              new QueryIterTriplePattern(qI, p, ec)
            }

        if (!graph.dsg.msgs.exists {
              case (_, v) => v.contains("Planning completed.")
            })
          graph.dsg.logVerboseMsg("Plan", "Planning completed.")

        if (!graph.dsg.msgs.exists {
              case (_, v) => v.contains("Executing started.")
            })
      ...
      case _ =>
        logger.error(
          "Jena ARQ has no registered StageGenerator. That should never happen."
        )
        !!!
    }
  }
    ...
    Await
      .result(
        crudServiceFS.thinSearch(
          Some(PathFilter("/", descendants = true)),
          Some(ff),
    ...
    Await
      .result(
        crudServiceFS.search(
          ...
          Some(ff),
          Some(DatesFilter(None, None)),
          PaginationParams(0, singleGetThreshold),
          withHistory = false,
          withData = true
        ),
        9.seconds
      ...
      Stream
        .iterate(
          Chunk(
            ...
            State(
              intermediateLimit,
              first.infotons.getOrElse(Seq.empty[Infoton]).length,
              first.totalHits
            )
          )
        ) {
          case Chunk(iid, data, State(intermediateLimit, c, t)) =>
            val ir = Await
              .result(crudServiceFS.scroll(iid, 60, withData = true), 9.seconds)
            val data = ir.infotons.getOrElse(Seq.empty[Infoton])
            Chunk(
              ir.iteratorId,
              data,
              State(config.intermediateLimit, c + data.length, t)
            )
        }
...
class DataFetcherImpl(val config: Config, val crudServiceFS: CRUDServiceFS)
    extends DataFetcher {
  val chunkSize = 100
...
class NamespaceException(msg: String) extends RuntimeException(msg: String) {
  override def toString = msg
}

class CmWellGraph(val dsg: DatasetGraphCmWell)
    extends GraphBase
    with LazyLogging {
  ...
  override def graphBaseFind(triple: Triple): ExtendedIterator[Triple] = {
    val data = dsg
      .findInDftGraph(triple.getSubject, triple.getPredicate, triple.getObject)
      .map(_.asTriple)
    new ExtendedIterator[Triple] {
      ...
      override def andThen[X <: Triple](
        iterator: util.Iterator[X]
...
case class Config(doNotOptimize: Boolean,
                  intermediateLimit: Long,
                  resultsLimit: Long,
                  verbose: Boolean,
                  finiteDuarationForDeadLine: FiniteDuration,
...
class DatasetGraphCmWell(
  ...
  def logMsg(category: String, msg: String) = {
    val time =
      new DateTime(System.currentTimeMillis() - relativeEpochTime).toString(fmt)
  ...
  def logVerboseMsg(category: String, msg: String) =
    if (config.verbose && !config.explainOnly)
      logMsg(s"$category ${Thread.currentThread.getId}", msg)

  override def findInSpecificNamedGraph(node: Node,
                                        node1: Node,
                                        node2: Node,
                                        node3: Node): util.Iterator[Quad] =
    findInAnyNamedGraphs(node, node1, node2) // todo quads

  override def findInAnyNamedGraphs(node: Node,
                                    node1: Node,
                                    node2: Node): util.Iterator[Quad] =
    findInDftGraph(node, node1, node2) // todo quads

  override def findInDftGraph(s: Node,
                              p: Node,
                              o: Node): util.Iterator[Quad] = {
  ...
  private def doFindInDftGraph(s: Node,
                               pred: Node,
                               o: Node): util.Iterator[Quad] = {

    def printNode(n: Node): String =
      if (n.isURI) n.getLocalName
      else if (n.isLiteral) n.getLiteral.toString()
      else n.toString()
    ...
    def containerPredicateMatches(containerPredicate: Node,
                                  field: Node,
                                  value: Node): Boolean = {
      JenaArqExtensionsUtils
        .explodeContainerPredicate(containerPredicate)
        .exists {
          case (name, eVal) =>
            val innerReprAsUri =
              NodeFactory.createURI(
                JenaArqExtensionsUtils.cmwellInternalUriPrefix + name
              )
            val objAsNode1 =
              if (eVal.isEmpty) Node.ANY else NodeFactory.createURI(eVal)
            val objAsNode2 =
              if (eVal.isEmpty) Node.ANY else NodeFactory.createLiteral(eVal)

            predicateMatches(innerReprAsUri, field) && (matches(
              value,
              objAsNode1
            ) || matches(value, objAsNode2))
        }
    }
    ...
    def getInfotonAndFilterFields: Iterator[Quad] = {
      arqCache.getInfoton(subject) match {
        case Some(i) => {
          ...
          allFieldsAsQuads.filter(
            q =>
              if (p.isURI && p.getURI
                    .startsWith(JenaArqExtensionsUtils.engineInternalUriPrefix))
                containerPredicateMatches(p, q.getPredicate, q.getObject)
              else
                predicateMatches(p, q.getPredicate) && matches(o, q.getObject)
          )
        }
        case None =>
          logger.debug(s"[arq] could not retrieve infoton: $subject");
          Iterator[Quad]()
      }
    }

    val quadFilter = (q: Quad) =>
      if (p.getURI.startsWith(JenaArqExtensionsUtils.engineInternalUriPrefix))
        containerPredicateMatches(p, q.getPredicate, q.getObject)
      else predicateMatches(p, q.getPredicate) && matches(o, q.getObject)
    ...
    def doSearchAndFilterFields: Iterator[Quad] = {
      ...
      if (cachedResults.nonEmpty) {
        logger.debug(
          s"Reusing caching results (amount = ${cachedResults.length}, key = $subVarName)"
        )
...
//          logVerboseMsg("Fetch", fieldFilter)
          ...
          def scroll(iteratorId: Option[String] = None) = {
            val scrollRes = Await.result(
              crudServiceFS.scroll(
                iteratorId.getOrElse(currentChunk.iteratorId),
                60,
                withData = true
              ),
              9.seconds
            )
            val quads = infotonsToQuadIterator(
              scrollRes.infotons.getOrElse(Seq.empty[Infoton])
            )
          ...
          override def hasNext: Boolean =
            count < config.intermediateLimit && (currentChunk.quads.hasNext || nextChunk.quads.hasNext)
    ...
    val results =
      if (p.isURI && p.getURI
        ...
        val filteredQuads =
          infotonResults.flatMap(infotonToQuadIterator).filter(quadFilter)
        ...
        (isConst(s), isConst(p), isConst(o)) match {
  ...
  override def listGraphNodes(): util.Iterator[Node] =
    Iterator[Node]() // this is a hack

  override def toString = "< CmWell DatasetGraph (AKA JenaDriver) >"

  // write - unimplemented
  override def addToNamedGraph(node: Node,
                               node1: Node,
                               node2: Node,
                               node3: Node): Unit = {}
  override def addToDftGraph(node: Node, node1: Node, node2: Node): Unit = {}
  override def deleteFromNamedGraph(node: Node,
                                    node1: Node,
                                    node2: Node,
                                    node3: Node): Unit = {}
  override def deleteFromDftGraph(node: Node,
                                  node1: Node,
                                  node2: Node): Unit = {}
  ...
  private val nullFormatter = new cmwell.formats.RDFFormatter(
    host,
    hash =>
      Some(JenaArqExtensionsUtils.cmwellInternalUriPrefix + hash + "#" -> None),
    false,
    false,
    false
  ) {
  ...
  private def asListOfNodes(triple: Triple) =
    List(triple.getSubject, triple.getPredicate, triple.getObject)

  private def matchesWithWildCards(triple: Triple,
                                   s: Node,
                                   p: Node,
                                   o: Node): Boolean = {
    def matches(node1: Node, node2: Node) =
      node1 == Node.ANY || node2 == Node.ANY || node1 == node2
    matches(triple.getSubject, s) && matches(triple.getPredicate, p) && matches(
      triple.getObject,
      o
    )
  }
...
class EmbedLimitQueryEngineFactory extends QueryEngineFactory with LazyLogging {
  override def accept(query: Query,
                      ...
                      context: Context): Boolean = true

  override def create(query: Query,
                      dataset: DatasetGraph,
                      initial: Binding,
  ...
  override def accept(op: Op,
                      dataset: DatasetGraph,
                      context: Context): Boolean = false

  // Should not be called because acceept/Op is false
  override def create(op: Op,
                      dataset: DatasetGraph,
                      inputBinding: Binding,
                      context: Context): Plan =
    throw new ARQInternalErrorException(
      "EmbedLimitQueryEngine: factory called directly with an algebra expression"
    )
}

class EmbedLimitQueryEngine(query: Query,
                            dataset: DatasetGraph,
                            initial: Binding = null,
                            context: Context = null)
    extends QueryEngineMain(query, dataset, initial, context) {
  override def eval(op: Op,
                    dsg: DatasetGraph,
                    input: Binding,
                    context: Context) =
    dsg.getDefaultGraph match {
      case cmwg: CmWellGraph =>
        val opSlice =
          new OpSlice(op, Long.MinValue, cmwg.dsg.config.resultsLimit)
