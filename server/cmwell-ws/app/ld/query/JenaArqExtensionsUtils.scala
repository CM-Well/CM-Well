...
object JenaArqExtensionsUtils {
  ...
  def buildCmWellQueryExecution(
    ...
    val driver = new DatasetGraphCmWell(
      host,
      config.copy(deadline = Some(config.finiteDuarationForDeadLine.fromNow)),
      crudServiceFS,
      arqCache,
  ...
  def mangleSubjectVariableNameIntoPredicate(t: Triple): Triple =
    new Triple(
      t.getSubject,
      mangleSubjectVariableNameIntoPredicate(t.getSubject, t.getPredicate),
      t.getObject
    )
  ...
  def mangleSubjectVariableNameIntoPredicate(key: String, t: Triple): Triple =
    new Triple(
      t.getSubject,
      mangleSubjectVariableNameIntoPredicate(
  ...
  def unmanglePredicate(p: Node): (String, Node) = {
    if (!p.isURI || !p.getURI.contains(manglingSeparator)) "" -> p
    else {
...
//      val (subVarName, sepAndPred) = p.getURI.splitAt(idxOfSep)
  ...
  def squashBySubject(triples: Seq[Triple]): Triple = {
    (triples.length: @switch) match {
      ...
      case _ =>
        val containerPredicate: Node = NodeFactory.createURI(
          engineInternalUriPrefix + triples
            .map { t =>
              t.getPredicate.getURI
                .replace(cmwellInternalUriPrefix, "") + ":" + objectToQpValue(
                t.getObject
              ).getOrElse("")
            }
            .mkString("|")
  ...
  val fakeQuad: Quad = {
    ...
    new Quad(
      emptyLtrl,
      emptyLtrl,
      NodeFactory.createURI(engineInternalUriPrefix + "FAKE"),
      emptyLtrl
    )
  }
  ...
  def queryToSseString(query: Query): String =
    Algebra.compile(query).toString(query.getPrefixMapping)
}

class JenaArqExtensionsUtils(arqCache: ArqCache,
  ...
  def innerReprToPredicate(innerReprPred: Node): Node = {
    if (innerReprPred.getURI.contains("/meta/sys") || innerReprPred.getURI
          .contains("msg://") || innerReprPred.getURI.contains(
          engineInternalUriPrefix + "FAKE"
        ))
      innerReprPred
    else {
      val (hash, localName) = {
        val splt =
          innerReprPred.getURI.replace(cmwellInternalUriPrefix, "").split('#')
        splt(0) -> splt(1)
      }
  ...
  def normalizeAsOutput(q: Quad): Quad =
    new Quad(
      q.getGraph,
      q.getSubject,
      innerReprToPredicate(q.getPredicate),
      q.getObject
    )

  private def defaultAmountCounter(t: Triple,
                                   graph: Option[CmWellGraph] = None) = {
    val ff = predicateToFieldFilter(t.getPredicate)(
      scala.concurrent.ExecutionContext.global
    ) //TODO: don't use global, pass ec properly
    val amount = dataFetcher.count(ff)

    graph.collect { case g if g.dsg.config.explainOnly => g.dsg }.foreach {
      dsg =>
        val pred = Try(unmanglePredicate(t.getPredicate)._2).toOption
          .getOrElse(t.getPredicate)
          .getURI
          .replace(cmwellInternalUriPrefix, "")
  ...
  // todo add DocTest :)
  def sortTriplePatternsByAmount(
    triplePatterns: Iterable[Triple],
    amountCounter: (Triple, Option[CmWellGraph]) => Long = defaultAmountCounter
  )(graph: CmWellGraph): Seq[Triple] = {
    def isVar(t: Triple) =
      t.getSubject.isVariable || (t.getSubject.isLiteral && t.getSubject.getLiteralValue == "")

    def isEngine(t: Triple) =
      t.getPredicate.getURI.contains(engineInternalUriPrefix)
    ...
    val squashedSubjects =
      engineTriples.map(t => unmanglePredicate(t.getPredicate)._1).toSet
    def isSquashed(t: Triple): Boolean = squashedSubjects(t.getSubject.getName)

    val (squashedActualTriples, nonSquashedActualTriples) =
      actualTriples.partition(isSquashed)
    val groupedByEngine = squashedActualTriples.groupBy(_.getSubject.getName)

    (engineTriples ++ nonSquashedActualTriples)
      .sortBy(amountCounter(_, Option(graph)))
      .foreach { triple =>
        allSorted += triple
        if (isEngine(triple)) {
  ...
  def predicateToFieldFilter(p: Node, obj: Node = emptyLtrl)(
    ...
    def toExplodedMangledFieldFilter(qp: String, value: Option[String]) = {
      ...
      RawSingleFieldFilter(
        Must,
        Equals,
        Right(HashedFieldKey(localName, hash)),
        value
      )
    }
    ...
    def evalAndAwait(rff: RawFieldFilter) = {
      ...
      /*val evalRes =*/
      Await.result(
        RawFieldFilter.eval(rff, typesCache, cmwellRDFHelper, None),
        9.seconds
      )
      //      filterNot(evalRes)(isTypeIncompatible)
    }

    def noneIfEmpty(s: String): Option[String] =
      if (s.isEmpty) None else Some(s)

    if (pred.getURI.contains(JenaArqExtensionsUtils.engineInternalUriPrefix)) {
      val fieldFilters =
        JenaArqExtensionsUtils.explodeContainerPredicate(pred).map {
          case t @ (name, value) =>
