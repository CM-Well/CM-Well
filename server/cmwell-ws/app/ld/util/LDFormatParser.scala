...
object LDFormatParser extends LazyLogging {
  ...
  case class ParsingResponse(infotons: Map[String, Map[DirectFieldKey, Set[FieldValue]]],
    ...
    def ⑃(that: ParsingResponse): ParsingResponse = {
      ...
      val deleteMap = {
        ...
        keys
          .map(k => k -> Set(this.deleteMap.get(k), that.deleteMap.get(k)).collect { case Some(x) => x }.flatten)
          .toMap
      }
  ...
  def parseGlobalExpressionsAsync(cmwellRDFHelper: CMWellRDFHelper, crudServiceFS: CRUDServiceFS)(
        ...
        globalMetaOpsStatements.foreach { stmt =>
          ...
          pLocalName match {
            ...
            case "graphAlias" if g.isDefined => {
              ...
              metaInfotons ++= Map[String, Map[DirectFieldKey, Set[FieldValue]]](
                path ->
  ...
  def validateAuth(authUtils: AuthUtils)(model: Model, dels: Resource => Boolean, token: Option[Token]): Unit = {
    ...
    if (unauthorizedPaths.nonEmpty)
      throw new security.UnauthorizedException(unauthorizedPaths.mkString("unauthorized paths:\n\t", "\n\t", "\n\n"))
  }

  case class DataSetConstructs(graphModelTuplesSeq: Seq[(Option[String], Model)],
  ...
  def modelsFromRdfDataInputStream(
    ...
    val graphModelsTuplesSeq = ds.listNames.asScala.foldLeft(modelsSeqWithdefaultModel) {
      case (acc, name) => {
        ...
        val (u2lMap, nsiMap) =
          convertMetaMapToInfotonFormatMap(cmwellRDFHelper, timeContext, m, urlToLastAcc.keySet.toSet)
  ...
  def updateDeleteMap(deleteFieldsMap: MMap[String, Set[(String, Option[String])]],
                      infotonPath: String,
  ...
  def getCmwellFieldNameForUrl(cmwellRDFHelper: CMWellRDFHelper,
                               timeContext: Option[Long],
                               predicateUrl: String,
                               localName: Option[String] = None): Option[String] = predicateUrl match {
    case "*" => Some("*")
    case _ => {
  ...
  def generateMetaData(predicate: String, value: FieldValue, md: MetaData = MetaData.empty): MetaData = {
    ...
    predicate match {
      ...
      case "modifiedDate" | "lastModified" =>
        md.copy(date = Some(Try(value.asInstanceOf[FDate].getDate).getOrElse(dt(value.toString))))
      ...
      case "linkTo" if value.isInstanceOf[FReference] =>
        md.copy(mdType = Some(LinkMetaData), linkTo = Some(value.asInstanceOf[FReference].getCmwellPath))
      ...
      case _                          => logger.warn(san"attribute: ${predicate}, is not treated in cmwell RDF imports."); md
    }
  }
  ...
  /**
    ...
    */
  def convertMetaMapToInfotonFormatMap(cmwellRDFHelper: CMWellRDFHelper,
                                       timeContext: Option[Long],
                                       model: Model,
  ...
  def createMetaNsInfoton(cmwellRDFHelper: CMWellRDFHelper,
                          ...
                          last: String,
                          url: String,
                          prefix: String): (String, Map[DirectFieldKey, Set[FieldValue]]) = {
  ...
  def inferShortNameFromUrl(url: String, pilledSuffix: String = ""): String = {
    ...
    if (url.isEmpty) {
      ...
      val (start, rest): Tuple2[String, String] =
        if (XMLChar.isNCNameStart(chosenPartOrInitials.head)) chosenPartOrInitials.splitAt(1)
