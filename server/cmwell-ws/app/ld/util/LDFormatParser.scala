...
object LDFormatParser extends LazyLogging {
  ...
  def createFieldValue(l: Literal, q: Option[String]): FieldValue = {
    ...
    l.getDatatypeURI match {
      ...
      case xsd: String if xsd.startsWith(XSDDatatype.XSD) =>
        getXSDType(xsd.drop(XSDDatatype.XSD.length + 1), l, lang, quad) //TODO: no need for lang here, will be the next case
      case rls: String if rls == RDFLangString.rdfLangString.getURI =>
        getXSDType("string", l, lang, quad)
      case fun: String if fun.startsWith("urn:x-hp-jena:Functor") => ???
      case str: String if str.contains('$') =>
        throw new IllegalArgumentException(
          s"custom types cannot contain '$$' character. please encode as '%24' or use another type. type recieved: '$str'"
        )
      case str: String if str.startsWith(normalizedCWD) =>
        FExternal(l.getLexicalForm, str.drop(normalizedCWD.length), quad)
      case str: String => FExternal(l.getLexicalForm, str, quad)
    }
  }

  def getXSDType(xsdType: String,
                 l: Literal,
                 lang: Option[String],
                 quad: Option[String]): FieldValue = xsdType match {
    case "float" =>
      FFloat(l.getFloat, quad) //convertXSD(l.getFloat, XSDDatatype.XSDfloat, l.getLexicalForm,lang,quad)
    case "double" =>
      FDouble(l.getDouble, quad) //convertXSD(l.getDouble, XSDDatatype.XSDdouble, l.getLexicalForm,lang,quad)
    case "int" =>
      FInt(l.getInt, quad) //convertXSD(l.getInt ,XSDDatatype.XSDint, l.getLexicalForm,lang,quad)
    case "long" =>
      FLong(l.getLong, quad) //convertXSD(l.getLong, XSDDatatype.XSDlong, l.getLexicalForm,lang,quad)
...
//  }
  ...
  case class ParsingResponse(
    infotons: Map[String, Map[DirectFieldKey, Set[FieldValue]]],
    ...
    def ⑃(that: ParsingResponse): ParsingResponse = {
      ...
      val metaData = {
        ...
        keys.map { k =>
          k -> {
            val metaDatas =
              Seq(this.metaData.get(k), that.metaData.get(k)).collect {
                case Some(x) => x
              }
      ...
      val deleteMap = {
        ...
        keys
          .map(
            k =>
              k -> Set(this.deleteMap.get(k), that.deleteMap.get(k)).collect {
                case Some(x) => x
              }.flatten
          )
          .toMap
      ...
      that.atomicUpdates.foreach {
        case pu @ (path, uuid) =>
          this.atomicUpdates.get(path) match {
            case None => atomicUpdatesBuilder += pu
            case Some(thisUuid) =>
              if (uuid != thisUuid)
                throw new IllegalArgumentException(
                  s"Atomic versioning must be unique per path. Document contained multiple base uuids: [$uuid,$thisUuid] for path [$path]"
                )
          }
      }
  ...
  def isGlobalMetaOp(stmt: Statement) = {
    ...
    val thisDocSub = {
      ...
      //meaning subject is: `<>`
      subj.isURIResource && (subj.getURI.isEmpty || new java.net.URI(
        subj.getURI
      ) == cwd)
    }
  ...
  def parseGlobalExpressionsAsync(
    ...
    models.foreach {
      case (g, m) => {
        ...
        globalMetaOpsStatements.foreach { stmt =>
          ...
          pLocalName match {
            case "replaceGraph"
                if o.isURIResource && pNameSpace.matches(metaOpRegex("sys")) => {
              val uri = o.asResource().getURI
              require(
                uri != "*",
                "trying to delete all quads with <*> ? naughty..."
              )
              require(
                uri.nonEmpty,
                "empty quad as value for `replaceGraph` is illegal..."
              )
              quadsToDelete += uri
              replaceGraphStatementsCounter += 1
            }
            case "fuzzyReplaceGraph"
                if pNameSpace.matches(metaOpRegex("sys")) => {
              val fuzzyQuad = o.toString
              require(
                fuzzyQuad.nonEmpty,
                "empty quad as value for `fuzzyReplaceGraph` is illegal..."
              )
            ...
            case "graphAlias" if g.isDefined => {
              ...
              metaInfotons ++= Map[String, Map[DirectFieldKey,
                                               Set[FieldValue]]](
                path ->
            ...
            case prefix if pNameSpace.matches(metaOpRegex("ns")) => {
              val url = o.toString
              val last = urlToLast.getOrElse(
                url,
                cmwellRDFHelper.nsUrlToHash(url, timeContext)._1
              )
              val (path, nsInfotonRepr) =
                createMetaNsInfoton(
                  cmwellRDFHelper,
                  timeContext,
                  last,
            ...
            case op =>
              throw new IllegalArgumentException(
                s"operation [$op] on <> with value [$o] is not defined."
              )
          }
        }
        require(
          replaceGraphStatementsCounter <= Settings.graphReplaceMaxStatements,
          s"maximum allowed `replaceGraph` statements per request exceeded (${replaceGraphStatementsCounter} > ${Settings.graphReplaceMaxStatements})."
        )
      }
    }
    ...
    val globalQuadsDeletionFuture = {
      if (quadsToDelete.isEmpty && fuzzyQuadsToDelete.isEmpty) {
        Future.successful(
          (
            IMap.empty[String, Set[(String, Option[String])]],
            Option.empty[String]
          )
        )
      } else {
        ...
        val fuzzyQuadsToDeleteImmutable = ISet
          .empty[String] ++ fuzzyQuadsToDelete
...
//        }.toList
        ...
        ) ::: fuzzyQuadsToDeleteImmutable
          .flatMap { q =>
            List(
              FieldFilter(Should, Contains, "quad", q),
              FieldFilter(Should, Contains, "system.quad", q)
            )
          }(
            scala.collection
              .breakOut[Set[String], FieldFilter, List[FieldFilter]]
          )

        implicit val searchTimeout = Some(
          Settings.graphReplaceSearchTimeout.seconds
        )
        ...
        crudServiceFS
          .thinSearch(
            pathFilter = None,
            fieldFilters =
              Some(MultiFieldFilter(Must, fieldFiltersForQuadsSearch)),
            datesFilter = None,
            paginationParams = PaginationParams(
              0,
              Settings.maxSearchResultsForGlobalQuadOperations
            ),
            withHistory = false,
            withDeleted = false,
          ...
          .map { searchThinResults =>
            require(
              searchThinResults.total <= Settings.maxSearchResultsForGlobalQuadOperations,
              "Request is too costly. " +
            ...
            searchThinResults.thinResults.map {
              _.path -> quadsToDeleteImmutable.map(
                q => "*" -> (Some(q): Option[String])
              )
            }.toMap -> Option(
              s"replaceGraph and/or fuzzyReplaceGraph ops resulted with ${searchThinResults.thinResults.length} infotons affected (markReplaced) implicitly"
            )
          }
      }
    }

    globalQuadsDeletionFuture.map {
      case (qDelMap, feedback) =>
        val allFeedbacks =
          feedback.fold(feedbacks.result())(feedbacks.+=(_).result())
        ParsingResponse(
  ...
  def validateAuth(
    authUtils: AuthUtils
  )(model: Model, dels: Resource => Boolean, token: Option[Token]): Unit = {
    ...
    val unauthorizedPaths =
      authUtils.filterNotAllowedPaths(paths, PermissionLevel.Write, token)
    if (unauthorizedPaths.nonEmpty)
      throw new security.UnauthorizedException(
        unauthorizedPaths.mkString("unauthorized paths:\n\t", "\n\t", "\n\n")
      )
  }

  case class DataSetConstructs(
    graphModelTuplesSeq: Seq[(Option[String], Model)],
    urlToLastMap: Map[String, String],
    metaNsInfotons: InfotonRepr,
  ...
  def modelsFromRdfDataInputStream(
    ...
    val metaNsInfotonsAcc =
      MMap.empty[String, Map[DirectFieldKey, Set[FieldValue]]]
    val urlToLastAcc = MMap.empty[String, String]
    val modelsSeqWithdefaultModel: Seq[(Option[String], Model)] = Seq(
      None -> defaultModel
    )
    val graphModelsTuplesSeq = (modelsSeqWithdefaultModel /: ds.listNames) {
      case (acc, name) => {
        ...
        val (u2lMap, nsiMap) =
          convertMetaMapToInfotonFormatMap(
            cmwellRDFHelper,
            timeContext,
            m,
            urlToLastAcc.keySet.toSet
          )
    ...
    val (urlToLastDefault, metaNsInfotonsDefault) =
      convertMetaMapToInfotonFormatMap(
        cmwellRDFHelper,
        timeContext,
        defaultModel,
    ...
    val feedbacks = List.newBuilder[String]
    val metaQuadsMap = (IMap
      .empty[String, Map[DirectFieldKey, Set[FieldValue]]] /: graphModelsTuplesSeq) {
      case (m, (subGraph, model)) => {
        val quadOpt = subGraph.flatMap(
          uri => Option(model.getNsURIPrefix(uri)).map(_ -> uri)
        )
        quadOpt match {
          case Some((prefix, url))
              if url != "*" && cmwellRDFHelper
                .getAliasForQuadUrl(url)
                .isEmpty => {
            feedbacks += s"Aliasing named graph [$url] as [$prefix]"
            m ++ Map[String, Map[DirectFieldKey, Set[FieldValue]]](
              s"/meta/quad/${Base64.encodeBase64URLSafeString(url)}" ->
                Map[DirectFieldKey, Set[FieldValue]](
    ...
    val newMetaNsInfotons = {
      ...
      val x = all.values
        .flatMap(_.getOrElse(prefixFKey, Set.empty))
        .groupBy(_.value)
        .values
        .filterNot(_.size == 1)
      require(
  ...
  def isQuadsDialect(dialect: String): Boolean = dialect match {
    ...
    case "TRIX"                       => true
    case _ =>
      throw new IllegalArgumentException(s"unknown RDF dialect: [$dialect]")
  }

  def changeQuadAccordingToDialect(quad: Option[String],
                                   dialect: String): Option[String] =
    quad.orElse {
  ...
  def rdfToInfotonsMap(cmwellRDFHelper: CMWellRDFHelper,
    ...
    require(
      dialectToLang.isDefinedAt(dialect),
      "the format " + dialect + " is unknown."
    )

    val DataSetConstructs(models, urlToLast, metaInfotonsMap, partialFeedback) =
      modelsFromRdfDataInputStream(cmwellRDFHelper, timeContext)(
        rdfData,
        dialect
      )

    val globalParsingResponse =
      parseGlobalExpressionsAsync(cmwellRDFHelper, crudServiceFS)(
        models,
        urlToLast,
        metaInfotonsMap,
        timeContext
    ...
    val parsingResults = models
      .map {
        case (subGraph, model) =>
          ...
          val (delSubs, regular) = {
            val (delSubs, regular) = model
              .listStatements()
              .partition(s => dels(s.getSubject.asResource))
          ...
          if (isOverwrite) {
            ...
            val metaNamespaces =
              model.listNameSpaces().count(_.contains("/meta/sys#"))
            if (metaNamespaces != 0 || subGraph.isEmpty) {
              require(
                metaNamespaces == 1,
                s"must use exactly one `/meta/sys#` namespace in document when forcing uniquness, namespaces found: ${model
                  .listNameSpaces()
                  .mkString("[", ",", "]")}"
              )
              val metaNs =
                model.listNameSpaces().find(_.contains("/meta/sys#")).get
              val props: Seq[Property] =
                Seq("type", "lastModified", "dataCenter", "indexTime").map(
                  model.createProperty(metaNs, _)
                )
          ...
          while (it.hasNext) {
            ...
            if (!dels(stmt.getSubject.asResource)) {
                  ...
                  if (!url.startsWith(cmwell)) {
                    cmwHosts += {
                      if (url.startsWith(http))
                        url.drop(http.length).takeWhile(_ != '/')
                      else if (url.startsWith(https))
                        url.drop(https.length).takeWhile(_ != '/')
                      else
                        throw new IllegalArgumentException(
                          s"the url: $url is weird"
                        )
                    }
                  }
                  if (url.endsWith("/meta/nn#")) Right(NnFieldKey(firstName))
                  else if (url.endsWith("/meta/sys#") && firstName == "markReplace")
                    Left(MarkReplace)
                  else if (url.endsWith("/meta/sys#") && firstName == "markDelete")
                    Left(MarkDelete)
                  else if (url.endsWith("/meta/sys#") && firstName == "fullDelete")
                    Left(FullDelete)
                  else if (url.endsWith("/meta/sys#") && firstName == "prevUUID")
                    Left(PrevUUID)
              ...
              val allowWriteSysFields = (fieldName: String) =>
                fieldName match {
                  case "uuid" | "path" | "parent" | "lastModified" |
                      "modifiedDate" | "indexTime" | "dataCenter" =>
              ...
              predicate match {
                case Right(fieldName) => {
                  ...
                  if (infotonsMap.contains(subject)) {
                    ...
                    val newValueSet = infoton.getOrElse(
                      fieldName,
                      Set[FieldValue]()
                    ) + value
                    infotonsMap.update(
                      subject,
                      infoton + (fieldName -> newValueSet)
                    )
                  } else {
                    infotonsMap.update(
                      subject,
                ...
                case Left(MarkDelete) =>
                  throw new IllegalArgumentException(
                    "when using 'markDelete' API, one must supply an anonymous node as value"
                  )
                case Left(FullDelete) => {
                  ...
                  val recursive: Boolean =
                    if (obj.isLiteral) obj.asLiteral().getBoolean
                      ...
                      throw new IllegalArgumentException(
                        s"was expecting a boolean value stating if the delete is recursive or not for path: $subject"
                      )

                  val fut = controllers.ApplicationUtils
                    .infotonPathDeletionAllowed(path, recursive, crudServiceFS)
                ...
                case Left(MarkReplace) => {
                  ...
                  if (cmwellFieldName.isEmpty)
                    throw new IllegalArgumentException(
                      s"CM-Well could not resolve predicate [${if (!obj.isResource) obj.toString
                      else {
                        val r = obj.asResource()
                ...
                case Left(PrevUUID) => {
                  ...
                  require(
                    uuid.isEmpty || uuid.matches("^[a-f0-9]{32}$"),
                    s"invalid uuid supplied for path: [$subject] and predicate prevUUID"
                  )
                  require(
                    !stmt.getSubject.isAnon,
                    "prevUUID predicates can't operate on anonymous subjects."
                  )
          ...
          val immutableInfotonsMap
            : IMap[String, IMap[DirectFieldKey, Set[FieldValue]]] = IMap[
            String,
            Map[DirectFieldKey, Set[FieldValue]]
          ]() ++ infotonsMap
          ...
          if (!skipValidation) {
            require(
              immutableInfotonsMap.keys
                .forall(
                  k => InfotonValidator.isInfotonNameValid(normalizePath(k))
                ),
  ...
  def createValueFromObject(obj: RDFNode, subGraph: Option[String] = None) = {
    if (obj.isAnon) {
      val node = obj.asNode
      if (node.isBlank)
        FReference(blank + anonNameMimic(node.getBlankNodeId), subGraph)
      else
        throw new RuntimeException(
          "object: %s is anon but not blank?".format(obj.toString)
        )
  ...
  def fieldValueFromObject(obj: RDFNode, quad: Option[String]) =
    if (obj.isLiteral) createFieldValue(obj.asLiteral, quad)
      ...
      FReference(obj.toString, quad) //(subjectFromStatement(obj.asResource,true),None,quad)

  def updateDeleteMap(
    deleteFieldsMap: MMap[String, Set[(String, Option[String])]],
    infotonPath: String,
    ...
    deleteFieldsMap.get(prependSlash(infotonPath)) match {
      case Some(fieldSet) if !fieldSet(fToDel) =>
        deleteFieldsMap.update(prependSlash(infotonPath), fieldSet + fToDel)
      case None =>
        deleteFieldsMap.update(prependSlash(infotonPath), Set(fToDel))
      case _ => //DO NOTHING (the deleted field already exist in the deleteFieldsMap)
    }
  }

  def updateDeleteValuesMap(
    cmwellRDFHelper: CMWellRDFHelper,
    timeContext: Option[Long],
    ...
    pvl.foreach {
      case (p, v) => {
        ...
        require(
          !predicateUrl.matches(metaOpRegex("(sys|ns)")),
          s"sys fields not allowed for markDelete failed for: [$predicateUrl${p.getLocalName}]"
        )
        val cmwAttribute = getCmwellFieldNameForUrl(
          cmwellRDFHelper,
          timeContext,
          predicateUrl,
          Some(p.getLocalName)
        ).get //TODO: protect against empty Option
        val validValue: FieldValue = fieldValueFromObject(v, quad)
        deleteValuesMap.get(infotonPath) match {
          case None =>
            deleteValuesMap.update(
              infotonPath,
              Map(cmwAttribute -> Set(validValue))
            )
          case Some(mss) =>
            mss.get(cmwAttribute) match {
              case None =>
                deleteValuesMap.update(
                  infotonPath,
                  mss.updated(cmwAttribute, Set(validValue))
                )
              case Some(sf) =>
                deleteValuesMap.update(
                  infotonPath,
                  mss.updated(cmwAttribute, sf + validValue)
                )
            }
        }
      }
  ...
  private def urlValidate(u: String): Boolean =
    Try(new java.net.URL(u)).isSuccess ||
      (u.startsWith(cmwell) && Try(
        new java.net.URL(http + u.drop(cmwell.length))
      ).isSuccess)

  def getCmwellFieldNameForUrl(
    cmwellRDFHelper: CMWellRDFHelper,
    timeContext: Option[Long],
    predicateUrl: String,
    ...
    predicateUrl match {
      ...
      case _ => {
        require(
          urlValidate(predicateUrl),
          s"the url: ${predicateUrl} is not a valid predicate url for ingest."
        )
        ...
        if (url.endsWith("/meta/nn#")) {
          if (firstName.contains("."))
            throw new IllegalArgumentException(
              s"nn (No Namespace) fields aren't allowed to contain dots. [$url$firstName]"
            )
          else Some(firstName)
        } else
          cmwellRDFHelper.urlToHash(url, timeContext).map { hash =>
            s"$firstName.$hash"
          }
      }
    }

  def updateMetaData(cmwMetaDataMap: MMap[String, MetaData],
                     subject: String,
                     predicate: String,
                     value: FieldValue) {
    cmwMetaDataMap.get(subject) match {
      case Some(md) =>
        cmwMetaDataMap.update(subject, generateMetaData(predicate, value, md))
      case None =>
        cmwMetaDataMap.update(subject, generateMetaData(predicate, value))
    }
  }

  def generateMetaData(predicate: String,
                       value: FieldValue,
                       md: MetaData = MetaData.empty): MetaData = {

    predicate match {
      case "type" =>
        value.toString match {
          case "OBJ" | "ObjectInfoton" | "CompoundInfoton" =>
            md.copy(mdType = Some(ObjectMetaData))
          case "FILE" | "FileInfoton" => md.copy(mdType = Some(FileMetaData))
          case "LINK" | "LinkInfoton" => md.copy(mdType = Some(LinkMetaData))
          case "DEL" | "DeletedInfoton" =>
            md.copy(mdType = Some(DeletedMetaData))
          case _ =>
            logger.warn(
              "type: " + value + ", is not yet treated in cmwell RDF imports."
            );
            md
        }
      case "modifiedDate" | "lastModified" =>
        md.copy(
          date = Some(
            Try(value.asInstanceOf[FDate].getDate).getOrElse(dt(value.toString))
          )
        )
      case "contentType" | "mimeType" =>
        md.copy(mdType = Some(FileMetaData), mimeType = Some(value.toString))
      case "data" =>
        md.copy(mdType = Some(FileMetaData), text = Some(value.toString))
      case "base64-data" =>
        md.copy(
          mdType = Some(FileMetaData),
          data = Some(Base64.decodeBase64(value.toString))
        )
      case "linkTo" if value.isInstanceOf[FReference] =>
        md.copy(
          mdType = Some(LinkMetaData),
          linkTo = Some(value.asInstanceOf[FReference].getCmwellPath)
        )
      case "linkTo" =>
        md.copy(mdType = Some(LinkMetaData), linkTo = Some(value.toString))
      case "linkType" =>
        md.copy(
          mdType = Some(LinkMetaData),
          linkType = Some(
            Try(value.asInstanceOf[FInt].value)
              .getOrElse(md.linkType.getOrElse(1))
          )
        )
      case "dataCenter" => md.copy(dataCenter = Some(value.toString))
      case "indexTime" =>
        md.copy(indexTime = Try(value.asInstanceOf[FLong].value).toOption)
      case "uuid" | "parent" | "path" =>
        md // these have no affect. better ignore without polluting the logs
      case _ =>
        logger.warn(
          "attribute: " + predicate + ", is not treated in cmwell RDF imports."
        );
        md
    }
  }
  ...
  def isCMWellSystemNS(url: String) =
    url.endsWith("/meta/sys#") || url.endsWith("/meta/nn#") || url.endsWith(
      "/meta/ns#"
    )
  ...
  /**
    ...
    */
  def convertMetaMapToInfotonFormatMap(
    cmwellRDFHelper: CMWellRDFHelper,
    timeContext: Option[Long],
    model: Model,
    urlExcludes: Set[String]
  ): (Map[String, String], InfotonRepr) = {

    val noJenaMap = model.getNsPrefixMap.toSeq.collect {
      case (shortName, uriValue)
          if !shortName.matches("""[Jj].\d+""") && shortName != "" =>
        uriValue -> shortName
    }.toMap
    ...
    val all = {
      ...
      it.foldLeft(Map.empty[String, (String, PrefixState)]) {
        ...
        case (m, url) if url.contains('$') =>
          throw new IllegalArgumentException(
            s"predicate namespace must not contain a dollar ('$$') sign: $url"
          )
        case (m, url) if !urlExcludes(url) && !m.contains(url) =>
          m.updated(url, cmwellRDFHelper.nsUrlToHash(url, timeContext))
        case (m, _) => m
      }
    }
    ...
    val (_, n) = all.foldLeft(
      Set.empty[String] -> Map
        .empty[String, Map[DirectFieldKey, Set[FieldValue]]]
    ) {
      ...
      case ((prefixes, result), (url, (hash, Create))) => {
        ...
        val entry =
          createMetaNsInfoton(cmwellRDFHelper, timeContext, hash, url, prefix)
  ...
  def createMetaNsInfoton(
    cmwellRDFHelper: CMWellRDFHelper,
    timeContext: Option[Long],
    last: String,
    url: String,
    prefix: String
  ): (String, Map[DirectFieldKey, Set[FieldValue]]) = {
    val uncollidedPrefixFuture = cmwellRDFHelper
      .getIdentifierForPrefixAsync(prefix, timeContext)
      .transform {
        case Success(`last`) | Failure(_: NoSuchElementException) =>
          Success(prefix)
        case Success(_)   => Success(s"$prefix-$last")
        case anythingElse => anythingElse
      }

    val uncollidedPrefix =
      uncollidedPrefixFuture.value.fold(
        Await.result(uncollidedPrefixFuture, Duration.Inf)
      )(_.get)
  ...
  def inferShortNameFromUrl(url: String, pilledSuffix: String = ""): String = {
    ...
    if (url.isEmpty) {
      ...
      val chosenPartOrInitials = domain
        .find(!blackList(_))
        .getOrElse(domain.flatMap(_.headOption).mkString)
      val (start, rest): Tuple2[String, String] =
        if (XMLChar.isNCNameStart(chosenPartOrInitials.head))
          chosenPartOrInitials.splitAt(1)
        else ("ns-", chosenPartOrInitials)
      val end: String = rest.flatMap { ch: Char =>
        ch match {
      ...
      if (XMLChar.isValidNCName(candidate) && !candidate.contains('.'))
        candidate
      else
        inferShortNameFromUrl(mUrl.dropRight(candidate.length + 1), candidate)
    }
  }
...
//  }
  ...
  def blackList(s: String): Boolean =
    s.length < 3 || s.matches(
      """(www\d*)|static|org|net|com|biz|gov|edu|https?"""
    )
...
//	}
  ...
  /**
    ...
    * @return
    */
  def shortWithUrlHash(short: String, url: String): String =
    short + "-" + crc32(url)
    ...
    if (su.matches("""((http(s?))|(cmwell))://\S+""")) {
      ...
      val v =
        if (su.startsWith(http)) su.drop(http.length + domainLength)
          ...
          "https." + su.drop(https.length + domainLength)
        else su.drop(cmwell.length)

      val u = v.takeWhile(_ != '/') match {
        case host9000 if host9000.endsWith(":9000") =>
          v.drop(host9000.length + 1)
        case cmw if cmw.toLowerCase.matches(""".*cm(\-)?well.*""") =>
          v.drop(cmw.length + 1)
        case connext if connext.toLowerCase.contains("connext") =>
          v.drop(connext.length + 1)
        case domain => v
      }
