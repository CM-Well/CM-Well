...
package object util {
  ...
  val uriSeparator
    : Set[Char] = Set('#', '/', ';') //TODO: does these possibilities cover every use case?
  ...
  def inferCharsetFromMimetipe(mimeType: String): String =
    mimeType.lastIndexOf("charset=") match {
      case i if (i != -1) => mimeType.substring(i + 8).trim
      case _              => "utf-8"
    }

  private[this] def makeMetaWithZero(
    path: String,
  ...
  def infotonFromMaps(cmwHostsSet: Set[String],
    ...
    metaData match {
      case Some(
        ...
        lazy val (_date, dc) =
          if (path.startsWith("/meta/"))
            DateTime.now(DateTimeZone.UTC) -> Settings.dataCenter
          else
            date.getOrElse(DateTime.now(DateTimeZone.UTC)) -> dataCenter
              .getOrElse(Settings.dataCenter)
        mdt match {
          case Some(ObjectMetaData) if path.startsWith("/meta/") =>
            makeMetaWithZero(path, fields)
          ...
          case Some(DeletedMetaData) =>
            DeletedInfoton(path, dc, indexTime, _date)
          case None =>
  ...
  def detectIfUrlBelongsToCmwellAndGetLength(subjectUrl: String): Int = {
    ...
    if (domain.contains("cm-well") || domain.startsWith("connext"))
      domain.length
    else 0
  }
}
...
package util {
  ...
  case class MetaData(mdType: Option[TypeMetaData],
