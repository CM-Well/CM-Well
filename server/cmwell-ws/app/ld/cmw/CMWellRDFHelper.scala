...
object CMWellRDFHelper {
  ...
  object PrefixResolvingError {
    def apply(prefix: String) =
      new RuntimeException(s"Failed to resolve prefix [$prefix]")
      with PrefixResolvingError
    def apply(prefix: String, cause: Throwable) =
      new RuntimeException(s"Failed to resolve prefix [$prefix]", cause)
      with PrefixResolvingError
  }
...
class CMWellRDFHelper @Inject()(val crudServiceFS: CRUDServiceFS,
  ...
  val newestGreatestMetaNsCacheImpl =
    TimeBasedAccumulatedNsCache(Map.empty, 0L, 2.minutes, crudServiceFS)(
  ...
  private[this] val transformFuncURLAndPrefix2URLAndPrefix
    : Try[(String, String)] => Option[(String, String)] = {
    case Success(urlAndPrefix)              => Some(urlAndPrefix)
    case Failure(_: NoSuchElementException) => None
    case Failure(e)                         => throw e
  }

  @inline def invalidate(
    nsID: String
  )(implicit timeout: Timeout): Future[Unit] =
    newestGreatestMetaNsCacheImpl.invalidate(nsID)
  @inline def invalidateAll()(implicit timeout: Timeout): Future[Unit] =
    newestGreatestMetaNsCacheImpl.invalidateAll()

  @deprecated(
    "API may falsely return None on first calls for some value",
    "Quetzal"
  )
  ...
  def hashToUrlAsync(hash: String, timeContext: Option[Long])(
    ...
    newestGreatestMetaNsCacheImpl
      ...
      .transform {
        ...
        case f @ Failure(_: ConflictingNsEntriesException) =>
          f.asInstanceOf[Try[String]]
        case Success((url, _)) => Success(url)
        case Failure(e) =>
          Failure(
            ServerComponentNotAvailableException(
              s"hashToUrlAsync failed for [$hash]",
              e
            )
          )
      }(ec)
  ...
  def urlToHashAsync(url: String, timeContext: Option[Long])(
    implicit ec: ExecutionContext
  ): Future[String] =
    newestGreatestMetaNsCacheImpl
      ...
      .transform {
        ...
        case Failure(e) =>
          Failure(
            ServerComponentNotAvailableException(
              s"urlToHashAsync failed for [$url]",
              e
            )
          )
  ...
  def getIdentifierForPrefixAsync(prefix: String, timeContext: Option[Long])(
    implicit ec: ExecutionContext
  ): Future[String] =
    newestGreatestMetaNsCacheImpl
      ...
      .transform {
        ...
        case Failure(e) =>
          Failure(
            ServerComponentNotAvailableException(
              s"getIdentifierForPrefixAsync failed for [$prefix]",
              e
            )
          )
        case success => success
    ...
    f.value.fold[Option[(String, String)]] {
      if (nsID.matches(looksLikeHashedNsIDRegex))
        ...
        Await
          .ready(f, 9.seconds)
          .value
          .flatMap(transformFuncURLAndPrefix2URLAndPrefix)
      else
        None
  ...
  /**
    ...
    */
  def nsUrlToHash(url: String,
                  timeContext: Option[Long]): (String, PrefixState) = {
    ...
    def inner(hash: String): Future[(String, PrefixState)] =
      hashToUrlAsync(hash, timeContext)(globalExecutionContext).transformWith {
        ...
        case Failure(_: NoSuchElementException) =>
          Future.successful(hash -> Create)
        case Failure(err) =>
          Future.failed(
            new Exception(
              s"nsUrlToHash.inner failed for url [$url] and hash [$hash]",
              err
            )
          )
        case Success(notSameUrl /* not same url */ ) => {
          ...
          logger.warn(
            s"double hashing url's [$url] hash [$hash] to [$doubleHash] because not same as [$notSameUrl]"
          )
          inner(doubleHash)
        }
  ...
  @inline def hashIterator(url: String) =
    Iterator.iterate(cmwell.util.string.Hash.crc32base64(url))(
      cmwell.util.string.Hash.crc32base64
    )

  val seqInfotonToSetString =
  ...
  def getTheFirstGeneratedMetaNsInfoton(
    url: String,
    ...
    hashChain.find(hashSet) match {
      ...
      case None =>
         ...
         */
        logger.warn(
          s"hashChain ${hashChain.mkString("[", ", ", "]")} did not contain a valid identifier for ${hashSet
            .mkString("[", ", ", "]")}"
        )
  ...
  def hashToInfotonAsync[T](
    hash: String
  )(out: (Infoton, (String, String)) => T)(ec: ExecutionContext): Future[T] =
    crudServiceFS
      .getInfotonByPathAsync(s"/meta/ns/$hash")
      .transform {
        case Failure(err) =>
          Failure(
            new IllegalStateException(s"could not load /meta/ns/$hash", err)
          )
        case Success(EmptyBox) =>
          Failure(new NoSuchElementException(s"could not load /meta/ns/$hash"))
        case Success(BoxedFailure(err)) =>
          Failure(
            new IllegalStateException(
              s"could not load /meta/ns/$hash from IRW",
              err
            )
          )
        case Success(FullBox(infoton)) =>
          validateInfoton(infoton).transform(
            ...
            e =>
              Failure(
                new IllegalStateException(
                  s"loaded invalid infoton for [$hash] / [$infoton]",
                  e
                )
            )
          )
  ...
  def getFirstHashForNsURL(
    url: String,
    infotons: Seq[Infoton],
    timeContext: Option[Long]
  ): Future[Either[String, Infoton]] = {
    ...
    def foldWhile(usedHashes: Set[String]): Future[Either[String, Infoton]] = {
      ...
      if (usedHashes(hash))
        ...
        hashToUrlAsync(hash, timeContext)(globalExecutionContext)
          .transformWith {
            case Success(`url`) =>
              infotons
                .find(_.name == hash)
                .fold[Future[Either[String, Infoton]]] {
                  logger.error(
                    s"hash [$hash] returned the right url [$url], but was not found in original seq?!?!?"
                  )
                  // getting the correct infoton anyway:
                  hashToInfotonAsync(hash)((infoton, _) => infoton)(
                    globalExecutionContext
                  ).map(Right.apply)(globalExecutionContext)
                }(
                  (Future
                    .successful[Either[String, Infoton]] _).compose(Right.apply)
                )
...
//   v
  ...
  private[this] val graphToAliasCache: LoadingCache[String, String] =
    CacheBuilder
      .newBuilder()
        ...
        new CacheLoader[String, String] {
          override def load(url: String): String = {
            ...
            f.onComplete {
              ...
              case Success(None) =>
                logger.trace(s"load for graph: $url is empty")
  ...
  private[this] val aliasToGraphCache: LoadingCache[String, String] =
    CacheBuilder
      .newBuilder()
      .maximumSize(Settings.quadsCacheSize)
      .expireAfterWrite(2, TimeUnit.MINUTES)
      .build {
        new CacheLoader[String, String] {
          override def load(alias: String): String = {
            val f =
              getQuadUrlForAliasAsyncActual(alias)(injectedExecutionContext)
            f.onComplete {
  ...
  def getAliasForQuadUrl(graphName: String): Option[String] =
    Try(graphToAliasCache.get(graphName)).toOption

  def getAliasForQuadUrlAsync(
    graph: String
    ...
    f.onComplete {
      ...
      case Failure(e) =>
        logger.error(s"getAliasForQuadUrlAsync for $graph failed", e)
    }
    f
  }

  def getQuadUrlForAlias(alias: String): Option[String] =
    Try(aliasToGraphCache.get(alias)).toOption

  def getQuadUrlForAliasAsync(
    alias: String
    ...
    f.onComplete {
      ...
      case Failure(e) =>
        logger.error(s"getQuadUrlForAliasAsync for $alias failed", e)
    }
    f
  }

  def getQuadUrlForAliasAsyncActual(
    alias: String
  )(implicit ec: ExecutionContext): Future[String] = {
    crudServiceFS
      .search(
      ...
      .map {
        case SearchResults(_, _, total, _, length, infotons, _) => {
          require(
            total != 0,
            s"the alias $alias is not associated to any graph"
          )
          ...
          val url = {
            ...
            require(
              byUrl.size == 1,
              s"group by url must be unambiguous: $byUrl"
            )
            ...
            require(
              urlSet.isDefined,
              s"url must have keys defined: $byUrl, $urlSet"
            )
            ...
            require(
              urls.head.value.isInstanceOf[String],
              "url value not a string"
            )
  ...
  private[this] def getAliasForQuadUrlAsync(
    graphName: String,
    byAlg: ByAlg = ByBase64
  )(implicit ec: ExecutionContext): Future[Option[String]] = {
