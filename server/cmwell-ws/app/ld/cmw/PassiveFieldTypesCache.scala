...
object PassiveFieldTypesCache {
  ...
  private[cmw] class PassiveFieldTypesCacheActor(
    ...
    override def receive: Receive = {
      ...
      case UpdateCache =>
        if (requestedCacheUpdates.nonEmpty) {
          requestedCacheUpdates.foreach { fk =>
            ...
            if (maybe eq null) {
              val lefuture = Left(
                getMetaFieldInfoton(fk)
                  .map(infoptToChars)(updatingExecutionContext)
              )
        ...
        if (maybe eq null) {
          ...
          maybe match {
            case Left(future) =>
              future.onComplete {
                case Failure(error) =>
                  self ! Put(internalKey, types, reportWhenDone, Some(sendr))
                case Success(chars) => {
                  if (types.diff(chars).nonEmpty)
                    self ! Put(
                      internalKey,
            ...
            case Right((_, chars)) => {
              if (types.diff(chars).nonEmpty) {
                cache.put(
                  internalKey,
                  Right(System.currentTimeMillis() -> (chars.union(types)))
                )
              }
              if (reportWhenDone) {
                sendr ! UpdateCompleted
      ...
      case UpdateAndGet(field: FieldKey) => {
        val sndr = sender()
        val rv = getMetaFieldInfoton(field).map(infoptToChars)(
          updatingExecutionContext
        )
        rv.onComplete { //field.metaPath is already completed as it is memoized in a lazy val if it is truly async
          case Failure(e) =>
            logger.error(s"failed to update cache for: ${field.metaPath}", e)
          case Success(types) => {
            ...
            if (maybe eq null) {
              ...
              maybe match {
                case Right((oTime, chars)) => {
                  ...
                  cache.put(
                    internalKey,
                    Right(math.max(oTime, nTime) → allTypes)
                  )
                  sndr ! allTypes
                }
    ...
    private def getMetaFieldInfoton(field: FieldKey): Future[Option[Infoton]] =
      crudService
        ...
        .transform {
          case Failure(err) =>
            Failure(
              new Exception(s"failed to getMetaFieldInfoton($field)", err)
            )
          case Success(BoxedFailure(err)) =>
            Failure(
              new Exception(
                s"failed to getMetaFieldInfoton($field) from IRW",
                err
              )
            )
...
trait PassiveFieldTypesCacheTrait {
  ...
  def update(fieldKey: FieldKey, types: Set[Char])(
    implicit ec: ExecutionContext
  ): Future[Unit]
}

abstract class PassiveFieldTypesCache(
  val cache: Cache[String, Either[Future[Set[Char]], (Long, Set[Char])]]
  ...
  private val cbf =
    implicitly[
      CanBuildFrom[MSet[FieldKey], (String, FieldKey), MSet[(String, FieldKey)]]
    ]
  ...
  def get(fieldKey: FieldKey, forceUpdateForType: Option[Set[Char]] = None)(
    ...
    case NnFieldKey(k)
        if k.startsWith("system.") || k.startsWith("content.") || k
          .startsWith("link.") =>
    ...
    case field =>
      Try {
        ...
        if (maybeEither eq null)
          (actor ? UpdateAndGet(field)).mapTo[Set[Char]].transform {
            case Success(s) if s.isEmpty =>
              Failure(
                new NoSuchElementException(
                  s"(async) empty type set for [$field] ([$forceUpdateForType],[$maybeEither])"
                )
              )
              ...
              forceUpdateForType match {
                case None =>
                  ...
                  if (types.isEmpty)
                    Future.failed(
                      new NoSuchElementException(
                        s"empty type set for [$field] ([$forceUpdateForType],[$maybeEither],${types
                          .mkString("[", ",", "]")})"
                      )
                    )
        ...
        case t: Throwable =>
          Future.failed[Set[Char]](
            new Exception(s"failed to  get([$field], [$forceUpdateForType])", t)
          )
      }.get
  }

  def update(fieldKey: FieldKey,
             types: Set[Char])(implicit ec: ExecutionContext): Future[Unit] =
    fieldKey match {
      case NnFieldKey(k)
          if k.startsWith("system.") || k.startsWith("content.") || k
            .startsWith("link.") =>
            ...
            case Left(future) =>
              future
                ...
                .recoverWith {
                  case err: Throwable => {
                    logger.error(
                      "cannot update cache. internalKey failure.",
                      err
                    )
                    doneFut
                  }
...
class passiveFieldTypesCacheImpl(crud: CRUDServiceFS,
                                 ec: ExecutionContext,
                                 sys: ActorSystem)
    extends
  ...
  private val props =
    Props(
      classOf[PassiveFieldTypesCache.PassiveFieldTypesCacheActor],
      crud,
      cache,
      ec
    )
  override def createActor: ActorRef =
    sys.actorOf(
      props,
      "passiveFieldTypesCacheImpl_" + PassiveFieldTypesCache.uniqueIdentifierForActorName
    )
}
