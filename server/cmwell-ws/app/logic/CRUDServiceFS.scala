...
@Singleton
class CRUDServiceFS @Inject()(implicit ec: ExecutionContext, sys: ActorSystem)
    extends LazyLogging {

  import cmwell.ws.Settings._

  lazy val passiveFieldTypesCache =
    new passiveFieldTypesCacheImpl(this, ec, sys)
  val level: ConsistencyLevel = ONE

  lazy val defaultParallelism = cmwell.util.os.Props.os.getAvailableProcessors
  lazy val zStore = ZStore(
    Dao(irwServiceDaoClusterName, irwServiceDaoKeySpace2, irwServiceDaoHostName)
  )
  lazy val zCache = new ZCache(zStore)

  lazy val irwService = IRWService.newIRW(
    Dao(
      irwServiceDaoClusterName,
      irwServiceDaoKeySpace2,
      irwServiceDaoHostName
    ),
  ...
  val ESMappingsCache = new SingleElementLazyAsyncCache[Set[String]](
    Settings.fieldsNamesCacheTimeout.toMillis,
    Set.empty
  )(ftsService.getMappings(withHistory = true))

  val metaNsCache =
    new SingleElementLazyAsyncCache[Set[String]](
      Settings.fieldsNamesCacheTimeout.toMillis,
      Set.empty
    )(fetchEntireMetaNsAsPredicates)

  private val fieldsSetBreakout =
    scala.collection.breakOut[Seq[Option[String]], String, Set[String]]

  private def fetchEntireMetaNsAsPredicates = {
    val chunkSize = 512
    def fetchFields(offset: Int = 0): Future[Seq[Infoton]] = {
      val fieldsFut = search(
        Some(PathFilter("/meta/ns", descendants = true)),
        paginationParams = PaginationParams(offset, chunkSize),
        withData = true
      )
      fieldsFut.flatMap { fields =>
        if (fields.length == chunkSize)
          fetchFields(offset + chunkSize).map(_ ++ fields.infotons)
        else Future.successful(fields.infotons)
      }
    }

    fetchFields().map { f =>
      val (fieldsInfotons, namespacesInfotons) =
        f.partition(_.path.count('/'.==) > 3)
      val prefixToUrl = (for {
      ...
      val fieldsSet = fieldsInfotons
        .map { infoton =>
          val lastTwoPathParts =
            infoton.path.split('/').reverseIterator.take(2).toArray.reverse
  ...
  def getInfoton(path: String,
                 offset: Option[Int],
                 length: Option[Int]): Future[Option[ContentPortion]] = {

    def listChildrenBoundedTime(
      path: String,
      offset: Option[Int],
      length: Option[Int]
    ): Future[Option[FTSSearchResponse]] = {
      val fut =
  ...
  def getInfotonHistory(path: String,
                        limit: Int): Future[InfotonHistoryVersions] = {
    val (_, uuidVec) = irwService.history(path, limit).sortBy(_._1).unzip
    if (uuidVec.isEmpty)
      Future.successful(InfotonHistoryVersions(Vector.empty[Infoton]))
    else
      irwService
        .readUUIDSAsync(uuidVec, level)
        .map(
          seq => InfotonHistoryVersions(seq.collect { case FullBox(i) => i })
        )
  }

  def getRawPathHistory(path: String,
                        limit: Int): Future[Vector[(Long, String)]] =
      ...
      .mapAsync(defaultParallelism) {
        case (_, uuid) =>
          irwService.readUUIDAsync(uuid).andThen {
            case Failure(fail) =>
              logger.error(
                s"uuid [$uuid] could not be fetched from cassandra",
                fail
              )
            case Success(EmptyBox) =>
              logger.error(
                s"uuid [$uuid] could not be fetched from cassandra: got EmptyBox"
              )
            case Success(BoxedFailure(e)) =>
              logger.error(
                s"uuid [$uuid] could not be fetched from cassandra: got BoxedFailure",
                e
              )
          }
      }
  ...
  def getInfotonsByPathOrUuid(
    paths: Vector[String] = Vector.empty[String],
    uuids: Vector[String] = Vector.empty[String]
  ): Future[BagOfInfotons] = {
    val futureInfotonsList: Future[List[Infoton]] = (paths, uuids) match {
      case (ps, us) if ps.isEmpty && us.isEmpty => Future.successful(Nil)
      case (ps, us) if us.isEmpty =>
        irwService
          .readPathsAsync(ps, level)
          .map(_.collect { case FullBox(i) => i }.toList)
      case (ps, us) if ps.isEmpty =>
        irwService
          .readUUIDSAsync(us, level)
          .map(_.collect { case FullBox(i) => i }.toList)
      case (ps, us) => {
        val f1 = irwService
          .readPathsAsync(ps, level)
  ...
  def putInfoton(infoton: Infoton,
                 isPriorityWrite: Boolean = false): Future[Boolean] = {
    require(
      infoton.kind != "DeletedInfoton",
    ...
    if (infoton.fields.map(_.map(_._2.size).sum).getOrElse(0) > 10000) {
      ...
      val payloadForIndirectLargeInfoton: Future[(Array[Byte], Array[Byte])] =
        infoton match {
          case i @ FileInfoton(
                _,
                _,
                ...
                _,
                _,
                Some(FileContent(Some(data), _, _, _)),
                _
            ...
            zStore.put(fi.content.flatMap(_.dataPointer).get, data).map { _ =>
              ...
              if (i.lastModified.getMillis == 0L)
                withPossibleEpoch -> CommandSerializer
                  .encode(WriteCommand(fi.copy(lastModified = new DateTime())))
          ...
          case i => {
            val t =
              if (i.lastModified.getMillis == 0L)
                payload -> CommandSerializer.encode(
                  WriteCommand(i.copyInfoton(lastModified = new DateTime()))
                )
  ...
  def putInfotons(infotons: Vector[Infoton],
    ...
    def kafkaWritesRes(infos: Vector[Infoton],
                       isPriorityWriteInner: Boolean): Future[Unit] = {
      Future
        .traverse(infos) {
          case infoton if infoton.lastModified.getMillis == 0L =>
            sendToKafka(
              WriteCommand(
                infoton
                  .copyInfoton(lastModified = DateTime.now(DateTimeZone.UTC)),
          ...
          case infoton =>
            sendToKafka(
              WriteCommand(
                infoton,
                validTid(infoton.path, tid),
                atomicUpdates.get(infoton.path)
              ),
              isPriorityWriteInner
            )
        }
  ...
  def deleteInfotons(
    deletes: List[(String, Option[Map[String, Set[FieldValue]]])],
    ...
    val commands: List[SingleCommand] = deletes.map {
      case (path, Some(fields)) =>
        DeleteAttributesCommand(
          ...
          fields,
          dt,
          validTid(path, tidOpt),
          atomicUpdates.get(path)
        )
      case (path, None) =>
        DeletePathCommand(
          path,
  ...
  def deleteInfoton(path: String,
                    data: Option[Map[String, Set[FieldValue]]],
                    isPriorityWrite: Boolean = false) = {
      ...
      val ups = inserts.map { i =>
        {
          ...
          val ins = i.fields match {
            ...
            case None =>
              eMap //TODO: should we block this option? regular DELETE could have been used instead...
          }
          UpdatePathCommand(
            i.path,
            del,
        ...
        .flatMap { _ =>
          if (regs.isEmpty) Future.successful(())
            ...
            Future.traverse(regs) {
              case cmd @ UpdatePathCommand(_, _, _, lastModified, _, _)
                  if lastModified.getMillis == 0L =>
                sendToKafka(
                  cmd.copy(lastModified = DateTime.now(DateTimeZone.UTC)),
                  isPriorityWrite
                )
  ...
  private def sendToKafka(command: SingleCommand,
                          isPriorityWrite: Boolean = false): Future[Unit] =
    sendToKafka(
      command.path,
      CommandSerializer.encode(command),
      isPriorityWrite
    )

  private def sendToKafka(path: String,
                          payload: Array[Byte],
                          isPriorityWrite: Boolean): Future[Unit] = {
    ...
    val topicName =
      if (isPriorityWrite) s"$persistTopicName.priority" else persistTopicName
    ...
    payloadForKafkaFut.flatMap { payloadForKafka =>
      ...
      injectFuture(kafkaProducer.send(pRecord, _)).map { recMD =>
        if (isPriorityWrite) {
          logger.info(
            s"sendToKafka priority for path [$path] and record [${recMD
              .offset()},${recMD.partition()}]"
          )
        }
      }
    }
  ...
  val thinSearchResultsBreakout = scala.collection
    .breakOut[Seq[FTSThinInfoton], SearchThinResult, Vector[SearchThinResult]]
  def thinSearch(pathFilter: Option[PathFilter] = None,
    ...
    implicit searchTimeout: Option[Duration] = None
  ): Future[SearchThinResults] = {

    val searchResultsFuture = {
      ftsService.thinSearch(
        pathFilter,
        fieldFilters,
        datesFilter,
        paginationParams,
...
//  }
  ...
  def search(
    ...
    def ftsResults2Dates(
      ftsResults: FTSSearchResponse
        ...
        irwService.readUUIDSAsync(ftsResults.infotons.map { i =>
          i.uuid
            ...
            if (infotonsSeq.exists(_._1.isEmpty)) {
              val esUuidsSet: Set[String] = ftsResults.infotons.map(_.uuid)(
                scala.collection.breakOut[Seq[Infoton], String, Set[String]]
              )
              val casUuidsSet: Set[String] = infotonsSeq.collect {
                case (FullBox(i), _) => i.uuid
              }(
                scala.collection
                  .breakOut[Vector[
                    (Box[Infoton], Option[Map[String, Set[FieldValue]]])
                  ], String, Set[String]]
              )
              logger.error(
            ...
            val infotons = {
              if (fieldSortParams eq NullSortParam) infotonsSeq.collect {
                case (FullBox(i), _) => i
              } else
  ...
  private def addExtras(infoton: Infoton,
                        extra: Option[Map[String, Set[FieldValue]]]): Infoton =
    infoton match {
  ...
  private def addIndexTime(fromCassandra: Seq[Infoton],
                           fromES: Seq[Infoton]): Seq[Infoton] = {
    val m = fromES.collect {
      case i if i.indexTime.isDefined => i.uuid -> i.indexTime.get
    }.toMap
    fromCassandra.map {
      case i: ObjectInfoton if m.isDefinedAt(i.uuid) && i.indexTime.isEmpty =>
        i.copy(indexTime = m.get(i.uuid))
      case i: FileInfoton if m.isDefinedAt(i.uuid) && i.indexTime.isEmpty =>
        i.copy(indexTime = m.get(i.uuid))
      case i: LinkInfoton if m.isDefinedAt(i.uuid) && i.indexTime.isEmpty =>
        i.copy(indexTime = m.get(i.uuid))
      case i: DeletedInfoton if m.isDefinedAt(i.uuid) && i.indexTime.isEmpty =>
        i.copy(indexTime = m.get(i.uuid))
      case i => i
    }
  }
  ...
  def getListOfDC: Future[Seq[String]] = {
    ftsService.listChildren("/meta/sys/dc", 0, 20).map { sr =>
      Settings.dataCenter +: sr.infotons
        .map(_.path.drop("/meta/sys/dc/".length))
    }
  }

  def getLastIndexTimeFor(
    dc: String = Settings.dataCenter,
    withHistory: Boolean,
    ...
    def mkVirtualInfoton(indexTime: Long): VirtualInfoton = {
      val fields =
        Map(
          "lastIdxT" -> Set[FieldValue](FLong(indexTime)),
          "dc" -> Set[FieldValue](FString(dc))
        )
      val fieldsWithFilter = fieldFilters.fold(fields)(
        ff =>
          fields + ("qp" -> Set[FieldValue](
            FString(Encoder.encodeFieldFilter(ff))
          ))
      )
      val fieldsWithFilterAndWh = fieldsWithFilter + ("with-history" -> Set[
        FieldValue
      ](FBoolean(withHistory)))
      VirtualInfoton(
        ObjectInfoton(
          s"/proc/dc/$dc",
          Settings.dataCenter,
          None,
          fieldsWithFilterAndWh
        )
      )
    }
  ...
  private def toFieldValues(ss: Set[String]): Set[FieldValue] =
    ss.map(FString.apply)
    ...
    ftsService
      ...
      .map { ftsResults =>
        IterationResults(
          ftsResults.scrollId,
          ftsResults.total,
          debugInfo = ftsResults.searchQueryStr
        )
      }
  }
...
//  }
  ...
  def scroll(scrollId: String,
             scrollTTL: Long,
             withData: Boolean): Future[IterationResults] = {
          ...
          irwService
            ...
            .map { infotonsSeq =>
              if (infotonsSeq.exists(_.isEmpty)) {
                ...
                val casUuidsSet: Set[String] = infotonsSeq.collect {
                  case FullBox(i) => i.uuid
                }(
                  scala.collection
                    .breakOut[Seq[Box[Infoton]], String, Set[String]]
                )
                logger.error(
                  "some uuids retrieved from ES, could not be retrieved from cassandra: " + esUuidsSet
                    .diff(casUuidsSet)
              ...
              val infotons = addIndexTime(infotonsSeq.collect {
                case FullBox(i) => i
              }, ftsResults.infotons)
              IterationResults(
                ftsResults.scrollId,
  ...
  def rFix(path: String,
           parallelism: Int = 1): Future[Source[(Boolean, String), NotUsed]] = {
  ...
  def info(path: String,
           limit: Int): Future[(CasInfo, EsExtendedInfo, ZStoreInfo)] =
    proxyOps.info(path, limit)
  ...
  def reactiveRawCassandra(uuid: String): Source[String, NotUsed] =
    irwService.getReactiveRawRow(uuid, QUORUM)

  // assuming not the only version of the infoton!
  def purgeUuid(infoton: Infoton): Future[Unit] = {
    irwService.purgeHistorical(infoton, isOnlyVersion = false, QUORUM).flatMap {
      _ =>
  ...
  def purgeUuidFromIndex(uuid: String, index: String): Future[Unit] = {
    ftsService
      .purgeByUuidsAndIndexes(Vector(uuid -> index))
      .map(_ => ()) //TODO also purge from ftsServiceNew
  }

  def purgePath(path: String,
                includeLast: Boolean,
                limit: Int): Future[Unit] = {

    import scala.language.postfixOps

    val casHistory = irwService.history(path, limit)
    val lastOpt =
      if (casHistory.nonEmpty) Some(casHistory.maxBy(_._1)) else None

    // union uuids from es and cas (and keeping indexes, if known):
    val allPossibleUuidsFut =
      ftsService.info(path, DefaultPaginationParams, withHistory = true).map {
        esInfo =>
          ...
          if (includeLast || lastOpt.isEmpty)
            ...
            esInfo
              .filterNot(_._1 == lastOpt.get._2) -> (allUuids - lastOpt.get._2)
      }
          ...
          case allUuids =>
            ...
            purgeJustByUuids.flatMap { bulkResponse =>
              if (bulkResponse.hasFailures) {
                throw new Exception(
                  "purge from es by uuids from all Indexes failed: " + bulkResponse
                    .buildFailureMessage()
                )
              } else {
                ...
                if (bulkResponse.hasFailures) {
                  throw new Exception(
                    "purge from es by uuids on specific Indexes failed: " + bulkResponse
                      .buildFailureMessage()
                  )
                } else {
                  val purgeHistoryFut =
                    if (includeLast || lastOpt.isEmpty) {
                      ...
                      Future.traverse(casHistory.map(_._2))(
                        irwService.purgeFromInfotonsOnly(_)
                      )
                    } else {
                      Future.traverse(casHistory.filter(lastOpt.get !=)) { h =>
                        irwService.purgeHistorical(
                          path,
                          h._2,
                          h._1,
                          isOnlyVersion = false,
    ...
    irwService
      ...
      .map { casHistory =>
        if (casHistory.isEmpty) Future.successful(())
          ...
          val sortedCasHistory =
            casHistory.sortBy(_._1).map { case (lm, uuid) => Version(lm, uuid) }
          val last = sortedCasHistory.last
          val prev = sortedCasHistory.init.lastOption

          def purgeLast(isTherePrev: Boolean) =
          ...
          def setPrevAsLast(pv: Version) =
            cmwell.util.concurrent.retry(3, 1.seconds) {
              irwService
                .setPathLast(
                  path,
                  new java.util.Date(pv.lastModified),
                  pv.uuid,
                  ConsistencyLevel.QUORUM
                )
                .flatMap { _ =>
                  irwService.readUUIDAsync(pv.uuid).flatMap { infpot =>
                    val prevInfoton = infpot.getOrElse(
                      throw new RuntimeException(
                        s"Previous infoton for path $path was not found under uuid ${pv.uuid}"
                      )
                    )
                    ftsService
                      .purgeByUuidsFromAllIndexes(Vector(pv.uuid))
                      .flatMap { _ =>
      ...
      retry(3, 1.seconds) {
        ...
        purgeEsByUuids.flatMap { bulkResponse =>
          if (bulkResponse.hasFailures) {
            ...
            val purgeFromInfoton =
              Future.traverse(uuids)(irwService.purgeFromInfotonsOnly(_))
  ...
  /**
    ...
    */
  private def injectFuture(
    f: Callback => java.util.concurrent.Future[RecordMetadata],
