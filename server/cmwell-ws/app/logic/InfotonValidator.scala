...
object InfotonValidator extends LazyLogging {
  def isInfotonNameValid(path: String): Boolean = {
    ...
    (!(noSlash.matches("_(.)*|(ii|zz|proc)(/(.)*)?"))) match {
      case true => true
      case false =>
        logger.warn(s"validation failed for infoton path: $path"); false
    }
  }
  ...
  def validateValueSize[K](fields: Fields[K]): Unit =
    if (fields.exists {
          case (_, s) => s.exists(_.size > cmwell.ws.Settings.maxValueWeight)
        })
      throw new IllegalArgumentException(
        "uploaded infoton, contains a value heavier than 16K."
      )
}
