...
@Singleton
class EagerAuthCache @Inject()(crudServiceFS: CRUDServiceFS)(
  ...
  def getRole(roleName: String): Option[JsValue] = {
    data.roles.get(roleName).orElse {
      Await
        .result(directReadFallback(s"/meta/auth/roles/$roleName"), 6.seconds)
        .map { role =>
          logger.debug(
            s"AuthCache role $roleName was not in memory, but added to Map"
          )
  ...
  def getUserInfoton(userName: String): Option[JsValue] = {
    data.users.get(userName).orElse {
      Await
        .result(directReadFallback(s"/meta/auth/users/$userName"), 6.seconds)
        .map { user =>
          logger.debug(
            s"AuthCache user $userName was not in memory, but added to Map"
          )
  ...
  def invalidate(): Future[Boolean] =
    if (isLoadingSemaphore) Future.successful(false) else load()
  ...
  private def load(): Future[Boolean] = {
    ...
    unsafeRetryUntil[AuthData](
      isSuccessful = !_.isEmpty,
      maxRetries = 10,
      delay = 5.seconds
    )(loadOnce())
      .map { d =>
  ...
  private def loadOnce(): Future[AuthData] = {
    ...
    crudServiceFS
      .search(
        Some(PathFilter("/meta/auth", descendants = true)),
        withData = true,
        paginationParams = PaginationParams(0, 2048)
      )
      .map { searchResult =>
        val data = searchResult.infotons
          ...
          .toMap
        val (usersData, rolesData) =
          cmwell.util.collections.partitionWith(data) { t =>
            val (path, payload) = t
        ...
        logger.debug(
          s"AuthCache Loaded with ${usersData.size} users and ${rolesData.size} roles."
        )
  ...
  private def directReadFallback(infotonPath: String) =
    crudServiceFS.getInfoton(infotonPath, None, None).map {
      ...
      case other =>
        if (!serverIsWarmingUp) {
          logger.warn(
            s"AuthCache Trying to read $infotonPath but got from CAS $other"
          )
        }
        None
    }

  private def extractPayload(infoton: Infoton): Option[JsValue] =
    infoton match {
      case FileInfoton(
          _,
          _,
          ...
          _,
          _,
          Some(FileContent(Some(payload), _, _, _)),
          _
          ) =>
        val jsValOpt = Try(Json.parse(payload)).toOption
        if (jsValOpt.isEmpty)
          logger.warn(
            s"AuthCache Infoton(${infoton.path}) has invalid JSON content."
          )
        jsValOpt
      case _ =>
        logger.warn(
          s"AuthCache Infoton(${infoton.path}) does not exist, or is not a FileInfoton with valid content."
        )
        None
    }

  private def combiner(ad1: AuthData, ad2: AuthData): AuthData =
    if (ad2.isEmpty) ad1 else ad2

  // todo - Leave JsValue for parsing only. Use case classes for User and Role !
  case class AuthData(users: Map[String, JsValue],
                      roles: Map[String, JsValue]) {
