...
class AuthUtils @Inject()(authCache: EagerAuthCache,
                          authorization: Authorization,
                          crudServiceFS: CRUDServiceFS) {
  def changePassword(token: Token,
                     currentPw: String,
                     newPw: String): Future[Boolean] = {
    if (!token.isValid) {
      ...
      authCache.getUserInfoton(token.username) match {
        case Some(user) if Authentication.passwordMatches(user, currentPw) => {
          ...
          val digest2Value =
            cmwell.util.string.Hash.md5(s"${token.username}:cmwell:$newPw")
          val newUserInfoton = user.as[JsObject] ++ JsObject(
            Seq(
              "digest" -> JsString(digestValue),
              "digest2" -> JsString(digest2Value)
            )
          )
  ...
  val useAuthorizationParam: Boolean =
  ...
  def isValidatedAs(tokenOpt: Option[Token], expectedUsername: String) =
    tokenOpt.exists(
      token => token.isValid && token.username == expectedUsername
    )
    ...
    tokenOpt match {
      case Some(token) if token.isValid => {
        authCache.getUserInfoton(token.username) match {
          case Some(user) =>
            paths.filterNot(
              path => authorization.isAllowedForUser((path, level), user)
            )
      ...
      case _ =>
        paths.filterNot(
          p => authorization.isAllowedForAnonymousUser(p -> level)
        )
    }
  }

  def isOperationAllowedForUser(op: Operation,
                                token: Option[Token],
                                evenForNonProdEnv: Boolean = false): Boolean = {
  ...
  def isWriteToMeta(level: PermissionLevel, path: String) = {
    level == PermissionLevel.Write && path.matches("/meta/.*") && !path.matches(
      "/meta/sys/dc/.*"
    )
  }
  ...
  private def getUser(tokenOpt: Option[Token]) =
    tokenOpt.collect {
      case token if token.isValid => authCache.getUserInfoton(token.username)
    }.flatten
