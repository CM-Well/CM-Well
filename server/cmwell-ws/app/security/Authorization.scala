...
object Authorization extends LazyLogging {
  ...
  implicit class StringExtensions(s: String) {
    ...
    def isSubfolderOf(path: String) = {
      ...
      s.appendSlash == normalizedPath || (path.length < s.length && s
  ...
  implicit class JsValueExtensions(v: JsValue) {
    def getArr(prop: String): Seq[JsValue] =
      (v \ prop).getOrElse(JsArray(Seq())) match {
...
class Authorization @Inject()(authCache: EagerAuthCache) extends LazyLogging {
  ...
  private def anonymousUser =
    authCache.getUserInfoton("anonymous").getOrElse(defaultAnonymousUser)
  ...
  def isAllowedForUser(request: (String, PermissionLevel),
                       user: JsValue,
                       username: Option[String] = None): Boolean = {
    def relevant(path: JsValue): Boolean = {
      val levels = (path \ "permissions")
        .as[String]
        .map {
          case 'r' => PermissionLevel.Read; case 'w' => PermissionLevel.Write
        }
        .toSet
    ...
    val rolesPaths =
      user.getArr("roles").flatMap(getRolesPaths).filter(relevant)
  ...
  def isOperationAllowedForUser(op: Operation, user: JsValue): Boolean = {
    def extractOperationsFrom(userOrRole: JsValue) =
      userOrRole
        .getArr("operations")
        .map(_.as[String])
        .collect { case Operation(op) => op }
        .toSet
  ...
  def inRoles(user: JsValue)(extractor: JsValue => Boolean) = {
    val roles =
      user.getArr("roles").flatMap(r => authCache.getRole(r.as[String]))
