...
import cmwell.fts.{FieldFilter, MultiFieldFilter, SingleFieldFilter}
...
case class SortedConsumeState(from: Long,
                              path: Option[String],
                              withHistory: Boolean,
                              withDeleted: Boolean,
                              withDescendants: Boolean,
                              qpOpt: Option[FieldFilter])
    extends ConsumeState {
  def asBulk(threshold: Long = Settings.consumeBulkThreshold) =
    BulkConsumeState(
      from,
      None,
      path,
...
case class BulkConsumeState(from: Long,
                            to: Option[Long],
                            path: Option[String],
                            withHistory: Boolean,
                            withDeleted: Boolean,
...
object ConsumeState {
  ...
  def parse[T <: ConsumeState: parser.Parser](s: String): Try[T] = {
    ...
    parser.parseAll(p, s) match {
      ...
      case Success(res, _) =>
        res.qpOpt
          .filter(fieldFilterHasKey("system.indexTime"))
          .fold[Try[T]](scala.util.Success(res)) { _ =>
            scala.util.Failure(
              new IllegalArgumentException(
                "consume API's qp must not contain \"system.indexTime\""
              )
            )
          }
    }
  ...
  def fieldFilterHasKey(key: String)(fieldFilter: FieldFilter): Boolean =
    fieldFilter match {
      case SingleFieldFilter(_, _, fieldName, _) => fieldName == key
      case MultiFieldFilter(_, fieldFilters) =>
        fieldFilters.exists(fieldFilterHasKey(key))
    }
  ...
  val parser = new cmwell.ws.util.FieldFilterParser {
    ...
    object Inner {
      ...
      private[this] def extractRawKeyAsFieldName(
        rff: RawFieldFilter
      ): Try[FieldFilter] =
        rff match {
          ...
          case RawSingleFieldFilter(fo, vo, Right(dfk), v) =>
            S(SingleFieldFilter(fo, vo, dfk.internalKey, v))
      ...
      val qp: Parser[Try[FieldFilter]] = "|" ~> unwrappedFieldFilters.^^(
        extractRawKeyAsFieldName
      )
    ...
    val innerBulkConsumeState = range ~ path ~ h ~ d ~ r ~ threshold ~ ff ^? {
      case from ~ toOpt ~ pathOpt ~ history ~ deleted ~ recursive ~ Some(
            thresholdConcrete
          ) ~ qpOpt =>
        BulkConsumeState(
          from,
    ...
    val sortedConsumeState = innerSortedConsumeState ||| innerBulkConsumeState
      .^^(_.asRegular)

    val bulkConsumeState = innerBulkConsumeState | innerSortedConsumeState.^^(
      _.asBulk(Settings.consumeBulkThreshold)
    )
  }
}
