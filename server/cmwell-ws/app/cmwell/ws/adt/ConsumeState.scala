...
case class SortedConsumeState(from: Long,
  ...
  def asBulk(threshold: Long = Settings.consumeBulkThreshold) =
    BulkConsumeState(from, None, path, withHistory, withDeleted, withDescendants, threshold, qpOpt)
}
...
object ConsumeState {
  ...
  def parse[T <: ConsumeState: parser.Parser](s: String): Try[T] = {
    ...
    parser.parseAll(p, s) match {
      ...
      case Success(res, _) =>
        res.qpOpt.filter(fieldFilterHasKey("system.indexTime")).fold[Try[T]](scala.util.Success(res)) { _ =>
