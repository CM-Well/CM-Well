...
sealed trait CMWellRequest {
  ...
  lazy val queryParameters
    : Map[String, Seq[String]] = requestHeader.queryString ++ queryParams
}

final class CreateConsumer private (
  protected override val requestHeader: RequestHeader,
  protected[this] override val queryParams: Map[String, Seq[String]]
) extends CMWellRequest
object CreateConsumer {
  def apply(rh: RequestHeader, qp: Map[String, Seq[String]]) =
    new CreateConsumer(rh, qp)
  def unapply(cc: CreateConsumer) = Some((cc.path, cc.queryParameters))
}

final class Search private (
  protected override val requestHeader: RequestHeader,
  protected[this] override val queryParams: Map[String, Seq[String]]
) extends CMWellRequest {
...
object Search {
  def apply(rh: RequestHeader, qp: Map[String, Seq[String]]) =
    new Search(rh, qp)
  def unapply(s: Search) =
    Some((s.path, s.base, s.host, s.uri, s.queryParameters))
}
