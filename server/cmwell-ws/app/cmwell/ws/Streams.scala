...
object Streams extends LazyLogging {
  ...
  val bo0 =
    scala.collection.breakOut[Seq[Infoton], ByteString, List[ByteString]]
  val bo1 = scala.collection.breakOut[Seq[Infoton], String, List[String]]
  val bo2 =
    scala.collection.breakOut[Seq[SearchThinResult], String, List[String]]
  val bo3 = scala.collection
    .breakOut[Seq[Infoton], SearchThinResult, List[SearchThinResult]]
  val parallelism =
    math.min(
      cmwell.util.os.Props.os.getAvailableProcessors,
      Settings.cassandraBulkSize
    )
  val endln = ByteString(cmwell.util.os.Props.endln)

  object Flows {

    def infotonToByteString(
      formatter: Formatter
    ): Flow[Infoton, ByteString, NotUsed] =
    ...
    def searchThinResultToByteString(
      formatter: Formatter
    ): Flow[SearchThinResult, ByteString, NotUsed] =
    ...
    def formattableToByteString(
      formatter: Formatter,
      length: Option[Long] = None
    ): Flow[Formattable, ByteString, NotUsed] = {
        ...
        length
          ...
          .collect {
            case Success(renderedString) =>
              if (renderedString.endsWith(cmwell.util.os.Props.endln))
                ByteString(renderedString)
            ...
            Flow
              .fromGraph(
                TakeWeighted[Formattable](
                  l,
                  true, {
                    case IterationResults(_, _, iSeqOpt, _, _) =>
                      iSeqOpt.fold(0)(_.size).toLong
    ...
    def searchThinResultToFatInfoton(
      crudServiceFS: CRUDServiceFS
    ): Flow[SearchThinResult, Infoton, NotUsed] =
      Flow[SearchThinResult]
        .mapAsyncUnordered(parallelism)(
          str => crudServiceFS.getInfotonByUuidAsync(str.uuid)
        )
        .collect { case FullBox(i) => i }

    val searchThinResultsFlattened
      : Flow[SearchThinResults, SearchThinResult, NotUsed] =
      Flow[SearchThinResults]
        .mapConcat { case SearchThinResults(_, _, _, str, _) => str.toList }

    def searchThinResultsToFatInfotons(
      crudServiceFS: CRUDServiceFS
    ): Flow[SearchThinResults, Infoton, NotUsed] =
      searchThinResultsFlattened.via(
        searchThinResultToFatInfoton(crudServiceFS)
      )

    def iterationResultsToFatInfotons(
      crudServiceFS: CRUDServiceFS
    ): Flow[IterationResults, Infoton, NotUsed] =
    ...
    val iterationResultsToInfotons: Flow[IterationResults, Infoton, NotUsed] =
      Flow[IterationResults]
        .collect { case IterationResults(_, _, Some(iSeq), _, _) => iSeq }
        .mapConcat(_.toList)
  }
...
class Streams @Inject()(crudServiceFS: CRUDServiceFS) extends LazyLogging {
  ...
  def scrollSourceToByteString(
    src: Source[IterationResults, NotUsed],
    formatter: Formatter,
    ...
    if (!withData) src.via(Flows.formattableToByteString(formatter, length))
      ...
      val s1: Source[Infoton, NotUsed] =
        length.fold(src.via(Flows.iterationResultsToFatInfotons(crudServiceFS))) {
          l =>
  ...
  ): Future[(Source[IterationResults, NotUsed], Long)] = {
    seqScrollSource(
      ScrollStarter(
        pathFilter,
        fieldFilter,
  ...
  def lazySeqScrollSource(scrollStarter: ScrollStarter, maxParallelism: Int)(
    ...
    if (readyToRunScrollFunctions.length <= maxParallelism)
      seqScrollSourceHandler(readyToRunScrollFunctions.map { f =>
        firstHitTupleApply(scrollStarter.withDeleted)(ec)(f())
      }, scrollStarter.withDeleted)
    else {
      val ScrollStarter(pf, ff, df, pagination, ttl, withHistory, withDeleted) =
        scrollStarter
      ...
      val sources = readyToRunScrollFunctions.map { f =>
        val lazilyAsyncSrc = () =>
          Source
            .fromFuture(f().map(singleScrollSourceHandler(withDeleted, ec)))
  ...
  private def firstHitTupleApply(withDeleted: Boolean)(
    implicit ec: ExecutionContext
  ): Future[IterationResults] => Future[(Future[IterationResults], Long)] =
    _.map { startScrollResult => //map the `Future[IterationResults]` (empty initial results) to a tuple of another `Future[IterationResults]` (this time with data) and the number of total hits for that index
      crudServiceFS.scroll(startScrollResult.iteratorId, 360, false) -> startScrollResult.totalHits
    }

  private def singleScrollSourceHandler(
    withDeleted: Boolean,
    ec: ExecutionContext
  )(firstHit: IterationResults): Source[IterationResults, NotUsed] =
    Source.unfoldAsync(firstHit) {
      case ir @ `firstHit` =>
        crudServiceFS
          .scroll(ir.iteratorId, 60, withData = false)
          .map(iir => Some(iir -> ir))(ec)
      case ir @ IterationResults(iteratorId, _, infotonsOpt, _, _) => {
        infotonsOpt
          .collect { case xs if xs.nonEmpty => ir }
          .fold(
            Future
              .successful(Option.empty[(IterationResults, IterationResults)])
          ) { ir =>
            crudServiceFS
              .scroll(iteratorId, 60, withData = false)
              .map(iir => Some(iir -> ir))(ec)
          }
      }
    }
  ...
  private def seqScrollSourceHandler(
            ...
            Source.unfoldAsync(ir) {
              case ir @ IterationResults(iteratorId, _, infotonsOpt, _, _) =>
                infotonsOpt
                  ...
                  .fold(
                    Future.successful(
                      Option.empty[(IterationResults, IterationResults)]
                    )
                  ) { ir =>
                    crudServiceFS
                      .scroll(iteratorId, 60, withData = false)
    ...
    sfeir.map {
      case Nil =>
        throw new IllegalStateException("must have at least one index")
      case source :: Nil => source
      case (first, h1) :: (second, h2) :: rest =>
        val source =
          Source.combine(first, second, rest.map(_._1): _*)(size => Merge(size))
  ...
  def scrollSource(pathFilter: Option[PathFilter] = None,
    ...
    val firstHitsTuple = crudServiceFS
      ...
      .flatMap { startScrollResult =>
        debugLogID.foreach(
          id => logger.info(s"[$id] startScrollResult: $startScrollResult")
        )
        crudServiceFS
          .scroll(startScrollResult.iteratorId, scrollTTL, withData = false)
          .map { firstScrollResult =>
            debugLogID.foreach(
              id =>
                logger
                  .info(s"[$id] scroll response: ${firstScrollResult.infotons
                    .fold("empty")(i => s"${i.size} results")}")
            )
          ...
          case ir @ IterationResults(iteratorId, _, infotonsOpt, _, _) => {
            infotonsOpt
              ...
              .fold(
                ...
                debugLogID.foreach(
                  id => logger.info(s"[$id] scroll request: $iteratorId")
                )
                crudServiceFS
                  ...
                  .andThen {
                    case Success(res) =>
                      debugLogID.foreach(
                        id =>
                          logger.info(s"[$id] scroll response: ${res.infotons
                            .fold("empty")(i => s"${i.size} results")}")
                      )
                    case Failure(err) =>
                      debugLogID.foreach(
                        id => logger.error(s"[$id] scroll source failed", err)
                      )
                  }
                  .map(iir => Some(iir -> ir))
              }
  ...
  def superScrollSource(pathFilter: Option[PathFilter] = None,
                        fieldFilter: Option[FieldFilter] = None,
                        datesFilter: Option[DatesFilter] = None,
                        paginationParams: PaginationParams =
                          DefaultPaginationParams,
  ...
  private def enrichWithDataAndFlatten(
    src: Source[Seq[Infoton], NotUsed]
  ): Source[Infoton, NotUsed] = {
    src
      .mapConcat {
        case infotons if infotons.isEmpty =>
          List(Future.successful(Vector.empty[Infoton]))
        case infotons => {
          infotons
            .map(_.uuid)(bo1)
  ...
  def qStream(
    ...
    Source
      .unfoldAsync[Long, Source[SearchThinResult, NotUsed]](firstTimeStamp) {
        timeStamp =>
          ...
          future.flatMap {
            case SearchThinResults(total, offset, length, results, _) => {
              if (total == 0) Future.successful(None)
                ...
                Future.successful(
                  Some(
                    results.maxBy(_.indexTime).indexTime -> Source(
                      results.toVector
                    )
                  )
                )
                ...
                require(
                  indexTime > 0,
                  "all infotons in iteration must have a valid indexTime defined"
                )

                //regular chunk with more than 1 indexTime
                if (results.exists(_.indexTime != indexTime)) {

                  val filteredByIndexTimeInfotons =
                    results.filter(_.indexTime < indexTime).toVector
                  val e = Source(filteredByIndexTimeInfotons)
                  val lastIndexTimeInChunk =
                    filteredByIndexTimeInfotons.maxBy(_.indexTime).indexTime
                ...
                //a chunk consisting infotons with the same indexTime
                  ...
                  fut.map {
                    case (_, 0L) => None
                    case (_, `lengthHint`) =>
                      Some(indexTime -> Source(results.toVector))
                    case (s, _) => {
                      val strs = s.mapConcat {
                        case IterationResults(_, _, infoptons, _, _) => {
                          infoptons.fold(List.empty[SearchThinResult])(_.map {
                            i =>
                              SearchThinResult(
                                i.path,
                                i.uuid,
