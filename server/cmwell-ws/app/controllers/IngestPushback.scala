...
import cmwell.util.concurrent.{
  Combiner,
  SimpleScheduler,
  SingleElementLazyAsyncCache
}
import cmwell.ws._
...
@Singleton
class IngestPushback @Inject()(
  backPressureToggler: BackPressureToggler,
  dashBoard: DashBoard,
  pbp: PlayBodyParsers
  ...
  lazy val bGMonitorProxy =
    new SingleElementLazyAsyncCache[OffsetsInfo](10000L, null)({
      Grid
        .serviceRef(BGMonitorActor.serviceName)
        .ask(GetOffsetInfo)(
          akka.util.Timeout(bgMonitorAskTimeout),
          Actor.noSender
        )
        .mapTo[OffsetsInfo]
    })
    ...
    bGMonitorProxy.getAndUpdateIfNeeded
      .map {
        case OffsetsInfo(partitionOffsetsInfos, _) => {
          val (persist, index) =
            partitionOffsetsInfos.values.partition(_.topic == "persist_topic")
          val persistLoad = persist.foldLeft(Map.empty[Int, Long]) {
            case (
                sumsByPartition,
                PartitionOffsetsInfo(_, partition, readOffset, writeOffset, _)
                ) =>
              sumsByPartition.updated(
                partition,
                sumsByPartition
                  .getOrElse(partition, 0L) + writeOffset - readOffset
              )
          }
          ...
          if (persistLoad.exists(_._2 > maximumQueueBuildupAllowed))
            Some(
              Results.ServiceUnavailable(
                "Persistence queue is full. You may try again later"
              )
            )
          else if (indexLoad.exists(_._2 > maximumQueueBuildupAllowed))
            Some(
              Results.ServiceUnavailable(
                "Index queue is full. You may try again later"
              )
            )
          else None
        }
      ...
      .recover {
        ...
        case e: Throwable => {
          logger.error(
            "unexpected error occurred in IngestPushback.filterByKLog()",
            e
          )
          Some(
            Results
  ...
  override def invokeBlock[A](
    ...
    def resOptToFilterBy(resOpt: Option[Result]) = resOpt.fold(block(request)) {
      result =>
        val requestTime = {
        ...
        if (requestTime >= ingestPushbackByServer) Future.successful(result)
          ...
          SimpleScheduler.schedule(ingestPushbackByServer - requestTime)(result)
    }
      ...
      PersistentDMap
        ...
        case "bar" =>
          Future.successful(
            Results
              .ServiceUnavailable(
                s"Ingests has been barred by an admin. Please try again later."
              )
          )
        case unknown =>
          Future.successful(
            Results.InternalServerError(
              s"unknown state for 'BACKPRESSURE_TRIGGER' [$unknown]"
            )
          )
      }
  }
