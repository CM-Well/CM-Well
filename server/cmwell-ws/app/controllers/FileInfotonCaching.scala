...
abstract class FileInfotonCaching(defaultAssetsMetadata: DefaultAssetsMetadata,
  ...
  def treatContentAsAsset(
    ...
    val dontCache = request.path.startsWith("/meta/") || request.path
      .startsWith("/proc/")
    ...
    defaultAssetsMetadata.assetInfoForRequest(request, path).map { aiOpt =>
      val (gzipRequested, gzipAvailable) = aiOpt.fold(false -> false) { ai =>
        ai._2.accepts("gzip") -> ai._1.compressedUrls
          .exists(_._1.endsWith(".gz"))
      }
      maybeNotModified(
        etag,
        lastModified,
  ...
  def result(length: Long,
    ...
    val response =
      Result(ResponseHeader(OK, Map(DATE -> currentTimeFormatted)), entity)
        .as(mimeType)
  ...
  def currentTimeFormatted =
    java.time.LocalDateTime.now().format(ResponseHeader.httpDateFormat)
  ...
  def getNotModified(request: Request[_],
                     lastModified: Long,
                     dontCache: Boolean = false): Option[Result] =
    for {
  ...
  def cacheableResult[A <: Result](
    ...
    val d = LocalDateTime
      .ofInstant(
        Instant.ofEpochMilli(lastModified.getMillis),
