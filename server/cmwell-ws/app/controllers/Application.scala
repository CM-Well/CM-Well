...
object ApplicationUtils {
  ...
  /**
    ...
    * @return
    */
  def infotonPathDeletionAllowed(
    path: String,
    recursive: Boolean,
    crudServiceFS: CRUDServiceFS
  )(implicit ec: ExecutionContext): Future[Either[String, Seq[String]]] = {
    ...
    if (!InfotonValidator.isInfotonNameValid(path)) Future.successful {
      ...
      case sr if sr.total > 500 =>
        Left(
          "recursive DELETE is forbidden for large (>500 descendants) infotons. DELETE descendants first."
        )
...
class Application @Inject()(
  bulkScrollHandler: BulkScrollHandler,
  ...
  assetsConfigurationProvider: AssetsConfigurationProvider
    ...
    extends FileInfotonCaching(
      assetsMetadataProvider.get,
      assetsConfigurationProvider.get
    )
  ...
  lazy val typesCache: passiveFieldTypesCacheImpl =
    crudServiceFS.passiveFieldTypesCache
  val fullDateFormatter =
    ISODateTimeFormat.dateTime().withZone(DateTimeZone.UTC)
  val successResponse = Ok(Json.obj("success" -> true))

  def isReactive[A](req: Request[A]): Boolean =
    req.getQueryString("reactive").fold(false)(!_.equalsIgnoreCase("false"))

  //TODO: validate query params

  private def requestSlashValidator[A](request: Request[A]): Try[Request[A]] =
    Try {
      if (request.path.toUpperCase(Locale.ENGLISH).contains("%2F"))
        throw new UnsupportedURIException(
          "%2F is illegal in the path part of the URI!"
        )
      else {
        val decodedPath = UriEncoding.decodePath(request.path, "UTF-8")
        val requestHeader = request.withTarget(
          RequestTarget(
            uriString = decodedPath + request.uri.drop(request.path.length),
            path = decodedPath,
  ...
  def handleTypesCacheGet =
    Action(_ => Ok(typesCache.getState).as(ContentTypes.JSON))
    ...
    Action.async(r => {
      ...
      def forbiddenOrElse(f: => Future[Result]) =
        if (!isAdminEvenNonProd(r))
          Future.successful(
            Forbidden(
              Json.obj("success" -> false, "message" -> "Not authorized")
            )
          )
        else f
      ...
      r.getQueryString("invalidate")
        .fold {
          if (r.getQueryString("invalidate-all")
                .fold(false)(asBoolean(_).getOrElse(true))) forbiddenOrElse {
            cmwellRDFHelper
            ...
            val quick = r
              .getQueryString("quick")
              .fold(false)(asBoolean(_).getOrElse(true))
              ...
              if (nsID.isEmpty)
                Future.successful(
                  BadRequest(
                    Json.obj(
                      "success" -> false,
                      "message" -> "You need to specify whom to invalidate"
                    )
                  )
                )
              else
    ...
    requestSlashValidator(originalRequest) match {
      ...
      case Success(request) =>
        request match {
          ...
          case Operation.createConsumer() =>
            handleCreateConsumerRequest(request)
          ...
          case Operation.purgeLast() | Operation.rollback() =>
            handlePurgeLast(request)
  ...
  def handlePull(req: Request[AnyContent]): Future[Result] = {
    ...
    extractFieldsMask(req, typesCache, cmwellRDFHelper, timeContext).flatMap {
      fieldsMask =>
        req.getQueryString("sub") match {
          case Some(sub) => {
            val p = Promise[Unit]()
            val source = Source.unfoldAsync[String, ByteString](sub) {
              subscription =>
                Subscriber
                  .pull(subscription)
                  .flatMap(
...
//              case _ if p.isCompleted => Future.successful(None)
                        case v if v.isEmpty =>
                          cmwell.util.concurrent.SimpleScheduler
                            .schedule[Option[(String, ByteString)]](3.seconds)(
                        ...
                        case _ => {
                          ...
                          val f = crudServiceFS
                            .getInfotonsByPathOrUuid(uuids = d.data)
                            .map { bag =>
                              val data = ByteString(
                                bag.infotons
                                  .map(
                                    i => formatter.render(i.masked(fieldsMask))
                                  )
                                  .mkString("", "\n", "\n"),
...
callback=< [URL] >
  ...
  def handleSubscribe(request: Request[AnyContent]): Future[Result] =
    Try {
      ...
      request
        ...
        .getOrElse(
          Right(cmwell.rts.PathFilter(new cmwell.rts.Path(path, true)))
        ) match {
        case Left(error) =>
          Future.successful(BadRequest(s"bad syntax for qp: $error"))
        case Right(rule) =>
          request.getQueryString("format").getOrElse("json") match {
            case FormatExtractor(format) =>
              request.getQueryString("method") match {
                case Some(m) if m.equalsIgnoreCase("pull") =>
                  Subscriber.subscribe(sub, rule, Pull(format)).map(Ok(_))
                    ...
                    case Some(url) =>
                      Subscriber
                        .subscribe(
                          sub,
                          rule,
                          Push(getHandlerFor(format, url, timeContext))
                        )
                        .map(Ok(_))
                    case None =>
                      Future.successful(
                        BadRequest("missing callback for method push")
                      )
                  }
                case _ =>
                  Future.successful(
                    BadRequest(
                      "unsupported or missing method for real time search "
                    )
                  )
              }
            case _ =>
              Future.successful(
                BadRequest(s"un-recognized type: ${request.headers("format")}")
              )
          }
      }
    }.recover(asyncErrorHandler).get

  def getHandlerFor(format: FormatType,
                    url: String,
                    timeContext: Option[Long]): (Seq[String]) => Unit = {
    uuids =>
      {
        ...
        val infotonsFut =
          crudServiceFS.getInfotonsByPathOrUuid(uuids = uuids.toVector)
        ...
        val formatter = formatterManager.getFormatter(
          format,
          timeContext,
          s"http://${cmwell.util.os.Props.machineName}:9000"
        )
        val futureRes = infotonsFut.flatMap { bag =>
          import cmwell.util.http.SimpleResponse.Implicits.UTF8StringHandler
          SimpleHttpClient
            .post[String](url, formatter.render(bag), Some(formatter.mimetype))
        }
  ...
  def handleMetaQuad(base64OfPartition: String) = Action.async { req =>
    val underscorePartition = cmwell.util.string.Base64
      .decodeBase64String(base64OfPartition, "UTF-8") // e.g: "_2"
      ...
      VirtualInfoton(
        ObjectInfoton(
          ...
          Map[String, Set[FieldValue]](
            "alias" -> Set(FString(s"partition_$partition")),
            "graph" -> Set(
              FReference(s"cmwell://meta/sys#partition_$partition")
            )
          )
        )
      )
  ...
  def handleProcGET(path: String) = Action.async { implicit req =>
    {
      val iPath: String = {
        ...
        if (noTrailingSlashes == "/proc" || noTrailingSlashes.isEmpty) "/proc"
        else "/proc/" + noTrailingSlashes
      }
      handleGetForActiveInfoton(req, iPath)
    }
  }

  private def handleGetForActiveInfoton(req: Request[AnyContent],
                                        path: String) =
    getQueryString("qp")(req.queryString)
      .fold(Success(None): Try[Option[RawFieldFilter]])(
      ...
      .map { qpOpt =>
        ...
        val withHistory =
          req.getQueryString("with-history").flatMap(asBoolean).getOrElse(false)
        val timeContext = req.attrs.get(Attrs.RequestReceivedTimestamp)
        val fieldsFiltersFut =
          qpOpt.fold[Future[Option[FieldFilter]]](
        ...
        fieldsFiltersFut.flatMap { fieldFilters =>
          activeInfotonGenerator
            ...
            .flatMap(
              iOpt => infotonOptionToReply(req, iOpt.map(VirtualInfoton.v2i))
            )
        }
      }
  ...
  def handleZzGET(key: String) = Action.async { implicit req =>
    {
      ...
      def mapToResp(task: Future[_]): Future[Result] = {
        ...
        task.onComplete {
          ...
          case Failure(e) =>
            p.success(Ok(s"""{"success":false,"message":"${Option(e.getMessage)
              .getOrElse(e.getCause.getMessage)}"}"""))
        }
        p.future
      }
      ...
      req.getQueryString("op") match {
        case _ if !allowed =>
          Future.successful(Forbidden("Not allowed to use zz"))
        case Some("purge") =>
        ...
        case Some("put") =>
          val value =
            req.getQueryString("payload").getOrElse("").getBytes("UTF-8")
          mapToResp(req.getQueryString("ttl") match {
            case Some(ttl) =>
              crudServiceFS.zStore.put(key, value, ttl.toInt, false)
        ...
        case None =>
          ...
          crudServiceFS.zStore.get(key).onComplete {
            case Success(payload) =>
              p.success(
                Ok(
                  if (req.getQueryString("format").contains("text"))
            ...
            case Failure(e) =>
              e match {
                case _: NoSuchElementException =>
                  p.success(NotFound("zz item not found"))
                case _ =>
                  p.success(
                    Ok(s"""{"success":false,"message":"${Option(e.getMessage)
                      .getOrElse(e.getCause.getMessage)}"}""")
                  )
              }
          }
  ...
  def handleUuidGET(uuid: String) = Action.async { implicit req =>
    {
      def allowed(infoton: Infoton,
                  level: PermissionLevel = PermissionLevel.Read) =
        authUtils
          .filterNotAllowedPaths(
            Seq(infoton.path),
            level,
      ...
      def fields =
        req.getQueryString("fields").map(FieldNameConverter.toActualFieldNames)
      ...
      if (!uuid.matches("^[a-f0-9]{32}$"))
        ...
        crudServiceFS.getInfotonByUuidAsync(uuid).flatMap {
          case FullBox(infoton)
              if isPurgeOp && allowed(infoton, PermissionLevel.Write) =>
                      ...
                      Ok(
                        formatter.render(
                          SimpleResponse(
                            success = true,
                            Some(
                              s"Note: $uuid was only purged from $index but not from CAS!"
                            )
                          )
                        )
          ...
          case FullBox(infoton) if isPurgeOp =>
          ...
          case FullBox(infoton) if allowed(infoton) =>
            extractFieldsMask(req, typesCache, cmwellRDFHelper, timeContext)
              .flatMap(
                fm => infotonOptionToReply(req, Some(infoton), fieldsMask = fm)
              )
          case FullBox(infoton) =>
            Future.successful(Forbidden("Not authorized"))
  ...
  def handleTrack(trackingId: String): Action[AnyContent] = Action.async {
    implicit request =>
      import cmwell.tracking.{InProgress, TrackingId, TrackingUtil}

      def getDataFromActor(trackingId: String) = trackingId match {
        case TrackingId(tid) =>
          logger.debug(s"Tracking: Trying to get data from $tid");
          TrackingUtil().readStatus(tid)
        case _ =>
          Future.failed(new IllegalArgumentException(s"Invalid trackingID"))
      }
      ...
      resultsFut
        .map { results =>
          val trackingHeaders =
            if (results.forall(_.status != InProgress))
              Seq("X-CM-WELL-TRACKING" -> "Done")
        ...
        .recover {
          ...
          case _: IllegalArgumentException =>
            BadRequest(errMsg("Invalid trackingID"))
          case _: NoSuchElementException =>
            Gone(
              errMsg(
                "This tracking ID was never created or its tracked request has been already completed"
              )
            )
          case _: AskTimeoutException =>
            ServiceUnavailable(errMsg("Tracking is currently unavailable"))
          case _ =>
            InternalServerError(errMsg("An unexpected error has occurred"))
        }
  }

  def handleDELETE(path: String) = Action.async(parse.raw) {
    implicit originalRequest =>
      requestSlashValidator(originalRequest) match {
        case Failure(e) => asyncErrorHandler(e)
        case Success(request) => {
          val normalizedPath = normalizePath(request.path)
          val isPriorityWrite =
            originalRequest.getQueryString("priority").isDefined
          if (!InfotonValidator.isInfotonNameValid(normalizedPath))
          ...
          else if (isPriorityWrite && !authUtils
            ...
            request.getQueryString("data") match {
              ...
              case None => {
                val fields: Option[Map[String, Set[FieldValue]]] =
                ...
                val p = Promise[Result]()
                for {
                  either <- infotonPathDeletionAllowed(
                    normalizedPath,
                    request
                      .getQueryString("recursive")
                      .getOrElse("true")
                      .toBoolean,
                    crudServiceFS
                  )
                  ...
                  (fields.isDefined, either) match {
                    case (true, _) =>
                      crudServiceFS
                        .deleteInfoton(normalizedPath, fields, isPriorityWrite)
                        .onComplete {
                          case Success(b) =>
                            p.success(Ok(Json.obj("success" -> b)))
                          case Failure(e) =>
                            p.success(
                              InternalServerError(
                                Json.obj(
                    ...
                    case (false, Left(msg)) =>
                      p.success(
                        BadRequest(
                          Json.obj("success" -> false, "message" -> msg)
                        )
                      )
                  }
                }
  ...
  private def handleStartScroll(request: Request[AnyContent]): Future[Result] =
    request
      ...
      .map { qpOpt =>
        ...
        val normalizedPath = normalizePath(request.path)
        val from = DateParser
          .parseDate(request.getQueryString("from").getOrElse(""), FromDate)
          .toOption
        val to = DateParser
          .parseDate(request.getQueryString("to").getOrElse(""), ToDate)
          .toOption
        val length =
          request.getQueryString("length").flatMap(asInt).getOrElse(10)
        val offset =
          request.getQueryString("offset").flatMap(asInt).getOrElse(0)
        val scrollTtl = request
          .getQueryString("session-ttl")
          .flatMap(asInt)
          .getOrElse(15)
          .min(60)
        val withDescendants = request.queryString.keySet("with-descendants") || request.queryString
          .keySet("recursive")
        ...
        val fieldsFiltersFut =
          qpOpt.fold(Future.successful(Option.empty[FieldFilter]))(
            rff =>
              RawFieldFilter
                .eval(rff, typesCache, cmwellRDFHelper, timeContext)
                .map(Some.apply)
        ...
        fieldsFiltersFut
          .transformWith {
            case Failure(err) => {
              val msg =
                s"failed to evaluate given qp [${qpOpt.fold("")(_.toString)}]"
              logger.error(msg, err)
              val res =
                FailedDependency(Json.obj("success" -> false, "message" -> msg))
              request.attrs
                .get(Attrs.RequestReceivedTimestamp)
                .fold(Future.successful(res)) { reqStartTime =>
                  val timePassedInMillis = System
                    .currentTimeMillis() - reqStartTime
                  if (timePassedInMillis > 9000L) Future.successful(res)
                  else
                    SimpleScheduler.schedule(
                      (9500L - timePassedInMillis).millis
                    )(res)
                }
            }
            case Success(fieldFilter) => {
              ...
              crudServiceFS
                ...
                .flatMap { startScrollResult =>
                  val rv = createScrollIdDispatcherActorFromIteratorId(
                  ...
                  fmFut.map { fm =>
                    Ok(
                      formatter.render(
                        startScrollResult.copy(iteratorId = rv).masked(fm)
                      )
                    ).as(formatter.mimetype)
                  }
                }
            }
          }
  ...
  private def createScrollIdDispatcherActorFromIteratorId(
    id: String,
    withHistory: Boolean,
    ttl: FiniteDuration
  ): String = {
    val ar =
      Grid.createAnon(classOf[IteratorIdDispatcher], id, withHistory, ttl)
   ...
   * }
   */
  private def addIndexTime(fromCassandra: Seq[Infoton],
                           uuidToindexTime: Map[String, Long]): Seq[Infoton] =
    fromCassandra.map {
      case i: ObjectInfoton if i.indexTime.isEmpty =>
        i.copy(indexTime = uuidToindexTime.get(i.uuid))
      case i: FileInfoton if i.indexTime.isEmpty =>
        i.copy(indexTime = uuidToindexTime.get(i.uuid))
      case i: LinkInfoton if i.indexTime.isEmpty =>
        i.copy(indexTime = uuidToindexTime.get(i.uuid))
      case i: DeletedInfoton if i.indexTime.isEmpty =>
        i.copy(indexTime = uuidToindexTime.get(i.uuid))
      case i => i
    }
  ...
  /**
    ...
    * @return
    */
  private def handleBoostedStream(
    ...
    request
      ...
      .map { qpOpt =>
        ...
        val timeContext = request.attrs.get(Attrs.RequestReceivedTimestamp)
        val fieldsMaskFut =
          extractFieldsMask(request, typesCache, cmwellRDFHelper, timeContext)
        val (withData, format) = {
          ...
          if (Set("nt", "ntriples", "nq", "nquads").exists(
                frmt.equalsIgnoreCase
              ) || frmt.toLowerCase
                .startsWith("json")) {
            Some("text") -> frmt
          } else {
                ...
                case Success(fieldFilter) => {
                  fieldsMaskFut
                    .flatMap { fieldsMask =>
                       ...
                       */
                      val forceUniqueness
                        : Boolean = withHistory && (formatType match {
                      ...
                      streams
                        ...
                        .map {
                          case (source, hits) => {
                            val s = streams.scrollSourceToByteString(
                              source,
                              formatter,
                              withData.isDefined,
                              withHistory,
                            ...
                            Ok.chunked(s)
                              .as(
                                overrideMimetype(formatter.mimetype, request)._2
                              )
  ...
  private def handleSuperStream(request: Request[AnyContent]): Future[Result] =
    request
      ...
      .map { qpOpt =>
        ...
        val withMeta = request.queryString.keySet("with-meta")
        val length = request.getQueryString("length").flatMap(asLong)
        val parallelism = request
          .getQueryString("parallelism")
          .flatMap(asInt)
          .getOrElse(Settings.sstreamParallelism)
        val pathFilter = Some(PathFilter(normalizedPath, withDescendants))
        val timeContext = request.attrs.get(Attrs.RequestReceivedTimestamp)
        val fieldsMaskFut =
          extractFieldsMask(request, typesCache, cmwellRDFHelper, timeContext)
                ...
                fieldsMaskFut.flatMap { fieldsMask =>
                  ...
                  streams
                    ...
                    .map {
                      case (src, hits) =>
                        val s =
                          streams.scrollSourceToByteString(
                            src,
                            formatter,
                            withData.isDefined,
                            withHistory,
            ...
            fieldsFiltersFut.transformWith {
              ...
              case Success(fieldFilter) => {
                fieldsMaskFut.flatMap { fieldsMask =>
                  ...
                  lazy val id =
                    cmwell.util.numeric.Radix64.encodeUnsigned(request.id)
                          ...
                          if (debugLog) scrollSourceToByteString.via {
                            new StreamEventInspector(
                              onUpstreamFinishInspection =
                                () => logger.info(s"[$id] onUpstreamFinish"),
                              onUpstreamFailureInspection = error =>
                                logger.error(s"[$id] onUpstreamFailure", error),
                              onDownstreamFinishInspection =
                                () => logger.info(s"[$id] onDownstreamFinish"),
                              onPullInspection =
                                () => logger.info(s"[$id] onPull"),
  ...
  def generateSortedConsumeFieldFilters(
    qpOpt: Option[String],
    path: String,
  ...
  private def transformFieldFiltersForConsumption(
    ...
    val uptoFilter =
      FieldFilter(Must, LessThan, "system.indexTime", (now - 10000).toString)
    ...
    fieldFilters match {
      ...
      case Some(should @ SingleFieldFilter(Should, _, _, _)) =>
        MultiFieldFilter(
          Must,
          should.copy(fieldOperator = Must) :: rangeFilters
        )
      case Some(ff) => MultiFieldFilter(Must, ff :: rangeFilters)
    }
  }

  private def handleQueueStream(
    request: Request[AnyContent]
  ): Future[Result] = {

    val indexTime: Long =
      request.getQueryString("index-time").flatMap(asLong).getOrElse(0L)
    val withMeta = request.queryString.keySet("with-meta")
    val length = request.getQueryString("length").flatMap(asLong)
    val lengthHint =
      request.getQueryString("length-hint").flatMap(asInt).getOrElse(3000)
    val normalizedPath = normalizePath(request.path)
...
//    val to = request.getQueryString("to")
    ...
    val (withData, format) = {
      ...
      if (Set("nt", "ntriples", "nq", "nquads").exists(frmt.equalsIgnoreCase) || frmt.toLowerCase
            .startsWith("json"))
        Some("text") -> frmt
      else None -> frmt
    }
        ...
        generateSortedConsumeFieldFilters(
          ...
          timeContext = timeContext
            ...
            case Success(
              ...
              Future.successful(
                Ok.chunked(ss.batch(128, identity)(_ ++ _))
                  .as(contentType)
              ) //TODO: `.withHeaders("X-CM-WELL-N" -> total.toString)`
            }
          }
          .recover(errorHandler)
      }
  ...
  def getQueryString(
    k: String
  )(implicit m: Map[String, Seq[String]]): Option[String] =
    m.get(k).flatMap(_.headOption)

  case class ExtractURLFormEnc(path: String)
      extends Action[Either[CMWellRequest, RawBuffer]] {

    def apply(
      request: Request[Either[CMWellRequest, RawBuffer]]
    ): Future[Result] =
      Try {
        request.body match {
  ...
  private def handleCreateConsumerRequest(
    request: Request[AnyContent]
  ): Future[Result] =
    handleCreateConsumer(
      request.path,
  ...
  private def handleCreateConsumer(
    path: String,
    requestReceivedTimestamp: Option[Long]
  )(createConsumerParams: Map[String, Seq[String]]): Future[Result] =
    Try {
      val indexTime = createConsumerParams
        .get("index-time")
        .flatMap(_.headOption.flatMap(asLong))
      val normalizedPath = normalizePath(path)
      val qpOpt = createConsumerParams.get("qp").flatMap(_.headOption)
      val withDescendants = createConsumerParams.contains("with-descendants") || createConsumerParams
        .contains("recursive")
      val withHistory = createConsumerParams.contains("with-history")
      val withDeleted = createConsumerParams.contains("with-deleted")
      val lengthHint = createConsumerParams
        .get("length-hint")
        .flatMap(_.headOption.flatMap(asLong))
      val consumeStateFut: Future[ConsumeState] = {
        val f = generateSortedConsumeFieldFilters(
          qpOpt,
          normalizedPath,
          withDescendants,
          withHistory,
      ...
      consumeStateFut
        .transformWith {
          case Failure(err) => {
            ...
            val res =
              FailedDependency(Json.obj("success" -> false, "message" -> msg))
            requestReceivedTimestamp.fold(Future.successful(res)) {
              reqStartTime =>
                val timePassedInMillis = System
                  .currentTimeMillis() - reqStartTime
                if (timePassedInMillis > 9000L) Future.successful(res)
                else
                  SimpleScheduler.schedule((9500L - timePassedInMillis).millis)(
                    res
                  )
            }
          }
  ...
  def handleConsumeRoute = Action.async { implicit request =>
    if (request.queryString.isEmpty)
      Future.successful(Ok(views.txt._consume(request)))
  ...
  def handleBulkConsumeRoute = Action.async { implicit request =>
    if (request.queryString.isEmpty)
      Future.successful(Ok(views.txt._bulkConsume(request)))
    else bulkScrollHandler.handle(request)
  }

  private[controllers] def handleConsume(
    request: Request[AnyContent]
  ): Future[Result] =
    Try {
      ...
      def wasSupplied(queryParamKey: String) =
        request.queryString.keySet(queryParamKey)
      ...
      if (wasSupplied("qp"))
        ...
        val timeContext = request.attrs.get(Attrs.RequestReceivedTimestamp)
        val sortedIteratorStateTry =
          ConsumeState.decode[SortedConsumeState](sortedIteratorID)
        ...
        sortedIteratorStateTry
          .map {
            case sortedConsumeState @ SortedConsumeState(
                  timeStamp,
                  path,
                  history,
                  deleted,
              ...
              val ffs =
                transformFieldFiltersForConsumption(
                  fieldFilters,
                  timeStamp,
              ...
              val (contentType, formatter) = requestedFormat match {
                case FormatExtractor(formatType) =>
                  ...
                  val m =
                    if (formatType.mimetype.startsWith("application/json"))
                      overrideMimetype(
                        "application/json-seq;charset=UTF8",
                        request
                      )._2
                    else
                      overrideMimetype(formatType.mimetype, request)._2
                ...
                .flatMap {
                  case sr: SearchThinResults if sr.thinResults.isEmpty => {
                    ...
                    val result = new Status(204)
                      .as(contentType)
                      .withHeaders(
                        "X-CM-WELL-POSITION" -> sortedIteratorID,
                        "X-CM-WELL-N-LEFT" -> "0"
                      )
                    Future.successful(result)
                  }
                  case SearchThinResults(total, _, _, results, _)
                      if results.nonEmpty => {
                    ...
                    if (results.length >= total)
                      expandSearchResultsForSortedIteration(
                        results,
                      ...
                      scrollFuture
                        .flatMap {
                          ...
                          case (iterationResultsEnum, hits) => {
                            logger.info(
                              s"sorted iteration encountered a large chunk [indexTime = $idxT]"
                            )

                            val id = ConsumeState.encode(
                              sortedConsumeState.copy(from = idxT)
                            )
                            val src = streams.scrollSourceToByteString(
                              iterationResultsEnum,
                              formatter,
                              withData,
                              history,
                              None,
                              Set.empty
                            ) // TODO: get fieldsMask instead of Set.empty

                            val result = Ok
                              .chunked(src)
                              .as(contentType)
                              .withHeaders(
                                "X-CM-WELL-POSITION" -> id,
                                "X-CM-WELL-N-LEFT" -> (total - hits).toString
                              )
                            Future.successful(result)
                          }
                        }
  ...
  def expandSearchResultsForSortedIteration(
    newResults: immutable.Seq[SearchThinResult],
    ...
    if (formatter.format.isThin) {
      ...
      Future.successful(
        Ok(body)
          ...
          .withHeaders(
            "X-CM-WELL-POSITION" -> id,
            "X-CM-WELL-N-LEFT" -> (total - newResults.length).toString
          )
      )
      ...
      travector(newResults)(
        str => crudServiceFS.getInfotonByUuidAsync(str.uuid).map(_ -> str.uuid)
      ).flatMap { newInfotonsBoxes =>
        val newInfotons = newInfotonsBoxes.collect {
          case (FullBox(i), _) => i
        }

        if (newInfotons.length != newInfotonsBoxes.length) {
          val (fails, nones) = cmwell.util.collections
            .partitionWith(newInfotonsBoxes.filter(_._1.isEmpty)) {
          ...
          if (nones.nonEmpty)
            logger.error(
              "some uuids could not be retrieved: " + nones
                .mkString("[", ",", "]")
            )
          fails.foreach {
        ...
        gqpModified.flatMap { infotonsAfterGQP =>
          ...
          ygModified.flatMap {
            case (false, infotonsAfterYg) => {
              val body =
                FormatterManager.formatFormattableSeq(
                  infotonsAfterYg,
                  formatter
                )
            ...
            case (true, infotonsAfterYg)
                if infotonsAfterYg.isEmpty || xg.isEmpty => {
              val body =
                FormatterManager.formatFormattableSeq(
                  infotonsAfterYg,
                  formatter
              ...
              val result = {
                if (newInfotonsBoxes.exists(_._1.isEmpty))
                  PartialContent(body)
                else Ok(body)
              }
  ...
  val (actorRefsCachedFactory, putInCache) = {
    val c: Cache[String, Future[Option[ActorRef]]] =
        ...
        case Success(None) =>
          Failure(
            new NoSuchElementException(
              "actor address was not found please see previous log for the exact reason why."
            )
          )
    ...
    Try {
      ...
      request
        ...
        .fold(
          Future.successful(
            BadRequest(
              "iterator-id query param is mandatory for this operation"
            )
          )
        ) { encodedActorAddress =>
          val xg = request.getQueryString("xg")
          ...
          if (!withData && xg.isDefined)
            Future.successful(
              BadRequest(
                "you can't use `xg` without also specifying `with-data`!"
              )
            )
          else {
            val timeContext = request.attrs.get(Attrs.RequestReceivedTimestamp)
            ...
            val itStateEitherFuture: Future[Either[String, IterationState]] = {
              actorRefsCachedFactory(
                Base64.decodeBase64String(encodedActorAddress, "UTF-8")
              ).flatMap { ar =>
                (ar ? GetID)(akka.util.Timeout(10.seconds))
                  .mapTo[IterationState]
                  .map {
                    case IterationState(id, wh, _)
                        if wh && (yg.isDefined || xg.isDefined) => {
                      Left(
            ...
            request.getQueryString("format").getOrElse("atom") match {
                ...
                itStateEitherFuture
                  .flatMap {
                    ...
                    case Right(IterationState(scrollId, withHistory, ar)) => {
                      ...
                      val futureThatMayHang: Future[String] = crudServiceFS
                        .scroll(scrollId, scrollTtl + 5, withData)
                        .flatMap { tmpIterationResults =>
                          fieldsMaskFut.flatMap { fieldsMask =>
                            val rv =
                              createScrollIdDispatcherActorFromIteratorId(
                                tmpIterationResults.iteratorId,
                                withHistory,
                                scrollTtl.seconds
                              )
                            val iterationResults = tmpIterationResults
                              .copy(iteratorId = rv)
                              .masked(fieldsMask)

                            val ygModified = yg match {
                              case Some(ygp)
                                  if iterationResults.infotons.isDefined => {
                                pathExpansionParser(
                                  ygp,
                                  ...
                                  case (ok, infotons) =>
                                    ok -> iterationResults.copy(
                                      infotons = Some(infotons)
                                    )
                                }
                              }
                              case _ =>
                                Future.successful(true -> iterationResults)
                            }
                            ...
                            ygModified.flatMap {
                              case (ok, iterationResultsAfterYg) => {
                                (xg, iterationResultsAfterYg.infotons) match {
                                  case t
                                      if t._1.isEmpty || t._2.isEmpty || !ok =>
                                    Future(
                                      formatter.render(iterationResultsAfterYg)
                                    )
                      ...
                      val prependInjections: () => ByteString =
                        formatter match {
                          case a: AtomFormatter => {
                            val it = Iterator.single(ByteString(a.xsltRef)) ++ Iterator
                              .continually(cmwell.ws.Streams.endln)
                            () =>
                              it.next()
                          }
                          case _ =>
                      ...
                      val injectOriginalFutureWith: String => ByteString =
                        ByteString(_, StandardCharsets.UTF_8)
                      val continueWithSource
                        : Source[ByteString, NotUsed] => Result = { src =>
                        ar ! GotIt
                        Ok.chunked(src)
                          .as(overrideMimetype(formatter.mimetype, request)._2)
                      }

                      guardHangingFutureByExpandingToSource[
                        String,
                        ByteString,
                        Result
                      ](futureThatMayHang, initialGraceTime, injectInterval)(
                        backOnTime,
                        prependInjections,
                  ...
                  .recover {
                    case err: Throwable => {
                      ...
                      logger.error(
                        s"[ID: $id] actor holding actual ES ID could not be found ($actorAddress)",
                        err
                      )
                      ExpectationFailed(
              ...
              case unrecognized: String =>
                Future.successful(
                  BadRequest(s"unrecognized format requested: $unrecognized")
                )
            }
          }
        }
      ...
      .map { qpOpt =>
        ...
        val pathFilter =
          if (normalizedPath.length > 1)
            Some(PathFilter(normalizedPath, withDescendants))
          else None
        val withHistory = request.queryString.keySet("with-history")
        val rawAggregationsFilters = AggregationsFiltersParser
          .parseAggregationParams(request.getQueryString("ap"))
        val timeContext = request.attrs.get(Attrs.RequestReceivedTimestamp)

        rawAggregationsFilters match {
          case Success(raf) =>
            val apfut = Future.traverse(raf)(
              RawAggregationFilter
                .eval(_, typesCache, cmwellRDFHelper, timeContext)
            )
            val fieldsFiltersFut =
              qpOpt.fold(Future.successful(Option.empty[FieldFilter]))(
                rff =>
                      ...
                      .map { aggResult =>
                        request
                          ...
                          case FormatExtractor(formatType) => {
                            ...
                            Ok(formatter.render(aggResult)).as(
                              overrideMimetype(formatter.mimetype, request)._2
                            )
                          }
                          case unrecognized: String =>
                            BadRequest(
                              s"unrecognized format requested: $unrecognized"
                            )
                        }
                      }
                  }
  ...
  private def handleSearch(r: Request[AnyContent]): Future[Result] =
    handleSearch(
      ...
      r.host,
      r.uri,
      r.attrs.get(Attrs.RequestReceivedTimestamp)
    )(r.queryString)
  ...
  private def handleSearch(
    ...
    getQueryString("qp")
      ...
      .map { qpOpt =>
        val from = DateParser
          .parseDate(getQueryString("from").getOrElse(""), FromDate)
          .toOption
        val to = DateParser
          .parseDate(getQueryString("to").getOrElse(""), ToDate)
          .toOption
        ...
        val rawSortParams = getQueryString("sort-by")
          .flatMap(SortByParser.parseFieldSortParams(_).toOption)
          .getOrElse(RawSortParam.empty)
        val withDescendants = queryString.keySet("with-descendants") || queryString
          .keySet("recursive")
        ...
        if (offset > Settings.maxOffset) {
          Future.successful(
            BadRequest(
              s"Even Google doesn't handle offsets larger than ${Settings.maxOffset}!"
            )
          )
        } else if (length > Settings.maxLength) {
          Future.successful(
            BadRequest(s"Length is larger than ${Settings.maxLength}!")
          )
        } else if (withHistory && (xg || yg || gqp))
          Future.successful(
            BadRequest(
              s"you can't mix `xg` nor `yg` with `with-history`: it makes no sense!"
            )
          )
        else if (!withData && (xg || (yg && getQueryString("yg").get.trim
                   .startsWith(">")) || (gqp && getQueryString("gqp").get.trim
                   .startsWith(">"))))
          Future.successful(
            BadRequest(
          ...
          val fieldSortParamsFut = RawSortParam.eval(
            rawSortParams,
            crudServiceFS,
            typesCache,
            cmwellRDFHelper,
                ...
                fieldSortParamsFut.flatMap { fieldSortParams =>
                  crudServiceFS
                    ...
                    .flatMap { unmodifiedSearchResult =>
                      val gqpModified = getQueryString("gqp").fold(
                        Future.successful(unmodifiedSearchResult.infotons)
                      ) { gqpPattern =>
                        gqpFilter(
                          gqpPattern,
                      ...
                      fSearchResult.flatMap {
                        case (ok, searchResult) =>
                          extractFieldsMask(
                            ...
                            val format =
                              getQueryString("format").fold("")("&format=".+)
                            val descendants =
                              getQueryString("with-descendants")
                                .fold("")("&with-descendants=".+)
                            val recursive = getQueryString("recursive").fold(
                              ""
                            )("&recursive=".+)
                            ...
                            val from = searchResult.fromDate.fold("")(
                              f =>
                                "&from=" + URLEncoder
                                  .encode(fullDateFormatter.print(f), "UTF-8")
                            )
                            val to = searchResult.toDate.fold("")(
                              t =>
                                "&to=" + URLEncoder
                                  .encode(fullDateFormatter.print(t), "UTF-8")
                            )

                            val qp =
                              getQueryString("qp").fold("")(
                            ...
                            val next =
                              searchResult.offset + searchResult.length - searchResult.total match {
                                case x if x < 0 =>
                                  Some(
                                    linkBase + "&offset=" + (searchResult.offset + searchResult.length)
                                  )
                                case _ => None
                              }

                            val previous =
                              (searchResult.offset - searchResult.length) match {
                                case dif if dif >= 0 =>
                                  Some(linkBase + "&offset=" + dif)
                                case dif
                                    if dif < 0 && -dif < searchResult.length =>
                                  Some(linkBase + "&offset=0")
                                case _ => None
                              }

                            val paginationInfo =
                              PaginationInfo(first, previous, self, next, last)
                                ...
                                if (ok)
                                  Ok(
                                    formatter.render(
                                      SearchResponse(
                                        paginationInfo,
                                        searchResult.masked(fieldsMask)
                                      )
                                    )
                                  ).as(
                                    overrideMimetype(
                                      formatter.mimetype,
                                      getQueryString("override-mimetype")
                                    )._2
                                  )
                                else
                                  InsufficientStorage(
  ...
  private def handleRead(request: Request[AnyContent],
                         recursiveCalls: Int = 30): Future[Result] =
    Try {
      ...
      val withHistory = request.queryString.keySet("with-history")
      val limit = request
        .getQueryString("versions-limit")
        .flatMap(asInt)
        .getOrElse(Settings.defaultLimitForHistoryVersions)
      val timeContext = request.attrs.get(Attrs.RequestReceivedTimestamp)
      ...
      if (offset > Settings.maxOffset) {
        ...
        Future.successful(
          BadRequest(s"Length is larger than ${Settings.maxOffset}!")
        )
          ...
          if (withHistory && (xg.isDefined || yg.isDefined))
              ...
              format.getOrElse("json") match {
                case f
                    if !Set(
                      ...
                      "nq",
                      "nquads"
                    )(f.toLowerCase) && !f.toLowerCase.startsWith("json") =>
                  Future.successful(
                    BadRequest(
                      Json.obj(
                        "success" -> false,
                ...
                case FormatExtractor(formatType) => {
                  ...
                  val infotonsSource =
                    crudServiceFS.getInfotonHistoryReactive(path)

                  val f: Formattable => ByteString =
                    formattableToByteString(formatter)
            ...
            crudServiceFS.getInfoton(path, Some(offset), Some(length)).flatMap {
              case Some(UnknownNestedContent(i)) =>
                ...
                Future.successful(
                  PartialContent(formatter.render(i))
                    .as(overrideMimetype(formatter.mimetype, request)._2)
                )
              case infopt => {
                ...
                extractFieldsMask(
                  ...
                  val toRes = (f: Future[(Boolean, Seq[Infoton])]) =>
                    f.map {
                      case ((true, xs)) =>
                        Ok(formatter.render(BagOfInfotons(xs)))
                          .as(overrideMimetype(formatter.mimetype, request)._2)
                      case ((false, xs)) =>
  ...
  def infotonOptionToReply(
    ...
    Try {
      ...
      val timeContext = request.attrs.get(Attrs.RequestReceivedTimestamp)
      (if (offset.isEmpty)
         actions.ActiveInfotonHandler.wrapInfotonReply(infoton)
       else infoton) match {
        case None => Future.successful(NotFound("Infoton not found"))
        case Some(DeletedInfoton(p, _, _, lm, _)) =>
          Future.successful(
            NotFound(s"Infoton was deleted on ${fullDateFormatter.print(lm)}")
          )
        case Some(LinkInfoton(_, _, _, _, _, to, lType, _)) =>
          lType match {
            case LinkType.Permanent =>
              Future.successful(
                Redirect(to, request.queryString, MOVED_PERMANENTLY)
              )
            case LinkType.Temporary =>
              Future.successful(
                Redirect(to, request.queryString, TEMPORARY_REDIRECT)
              )
            case LinkType.Forward if recursiveCalls > 0 =>
              handleRead(
                request
                  .withTarget(
                    RequestTarget(
                      uriString = to + request.uri.drop(request.path.length),
                      path = to,
                      queryString = request.queryString
                    )
                  )
            ...
            case LinkType.Forward =>
              Future.successful(
                BadRequest("too deep forward link chain detected!")
              )
          }
        case Some(i) =>
          ...
          def infotonIslandResult(prefix: String, suffix: String) = {
            val infotonStr = formatterManager
              .getFormatter(JsonlType, timeContext)
              .render(maskedInfoton)
          ...
          request.getQueryString("format") match {
            case Some(FormatExtractor(formatType)) => {
              ...
              Future.successful(
                Ok(formatter.render(maskedInfoton))
                  .as(overrideMimetype(formatter.mimetype, request)._2)
              )
            }
            // default format
            case _ =>
              i match {
                case f: FileInfoton
                    if request.headers.get("x-compile-to").contains("js") =>
                  val scalaJsSource =
                    new String(f.content.get.data.get, "UTF-8")
                ...
                case c: CompoundInfoton
                    if c.children.exists(
                      _.name.equalsIgnoreCase("index.html")
                    ) =>
                  Future.successful(
                    Redirect(
                      routes.Application
                        .handleGET(s"${c.path}/index.html".substring(1))
                    )
                  )
                // ui
                case _ => {
                  ...
                  cachedSpa.getContent(isOldUi).flatMap { markup =>
                    if (markup eq null)
                      Future.successful(
                        ServiceUnavailable(
                          "System initialization was not yet completed. Please try again soon."
                        )
                      )
                    else
  ...
  def isMarkdown(mime: String): Boolean =
    mime.startsWith("text/x-markdown") || mime.startsWith(
  ...
  def handlePutInfoton(path: String): Request[RawBuffer] => Future[Result] = {
    implicit originalRequest =>
      requestSlashValidator(originalRequest)
        .map { request =>
          ...
          } else if (isPriorityWrite && !authUtils.isOperationAllowedForUser(
                       security.PriorityWrite,
                       authUtils.extractTokenFrom(originalRequest),
                       evenForNonProdEnv = true
                     )) {
            ...
            request match {
              case XCmWellType.Object() => {
                val bodyBytes = request.body
                  .asBytes()
                  .fold(Array.emptyByteArray)(_.toArray[Byte])
              ...
              case XCmWellType.File() => {
                ...
                if (content.isEmpty)
                  Future.successful(
                    BadRequest(
                      Json.obj("success" -> false, "cause" -> "empty content")
                    )
                  )
                else {
              ...
              case XCmWellType.Link() => {
                ...
                val linkTypeStr =
                  request.headers.get("X-CM-WELL-LINK-TYPE").getOrElse("1")
              ...
              case _ =>
                Future.successful(
                  BadRequest(
                    Json.obj("success" -> false, "cause" -> "unrecognized type")
                  )
                )
            }
        }
  ...
  private def jsonToFields(
    jsonBytes: Array[Byte]
  ): Try[Map[String, Set[FieldValue]]] = {

    Try(Json.parse(jsonBytes))
      .flatMap {
        case JsObject(fields) =>
          val data: MultiMap[String, FieldValue] =
            new HashMap[String, collection.mutable.Set[FieldValue]]()
            with MultiMap[String, FieldValue]
          fields.foreach {
            case (k, JsString(s)) => data.addBinding(k, FString(s))
            case (k, JsNumber(n)) =>
              data.addBinding(k, Encoders.numToFieldValue(n))
            ...
            case _ =>
              Failure(
                new IllegalArgumentException("Json depth level allowed is 1")
              )
          }
          Success(data.map { case (x, y) => (x, y.toSet) }.toMap)
        case _ =>
          Failure(new IllegalArgumentException("Json value must be an object"))
      }
      .recoverWith {
        case parseException =>
          Failure(
            new IllegalArgumentException(
              "Request body is not a recognized json",
              parseException
            )
          )
      }
    ...
    req.getQueryString("op") match {
      ...
      case Some("change-password") => {
        ...
        if (Seq(currentPassword, newPassword, token).forall(_.isDefined)) {
          authUtils
            .changePassword(token.get, currentPassword.get, newPassword.get)
            .map {
              ...
              case _ =>
                Forbidden(
                  Json.obj(
                    "success" -> false,
                    "message" -> "Current password does not match given token"
                  )
                )
            }
        } else {
          Future.successful(
            BadRequest(
              Json
                .obj("success" -> false, "message" -> "insufficient arguments")
            )
          )
        }
      }
      case Some("invalidate-cache") => {
        if (isAdminEvenNonProd(req))
          authUtils
            .invalidateAuthCache()
            .map(isSuccess => Ok(Json.obj("success" -> isSuccess)))
        else
      ...
      case Some(unknownOp) =>
        Future.successful(
          BadRequest(
            Json.obj(
              "success" -> false,
              "message" -> s"`$unknownOp` is not a valid operation"
            )
          )
        )
      case None =>
        Future.successful(
          BadRequest(
            Json.obj(
              "success" -> false,
              "message" -> "`op` query parameter was expected"
            )
          )
        )
    }
  ...
  def handleFix(req: Request[AnyContent]): Future[Result] = {
    if (!authUtils.isOperationAllowedForUser(
          security.Overwrite,
          authUtils.extractTokenFrom(req)
        ))
  ...
  def handleVerify(req: Request[AnyContent]): Future[Result] = {
    ...
    val formatter = getFormatter(req, formatterManager, "json")
    f.map { b =>
      Ok(formatter.render(SimpleResponse(b, None)))
        .as(overrideMimetype(formatter.mimetype, req)._2)
    }
  }
    ...
    crudServiceFS
      ...
      .map {
        case (cas, es, zs) => {
          val c = cas.map {
            case (u, io) =>
              s"cas $u ${io.map(i => formatterManager.jsonFormatter.render(i)).getOrElse("none")}"
          }
  ...
  def handleFixDc(req: Request[AnyContent]): Future[Result] = {
    if (!authUtils.isOperationAllowedForUser(
      ...
      val formatter = getFormatter(req, formatterManager, "json")
      f.map { bs =>
          Ok(formatter.render(SimpleResponse(bs, None)))
            .as(overrideMimetype(formatter.mimetype, req)._2)
        }
  ...
  def handlePurgeAll(req: Request[AnyContent]) =
    handlePurge(req, includeLast = true)

  def handlePurgeHistory(req: Request[AnyContent]) =
    handlePurge(req, includeLast = false)

  def handlePurgeLast(req: Request[AnyContent]) =
    handlePurge(req, includeLast = true, onlyLast = true)

  private def handlePurge(req: Request[AnyContent],
                          includeLast: Boolean,
                          onlyLast: Boolean = false): Future[Result] = {
    if (req.getQueryString("2").isDefined) handlePurge2(req)
    else {

      lazy val notImplemented = Future.failed(
        new NotImplementedError(
          "This specific version of CM-Well does not support this operation."
        )
      )
      ...
      if (!allowed) {
        ...
        p.completeWith(
          Future
            .successful(BadRequest("Purging Root Infoton does not make sense!"))
        )
      } else {
        val formatter = getFormatter(req, formatterManager, "json")
        val limit = req
        ...
        val res =
          if (onlyLast)
            ...
            crudServiceFS.purgePath(path, includeLast, limit)
          else
            notImplemented // CRUDServiceFS.purgePath(path, includeLast, limit)

        res.onComplete {
          case Success(_) =>
            p.completeWith(
              Future.successful(
                Ok(formatter.render(SimpleResponse(true, None)))
                  .as(overrideMimetype(formatter.mimetype, req)._2)
              )
            )
          case Failure(e) =>
            p.completeWith(
              Future.successful(
                Ok(
                  formatter.render(
                    SimpleResponse(false, Option(e.getCause.getMessage))
                  )
                ).as(overrideMimetype(formatter.mimetype, req)._2)
              )
            )
      ...
      .recoverWith {
        case err: Throwable => {
          ...
          crudServiceFS.ftsService.uinfo(uuid).map { vec =>
            ...
            Ok(payload)
              .as(overrideMimetype("application/json;charset=UTF-8", req)._2)
          }
        }
      }
      .recoverWith(asyncErrorHandler)
  ...
  def handleRawUUID(uuid: String, isReactive: Boolean, limit: Int)(
    ...
    if (isReactive) {
      val src = crudServiceFS
        .reactiveRawCassandra(uuid)
        .intersperse("\n")
        .map(ByteString(_, StandardCharsets.UTF_8))
      Future.successful(
        Ok.chunked(src)
          .as(defaultMimetypeToReturnedMimetype("text/csv;charset=UTF-8"))
      )
    } else
      crudServiceFS.getRawCassandra(uuid).flatMap {
        case (payload, _) if payload.lines.size < 2 =>
          handleRawPath("/" + uuid, isReactive, limit)(
            defaultMimetypeToReturnedMimetype
          )
        case (payload, mimetype) =>
          Future.successful(
            Ok(payload).as(defaultMimetypeToReturnedMimetype(mimetype))
          )
      }
  }
  ...
  def handleRawPath(path: String, isReactive: Boolean, limit: Int)(
    ...
    if (isReactive) {
      ...
      Future.successful(
        Ok.chunked(pathHeaderSource.concat(src))
          .as(defaultMimetypeToReturnedMimetype("text/csv;charset=UTF-8"))
      )
    } else
  ...
  def handleZzPost(uzid: String) = Action.async(parse.raw) { implicit req =>
    ...
    req.body.asBytes() match {
      case Some(payload) if allowed =>
        ...
        crudServiceFS.zStore
          .put(
            ...
            ttl,
            req.queryString.keySet("batched")
          )
      ...
      case None if allowed =>
        Future.successful(BadRequest("POST body may not be empty!"))
  ...
  private def isAdminEvenNonProd(r: Request[_]): Boolean = {
    authUtils.isOperationAllowedForUser(
      Admin,
      authUtils.extractTokenFrom(r),
      evenForNonProdEnv = true
    )
  }
...
case class ParamExtractor(name: String, value: String => Boolean) {
  def unapply(request: RequestHeader): Boolean =
    request.getQueryString(name).exists(value)
}
...
case class CMWellPostType(xCmWellType: String) {
  def unapply(request: RequestHeader): Boolean =
    request.headers
      .get("X-CM-WELL-TYPE")
      .exists(_.equalsIgnoreCase(xCmWellType))
}

@Singleton
class CachedSpa @Inject()(crudServiceFS: CRUDServiceFS)(
  implicit ec: ExecutionContext
) extends LazyLogging {

  val oldCache =
    new SingleElementLazyAsyncCache[String](600000)(doFetchContent(true))
  val newCache =
    new SingleElementLazyAsyncCache[String](600000)(doFetchContent(false))
  ...
  private def doFetchContent(isOldUi: Boolean): Future[String] = {
    ...
    crudServiceFS.getInfotonByPathAsync(path).collect {
      case FullBox(FileInfoton(_, _, _, _, _, Some(c), _)) =>
        new String(c.data.get, "UTF-8")
      case somethingElse => {
