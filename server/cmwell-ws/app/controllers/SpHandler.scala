...
class SpHandlerController @Inject()(crudServiceFS: CRUDServiceFS)(
  ...
  def handleSparqlPost(formatByRest: String = "") =
    Action.async(parse.tolerantText) { implicit req =>
      handlePost(req, "") { req => (rp: RequestParameters) =>
        /* no need to parse, returning the case class right away */
        OverallSparqlQuery(req.body, req.host, rp)
      }
    }

  def handleSpPost(formatByRest: String = "") =
    Action.async(parse.tolerantText) { implicit req =>
      handlePost(req, formatByRest) { req =>
        parser.parseQuery(req.body.filterNot(_ == '\r'))
      }
    }

  private def handlePost[T](req: Request[String], formatByRest: String = "")(
    parse: Request[String] => (RequestParameters => T)
  ) = {
    ...
    Try(parse(req)) match {
      case Failure(_) =>
        Future.successful(
          wsutil
            .exceptionToResponse(new IllegalArgumentException("Parsing error"))
        )
      case Success(fun) => {
        val formatByRestOpt =
          if (formatByRest.nonEmpty) Some(formatByRest) else None
        ...
        Try(fun(rp)) match {
          ...
          case Success(paq) => {
            val qrFut =
              if (rp.bypassCache) task(paq) else viaCache(crudServiceFS)(paq)
            val resultsAndStatsFut = qrFut.map {
              case Plain(content, stats) =>
                Ok(content).withHeaders(stats.toSeq: _*) -> stats
              case Filename(path, stats) =>
                Ok.sendPath(
                    Paths.get(path),
                    onClose = () => files.deleteFile(path)
                  )
                  .withHeaders(stats.toSeq: _*) -> stats
              case ThroughPipe(_, _) => ???
              case RemoteFailure(e, stats) =>
                wsutil
                  .exceptionToResponse(e)
                  .withHeaders(stats.toSeq: _*) -> stats
              case ShortCircuitOverloaded(activeRequests, stats) =>
                ServiceUnavailable("Busy, try again later").withHeaders(
                  Seq(
                    "Retry-After" -> "10",
                    "X-CM-WELL-N-ACT" -> activeRequests.toString
                  ) ++ stats.toSeq: _*
                ) -> stats
              case Status(_, _) => !!!
            }
            ...
            val (resultsFut, trailersFut) = resultsAndStatsFut.map(_._1) -> resultsAndStatsFut
              .map(_._2.toSeq)
            wsutil.keepAliveByDrippingNewlines(
              resultsFut,
              extraTrailers = trailersFut
            )
          }
        }
      }
...
object SpHandler extends LazyLogging {
  ...
  def serializer(qr: QueryResponse): Array[Byte] = qr match {
    case Plain(content, stats) =>
      (Json.toJson(stats).toString() + "\n" + content).getBytes("UTF-8")
    case _ => !!!
  }
  def isCachable(qr: QueryResponse): Boolean = qr match {
    case Plain(_, _) => true
    case _           => false
  }

  def viaCache(crudServiceFS: CRUDServiceFS) =
    cmwell.zcache.l1l2(task)(digest, deserializer, serializer, isCachable)(
...
class SPParser extends RegexParsers {
  ...
  val sparql = "(?i)sparql".r ^^ { _ =>
    {
      (sources: Seq[String], imports: Seq[String], queries: Seq[String]) =>
        (rp: RequestParameters) =>
          SparqlQuery(sources, imports, queries, rp)
    }
  }
  ...
  val gremlin = "(?i)gremlin".r ^^ { _ =>
    {
      (sources: Seq[String], imports: Seq[String], queries: Seq[String]) =>
        (rp: RequestParameters) =>
          GremlinQuery(sources, imports, queries, rp)
    }
  }

  val queryLiteral: Parser[
    (Seq[String], Seq[String], Seq[String]) => (
      RequestParameters
    ) => PopulateAndQuery
  ] = sparql | gremlin
  def queryHeadLine: Parser[
    (Seq[String], Seq[String], Seq[String]) => (
      RequestParameters
    ) => PopulateAndQuery
  ] =
    whiteSpaces ~> queryLiteral <~ newline

  def queries: Parser[
    (Seq[String], Seq[String]) => (RequestParameters) => PopulateAndQuery
  ] =
    queryHeadLine ~ repsep(anythingOrEmpty, newline) <~ whiteSpaces ^^ {
      case func ~ queries =>
        (sources: Seq[String], imports: Seq[String]) =>
          (rp: RequestParameters) =>
            func(
              sources,
              imports,
              queries.mkString("\n").split(minuses).map(_.trim)
            )(rp)
    }

  def populateAndQuery: Parser[(RequestParameters) => PopulateAndQuery] =
    sources ~ repN(1, newline) ~ imports ~ repN(1, newline) ~ queries ^^ {
      case s ~ _ ~ i ~ _ ~ f =>
        (rp: RequestParameters) =>
          f(s, i)(rp)
      ...
      case s ~ _ ~ f =>
        (rp: RequestParameters) =>
          f(s, Seq())(rp) // import is optional. when not provided, is an empty Seq
    }
  ...
  def parseQuery(input: String): (RequestParameters) => PopulateAndQuery = {
    def throwError(msg: String) =
      throw new ParsingException(s"error: $msg, accured while parsing: $input")
...
object PopulateAndQuery extends LazyLogging {
  ...
  def httpRequest2(path: String): Future[Either[(Int, String), InputStream]] = {
    ...
    val (justPath, queryParams) =
      if (!path.contains('?')) path -> "?" else path.splitAt(path.indexOf('?'))
    val parsedQueryParams = queryParams
      .substring(1)
      .split('&')
      .map { kv =>
        val s = kv.split('=');
        (if (s.length > 0) s(0) else "") -> (if (s.length > 1) s(1) else "")
      }
      .toSeq

    val pathToGet = "http://localhost:9000" + (if (pathHasURLEncodedQps) path
                                               else justPath)
    val qpsToGet = if (pathHasURLEncodedQps) Seq() else parsedQueryParams

    cmwell.util.http.SimpleHttpClient
      .get(pathToGet, qpsToGet, textPlainUtf8)
      .map {
        case SimpleResponse(respCode, _, (_, inputStream))
            if nonError(respCode) =>
          Right(inputStream)
        case SimpleResponse(respCode, _, (_, inputStream)) => {
          val respBody =
  ...
  def retryFetchingIfWasEmpty(
    path: String
  ): Future[Either[(Int, String), Dataset]] = {
    lazy val errMsg = {
      val reason =
        if (path.toLowerCase.contains("with-data")) "this might be a data issue"
        else "try adding `with-data`"
...
//        lengthOpt.fold(false)(_ > actualRetrievedInfotonsAmount)
      ...
      httpRequest2(path).flatMap {
        case Left(respCodeAndTxt) =>
          Future.successful(Left(respCodeAndTxt)) // not retrying when status code > 200
        case Right(is) => {
  ...
  def loadRdfToDataset(
    ntriplesOrNquads: InputStream,
    ds: Dataset = DatasetFactory.createGeneral()
  ): Dataset = {
...
case object Subgraph extends TimedRequestKind {
  override def toString = "Subgraph"
}
case object Sparql extends TimedRequestKind {
  override def toString = "SPARQL  "
}
case object Gremlin extends TimedRequestKind {
  override def toString = "Gremlin "
}
case object Import extends TimedRequestKind {
  override def toString = "Import  "
}
...
abstract class PopulateAndQuery {
  ...
  ): Future[(String, Map[String, String])]
  ...
  case class TimedRequest(path: String,
                          kind: TimedRequestKind,
                          started: Long,
                          ended: Long) {
    private val fmt: DateTimeFormatter =
      ISODateTimeFormat.hourMinuteSecondMillis
    private def formatTime(millis: Long) =
      new DateTime(millis - relativeEpochTime).toString(fmt)
    override def toString: String =
      s"${formatTime(started)} ${formatTime(ended)} $kind $path"
    def toString(manipulator: String => String): String =
      s"${formatTime(started)} ${formatTime(ended)} ${formatTime(
        ended - started + relativeEpochTime
      )} $kind ${manipulator(path)}"
  }

  protected def start(id: String, kind: TimedRequestKind): TimedRequest =
    TimedRequest(id, kind, System.currentTimeMillis, 0)
  protected def end(tr: TimedRequest): TimedRequest =
    tr.copy(ended = System.currentTimeMillis)
...
//  lazy val populateLanguage = if(rp.quads) Lang.NQUADS else Lang.NTRIPLES
  ...
  def populate(
    handler: (
      (Option[TimedRequest], Either[(Int, String), Dataset])
    ) => (Option[TimedRequest], T)
  ): Seq[Future[(Option[TimedRequest], T)]] = {
    ...
    def validateUrl(url: String) = {
      ...
      def validKeyValuePair(key: String, value: String): Boolean =
        key -> value match {
          case (k, v) if k.toLowerCase == "op" =>
            Set("search", "read")(v.toLowerCase)
          case (k, v) if k.toLowerCase == "format" =>
            v.toLowerCase == populateFormat
          case _ => true
        }

      val queryString = url.dropWhile(_ != '?').tail
      queryString.split('&').map(_.split("=", 2)).foreach { kv =>
        require(validKeyValuePair(kv(0), kv(1)))
      }
    }

    def timeWrapIfNeeded[T](
      id: String,
      f: => Future[T]
    ): Future[(Option[TimedRequest], T)] = {
      if (isTiming) {
    ...
    sources
      .map(populatePlaceHolders)
      .map((addFormat _).andThen { path =>
        validateUrl(path);
        timeWrapIfNeeded(path, retryFetchingIfWasEmpty(path)).map(handler)
      })
  }

  def extractBody(
    tRes: (Option[TimedRequest], Either[(Int, String), Dataset])
  ): (Option[TimedRequest], Either[(Int, String), Dataset]) = tRes
...
case class SparqlQuery(sources: Seq[String],
  ...
  override def evaluate(
    ...
    case class PopulateResults(verboseText: String,
                               respCodes: Seq[Int],
                               dataSet: Dataset) {
      def merge(other: PopulateResults): PopulateResults =
        PopulateResults(
          verboseText + "\n" + other.verboseText,
          respCodes ++ other.respCodes,
          JenaUtils.merge(dataSet, other.dataSet)
        )
    }

    val ntriplesFutures
      : Seq[Future[(Option[TimedRequest], Either[(Int, String), Dataset])]] =
      populate(extractBody)
    ...
    def convert(rawFetchResult: (Option[TimedRequest], T)): PopulateResults = {
      ...
      valueOrError match {
        case Left((respCode, errMsg)) => {
          PopulateResults(
            trOpt
              .map(_.toString(_.padOrEllide(64)) + " 0       ")
              .getOrElse("") + errMsg + "\n",
            List(respCode),
        ...
        case Right(dataset) => {
          PopulateResults(
            trOpt
              .map(
                _.toString(_.padOrEllide(64)) + " " + JenaUtils
                  .size(dataset) + "\n"
              )
              .getOrElse(""),
            Nil,
    ...
    val populateResultsFut =
      (ntriplesFutures.head.map(convert) /: ntriplesFutures.tail) {
        case (fm, fs) =>
          fm.flatMap { m =>
...
//    if(rp.disableImportsCache) { Importers.all.foreach(_.invalidateCaches()) }
    ...
    val (jarImportsPaths, sprqlImportsPaths, sourceImportPaths) =
      partition3(imports) {
    ...
    val importsFut = for {
      ...
      iq <- queriesImporter
        .fetch(sprqlImportsPaths)
        .map(_.map(populatePlaceHolders))
      is <- sourcesImporter.fetch(sourceImportPaths)
    } yield (ij, iq, is)

    populateResultsFut.flatMap { perfAndDs =>
      {
        ...
        importsFut.map {
          case (ij, iq, is) =>
            ij.foreach(
              jenaFunc =>
                FunctionRegistry.get
                  .put(s"jar:${jenaFunc.getClass.getName}", jenaFunc.getClass)
            )
            is.foreach {
              case NamedAnonJenaFuncImpl(name, impl) =>
                FunctionRegistry.get.put(s"jar:$name", impl.getClass)
            }

            val sparqlTime = concreteQueries
              .map { query =>
                if (isTiming) {
                  ...
                  val expandedDs =
                    if (iq.isEmpty) ds
                      ...
                      val dss = iq.map { q =>
                        ...
                        verboseImportsSb.append(
                          end(tr).toString(_.padOrEllide(64)) + " " + JenaUtils
                            .size(curDs) + "\n"
                        )
                        curDs
                      }
                  ...
                  verboseImportsSb.toString() + end(tr).toString(
                    _.padOrEllide(64)
                  ) + " " + size
                } else {
                  val expandedDs =
                    if (iq.isEmpty) ds
                      ...
                      JenaUtils.flatten(
                        iq.map(q => JenaUtils.expandDataset(ds, q)) :+ ds
                      )
            ...
            val stats =
              if (respCodes.nonEmpty)
                Map("X-CM-WELL-SG-RS" -> respCodes.mkString(","))
  ...
  def evaluateSparql(sparql: String,
                     ds: Dataset,
                     rp: RequestParameters,
                     outputStream: OutputStream): Long = {
    val dsWithoutSsytemBlankNodes = JenaUtils.filter(ds) { s =>
      !(s.getSubject.isAnon && s.getPredicate.getNameSpace.contains("meta/sys"))
    }
      ...
      (rp.format match {
        ...
        case _ =>
          throw new IllegalArgumentException(
            s"Unsupported format for SELECT: ${rp.format}"
          )
...
case class GremlinQuery(sources: Seq[String],
                        imports: Seq[String],
                        queries: Seq[String],
                        rp: RequestParameters)
    extends PopulateAndQuery {
  ...
  def engine =
    SgEngines.engines.getOrElse(
      "Gremlin",
      throw new Exception("Could not load Gremlin engine!")
    )
  ...
  override def evaluate(
    ...
    val first = ntriplesFutures.head.map(_._2).map {
      ...
      case (fm, fs) =>
        fm.flatMap(
          m =>
            fs.map(_._2 match {
              case Left(_)   => m
              case Right(ds) => JenaUtils.merge(ds, m)
            })
        )
    }
    ...
    modelFuture.map { m =>
      ...
      val ds = DatasetFactory.create(JenaUtils.filter(m.getDefaultModel) { s =>
        !s.getPredicate.getNameSpace.contains("meta/sys")
      })
...
case class OverallSparqlQuery(query: String,
                              host: String,
                              rp: RequestParameters)

case object StatusRequest

case class RequestParameters(format: String,
                             quads: Boolean,
                             verbose: Boolean,
                             showGraph: Boolean,
                             forceUsingFile: Boolean,
                             disableImportsCache: Boolean,
                             execLogging: Boolean,
                             doNotOptimize: Boolean,
                             intermediateLimit: Long,
                             resultsLimit: Long,
                             explainOnly: Boolean,
                             customParams: Map[String, String] = Map(),
                             bypassCache: Boolean)
object RequestParameters {
  def apply(req: Request[_], formatByRest: Option[String]) =
    new RequestParameters(
      // todo formatByRest is deprecated. One day we should be stop respecting it.
      format =
        formatByRest.getOrElse(req.getQueryString("format").getOrElse("ascii")),
      ...
      intermediateLimit = req
        .getQueryString("intermediate-limit")
        .map(_.toLong)
        .getOrElse(10000), // this one is for _sparql, it's not in use in _sp
      resultsLimit = req
        .getQueryString("results-limit")
        .map(_.toLong)
        .getOrElse(10000), // this one is for _sparql, it's not in use in _sp
      explainOnly = req.getQueryString("explain-only").isDefined, // this one is for _sparql, it's not in use in _sp
      customParams = req.queryString.toSeq.collect {
        case (key, values) if key.startsWith("sp.") =>
          "(?i)sp\\.".r.replaceAllIn(key, "").toLowerCase -> values.head
      }.toMap,
...
object SgEngines extends LazyLogging {
  ...
  private def loadSgEngine(jarPath: String,
                           langName: String): Option[SgEngineClient] = {
    Try {
      val className = s"cmwell.plugins.impl.${langName}Parser"
      val excludes = Seq("cmwell.plugins.spi", "org.apache.jena.")
      ChildFirstURLClassLoader
        .loadClassFromJar[SgEngineClient](className, jarPath, excludes)
    }.toOption
  }
}
...
trait Importer[A] { this: LazyLogging =>
  ...
  def readFileInfoton(path: String): Future[FileInfotonContent] = {
    import scala.concurrent.ExecutionContext.Implicits.global
    crudServiceFS
      .getInfoton(path, None, None)
      .map(
        res =>
          (res: @unchecked) match {
            case Some(
                Everything(FileInfoton(_, _, _, _, fields, Some(content), _))
                ) =>
              FileInfotonContent(content.data.get, fields.getOrElse(Map()))
            case x =>
              logger.debug(s"Could not fetch $path, got $x");
              throw new RuntimeException(s"Could not fetch $path")
        }
      )
  }

  protected def extractFileNameFromPath(path: String) =
...
class QueriesImporter(override val crudServiceFS: CRUDServiceFS)
    ...
    CacheBuilder
      ...
      .build(new CacheLoader[String, String] {
        override def load(key: String) =
          Await.result(readTextualFileInfoton(key), 5.seconds)
      })
  ...
  private lazy val directoriesCache: LoadingCache[String, Seq[String]] =
    CacheBuilder
      ...
      .build(new CacheLoader[String, Seq[String]] {
        override def load(key: String) =
          Await.result(listChildren(key), 5.seconds)
      })

  def fetch(importsPaths: Seq[String]) = {
    explodeWildcardImports(importsPaths).flatMap(
      actualImports => fetchRec(actualImports.toSet)
    )
  }

  def fetchRec(
    importsPaths: Set[String],
    alreadyFetched: Set[String] = Set.empty[String]
    ...
    if (importsPaths.isEmpty) Future.successful(Vector.empty[String])
      ...
      doFetch(importsPaths).flatMap { queries =>
        ...
        val nextToFetch = queries
          .flatMap(extractInlineImportsFrom)
          .toSet &~ nextAlreadyFetched
  ...
  private def extractInlineImportsFrom(query: String): Set[String] = {
    query.lines
      .filter(_.startsWith(importDirective))
  ...
  private def explodeWildcardImports(importsPaths: Seq[String]) = {
    import scala.concurrent.ExecutionContext.Implicits.global
    val (wildcardImports, plainImports) = importsPaths
      .map(toAbsolute)
      .partition(imprt => wildcards.exists(imprt.endsWith))
    val normalizedWildcardImports = wildcardImports.map(_.dropRight(2)) // get rid of `/<wildcard>`
    Future
      .traverse(normalizedWildcardImports)(directoriesCache.getAsync)
      .map(_.flatten ++ plainImports)
      .map(_.distinct)
  }

  private def toAbsolute(path: String) =
    if (path.startsWith("/")) path else s"$defaultBasePath/$path"

  private def listChildren(path: String) = {
    crudServiceFS
      .thinSearch(Some(PathFilter(path, descendants = false)))
      .map(_.thinResults.map(_.path))
  }

  private def readTextualFileInfoton(path: String) = {
    readFileInfoton(path).map {
      case FileInfotonContent(data, _) => new String(data, "UTF-8")
    }
  }
}
class JarsImporter(override val crudServiceFS: CRUDServiceFS)
    extends Importer[JenaFunction]
    with SpFileUtils { self =>
  import cmwell.util.collections.LoadingCacheExtensions
  import cmwell.util.concurrent.travector
  ...
  case class LoadedJar(tempPhysicalPath: String,
                       jenaFunctions: Seq[JenaFunction])
  ...
  private lazy val jarsCache: LoadingCache[String, LoadedJar] =
    CacheBuilder
      ...
      .removalListener(new RemovalListener[String, LoadedJar] {
        override def onRemoval(
          notification: RemovalNotification[String, LoadedJar]
        ): Unit = {
          new File(notification.getValue.tempPhysicalPath).delete()
        }
      })
      .build(new CacheLoader[String, LoadedJar] {
        override def load(key: String) =
          self.load(Await.result(readFileInfoton(key), 15.seconds))
      })

  override def fetch(paths: Seq[String]): Future[Vector[JenaFunction]] =
    travector(paths) { path =>
      jarsCache.getAsync(s"$mandatoryBaseJarsPath$path")
    }.map(_.flatMap(_.jenaFunctions))

  private def load(fileInfotonContent: FileInfotonContent): LoadedJar = {
    val FileInfotonContent(data, fields) = fileInfotonContent
    val excludes = fields.getOrElse("exclude", Set()).collect {
    ...
    val loader =
      ChildFirstURLClassLoader.Loader(path, Seq("org.apache.jena") ++ excludes)
    val classNames = extractClassNamesFromJarFile(path)
    val jenaFunctions =
      classNames.map(cn => Try(loader.load[JenaFunction](cn))).collect {
        case Success(jf) => jf
      }
...
class SourcesImporter(override val crudServiceFS: CRUDServiceFS)
    extends Importer[NamedAnonJenaFuncImpl]
    with LazyLogging {
  import cmwell.util.collections.LoadingCacheExtensions
  import cmwell.util.concurrent.travector
  ...
  override def fetch(
    paths: Seq[String]
  ): Future[Vector[NamedAnonJenaFuncImpl]] = {
    import scala.concurrent.ExecutionContext.Implicits.global
    travector(paths) { path =>
      functionsCache.getAsync(s"$mandatoryBaseSourcesPath$path")
    }
  }
    ...
    CacheBuilder
      ...
      .build(new CacheLoader[String, NamedAnonJenaFuncImpl] {
        override def load(key: String) =
          eval(
            new String(
              Await.result(readFileInfoton(key), 15.seconds).content,
              "UTF-8"
            ),
            className = extractFileNameFromPath(key).replace(".scala", "")
          )
      })
  ...
  def eval(source: String, className: String): NamedAnonJenaFuncImpl = { // WARNING: Black magic.
    ...
    val impl = tb
      .eval(
        tb.parse(s"import scala._\nimport Predef._\n$source\n new $className()")
      )
      .asInstanceOf[JenaFunction]
...
object JenaUtils {
  ...
  def expandDataset(dataset: Dataset, constructQuery: String) = {
    ...
    require(
      ...
      "cannot expand dataset using non-construct query"
    )
    QueryExecutionFactory.create(query, dataset).execConstructDataset()
  }

  def size(dataset: Dataset): Long = {
    dataset.getDefaultModel.size + getNamedModels(dataset).map {
      case (n, m) => m.size
    }.sum
  }
  ...
  def flatten(dss: Seq[Dataset]): Dataset = {
    ...
    dss.foreach { ds =>
      ...
      getNamedModels(ds).foreach {
        case (name, namedModel) =>
  ...
  def filter(ds: Dataset)(predicate: Statement => Boolean): Dataset = {
    ...
    getNamedModels(ds).foreach {
      case (name, model) =>
        filteredDs.addNamedModel(name, filter(model)(predicate))
    }
    filteredDs
  }
  ...
  def discardQuadsAndFlattenAsTriples(ds: Dataset): Model = {
    ...
    val allTriples = ds.getDefaultModel.listStatements ++ getNamedModels(ds).values
      .flatMap(_.listStatements)
...
trait SpFileUtils extends LazyLogging {
  private val basePath = scala.sys.env
    .getOrElse("TMPDIR", "/tmp/") + Settings.queryResultsTempFileBaseName

  protected def generateTempFileName =
    s"${basePath}_${java.lang.System.currentTimeMillis}_${Random.alphanumeric.take(10).mkString}"
  ...
  protected def deleteTempFiles(): Unit = {
    ...
    if (prefix.isEmpty) {
      logger.warn(
        "CW is trying to delete tempFiles but file prefix is empty. HALTING AND NOT DELETING // we only want to delete our temp files..."
      )
    } else {
      Try {
        Option(new File(directory)).foreach { f =>
          val cwTmpFiles = f.list(new FilenameFilter() {
            override def accept(file: File, filename: String) =
