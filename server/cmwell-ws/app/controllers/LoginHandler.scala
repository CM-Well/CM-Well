...
@Singleton
class LoginHandler @Inject()(authCache: EagerAuthCache)(
  ...
  def login: Action[AnyContent] = Action.async { implicit req =>
    val exp: Option[DateTime] =
      req.getQueryString("exp").map(parseShortFormatDuration)
    ...
    def loginDigest =
      digestAuthenticate(authCache)(req).map(
        status =>
          if (status.isAuthenticated) grantToken(status.username, exp)
    ...
    def loginBasic = {
      ...
      authCache.getUserInfoton(username) match {
        case Some(user) if Authentication.passwordMatches(user, pass) =>
          grantToken(username, exp)
    ...
    (whichAuthType match {
      case Some(Basic) =>
        cmwell.util.concurrent.delayedTask(penalty)(loginBasic)
      case Some(Digest) =>
        cmwell.util.concurrent
          .delayedTask(penalty / 2)(loginDigest)
          .flatMap(identity)
      case None =>
...
//  }
  ...
  private def parseShortFormatDuration(
    ...
    val durs = Seq("d", "h", "m")
      .map(
        part =>
          part -> s"(\\d+)(?i)$part".r
            .findFirstMatchIn(shortFormatDuration)
            .map(_.group(1).toInt)
            .getOrElse(0)
      )
      .toMap
