...
@Singleton
class BulkScrollHandler @Inject()(
  ...
  def infotonWriteable(formatter: Formatter) =
    new Writeable[Infoton](
      formattableToByteString(formatter),
      Some(formatter.mimetype)
    )

  sealed trait RangeForConsumption
  case class CurrRangeForConsumption(from: Long, to: Long, nextTo: Option[Long])
      extends RangeForConsumption
  case class NextRangeForConsumption(from: Long, to: Option[Long])
      extends RangeForConsumption
  ...
  def fieldsFiltersFromTimeframeAndOptionalFilters(
    from: Long,
    to: Long,
    ffsOpt: Option[FieldFilter]
  ): FieldFilter =
    ffsOpt.fold(onlyTimeframeFieldFilters(from, to)) {
      case ff @ SingleFieldFilter(Should, _, _, _) =>
        MultiFieldFilter(
          Must,
          MultiFieldFilter(Must, Seq(ff)) :: getFieldFilterSeq(from, to)
        )
      case ff: SingleFieldFilter =>
        MultiFieldFilter(Must, ff :: getFieldFilterSeq(from, to))
      case ff @ MultiFieldFilter(Should, _) =>
        MultiFieldFilter(
          Must,
          MultiFieldFilter(Must, Seq(ff)) :: getFieldFilterSeq(from, to)
        )
      case ff: MultiFieldFilter =>
        MultiFieldFilter(Must, ff :: getFieldFilterSeq(from, to))
    }

  def fieldsFiltersforSortedSearchFromTimeAndOptionalFilters(
    from: Long,
    ffsOpt: Option[FieldFilter]
  ): FieldFilter = {
    val fromFilter: FieldFilter =
      SingleFieldFilter(
        Must,
        GreaterThanOrEquals,
        "system.indexTime",
    ...
    ffsOpt.fold(fromFilter) {
      case ff @ SingleFieldFilter(Should, _, _, _) =>
        MultiFieldFilter(
          Must,
          Seq[FieldFilter](MultiFieldFilter(Must, Seq(ff)), fromFilter)
        )
      case ff: SingleFieldFilter => MultiFieldFilter(Must, Seq(ff, fromFilter))
      case ff @ MultiFieldFilter(Should, _) =>
        MultiFieldFilter(Must, Seq(MultiFieldFilter(Must, Seq(ff)), fromFilter))
  ...
  def findValidRange(
    thinSearchParams: ThinSearchParams,
    from: Long,
    threshold: Long,
    timeout: FiniteDuration
  )(implicit ec: ExecutionContext): Future[CurrRangeForConsumption] = {

    logger.debug(
      s"findValidRange: from[$from], threshold[$threshold], tsp[$thinSearchParams]"
    )
      ...
      crudServiceFS
        ...
        .map {
          case SearchThinResults(_, _, _, results, _) =>
            ...
            results.headOption.fold(now)(
              i => math.max(i.indexTime, math.min(now, from + 1729L))
    ...
    toSeed.flatMap { to =>
      logger.debug(
        s"findValidRange: toSeed[$to], from[$from], tsp[$thinSearchParams]"
      )
      ...
      cmwell.util.algorithms
        .binRangeSearch(from, to, now, threshold, 0.5, timeout)(searchFunction)
        .map {
  ...
  def createPathFilter(path: Option[String], recursive: Boolean) =
    path.flatMap { p =>
      if (p == "/" && recursive) None
      ...
      crudServiceFS
        ...
        .flatMap {
          case SearchThinResults(_, _, _, results, _) => {
            ...
            results.headOption.fold(consumeEverythingWithoutNarrowingSearch) {
              i =>
                // first infoton found, gets to be the new from instead of 0, and we are going to find a real valid range
                val thinSearchParams =
                  ThinSearchParams(pf, ff, withHistory, withDeleted)
                findValidRange(
                  thinSearchParams,
                  i.indexTime,
    ...
    (extractInferredFormatWithData(request) match {
      case (fmt, b)
          if Set(
            ...
            "nq",
            "nquads"
          )(fmt.toLowerCase) || fmt.toLowerCase
            .startsWith("json") =>
        Success(fmt -> b)
      case (badFormat, _) =>
        Failure(
          new IllegalArgumentException(
            s"requested format ($badFormat) is invalid for as streamable response."
          )
        )
    }).map {
      case (format, forceData) =>
        val withData: Option[String] =
          request.getQueryString("with-data").orElse {
          ...
          ConsumeState
            ...
            .map(
              bcs =>
                bcs.copy(
                  to = bcs.to
                    .orElse(request.getQueryString("to-hint").flatMap(asLong))
              )
            ) match {
            case Success(
            ...
            case Success(
                state @ BulkConsumeState(_, Some(t), _, _, _, _, _, _)
                ) =>
                  ...
                  def getScrollSource() = {
                    (if (wasSupplied("slow-bulk")) {
                       streams.scrollSource(
                         ...
                         fieldFilters = Option(
                           fieldsFiltersFromTimeframeAndOptionalFilters(
                             from,
                             to,
                             ffOpt
                           )
                      ...
                      case (src, hits) =>
                        val s: Source[Infoton, NotUsed] = {
                          if (withData)
                            src.via(
                              Flows.iterationResultsToFatInfotons(crudServiceFS)
                            )
