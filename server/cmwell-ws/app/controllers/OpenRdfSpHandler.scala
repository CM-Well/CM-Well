...
class OpenRdfSpHandler @Inject()(
  ...
  val cmWellReadOnlySailConnection = new CmWellReadOnlySailConnection(
    cmWellTripleSource
  )
  val cmWellReadOnlySail = new CmWellReadOnlySail(cmWellReadOnlySailConnection)
  val sailConn =
    new CmWellSailRepositoryConnection(
      cmWellReadOnlySailConnection,
      cmWellReadOnlySail
    )
  ...
  def handleSsparqlPost(): Action[String] = Action.async(parse.tolerantText) {
    implicit request =>
      Try(
        QueryParserUtil
          .parseOperation(QueryLanguage.SPARQL, request.body, null)
      ) match {
        case Success(po) =>
          ...
          evaluateQuery(po, formatOpt, withoutMeta)
            .map[Result](Ok.apply)
            .recover {
  ...
  private def evaluateQuery(operation: ParsedOperation,
                            ...
                            withoutMeta: Boolean): Future[String] =
    operation match {
      ...
      case _ =>
        Future.failed(new IllegalArgumentException("Unsupported query type"))
    }

  private def evaluateSelect(ptq: ParsedTupleQuery,
                             format: String,
                             withoutMeta: Boolean): Future[String] = {
    val query =
      sailConn.prepareTupleQuery(QueryLanguage.SPARQL, ptq.getSourceString)
  ...
  private def evaluateConstruct(pgq: ParsedGraphQuery,
                                format: String,
                                withoutMeta: Boolean): Future[String] = {
    val query =
      sailConn.prepareGraphQuery(QueryLanguage.SPARQL, pgq.getSourceString)
  ...
  private def getSelectResultsHandler(
    format: String,
    os: OutputStream
  ...
  private def getConstructResultsHandler(format: String,
                                         os: OutputStream): RDFWriter =
    format match {
...
class CmWellSailRepositoryConnection(
  cmWellReadOnlySailConnection: CmWellReadOnlySailConnection,
  cmWellReadOnlySail: CmWellReadOnlySail
)(implicit ec: ExecutionContext)
    extends SailRepositoryConnection(
      new SailRepository(cmWellReadOnlySail),
      cmWellReadOnlySailConnection
    )

class CmWellReadOnlySail(
  cmWellReadOnlySailConnection: CmWellReadOnlySailConnection
) extends Sail {
  ...
  override def getSupportedIsolationLevels: util.List[IsolationLevel] =
    Seq(IsolationLevels.NONE)

  override def getDataDir: File = null
  override def setDataDir(dataDir: File): Unit = {}
}

class CmWellReadOnlySailConnection(cmWellTripleSource: CmWellTripleSource)(
  implicit ec: ExecutionContext
  ...
  override def removeStatement(op: UpdateContext,
                               subj: Resource,
                               pred: IRI,
                               obj: Value,
                               contexts: Resource*): Unit = ???
  ...
  override def getNamespaces
    : CloseableIteration[_ <: Namespace, SailException] =
    new EmptyIteration // todo should we list namespaces?

  override def clear(contexts: Resource*): Unit = ???
  override def removeStatements(subj: Resource,
                                pred: IRI,
                                obj: Value,
                                contexts: Resource*): Unit =
    ???
  override def addStatement(subj: Resource,
                            pred: IRI,
                            obj: Value,
                            contexts: Resource*): Unit = ???
  override def addStatement(op: UpdateContext,
                            subj: Resource,
                            pred: IRI,
                            obj: Value,
                            contexts: Resource*): Unit = ???
  ...
  override def evaluate(
    ...
    SesameExtensions.SortByCardinalityQueryOptimizer.optimize(
      tupleExpr,
      dataset,
      bindings
    )

    new SimpleEvaluationStrategy(cmWellTripleSource, dataset, null)
  ...
  override def getNamespace(prefix: String): String =
    ??? // should we connect it to meta/ns ???????
...
class CmWellTripleSource(tripleStore: TripleStore)(
  implicit ec: ExecutionContext
) extends TripleSource {
  ...
  override def getStatements(
    ...
    originalIterator(subj, pred, obj, contexts: _*)
      .asInstanceOf[CloseableIteration[_ <: Statement,
                                       QueryEvaluationException]]
  }

  // FIXME: this is temp. hack to map CloseableIteration of different types of Exceptions.
  def originalIterator(
    subj: Resource,
    pred: IRI,
    obj: Value,
    contexts: Resource*
  ): CloseableIteration[_ <: Statement, OpenRDFException] = {
    import scala.collection.JavaConversions._
    ...
    val tp = TriplePattern(
      Option(subj).map(_.stringValue),
