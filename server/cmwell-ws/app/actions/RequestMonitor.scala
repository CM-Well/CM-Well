...
class RequestMonitor(cmwellRequest: CmwellRequest)
    extends Actor
    with LazyLogging {
  ...
  override def receive: Receive = {
    ...
    case GetRequestShort =>
      ...
      sender ! cmwellRequest.copy(
        requestBody = cmwellRequest.requestBody.take(100)
      )
  }
}
...
object RequestMonitor extends LazyLogging {
  ...
  def init: Unit = {
    requestContainer =
      Grid.create(classOf[RequestsContainer], "RequestsContainer")
  }

  def add(requestType: String,
          path: String,
          queryString: String,
          requestBody: String,
          now: Long): Unit = {
    requestContainer ! CmwellRequest(
      requestType,
      path,
      queryString,
      now,
  ...
  def getAllRequestPaths
    : Future[Map[String, Iterable[(String, CmwellRequest)]]] = {
    startMonitoring
  ...
  def requestsInfoton(path: String,
      ...
      Some(
        VirtualInfoton(
          FileInfoton(
            ...
            dc,
            None,
            content = Some(FileContent(bd.getBytes, "text/x-markdown"))
          )
        )
      )
    }
...
class RequestsContainer extends Actor with LazyLogging {
  ...
  override def receive: Actor.Receive = {
    ...
    case GetRequestActors =>
      logger.info(
        s"Received GetRequestActors has (${context.children.size}) children"
      )
      val s = sender()
      val x = context.children.map { c =>
        logger.debug(
          s"path: ${c.path.toSerializationFormatWithAddress(Grid.me)}"
        )
        val pathB64 = Base64.encodeBase64String(
          c.path.toSerializationFormatWithAddress(Grid.me)
        )
    ...
    case StartRequestMonitoring =>
      ...
      taskCanceller = Some(
        context.system.scheduler
          .scheduleOnce(5 minutes, self, StopRequestMonitoring)
      )
