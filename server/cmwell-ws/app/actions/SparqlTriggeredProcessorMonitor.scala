...
object SparqlTriggeredProcessorMonitor extends LazyLogging {
  ...
  def jobsDataToTuple(lines: Iterable[Row]) =
    for (line <- lines) yield MarkdownTuple(line.toSeq: _*)

  def generateTables(path: String,
      ...
      Some(
        VirtualInfoton(
          FileInfoton(
            ...
            None,
            content =
              Some(FileContent(content.getBytes("utf-8"), "text/x-markdown"))
          )
        )
      )
    }
