...
class ActiveInfotonGenerator @Inject()(backPressureToggler: controllers.BackPressureToggler,
  ...
  def getColoredStatus(clr: Color): String = {
    val (color, status) =
      Map(Green -> ("green", "Green"), Yellow -> ("yellow", "Yellow"), Red -> ("red", "Red"), Grey -> ("grey", "N/A"))(
        clr
      )
  ...
  private[this] def getClusterDetailedHealthNew
    : Map[String,
          ((String, Color), (String, Color), (String, Color), (String, Color), (String, Color), (String, Color))] = {
      ...
      bgStatsFut.map { bgStat =>
        ...
        keys.map { k =>
          val wsMessage = cs.wsStat.getOrElse(k, WebDown()) match {
            ...
            case wr: WebBadCode =>
              (s"Web is returning code: ${wr.code}<br>Response time: ${wr.responseTime} ms", colorAdapter(wr.getColor))
  ...
  private[this] def getDetailedBgStatus(): Future[Map[String, Set[(String, Iterable[PartitionOffsetsInfo])]]] = {
    ...
    offsetInfoFut.map { offsetInfo =>
      val partitionsInfos = offsetInfo.partitionsOffsetInfo.values.groupBy(_.partition).map {
  ...
  private[this] def generateHealthMarkdown(now: DateTime): String = {
    ...
    val ((ws, wsTime), (bg, bgTime), (es, esTime), (ca, caTime), (zk, zkTime), (kf, kfTime), controlNode, masters) =
      getClusterHealth
...
"""
  ...
  private[this] val ghdfBreakout = scala.collection.breakOut[Seq[
    (String,
     ((String, Color.Color),
      (String, Color.Color),
      (String, Color.Color),
...
"""
  ...
  private def generateDetailedHealthCsvData(): String = {
    ...
    clusterHealth
      .map(
        r =>
          s"${r._1},${getColoredStatus(r._2._1._2)}<br>${r._2._1._1},${getColoredStatus(r._2._2._2)}<br>${r._2._2._1},${getColoredStatus(
            r._2._3._2
          )}<br>${r._2._3._1},${getColoredStatus(r._2._4._2)}<br>${r._2._4._1},${zkStatusString(r._2._5)},${getColoredStatus(
            r._2._6._2
          )}<br>${r._2._6._1}"
      )
      .mkString("\n")
...
"""
  ...
  def generateInfoton(host: String,
                      path: String,
                      now: Long,
                      length: Int = 0,
                      offset: Int = 0,
                      ...
                      timeContext: Option[Long]): Future[Option[VirtualInfoton]] = {
    ...
    def compoundDC = crudServiceFS.getListOfDC.map { seq =>
      {
        val dcKids: Seq[Infoton] =
          seq.map(dc => VirtualInfoton(ObjectInfoton(s"/proc/dc/$dc", dc, None, d, None)).getInfoton)
        Some(
          VirtualInfoton(
            CompoundInfoton("/proc/dc",
                            dc,
                            None,
                            d,
                            None,
      ...
      crudServiceFS
        ...
        .flatMap {
          case FullBox(ObjectInfoton(_, _, _, _, Some(fields), _)) =>
            ...
            qp.fold(Success(None): Try[Option[RawFieldFilter]])(FieldFilterParser.parseQueryParams(_).map(Some.apply))
              .map { qpOpt =>
    ...
    path.dropTrailingChars('/') match {
        ...
        Some(
          VirtualInfoton(
            FileInfoton(path,
                        dc,
                        None,
                        content = Some(FileContent(generateHealthMarkdown(d).getBytes, "text/x-markdown")))
          )
        )
      case "/proc/health-detailed" =>
        Some(VirtualInfoton(ObjectInfoton(path, dc, None, d, generateHealthDetailedFields)))
        ...
        Some(
          VirtualInfoton(
            FileInfoton(path,
                        dc,
                        None,
                        content = Some(FileContent(generateDetailedHealthMarkdown(d).getBytes, "text/x-markdown")))
          )
        )
        ...
        Some(
          VirtualInfoton(
            FileInfoton(path,
                        dc,
                        None,
                        content = Some(FileContent(generateDetailedHealthCsvPretty().getBytes, "text/html")))
          )
        )
      case "/proc/bg.md" =>
        Some(
          VirtualInfoton(
            FileInfoton(path, dc, None, content = Some(FileContent(generateBgMarkdown.getBytes, "text/x-markdown")))
          )
        )
        ...
        Some(
          VirtualInfoton(
            FileInfoton(path,
                        dc,
                        None,
                        content = Some(FileContent(generateIteratorMarkdown.getBytes, "text/x-markdown")))
          )
        )
      ...
      case "/proc/members-all.csv" =>
        GridMonitoring.members(
          ...
          format = CsvPretty,
          contentTranformator =
            payload => views.html.csvPretty(s"${Settings.clusterName} - Members-All", payload.replace("\n", "\\n")).body
        )
        ...
        Some(
          VirtualInfoton(
            ObjectInfoton(
              ...
              None,
              new DateTime(),
              Some(Map("url" -> Set[FieldValue](FString(url)), "url_hash" -> Set[FieldValue](FString(crc32(url)))))
            )
          )
        )
      }
  ...
  private[this] def procKids: Vector[Infoton] = {
    ...
    Vector(
      ...
      VirtualInfoton(
        FileInfoton("/proc/health-detailed.md", dc, None, md, None, Some(FileContent("text/x-markdown", -1L)))
      ),
      VirtualInfoton(FileInfoton("/proc/health-detailed.csv", dc, None, md, None, Some(FileContent("text/html", -1L)))),
      VirtualInfoton(
        FileInfoton("/proc/search-contexts.md", dc, None, md, None, Some(FileContent("text/x-markdown", -1L)))
      ),
      VirtualInfoton(
        FileInfoton("/proc/members-active.md", dc, None, md, None, Some(FileContent("text/x-markdown", -1L)))
      ),
      VirtualInfoton(
        FileInfoton("/proc/members-all.md", dc, None, md, None, Some(FileContent("text/x-markdown", -1L)))
      ),
      ...
      VirtualInfoton(
        FileInfoton("/proc/actors-diff.md", dc, None, md, None, Some(FileContent("text/x-markdown", -1L)))
      ),
      ...
      VirtualInfoton(
        FileInfoton("/proc/dc-distribution.md", dc, None, md, None, Some(FileContent("text/x-markdown", -1L)))
      ),
