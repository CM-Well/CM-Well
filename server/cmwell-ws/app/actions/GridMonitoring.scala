...
object GridMonitoring {
  ...
  def durationToWords(duration: Long): String = {
    ...
    s"$days " + "%02d".format(hrs) + ":" + "%02d".format(mins) + ":" + "%02d"
      .format(secs)
  }
  //
  ...
  def members(
    ...
    contentTranformator: String => String = identity
    ...
    members.map { m =>
      ...
      val (payload, mimeType) = format match {
        case Markdown =>
          MarkdownTable(
            MarkdownTuple(headers: _*),
            tupls.map(MarkdownTuple(_: _*))
          ).get -> "text/x-markdown"
        case CsvPretty =>
      ...
      Some(
        VirtualInfoton(
          FileInfoton(
            ...
            None,
            content =
              Some(FileContent(contentTranformator(payload).getBytes, mimeType))
          )
        )
      )
    }
  ...
  def singletons(path: String, dc: String): Future[Option[VirtualInfoton]] = {
    ...
    val md = MarkdownTable(
      MarkdownTuple("Singleton", "Role", "Location"),
      body.toSeq
    ).get
    Future.successful(
      Some(
  ...
  def actors(path: String, dc: String): Future[Option[VirtualInfoton]] = {
    ...
    allActorsF.map { allActors =>
      val actors = allActors.map(
        m => m.actors.map(a => (m.host, a.name, a.latency.toString))
      )
      val actorsFlatten = actors.flatten
      val body = actorsFlatten.toSeq
        .sortBy(_._1)
        .map(t => MarkdownTuple(t._1, t._2, t._3))
      val md = MarkdownTable(
        MarkdownTuple("Member", "Actor", "Latency (ms)"),
        body
      ).get
      Some(
        VirtualInfoton(
  ...
  def actorsDiff(path: String, dc: String): Future[Option[VirtualInfoton]] = {
    ...
    membersWithActorsF.map { membersWithActors =>
      val actorsMap =
        membersWithActors.map(mwa => mwa.host -> mwa.actors.map(_.name)).toMap
      ...
      val bodyData = actors.map { actor =>
        actorsMap.keySet.toSeq.sorted.map { host =>
          if (actorsMap(host).contains(actor))
            s"<span style='color:green'>$actor</span>"
      ...
      val md = MarkdownTable(
        MarkdownTuple(actorsMap.keySet.toSeq.sorted: _*),
        body.toSeq
      ).get
      Some(
        VirtualInfoton(
