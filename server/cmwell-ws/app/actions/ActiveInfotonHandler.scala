...
//  private[this] val activeInfotons = Set[String]("/proc","/proc/node")
  ...
  def wrapInfotonReply(infoton: Option[Infoton]): Option[Infoton] =
    infoton match {
  ...
  private[this] def wrap(infoton: Infoton): Infoton = infoton match {
    case cp @ CompoundInfoton("/", _, _, _, _, children, _, length, total, _) =>
      cp.copy(
        children = v2i(
          VirtualInfoton(ObjectInfoton("/proc", Settings.dataCenter))
        ) +: children,
        total = total + 1,
        length = length + 1
      )
    case cp @ CompoundInfoton(
          "/meta/ns",
          _,
          _,
          _,
      ...
      cp.copy(
        children = v2i(
          VirtualInfoton(ObjectInfoton("/meta/ns/sys", Settings.dataCenter))
        ) +: v2i(
