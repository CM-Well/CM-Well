...
@Singleton
class DashBoard @Inject()(crudServiceFS: CRUDServiceFS)(
  ...
  type FutureColorTuple4 =
    (Future[Color], Future[Color], Future[Color], Future[Color])
  type Ratio = (Int, Int)
  ...
  type ComponentHealthFutureTuple =
    (Future[ComponentHealth], Future[ComponentHealth], Future[ComponentHealth])
  type TimedHealth = (ComponentHealth, DateTime)
  type HealthTimedData =
    (TimedHealth,
     TimedHealth,
     TimedHealth,
     TimedHealth,
     ...
     String,
     Set[String])
  type HealthTimedDataWithBG = (TimedHealth,
                                TimedHealth,
                                TimedHealth,
                                TimedHealth,
                                TimedHealth,
    ...
    private[this] val hostsDaemonCancellable = {
      ...
      SimpleScheduler.scheduleAtFixedRate(0 seconds, 1 minutes) {
        crudServiceFS.getInfoton("/meta/sys", None, None).map {
          case None =>
            logger.error("could not retrieve data from local CRUDService")
          case Some(Everything(metaSys)) =>
            metaSys.fields match {
              case None =>
                logger
                  .error(
                    "/meta/sys retrieved from local CRUDService, but does not contain fields"
                  )
              case Some(m: Fields) => {
          ...
          case Some(UnknownNestedContent(_)) =>
            !!! //  not reachable! case written to silence compilation warnings
        }
      }
    }

    def getCachedHostsSet = hosts
    def foreachHost[T](doWithHost: String => T): Future[Set[T]] =
      Future(hosts.map(doWithHost))
  }
  ...
  def getCmwellWSRatio: Future[Either[String, (Ratio, String)]] = {
    ...
    val fsfes: Future[Set[Future[Either[String, (String, Int)]]]] =
      foreachHost { host =>
        Http
    ...
    val ffsew = fsfes.map[Future[Set[Either[String, (String, Int)]]]](
      Future.sequence(_)
  ...
  def getStatusForAll = {
    val fSet = foreachHost { host =>
      ...
      val (bg, es) = {
        val ff = Http
          .get(s"http://$host/proc/node?format=json")
          .map(r => Json.parse(r.payload).\("fields").get)
          .recover {
        ...
        ff.map(_.\("batch_color")(0).as[String]) -> ff.map(
          _.\("es_color")(0).as[String]
        )
      }
      val ca = Http
        .get(s"http://$host/health/cas")(
          SimpleResponse.Implicits.UTF8StringHandler,
          ec,
          sys,
          mat
        )
  ...
  def generateHealthData: ComponentHealthFutureTuple = {
    ...
    val esTuple =
      dashBoard.getElasticsearchStatus().map(t => (Color(t._1) -> t._2))
    ...
    wsTuple.onFailure {
      case ex: Throwable =>
        logger.error("data could not be retrieved for ws.", ex)
    }
    esTuple.onFailure {
      case ex: Throwable =>
        logger.error("data could not be retrieved for es.", ex)
    }
    caTuple.onFailure {
      case ex: Throwable =>
        logger.error("data could not be retrieved for ca.", ex)
    }
      ...
      stf
        .map {
          case (host, (wsResponse, bgColor, cassStats, esColor)) => {
            val ws =
              wsResponse.map(r => if (r == 200) Green else Yellow).recover {
                case _ => Red
              }
            ...
            val ca = cassStats.map(cassandraStatusToColor).recover {
              case ex: java.net.ConnectException => {
                logger
                  .error(s"could not retrieve data from $host/health/cas", ex)
                Grey
              }
            }
  ...
  object DashBoardCache {
    ...
    @volatile private[this] var detail: DetailedHealthData =
      Map.empty[String, ColorTuple4]
    ...
    def cacheAndGetHealthData: HealthTimedData = {
      ...
      ws.onComplete {
        case Success(v) => {
          wsData = v; wsTime = new DateTime(); wsp.success(v -> wsTime)
        }
        case Failure(e) => wsp.failure(e)
      }
      es.onComplete {
        case Success(v) => {
          esData = v; esTime = new DateTime(); esp.success(v -> esTime)
        }
        case Failure(e) => esp.failure(e)
      }
      ca.onComplete {
        case Success(v) => {
          caData = v; caTime = new DateTime(); cap.success(v -> caTime)
        }
        case Failure(e) => cap.failure(e)
      }

      //always have a successful future with valid data
      val wsr: Future[TimedHealth] = wsp.future.recover {
        case _ => wsData -> wsTime
      }
      val esr: Future[TimedHealth] = esp.future.recover {
        case _ => esData -> esTime
      }
      val car: Future[TimedHealth] = cap.future.recover {
        case _ => caData -> caTime
      }

      //make sure to complete successfully within the specified timeout
      val deadline = timeOut.fromNow
      val wsth =
        Try(Await.result(wsr, deadline.timeLeft)).getOrElse(wsData -> wsTime)
      val esth =
        Try(Await.result(esr, deadline.timeLeft)).getOrElse(esData -> esTime)
      val cath =
        Try(Await.result(car, deadline.timeLeft)).getOrElse(caData -> caTime)
    ...
    def cacheAndGetDetailedHealthData: DetailedHealthTimedData = {
      ...
      val xs: Map[String, FutureColorTuple4] =
        Try(Await.result(f, timeOut)).getOrElse[Map[String, FutureColorTuple4]](
            ...
            .map(h => {
              ...
              val tOpt: Option[FutureColorTuple4] = detail
                ...
                .map(
                  c =>
                    (
                      Future.successful(c._1),
                      Future.successful(c._2),
                      Future.successful(c._3),
                      Future.successful(c._4)
                  )
                )
              val t: FutureColorTuple4 =
                tOpt.getOrElse((fGrey, fGrey, fGrey, fGrey))
              h -> t
            })
            .toMap
        )
      ...
      val ys = xs.toList.flatMap {
        case (_, (f1, f2, f3, f4)) => Seq(f1, f2, f3, f4)
      }
