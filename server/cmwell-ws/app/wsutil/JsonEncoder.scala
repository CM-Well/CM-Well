...
object JsonEncoder {
      ...
      x =>
        x._1.replace(oldBase, newBase) -> x._2.map {
          case FString(str, l, q) =>
            FString(str.replace(oldBase, newBase), l, q)
          case FReference(ref, q) =>
            FReference(ref.replace(oldBase, newBase), q)
  ...
  /**
    ...
    * @return
    */
  def encodeIterationResults(scrollResults: IterationResults,
                             cmwellBase: String): JsValue = {
    if (cmwellBase != null) {
      Json.toJson(
        IterationResults(
          scrollResults.iteratorId,
          scrollResults.totalHits,
          scrollResults.infotons match {
            case Some(vi) =>
              Some(vi.map(i => alterBase(i, cmwellBase)));
            case None => None
          }
        )
      )
  ...
  def encodeAggregationResponse(
    aggregationsResponse: AggregationsResponse
  ): JsValue = {
  ...
  def encodeBagOfInfotons(bag: BagOfInfotons, cmwellBase: String): JsValue = {
    if (cmwellBase != null)
      Json.toJson(
        BagOfInfotons(bag.infotons.map(i => alterBase(i, cmwellBase)))
      )
    else
      Json.toJson(bag)
