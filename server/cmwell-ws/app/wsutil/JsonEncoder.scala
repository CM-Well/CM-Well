...
object JsonEncoder{
  ...
  private def alterFieldsBase(fields: Map[String, Set[FieldValue]], oldBase: String , newBase: String): Map[String, Set[FieldValue]] = fields.map(x => x._1.replace(oldBase, newBase) -> x._2.map{
  ...
  private def alterBase(infoton: Infoton, newBase: String): Infoton = {
    ...
    infoton match {
      ...
      case ci @ CompoundInfoton(path, dc, idxT, lastModified, Some(fields), children, offset, length, total,_) =>
        new CompoundInfoton(path, dc, idxT, lastModified,Some(alterFieldsBase(fields,cmwellDefaultBase, newBase)), children, offset, length, total) {
      ...
      case fi @ FileInfoton(path, dc, idxT, lastModified, Some(fields), content,_) =>
        new FileInfoton(path, dc, idxT, lastModified, Some(alterFieldsBase(fields,cmwellDefaultBase, newBase)), content) {
      ...
      case li @ LinkInfoton(path, dc, idxT, lastModified, Some(fields), linkTo, linkType,_) =>
        new LinkInfoton(path, dc, idxT, lastModified, Some(alterFieldsBase(fields,cmwellDefaultBase, newBase)), linkTo, linkType){
