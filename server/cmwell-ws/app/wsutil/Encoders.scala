...
object Encoders {
  ...
  private def parseDate(str: String) = {
    if (str.contains(".")) withDotdateParser.parseDateTime(str)
    else withoutDotdateParser.parseDateTime(str)
  }
  ...
  implicit object FieldValueEncoder extends Format[FieldValue] {
    ...
    override def reads(json: JsValue): JsResult[FieldValue] = json match {
      case JsString(str) if FReference.isUriRef(str) =>
        JsSuccess(FReference(str))
  ...
  implicit object InfotonEncoder extends Format[Infoton] {
    ...
    private def processSystem(node: JsValue): (String, DateTime, String) = {
      ...
      val dataCenter =
        (node \ "dataCenter").asOpt[String].getOrElse(Settings.dataCenter)
      pathOpt match {
        case None =>
          throw InfotonParsingException("No path specified in system")
    ...
    private def processFileContent(node: JsValue): FileContent = {
      ...
      (mimeTypeOpt, lengthOpt, dataOpt, base64dataOpt) match {
        case (Some(_), None, None, None) =>
          throw InfotonParsingException(
            "Not enough parameters in the file content section"
          )
        case (None, _, _, _) =>
          throw InfotonParsingException(
            "No mimetype specified in the file content section"
          )
        case (_, _, Some(d), Some(d64)) =>
          throw InfotonParsingException(
            "Both data and base64-data are present in file content section"
          )
    ...
    private def processObjectInfoton(node: JsValue): ObjectInfoton = {
      ...
      (systemFieldOpt, fieldsOpt) match {
        case (None, _) =>
          throw InfotonParsingException("System object is missing")
        case (Some(sf), Some(f)) =>
    ...
    private def processFileInfoton(node: JsValue): FileInfoton = {
      ...
      (systemFieldOpt, fieldsOpt, fileContentsOpt) match {
        ...
        case (None, _, _) =>
          throw InfotonParsingException("System field is not present")
    ...
    private def processLinkInfoton(node: JsValue): LinkInfoton = {
      ...
      (systemFieldOpt, fieldsOpt, linkTo, linkType) match {
        case (None, _, _, _) =>
          throw InfotonParsingException("System field is not present")
        case (_, _, _, None) =>
          throw InfotonParsingException("LinkTo field is not present")
        case (_, _, None, _) =>
          throw InfotonParsingException("LinkType field is not present")
    ...
    private def getSystem(infoton: Infoton): JsValue = {
      ...
      structure += "lastModified" -> Json.toJson(
        infoton.lastModified
          .toString(ISODateTimeFormat.dateTime().withZone(DateTimeZone.UTC))
      )
    ...
    private def getContent(fc: FileContent): JsValue = {
      ...
      (fc.data, MimeTypeIdentifier.isTextual(fc.mimeType)) match {
        ...
        case (Some(d), false) =>
          structure += "base64-data" -> Json.toJson(
            new Base64(0).encodeToString(d)
          )
      }
  ...
  implicit object IterationResultsEncoder extends Format[IterationResults] {
    ...
    def writes(o: IterationResults): JsValue = {
      ...
      o.infotons.collect {
        case is => structure += "infotons" -> Json.toJson(is)
      }
  ...
  implicit object PaginationInfoEncoder extends Format[PaginationInfo] {
    ...
    def writes(o: PaginationInfo): JsValue = {
      ...
      if (o.previous.isDefined)
        structure += "previous" -> Json.toJson(o.previous.get)
      ...
      af match {
        ...
        case SignificantTermsAggregationFilter(
          ...
          backGroundTerm.foreach { kv =>
            structure += "background term" -> JsObject(
              Seq("key" -> JsString(kv._1), "value" -> JsString(kv._2))
            )
          }
  ...
  def aggregationsResponseToJson(asr: AggregationsResponse): JsArray = {
    JsArray(asr.responses.map { ar =>
      ...
      if (ar.isInstanceOf[BucketsAggregationResponse]) {
        intStructure += "buckets" -> Json.toJson(
          ar.asInstanceOf[BucketsAggregationResponse].buckets
        )
      }
  ...
  implicit object AggregationsResponseEncoder
      extends Format[AggregationsResponse] {
...
case class SystemFields(path: String,
