...
object Encoders {
  ...
  def numToFieldValue(num: BigDecimal): FieldValue = {
    Try(num.toIntExact) match {
      ...
      case Failure(_) => Try(num.toLongExact) match {
        case Success(long) => FLong(long)
        case Failure(_) => num.toBigIntExact() match {
          case Some(bigInt) => FBigInt(bigInt.underlying())
          case None => num.isDecimalFloat match {
            case true => FFloat(num.floatValue())
            case false => num.isDecimalDouble match {
  ...
  implicit object InfotonEncoder extends Format[Infoton] {
    ...
    private def processSystem(node : JsValue) : ( String, DateTime, String ) = {
      ...
      val lmOpt = (node \ "lastModified").asOpt[String].orElse((node \ "modifiedDate").asOpt[String]).flatMap(cmwell.util.string.parseDate)
    ...
    private def processFileContent(node : JsValue) : FileContent = {
      ...
      (mimeTypeOpt, lengthOpt, dataOpt, base64dataOpt) match {
        case (Some(_), None, None, None) => throw InfotonParsingException("Not enough parameters in the file content section")
        case (None, _, _, _) => throw InfotonParsingException("No mimetype specified in the file content section")
        case (_, _, Some(d), Some(d64)) => throw InfotonParsingException("Both data and base64-data are present in file content section")
    ...
    private def getSystem(infoton:Infoton) : JsValue = {
      ...
      structure += "lastModified" -> Json.toJson(infoton.lastModified.toString(ISODateTimeFormat.dateTime().withZone(DateTimeZone.UTC)))
      ...
      asr.responses.map{ ar =>
        ...
        ar match {
          case sar:StatsAggregationResponse =>
            intStructure += "results" -> JsObject(Seq(
