...
case class NnFieldKey(externalKey: String) extends DirectFieldKey {
  ...
  override def infoPath = {
    if (externalKey.startsWith("system.") || externalKey.startsWith("content.") || externalKey.startsWith("link."))
...
case class RawMultiFieldFilter(override val fieldOperator: FieldOperator = Must, filters: Seq[RawFieldFilter])
    extends RawFieldFilter
...
object RawFieldFilter extends PrefixRequirement {
  def eval(rff: RawFieldFilter,
           ...
           timeContext: Option[Long])(implicit ec: ExecutionContext): Future[FieldFilter] = rff match {
    case UnevaluatedQuadFilter(fo, vo, alias) => {
      val fieldFilterWithExplicitUrlOpt =
        cmwellRDFHelper.getQuadUrlForAlias(alias).map(v => SingleFieldFilter(fo, vo, "system.quad", Some(v)))
      prefixRequirement(
  ...
  def mkSingleFieldFilter(fieldOp: FieldOperator, valueOp: ValueOperator, fieldName: String, value: Option[String]) =
    valueOp match {
      case Equals
...
//                                                (scala.concurrent.ExecutionContext.Implicits.global)
  ...
  def eval(rsps: RawSortParam,
           ...
           cmwellRDFHelper: CMWellRDFHelper,
           timeContext: Option[Long])(implicit ec: ExecutionContext): Future[SortParam] = rsps match {
    ...
    case RawFieldSortParam(rfsp) => {
      ...
      Future
        ...
        .flatMap(
          pairs =>
            indexedFieldsNamesFut.map { indexedFieldsNamesWithTypeConcatenation =>
              {
...
object FieldKey extends LazyLogging with PrefixRequirement {
  ...
  def eval(fieldKey: Either[UnresolvedFieldKey, DirectFieldKey],
           cache: PassiveFieldTypesCacheTrait,
           cmwellRDFHelper: CMWellRDFHelper,
  ...
  def resolvePrefix(cmwellRDFHelper: CMWellRDFHelper,
                    timeContext: Option[Long],
                    first: String,
                    requestedPrefix: String)(implicit ec: ExecutionContext): Future[(String, String)] = {
