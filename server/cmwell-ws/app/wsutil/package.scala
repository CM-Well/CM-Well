...
import cmwell.web.ld.exceptions.{
  UnretrievableIdentifierException,
  UnsupportedURIException
}
import cmwell.ws.Settings
import cmwell.ws.util.{
  ExpandGraphParser,
  FieldNameConverter,
  PathGraphExpansionParser,
  TypeHelpers
}
...
package object wsutil extends LazyLogging {
  ...
  implicit class StringExtensions(s: String) {
    def dropRightWhile(p: (Char) => Boolean): String =
      s.dropRight(s.reverseIterator.takeWhile(p).length)
  ...
  def overrideMimetype(default: String,
                       overrideMime: Option[String]): (String, String) =
    overrideMime match {
  ...
  def filterInfoton(f: SingleFieldFilter, i: Infoton): Boolean = {
    require(
      f.valueOperator == Contains || f.valueOperator == Equals,
      s"unsupported ValueOperator: ${f.valueOperator}"
    )
    ...
    f.fieldOperator match {
      case Should | Must =>
        i.fields
          .flatMap(
            _.get(f.name).map(
              _.exists(fv => f.value.forall(v => valOp(fv.value.toString, v)))
            )
          )
          .getOrElse(false)
      case MustNot =>
        i.fields
          .flatMap(
            _.get(f.name).map(
              _.forall(fv => !f.value.exists(v => valOp(fv.value.toString, v)))
            )
          )
          .getOrElse(true)
    }
  ...
  type RawField[Op <: FieldValeOperator] =
    (Op, Either[UnresolvedFieldKey, DirectFieldKey])
  sealed trait RawAggregationFilter
  case class RawStatsAggregationFilter(name: String = "Statistics Aggregation",
  ...
  case class RawTermAggregationFilter(name: String = "Term Aggregation",
                                      field: RawField[FieldValeOperator],
                                      size: Int = 10,
                                      subFilters: Seq[RawAggregationFilter] =
                                        Seq.empty)
      extends RawAggregationFilter

  case class RawHistogramAggregationFilter(
    name: String = "Histogram Aggregation",
    field: RawField[FieldValeOperator],
    interval: Int,
    minDocCount: Int,
    extMin: Option[Long],
  ...
  case class RawSignificantTermsAggregationFilter(
    name: String = "Signigicant Terms Aggregation",
    field: RawField[FieldValeOperator],
  ...
  object RawAggregationFilter {
    private[this] val lbo = scala.collection
      .breakOut[Set[String], AggregationFilter, List[AggregationFilter]]
    private def uniq(fn: String, name: String) = {
      if (fn.length > 1 && fn.tail.head == '$') s"-${fn.head}- $name"
      else "-s- " + name
    }
    def eval(
      af: RawAggregationFilter,
      cache: PassiveFieldTypesCache,
      cmwellRDFHelper: CMWellRDFHelper,
      timeContext: Option[Long]
    )(implicit ec: ExecutionContext): Future[List[AggregationFilter]] =
      af match {
        ...
        case RawTermAggregationFilter(name, (op, fk), size, rawSubFilters)
            if rawSubFilters.nonEmpty => {
          val ff = FieldKey.eval(fk, cache, cmwellRDFHelper, timeContext)
          Future
            .traverse(rawSubFilters)(
              eval(_, cache, cmwellRDFHelper, timeContext)
            )
            .flatMap { subFilters =>
              ff.map { fns =>
                fns.map { fn =>
        ...
        case RawHistogramAggregationFilter(
            name,
            (op, fk),
            interval,
            minDocCount,
        ...
        case RawSignificantTermsAggregationFilter(
            name,
            (op, fk),
            None,
            minDocCount,
            ...
            .flatMap { subFilters =>
              ff.map { fns =>
                fns.map { fn =>
                  ...
                  SignificantTermsAggregationFilter(
                    uname,
                    Field(op, fn),
                    None,
                    minDocCount,
        ...
        case RawCardinalityAggregationFilter(
            name,
            (op, fk),
            precisionThreshold
            ) =>
  ...
  case class FieldKeyPattern(
    fieldKey: Either[UnresolvedFieldKey, DirectFieldKey]
  ) extends FieldPattern

  case class FilteredField[FP <: FieldPattern](
    fieldPattern: FP,
    rawFieldFilterOpt: Option[RawFieldFilter]
  )
  case class LevelExpansion(filteredFields: List[FilteredField[FieldPattern]])

  sealed abstract class DirectedExpansion
  case class ExpandUp(filteredFields: List[FilteredField[FieldKeyPattern]])
      extends DirectedExpansion
  case class ExpandIn(filteredFields: List[FilteredField[FieldPattern]])
      extends DirectedExpansion
  ...
  def getByPath(path: String, crudServiceFS: CRUDServiceFS)(
    implicit ec: ExecutionContext
  ): Future[Infoton] =
    crudServiceFS.irwService
      .readPathAsync(path, crudServiceFS.level)
      .map(_.getOrElse(GhostInfoton(path)))
  type F[X] = (X, Option[List[RawFieldFilter]])
  type EFX = Either[F[Future[Infoton]], F[Infoton]]

  def filterByRawFieldFiltersTupled(cache: PassiveFieldTypesCache,
                                    cmwellRDFHelper: CMWellRDFHelper,
                                    timeContext: Option[Long])(
    tuple: (Infoton, Option[List[RawFieldFilter]])
  )(implicit ec: ExecutionContext): Future[Boolean] = tuple match {
    case (i, None) => Future.successful(true)
    case (i, Some(filters)) =>
      filterByRawFieldFilters(cache, cmwellRDFHelper, timeContext)(i, filters)
  }

  def filterByRawFieldFilters(cache: PassiveFieldTypesCache,
                              cmwellRDFHelper: CMWellRDFHelper,
                              timeContext: Option[Long])(
    infoton: Infoton,
    filters: List[RawFieldFilter]
...
//  }
  ...
  def expandIn(
    filteredFields: List[FilteredField[FieldPattern]],
    ...
    val expansionFuncsFut = Future.traverse(filteredFields) {
      ...
      case FilteredField(FieldKeyPattern(Right(dfk)), rffo) =>
        Future.successful((dfk.internalKey == _, rffo))
      case FilteredField(FieldKeyPattern(Left(rfk)), rffo) =>
        FieldKey
          .resolve(rfk, cmwellRDFHelper, timeContext)
          .map(fk => (fk.internalKey == _, rffo))
    ...
    expansionFuncsFut.flatMap { funs =>
      ...
      val fieldsMaps =
        infotonsToExpand.map(_.fields).collect { case Some(m) => m }

      // maps reduced into 1 aggregated fields `Map`
      val fieldsReduced = if (fieldsMaps.nonEmpty) fieldsMaps.reduce {
        (m1, m2) =>
          m1.foldLeft(m2) {
          ...
          funs.flatMap {
            ...
            case (_, rffo) =>
              values.collect {
                ...
                breakOut[
                  Set[FieldValue],
                  (String, Option[RawFieldFilter]),
                  List[(String, Option[RawFieldFilter])]
                ]
              )
          }
      }(
      ...
      val pathToFiltersMap = cmwPathFilterOptionPairs
        .groupBy(_._1)
        .mapValues(ps => Option.sequence(ps.unzip._2))
        .toSeq
      ...
      val (l, r) = partitionWith(pathToFiltersMap) {
        case (path, rffso) => {
          infotonsRetrievedCache
            .get(path)
            .fold[EFX](
              Left(getByPath(path, cmwellRDFHelper.crudServiceFS) -> rffso)
            ) { i =>
              Right(i -> rffso)
            }
        }
      ...
      val lInfotonsFut = Future
        .traverse(l) {
          ...
          case (fi, Some(filters)) =>
              ...
              case i =>
                filterByRawFieldFilters(cache, cmwellRDFHelper, timeContext)(
                  i,
                  filters
                ).map {
                  case true  => Some(i)
      ...
      val rInfotonsFut = Future
        .traverse(r) {
          case t @ (i, _) =>
            filterByRawFieldFiltersTupled(cache, cmwellRDFHelper, timeContext)(
              t
            ).map {
              case true  => Some(i)
              case false => None
  ...
  def expandUp(
    filteredFields: List[FilteredField[FieldKeyPattern]],
    ...
    def mkFieldFilters2(ff: FilteredField[FieldKeyPattern],
      ...
      val internalFieldNameFut = fkp match {
        ...
        case FieldKeyPattern(Left(unfk)) =>
          FieldKey
            .resolve(unfk, cmwellRDFHelper, timeContext)
            .map(_.internalKey)
      }
      val filterFut: Future[FieldFilter] = internalFieldNameFut.map {
        internalFieldName =>
          urls match {
            case Nil =>
            ...
            case url :: Nil =>
              SingleFieldFilter(
                rffo.fold[FieldOperator](outerFieldOperator)(_ => Must),
                Equals,
                internalFieldName,
                Some(url)
              )
      ...
      rffo.fold[Future[FieldFilter]](filterFut) { rawFilter =>
        RawFieldFilter
          .eval(rawFilter, typesCache, cmwellRDFHelper, timeContext)
          .flatMap { filter =>
            filterFut.map(
              ff => MultiFieldFilter(outerFieldOperator, List(ff, filter))
            )
          }
      }
    }
    ...
    Future
      .traverse(population.grouped(chunkSize)) { infotonsChunk =>
        val urls: List[String] =
          infotonsChunk.map(i => pathToUri(i.path))(breakOut)
        ...
        val fieldFilterFut = filteredFields match {
          ...
          case _ =>
            Future
              .traverse(filteredFields)(mkFieldFilters2(_, Should, urls))
              .map(MultiFieldFilter(Must, _))
        }
        fieldFilterFut.transformWith {
          ...
          case Success(ffs) =>
            cmwellRDFHelper.crudServiceFS
              ...
              .flatMap(sr => {
                val (inCache, toFetch) =
                  sr.thinResults.partition(i => cache.contains(i.path))
                cmwellRDFHelper.crudServiceFS
                  .getInfotonsByUuidAsync(toFetch.map(_.uuid))
                  .map {
                    _ -> inCache.map(i => cache(i.path))
                  }
              })
          case anotherFailures =>
            Future.fromTry(
              anotherFailures.asInstanceOf[Try[(Seq[Infoton], Seq[Infoton])]]
            )
        }
      }
      .map {
  ...
  def deepExpandGraph(
    ...
    cache: PassiveFieldTypesCache,
    timeContext: Option[Long]
  )(implicit ec: ExecutionContext): Future[(Boolean, Seq[Infoton])] = {

    def expandDeeper(
      expanders: List[LevelExpansion],
      infotonsToExpand: Seq[Infoton],
      infotonsRetrievedCache: Map[String, Infoton]
    ): Future[(Boolean, Seq[Infoton])] = expanders match {
      case Nil =>
        Future.successful(
          true -> infotonsRetrievedCache.values
            .filterNot(_.isInstanceOf[GhostInfoton])
            .toSeq
        )
          ...
          case (lInfotons, rInfotons) =>
            expandDeeper(
              ...
              infotonsRetrievedCache ++ lInfotons.map(i => i.path -> i)(
                scala.collection
                  .breakOut[Seq[Infoton], (String, Infoton), Map[String,
                                                                 Infoton]]
              )
            )
        }
  ...
  def gqpFilter(
    ...
    logger.trace(
      s"gqpFilter with infotons: [${infotons.map(_.path).mkString(", ")}]"
    )
        ...
        case ExpandIn(filteredFields) =>
          expandIn(
            ...
            iv._2.map(i => i.path -> i)(
              scala.collection
                .breakOut[Vector[Infoton], (String, Infoton), Map[String,
                                                                  Infoton]]
            ),
            cmwellRDFHelper,
            typesCache,
    ...
    def nextFilteringHop(
      dexp: DirectedExpansion,
      dexps: List[DirectedExpansion],
      survivors: Vector[(Infoton, Vector[Infoton])]
    ): Future[Vector[Infoton]] = {
      Future
        .traverse(survivors)(filterByDirectedExpansion(dexp))
        .flatMap { s =>
          val newSurvivors = s.filter(_._2.nonEmpty)
          if (newSurvivors.isEmpty || dexps.isEmpty)
            Future.successful(newSurvivors.map(_._1))
          else nextFilteringHop(dexps.head, dexps.tail, newSurvivors)
        }
        .andThen {
          case Success(is) =>
            logger.trace(
              s"nextFilteringHop: finished with survivors[${is.map(_.path).mkString(", ")}]"
            )
          case Failure(ex) =>
            logger.error(s"nextFilteringHop($dexp,$dexps,$survivors)", ex)
        }
    }
        ...
        case PathsExpansion(paths) =>
          paths.foldLeft(Future.successful(Vector.empty[Infoton])) {
            ...
            case (vecFut, PathExpansion(segments)) =>
              vecFut.flatMap { vec =>
                val candidates: Vector[(Infoton, Vector[Infoton])] =
                  infotons.collect {
                    case i if !vec.contains(i) =>
                      i -> Vector(i)
                  }(
                    breakOut[Seq[Infoton], (Infoton, Vector[Infoton]), Vector[
                      (Infoton, Vector[Infoton])
                    ]]
                  )
                logger.trace(s"appending: [${segments.mkString(", ")}] to vec[${vec
                  .map(_.path)
                  .mkString(", ")}] with candidates[${candidates.map(_._1.path).mkString(", ")}]")
                nextFilteringHop(segments.head, segments.tail, candidates)
                  .map(_ ++: vec)
              }
          }
      } match {
  ...
  def pathExpansionParser(
    ...
    /**
      ...
      */
    def adjustResults(
      infoCachedOrRetrieved: (Seq[Infoton], Seq[Infoton]),
      tail: List[DirectedExpansion]
    ): (Option[Expander], Seq[Infoton]) = {
      val (retrieved, cached) = infoCachedOrRetrieved
      tail.headOption
        .map[Expander](h => (h, tail.tail, cached ++ retrieved)) -> retrieved
    }

    def expandDeeper(
      expanders: List[Option[Expander]],
      cache: Map[String, Infoton]
    ): Future[(Boolean, Seq[Infoton])] = {
      if (expanders.forall(_.isEmpty))
        Future.successful(
          true -> cache.values.filterNot(_.isInstanceOf[GhostInfoton]).toSeq
        )
        ...
        Future
          .traverse(expanders) {
            ...
            case Some((ExpandIn(ffs), tail, population)) =>
              expandIn(
                ...
                typesCache,
                timeContext
              ).map(newPop => adjustResults(newPop, tail))
          ...
          .flatMap { expanderRetrievedInfotonPairs =>
            val (newExpanders, retrievedInfotons) =
              expanderRetrievedInfotonPairs.unzip
            val newCache = retrievedInfotons.foldLeft(cache) {
              case (accache, additions) =>
                accache ++ additions.map(i => i.path -> i)
            }
      ...
      case PathsExpansion(paths) => {
        ...
        expandDeeper(
          ...
          infotons.map(i => i.path -> i)(
            scala.collection
              .breakOut[Seq[Infoton], (String, Infoton), Map[String, Infoton]]
          )
        )
      }
    } match {
  ...
  def isPathADomain(path: String): Boolean =
    path.dropWhile(_ == '/').takeWhile(_ != '/').contains('.')
  ...
  def exceptionToResponse(throwable: Throwable): Result = {
    val (status, eHandler): (Status, Throwable => String) = throwable match {
      case _: ServerComponentNotAvailableException =>
        ServiceUnavailable -> { _.getMessage }
      case _: TooManyNsRequestsException =>
        ServiceUnavailable -> { _.getMessage }
      case _: ConflictingNsEntriesException =>
        ExpectationFailed -> { _.getMessage }
      case _: TimeoutException    => ServiceUnavailable -> { _.getMessage }
      case _: SpaMissingException => ServiceUnavailable -> { _.getMessage }
      case _: UnretrievableIdentifierException =>
        UnprocessableEntity -> { _.getMessage }
      case _: security.UnauthorizedException => Forbidden -> { _.getMessage }
      case _: org.apache.jena.shared.JenaException =>
        BadRequest -> { _.getMessage }
      case _: cmwell.web.ld.exceptions.ParsingException =>
        BadRequest -> { _.getMessage }
      ...
      case _: com.datastax.driver.core.exceptions.InvalidQueryException =>
        ExpectationFailed -> { _.getMessage }
      case _: com.datastax.driver.core.exceptions.DriverException =>
        ServiceUnavailable -> { _.getMessage }
    ...
    fieldsOpt.map(FieldNameConverter.toActualFieldNames) match {
      case Some(Success(fields)) =>
        travset(fields) {
          ...
          case Left(u) =>
            FieldKey.resolve(u, cmwellRDFHelper, timeContext).map(_.internalKey)
        }
  ...
  def extractInferredFormatWithData(
    req: Request[_],
    defaultFormat: String = "json"
  ): (String, Boolean) = {
    val format = req.getQueryString("format").getOrElse(defaultFormat)
    val withData =
      req.getQueryString("with-data").fold(!noDataFormat(format))(_ != "false")
    format -> withData
  }

  val endln = ByteString(cmwell.util.os.Props.endln)

  def guardHangingFutureByExpandingToSource[T, U, V](
    futureThatMayHang: Future[T],
        ...
        val source = Source.unfoldAsync(false) { isCompleted =>
          {
            if (isCompleted) Future.successful(None)
              ...
              p2.tryCompleteWith(
                futureThatMayHang
                  .map(t => Some(true -> injectOriginalFutureWith(t)))
              )
  ...
  def keepAliveByDrippingNewlines(
    ...
    val isEmptySuccessfulFuture =
      extraTrailers.value.fold(false)(_.fold(err => false, _.isEmpty))
    if (isEmptySuccessfulFuture)
      guardHangingFutureByExpandingToSource[Result,
                                            Source[ByteString, _],
                                            Result](
        response,
        7.seconds,
        ...
        source => {
          val r =
            Ok.chunked(source.flatMapConcat(x => x))
      ...
      val injectOriginalFutureWith: Result => Source[HttpChunk, _] =
        _.body.dataStream
          .map(HttpChunk.Chunk)
          .concat(
            Source
              ...
              .map(
                nonEmptyTrailers =>
                  HttpChunk.LastChunk(new Headers(nonEmptyTrailers))
              )
          )
      val continueWithSource: Source[Source[HttpChunk, _], NotUsed] => Result =
        source => {
          Result(
...
(                                                                          1.  start a group to capture protocol + optional domain
 (cmwell:/)                                                                2.  if protocol is `cmwell:` no need to specify domain
  ...
  def pathStatusAsInfoton(ps: PathStatus): Infoton = {
    ...
    val fields: Option[Map[String, Set[FieldValue]]] = Some(
      Map("trackingStatus" -> Set(FString(status.toString)))
    )
    VirtualInfoton(
      ObjectInfoton(path, Settings.dataCenter, None, fields = fields)
    )
  }
    ...
    request.getQueryString("format").getOrElse(defaultFormat) match {
      ...
      case unknownFormat =>
        throw new IllegalArgumentException(
          s"Format $unknownFormat is not supported"
        )
    }

  def isSystemProperty(firstDotLast: (String, String)): Boolean =
    firstDotLast match {
