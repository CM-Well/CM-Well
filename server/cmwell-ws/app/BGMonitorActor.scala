...
class BGMonitorActor(zkServers: String,
  ...
  val topics = Seq(
    "persist_topic",
    "persist_topic.priority",
    "index_topic",
    "index_topic.priority"
  )
  ...
  val topicsPartitionsAndGroups = partitionsForTopics.flatMap {
    ...
    case ("persist_topic.priority", partitions) =>
      partitions.map { partition =>
        (
          new TopicPartition("persist_topic.priority", partition),
          s"imp.p.$partition"
        )
      }
    ...
    case ("index_topic.priority", partitions) =>
      partitions.map { partition =>
        (
          new TopicPartition("index_topic.priority", partition),
          s"indexer.p.$partition"
        )
      }
  }
  ...
  val topicsPartitionsAndConsumers = topicsPartitionsAndGroups.map {
    case (topicPartition, groupId) =>
      ...
      (
        topicPartition,
        new KafkaConsumer[Array[Byte], Array[Byte]](kafkaConsumerProps)
      )
  }

  val topicsPartitions = topicsPartitionsAndConsumers.keys

  var previousOffsetInfo: OffsetsInfo =
    OffsetsInfo(Map.empty[String, PartitionOffsetsInfo], DateTime.now())
  @volatile var currentOffsetInfo: OffsetsInfo =
    OffsetsInfo(Map.empty[String, PartitionOffsetsInfo], DateTime.now())
  ...
  val redSince: collection.concurrent.Map[Int, Long] =
  ...
  private def generateOffsetsInfo = {
    ...
    def calculateOffsetInfo(): Future[(OffsetsInfo, Long)] = {
      ...
      Future {
        blocking {
          ...
          val topicPartitionsWriteOffsets = topicsPartitionsAndConsumers.head._2
            .endOffsets(topicsPartitions.asJavaCollection)
          val partitionsOffsetsInfo: Map[String, PartitionOffsetsInfo] =
            topicPartitionsWriteOffsets.asScala.map {
              case (topicPartition, writeOffset) =>
                val streamId = topicPartition.topic() match {
                  case "persist_topic" =>
                    s"imp.${topicPartition.partition()}_offset"
                  case "persist_topic.priority" =>
                    s"imp.${topicPartition.partition()}.p_offset"
                  case "index_topic" =>
                    s"indexer.${topicPartition.partition()}_offset"
                  case "index_topic.priority" =>
                    s"indexer.${topicPartition.partition()}.p_offset"
                }
    ...
    calculateOffsetInfo().onComplete {
      case Success((info, duration)) =>
        logger.debug(
          s"calculate offset info successful: \nInfo:$info\nDuration:$duration"
        )
        val now = System.currentTimeMillis()
        if (now - statusesCheckedTime > 1 * 60 * 1000) {
          logger.debug(
            s"more than 1 minute has past since last checked statuses, let's check"
          )
          ...
          try {
              ...
              case (key, partitionInfo) =>
                ...
                if (partitionStatus == Red) {
                  ...
                  redSince.get(partitionInfo.partition) match {
                    case None =>
                      logger.warn(
                        s"BG status for partition ${partitionInfo.partition} turned RED"
                      )
                      redSince.putIfAbsent(partitionInfo.partition, currentTime)
                    case Some(since)
                        if ((currentTime - since) > 15 * 60 * 1000) =>
                      logger.error(
            ...
            currentOffsetInfo =
              info.copy(partitionsOffsetInfo = partitionsOffsetInfoUpdated)
          } catch {
            case t: Throwable =>
              logger.error("exception ingesting offset info", t)
          }
        } else if (currentOffsetInfo.partitionsOffsetInfo.nonEmpty) {
          currentOffsetInfo =
            info.copy(partitionsOffsetInfo = info.partitionsOffsetInfo.map {
              case (topic, info) =>
                (
                  topic,
                  info.copy(
                    partitionStatus = currentOffsetInfo
                      .partitionsOffsetInfo(topic)
                      .partitionStatus
                  )
                )
            })
        ...
        context.system.scheduler
          .scheduleOnce(
            math.max(10000, lastFetchDuration).milliseconds,
            self,
            CalculateOffsetInfo
          )
...
case class OffsetsInfo(partitionsOffsetInfo: Map[String, PartitionOffsetsInfo],
                       timeStamp: DateTime)
