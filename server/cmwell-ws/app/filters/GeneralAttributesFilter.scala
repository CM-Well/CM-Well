...
/**
  ...
  * Time: 8:04 AM
  */
class GeneralAttributesFilter @Inject()(implicit val mat: Materializer,
                                        ec: ExecutionContext)
    extends Filter
    with TypeHelpers {
  override def apply(f: RequestHeader => Future[Result])(rh: RequestHeader) = {
    f(
      rh.addAttr(Attrs.RequestReceivedTimestamp, System.currentTimeMillis())
        ...
        ) // TODO: might be good to add ip to logs when user misbehaves
    )
  }
}

object Attrs {
  val RequestReceivedTimestamp: TypedKey[Long] =
    TypedKey.apply[Long]("RequestReceivedTimestamp")
