...
class AuthFilter @Inject()(
...
)(implicit override val mat: Materializer, ec: ExecutionContext)
    extends Filter {

  private val useAuthorizationParam =
    java.lang.Boolean.getBoolean("use.authorization")
      ...
      case 2 =>
        ...
        if (allowed) {
          ...
          if (PermissionLevel(requestHeader.method) == PermissionLevel.Read)
            ...
            Future(
              Results.Forbidden(Json.obj("success" -> false, "message" -> msg))
            )
        }
  ...
  private def isAuthenticatedAndAuthorized(
    requestHeader: RequestHeader
  ): (Boolean, String) = {
    def isRequestWriteToMeta =
      authUtils.isWriteToMeta(
        PermissionLevel(requestHeader.method),
        requestHeader.path
      )

    if ((!useAuthorizationParam && !isRequestWriteToMeta) || irrelevantPaths
          .exists(requestHeader.path.startsWith))
      return (true, "")

    def withMsg(allowed: Boolean, msg: String) =
      (allowed, if (allowed) "" else msg)

    val request = (
      normalizePath(requestHeader.path),
      PermissionLevel(requestHeader.method, requestHeader.getQueryString("op"))
    )
      ...
      case Some(token) if token.isValid => {
        authCache.getUserInfoton(token.username) match {
          case Some(user) =>
            withMsg(
              authorization
                .isAllowedForUser(request, user, Some(token.username)),
          ...
          case None =>
            (false, s"Username ${token.username} was not found in CM-Well")
        }
      }
      case Some(_) =>
        (false, "given token is not valid (not signed or expired)")
      case None =>
        withMsg(
          authorization.isAllowedForAnonymousUser(request),
