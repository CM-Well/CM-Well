...
case class FailingIRWServiceMockup(storageDao: Dao, errorModuloDivisor: Int)
  ...
  override def readUUIDAsync(
    ...
    logger.debug(
      s"readUUIDAsync for uuid:$uuid. errorModuloDividend=$errorModuloDividend"
    )
  ...
  override def readPathAsync(path: String, level: ConsistencyLevel)(
    ...
    errorModuloDividend += 1
    logger.debug(
      s"readPathAsync for path:$path. errorModuloDividend=$errorModuloDividend"
    )
    if (errorCount < 3 && errorModuloDividend % errorModuloDivisor == 0) {
      errorCount += 1
      logger.debug(
        s"throwing DriverException in readPathAsync for infoton path:${path}"
      )
      throw new DriverException("test exception")
    } else {
  ...
  override def writeAsync(infoton: Infoton,
                          level: ConsistencyLevel,
                          skipSetPathLast: Boolean)(
    implicit ec: ExecutionContext
  ): Future[Infoton] = {

    errorModuloDividend += 1
    logger.debug(
      s"writeAsync for path:${infoton.path}. errorModuloDividend=$errorModuloDividend"
    )
    if (errorCount < 3 && errorModuloDividend % errorModuloDivisor == 0) {
      logger.debug(
      ...
      errorCount += 1
      logger.debug(
        s"throwing DriverException in writeAsync for infoton path:${infoton.path}, uuid:${infoton.uuid}"
      )
      throw new DriverException("test exception")
    } else
