...
object FailingFTSServiceMockup {
  def apply(esClasspathYml: String, errorModuloDivisor: Int) =
    new FailingFTSServiceMockup(ConfigFactory.load(),
                                esClasspathYml,
                                errorModuloDivisor)
}

class FailingFTSServiceMockup(config: Config,
                              esClasspathYaml: String,
                              errorModuloDivisor: Int)
  ...
  override def executeIndexRequests(indexRequests: Iterable[ESIndexRequest])(
    ...
    if (errorModuloDividend % errorModuloDivisor == 0)
      ...
      SimpleScheduler.scheduleFuture(15.seconds)(
  ...
  override def executeBulkIndexRequests(
    ...
    errorModuloDividend += 1
    logger.error(
      s"executeBulkIndexRequests: errorModuloDividend=$errorModuloDividend"
    )
    if (errorModuloDividend % errorModuloDivisor == 2 && errorCount <= 2) {
      ...
      super.executeBulkIndexRequests(indexRequests,
                                     numOfRetries,
                                     waitBetweenRetries)
    }
  }
