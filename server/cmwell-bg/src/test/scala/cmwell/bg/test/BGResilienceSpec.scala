...
class BGResilienceSpec
  ...
  override def beforeAll = {
    ...
    bgConfig.withValue("cmwell.bg.esActionsBulkSize",
                       ConfigValueFactory.fromAnyRef(100))

    actorSystem = ActorSystem("cmwell-bg-test-system")

    cmwellBGActor = actorSystem.actorOf(
      CMWellBGActor.props(0,
                          bgConfig,
                          testIRWMockupService,
                          ftsServiceES,
                          zStore,
                          offsetsService)
    )
  ...
  "Resilient BG" should "process commands as usual on circumvented BGActor (periodically failing IRWService) after suspending and resuming" in {
    ...
    val writeCommands = Seq.tabulate(numOfCommands) { n =>
      val infoton =
        ObjectInfoton(
          ...
          indexTime = None,
          fields =
            Some(Map("games" -> Set(FieldValue("Taki"), FieldValue("Race"))))
        )
    ...
    for (i <- 0 to numOfCommands - 1) {
      val nextResult =
        Await.result(
          irwService.readPathAsync(s"/cmt/cm/bg-test/circumvented_bg/info$i"),
          5.seconds
        )
    ...
    for (i <- 0 to numOfCommands - 1) {
      val searchResponse = Await.result(
        ftsServiceES.search(
          pathFilter = None,
          fieldsFilter = Some(
            SingleFieldFilter(Must,
                              Equals,
                              "system.path",
                              Some(s"/cmt/cm/bg-test/circumvented_bg/info$i"))
          ),
          datesFilter = None,
