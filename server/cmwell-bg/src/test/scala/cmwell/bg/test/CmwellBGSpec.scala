...
class CmwellBGSpec extends AsyncFunSpec with BeforeAndAfterAll with Matchers with Inspectors with LazyLogging {
  ...
  def executeAfterCompletion[T](f: Future[_], timeout: FiniteDuration = 5.minutes)(
    body: => Future[T]
...
//    ftsServiceES = FTSServiceNew("es.test.yml")
    ...
    cmwellBGActor =
      actorSystem.actorOf(CMWellBGActor.props(0, bgConfig, irwService, ftsServiceES, zStore, offsetsService))
  ...
  describe("CmwellBG should") {
    ...
    val useNewlyCreatedAsBaseInfoton = okToStartPromise.future.flatMap { _ =>
      val pRecords = Seq.tabulate(20) { n =>
        val infoton = ObjectInfoton(path = s"/cmt/cm/bg-test/baseInfoton/info$n",
                                    dc = "dc",
                                    indexTime = None,
        ...
        val infoton = ObjectInfoton(path = s"/cmt/cm/bg-test/baseInfoton/info19",
                                    dc = "dc",
                                    indexTime = None,
    ...
    def afterFirst[T](body: => Future[T])(implicit ec: ExecutionContext): Future[T] =
    ...
    val writeCommandsProccessing = afterFirst {
      ...
      val writeCommands = Seq.tabulate(10) { n =>
        val infoton = ObjectInfoton(path = s"/cmt/cm/bg-test1/info$n",
                                    dc = "dc",
                                    indexTime = None,
...
//
      ...
      sendToKafkaProducer(pRecord).flatMap { recordMetaData =>
        scheduleFuture(3.seconds) {
          irwService.readPathAsync("/cmt/cm/bg-test1/info0", ConsistencyLevel.QUORUM).map {
            ...
            case somethingElse =>
              fail(s"expected a deleted infoton, but got [$somethingElse] from irw (recoredMetaData: $recordMetaData).")
          }
        }
      }
    }
    ...
    val indexTimeAddedToNonOverrideCmds = afterFirst {
      val writeCommands = Seq.tabulate(10) { n =>
        val infoton = ObjectInfoton(path = s"/cmt/cm/bg-test/indexTime/info$n",
                                    dc = "dc",
                                    indexTime = None,
                                    fields = Some(Map("a" -> Set(FieldValue("b"), FieldValue("c")))))
        WriteCommand(infoton)
    ...
    val markInfotonAsHistory = executeAfterCompletion(indexAllInfotons) {
      ...
      val pRecord =
        new ProducerRecord[Array[Byte], Array[Byte]]("persist_topic", CommandSerializer.encode(writeCommand))
      sendToKafkaProducer(pRecord).flatMap { recordMetadata =>
        scheduleFuture(5.seconds) {
          ...
          val f1 = ftsServiceES.search(
            ...
            fieldsFilter = Some(
              MultiFieldFilter(Must,
                               Seq(FieldFilter(Must, Equals, "system.path", "/cmt/cm/bg-test1/info1"),
    ...
    val processOverrideCommands = afterFirst {
      ...
      sendEm.flatMap { recordMetaDataSeq =>
        scheduleFuture(3000.millis)(
          ftsServiceES.search(pathFilter = Some(PathFilter("/cmt/cm/bg-test3", true)),
                              fieldsFilter = None,
                              datesFilter = None,
                              paginationParams = DefaultPaginationParams)
        ).map { response =>
    ...
    val deeplyNestedOverrideCommands = afterFirst {
      ...
      sendIt.flatMap { recordMetaData =>
        scheduleFuture(10.seconds) {
          ...
          val f0 = ftsServiceES.search(pathFilter = Some(PathFilter("/cmt/cm/bg-test4", descendants = true)),
                                       fieldsFilter = None,
                                       datesFilter = None,
                                       paginationParams = DefaultPaginationParams)
    ...
    def verifyBgTest5() = {
      ...
      val f1 = ftsServiceES.search(
        ...
        fieldsFilter = Some(
          MultiFieldFilter(Must,
                           Seq(FieldFilter(Must, Equals, "system.path", "/cmt/cm/bg-test5/infobj"),
                               FieldFilter(Must, Equals, "system.current", "false")))
        ),
        datesFilter = None,
        paginationParams = DefaultPaginationParams,
        ...
        ftsServiceES
          ...
          .flatMap { res =>
            if (res.total >= numOfVersionsToExpect) Future.successful(res)
              ...
              Future.failed(new IllegalStateException(s"Waited for over 30s, last res: ${res.toString}"))
    ...
    val version4IngestAndVerify = executeAfterCompletion(version2IngestAndVerify) {
      val infoton = ObjectInfoton(
        ...
        new org.joda.time.DateTime(currentTime + 10000),
        fields =
          Some(Map("No" -> Set(FieldValue("U"), FieldValue("Go")), "But" -> Set(FieldValue("I don't need the ATM."))))
      )
...
//    }
    ...
    it("process OverrideCommands correctly by keeping its original indexTime and not generating a new one")(
    ...
    it("process OverrideCommands correctly by not grouping same path commands together for merge")(
    ...
    describe("process OverrideCommands correctly by keeping history in correct order") {
      ...
      it("and then ingesting version 1 and verifying version 1 is history, while version 3 stays current")(
      ...
      it("and then ingesting version 5 and verifying version 1-4 are history, while version 5 became current")(
