...
class CmwellBGSpec
  ...
  def sendToKafkaProducer(
    pRecord: ProducerRecord[Array[Byte], Array[Byte]]
  ...
  def executeAfterCompletion[T](f: Future[_],
...
//    ftsServiceES = FTSServiceNew("es.test.yml")
  ...
  describe("CmwellBG should") {
    ...
    def afterFirst[T](
      ...
      executeAfterCompletion(useNewlyCreatedAsBaseInfoton)(body)(ec)
...
//        logger.info(s"waiting for 5 seconds for $recordMetaDataSeq")
        scheduleFuture(5.seconds) {
          irwService
            .readPathAsync("/cmt/cm/bg-test1/info1", ConsistencyLevel.QUORUM)
            .map { infopt =>
...
//    }
//
    val processDeletePathCommands =
      executeAfterCompletion(writeCommandsProccessing) {
        ...
        sendToKafkaProducer(pRecord).flatMap { recordMetaData =>
          scheduleFuture(3.seconds) {
            irwService
              .readPathAsync("/cmt/cm/bg-test1/info0", ConsistencyLevel.QUORUM)
              .map {
                case FullBox(i: DeletedInfoton) => succeed
                case somethingElse =>
                  fail(
                    s"expected a deleted infoton, but got [$somethingElse] from irw (recoredMetaData: $recordMetaData)."
                  )
              }
          }
        }
    ...
    val parentsCreation = executeAfterCompletion(writeCommandsProccessing) {
      Future
        .traverse(Seq("/cmt/cm/bg-test1", "/cmt/cm", "/cmt")) { path =>
    ...
    val groupedWriteCommands = afterFirst {
      ...
      val writeCommands = Seq.tabulate(20) { i =>
        val infoton =
          ObjectInfoton(path = infotonPath,
                        dc = "dc",
                        indexTime = None,
                        lastModified =
                          new org.joda.time.DateTime(currentTime + i),
      ...
      sendEm.flatMap { recordMetaDataSeq =>
        ...
        cmwell.util.concurrent
          .unsafeRetryUntil[cmwell.util.Box[Infoton]]({ bi =>
          ...
          .flatMap { infopt =>
            irwService.historyAsync(infotonPath, 20).flatMap { v =>
              irwService.readUUIDSAsync(v.map(_._2)).map { histories =>
                withClue(histories) {
                  infopt should not be empty
                  (infopt.get.fields.get should contain)
                    .theSameElementsAs(expectedFields)
                }
              }
            }
          }
    ...
    val indexTimeAddedToNonOverrideCmds = afterFirst {
      ...
      sendEm
        ...
        .flatMap { searchResults =>
          ...
          Future
            .traverse(searchResults.infotons) { i =>
              irwService.readUUIDAsync(i.uuid)
            }
            .map { irwResults =>
              withClue(ftsSortedPaths, irwResults) {
                val irwSortedPaths =
                  irwResults.sortBy(_.get.indexTime.get).map(_.get.path)
                (ftsSortedPaths should contain).theSameElementsInOrderAs(
                  irwSortedPaths
                )
              }
            }
    ...
    val markInfotonAsHistory = executeAfterCompletion(indexAllInfotons) {
      val writeCommand =
        WriteCommand(
          ObjectInfoton("/cmt/cm/bg-test1/info1",
                        "dc",
                        None,
      ...
      val pRecord =
        new ProducerRecord[Array[Byte], Array[Byte]](
          "persist_topic",
          CommandSerializer.encode(writeCommand)
        )
      sendToKafkaProducer(pRecord).flatMap { recordMetadata =>
        scheduleFuture(5.seconds) {
          ...
          val f1 = ftsServiceES.search(
            ...
            fieldsFilter = Some(
              MultiFieldFilter(
                Must,
                Seq(FieldFilter(Must,
                                Equals,
                                "system.path",
                                "/cmt/cm/bg-test1/info1"),
          ...
          for {
            ...
            withClue(ftsSearchResponse1, ftsSearchResponse2, irwHistoryResponse) {
    ...
    val processOverrideCommands = afterFirst {
      ...
      val overrideCommands = Seq.tabulate(numOfInfotons) { n =>
        val infoton = ObjectInfoton(
          ...
          fields = Some(
            Map(
              "pearls" -> Set(FieldValue("Ubuntu"), FieldValue("shmubuntu"))
            )
          )
        )
      ...
      sendEm.flatMap { recordMetaDataSeq =>
        scheduleFuture(3000.millis)(
          ftsServiceES.search(pathFilter =
                                Some(PathFilter("/cmt/cm/bg-test3", true)),
                              fieldsFilter = None,
                              datesFilter = None,
                              paginationParams = DefaultPaginationParams)
        ).map { response =>
    ...
    val reProcessNotIndexedOWCommands = afterFirst {
      ...
      val infotons = Seq.tabulate(5) { n =>
        ObjectInfoton(
          ...
          fields = Some(
            Map(s"a$n" -> Set(FieldValue(s"b$n"), FieldValue(s"c${n % 2}")))
          )
        )
      }
      val owCommands = infotons.map { i =>
        OverwriteCommand(i)
      }
              ...
              scheduleFuture(5.seconds) {
                sendAgain.flatMap { _ =>
                  ftsServiceES
                    .search(
                      pathFilter =
                        Some(PathFilter("/cmt/cm/bg-test/re_process_ow", true)),
                      fieldsFilter = None,
                      datesFilter = None,
                      paginationParams = DefaultPaginationParams,
                      sortParams =
                        FieldSortParams(List(("system.indexTime" -> Desc))),
                      withHistory = false,
                      debugInfo = true
                    )
                    .map { res =>
                      withClue(res,
                               res.infotons.head.lastModified.getMillis,
                               currentTime) {
                        res.infotons.size should equal(1)
                        res.infotons.head.lastModified.getMillis should equal(
                          currentTime + 4
                        )
                      }
                    }
    ...
    val notGroupingOverrideCommands = afterFirst {
      ...
      val overrideCommands = Seq.tabulate(numOfInfotons) { n =>
        val infoton = ObjectInfoton(
          ...
          fields = Some(
            Map(
              s"Version${n % 3}" -> Set(FieldValue(s"a$n"),
                                        FieldValue(s"b${n % 2}"))
            )
          )
        )
      ...
      sendEm.flatMap { recordMetaDataSeq =>
        scheduleFuture(10.seconds) {
          ftsServiceES
            .search(
              pathFilter =
                Some(PathFilter("/cmt/cm/bg-test/override_not_grouped", false)),
              fieldsFilter = None,
              datesFilter = None,
              paginationParams = DefaultPaginationParams,
              withHistory = true,
    ...
    val persistAndIndexLargeInfoton = afterFirst {
      ...
      val fatFoton = ObjectInfoton("/cmt/cm/bg-test-fat/fatfoton1",
                                   "dcc",
                                   None,
                                   lotsOfFields)
      ...
      sendIt.flatMap { recordMetaData =>
        scheduleFuture(10.seconds) {
          val readReply =
            irwService.readPathAsync("/cmt/cm/bg-test-fat/fatfoton1")
          val searchReply =
            ftsServiceES.search(pathFilter =
                                  Some(PathFilter("/cmt/cm/bg-test-fat", true)),
                                fieldsFilter = None,
                                datesFilter = None,
                                paginationParams = DefaultPaginationParams)
    ...
    val deeplyNestedOverrideCommands = afterFirst {
      ...
      val infoton = ObjectInfoton(
        ...
        fields = Some(
          Map("whereTo" -> Set(FieldValue("The"), FieldValue("ATM!")))
        )
      )
      ...
      sendIt.flatMap { recordMetaData =>
        scheduleFuture(10.seconds) {
          ...
          val f0 = ftsServiceES.search(
            pathFilter =
              Some(PathFilter("/cmt/cm/bg-test4", descendants = true)),
            fieldsFilter = None,
            datesFilter = None,
            paginationParams = DefaultPaginationParams
          )

          val f1 = irwService.readPathAsync("/cmt/cm/bg-test4",
                                            ConsistencyLevel.QUORUM)
          val f2 = irwService.readPathAsync("/cmt/cm/bg-test4/deeply",
                                            ConsistencyLevel.QUORUM)
          val f3 =
            irwService.readPathAsync("/cmt/cm/bg-test4/deeply/nested",
                                     ConsistencyLevel.QUORUM)
          val f4 =
            irwService.readPathAsync("/cmt/cm/bg-test4/deeply/nested/overwrite",
                                     ConsistencyLevel.QUORUM)
          val f5 = irwService.readPathAsync(
            "/cmt/cm/bg-test4/deeply/nested/overwrite/infobj",
            ConsistencyLevel.QUORUM
          )
          ...
          for {
            ...
            withClue(r0, r1, r2, r3, r4, r5, recordMetaData) {
              ...
              paths shouldNot contain(
                "/cmt/cm/bg-test4/deeply/nested/overwrite"
              )
    ...
    def verifyBgTest5() = {
      ...
      val f2 = irwService.readPathAsync("/cmt/cm/bg-test5/infobj",
                                        ConsistencyLevel.QUORUM)
    ...
    def waitForIt(
      numOfVersionsToExpect: Int
    )(implicit ec: ExecutionContext): Future[FTSSearchResponse] = {
      ...
      def waitForItInner(): Future[FTSSearchResponse] = {
        ftsServiceES
          ...
          .flatMap { res =>
            ...
            else if (System.currentTimeMillis() - startTime > 30000L)
              Future.failed(
                new IllegalStateException(
                  s"Waited for over 30s, last res: ${res.toString}"
                )
              )
    ...
    val version3IngestAndVerify = afterFirst {
      val infoton = ObjectInfoton(
        ...
        new org.joda.time.DateTime(currentTime),
        fields = Some(
          Map("GoTo" -> Set(FieldValue("draw"), FieldValue("money")))
        )
      )
      ...
      sendIt(infoton).flatMap { recordMetaData =>
        waitForIt(1).flatMap { ftsRes =>
          verifyBgTest5().map {
            case t @ (currFTSRes, histFTSRes, currPathIRWBox, historiesIRW) =>
              withClue(t -> ftsRes) {
                val currPathIRW = currPathIRWBox.toOption
                currFTSRes.total should be(1)
                currFTSRes.infotons.head.indexTime.value should be(
                  currentTime + 42
                )
    ...
    val version1IngestAndVerify =
      executeAfterCompletion(version3IngestAndVerify) {
        val infoton = ObjectInfoton(
          path = s"/cmt/cm/bg-test5/infobj",
          dc = "dc",
          ...
          fields = Some(
            Map(
              "whereTo" -> Set(FieldValue("Techno"), FieldValue("Doron"))
            )
          )
        )
            ...
            verifyBgTest5().map {
              case t @ (currFTSRes, histFTSRes, currPathIRWBox, historiesIRW) =>
                withClue(t -> ftsRes) {
                  ...
                  currPathIRW shouldBe defined
                  currPathIRW.value.indexTime.value should be(currentTime + 42)
                  currPathIRW.value.uuid shouldEqual currFTSRes.infotons.head
                    .uuid
    ...
    val version2IngestAndVerify =
      executeAfterCompletion(version1IngestAndVerify) {
        val infoton = ObjectInfoton(
          path = s"/cmt/cm/bg-test5/infobj",
          dc = "dc",
    ...
    val version4IngestAndVerify =
      executeAfterCompletion(version2IngestAndVerify) {
        val infoton = ObjectInfoton(
          path = s"/cmt/cm/bg-test5/infobj",
          dc = "dc",
          ...
          new org.joda.time.DateTime(currentTime + 10000),
          fields = Some(
            Map("No" -> Set(FieldValue("U"), FieldValue("Go")),
              ...
              verifyBgTest5().map {
                case t @ (currFTSRes,
                  ...
                  withClue(t -> ftsRes) {
                    ...
                    currPathIRW shouldBe defined
                    currPathIRW.value.indexTime.value should be(
                      currentTime + 23
                    )
                    currPathIRW.value.uuid shouldEqual currFTSRes.infotons.head
                      .uuid
    ...
    val version5IngestAndVerify =
      executeAfterCompletion(version4IngestAndVerify) {
        val infoton = ObjectInfoton(
          path = s"/cmt/cm/bg-test5/infobj",
          dc = "dc",
          ...
          new org.joda.time.DateTime(currentTime + 20000),
          fields = Some(
            Map("So" -> Set(FieldValue("Why you asked?")),
    ...
    it("use the cache of newly created infotons as baseInfoton before merge")(
      useNewlyCreatedAsBaseInfoton
    )
    ...
    it(
      "add index time to non override commands and update indexTime in Cassandra"
    )(
      indexTimeAddedToNonOverrideCmds
    )
    it("mark infoton as history if newer version is sent for it")(
      markInfotonAsHistory
    )
    it("process WriteCommands containing fat infoton")(
      persistAndIndexLargeInfoton
    )
...
//
//    }
    it("re process OW commands even if were not indexed at first")(
      reProcessNotIndexedOWCommands
    )
    it(
      "process OverrideCommands correctly by keeping its original indexTime and not generating a new one"
    )(processOverrideCommands)
    it(
      "process group of writecommands in short time while keeping all fields (in case of the data being splitted to several versions, last version must contain all data)"
    )(groupedWriteCommands)
    it(
      "process OverrideCommands correctly by not grouping same path commands together for merge"
    )(
      notGroupingOverrideCommands
    )
    it("process OverrideCommands correctly by creating parents if needed")(
      deeplyNestedOverrideCommands
    )
    describe(
      "process OverrideCommands correctly by keeping history in correct order"
    ) {
      it(
        "while ingesting version 3 first, and verifying  version 3 is current"
      )(
        version3IngestAndVerify
      )
      it(
        "and then ingesting version 1 and verifying version 1 is history, while version 3 stays current"
      )(version1IngestAndVerify)
      it(
        "and then ingesting version 2, which is history but with newer indexTime, and verifying version 2&1 are history, while version 3 stays current"
      )(version2IngestAndVerify)
      it(
        "and then ingesting version 4 with older indexTime and verifying version 1-3 are history, while version 4 became current"
      )(version4IngestAndVerify)
      it(
        "and then ingesting version 5 and verifying version 1-4 are history, while version 5 became current"
      )(version5IngestAndVerify)
    }
