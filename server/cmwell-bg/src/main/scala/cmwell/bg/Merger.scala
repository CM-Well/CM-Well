...
final case class NullUpdate(path: String, tids: Seq[String], evictions: Seq[(String,Option[String])]) extends MergeResponse {
  override def merged = None
}
final case class RealUpdate(infoton: Infoton,tids: Seq[String], evictions: Seq[(String,Option[String])]) extends MergeResponse {
...
class Merger(config:Config) extends LazyLogging {
  ...
  private def merge_f(last_fields: Option[Map[String, Set[FieldValue]]], current_fields: Option[Map[String, Set[FieldValue]]] ): Option[Map[String, Set[FieldValue]]] = {
  ...
  private def delete_f( current_fields: Option[Map[String, Set[FieldValue]]], deleteFields: Map[String, Set[FieldValue]]): Option[Map[String, Set[FieldValue]]] = {
  ...
  private def update_f( current_fields:Option[Map[String, Set[FieldValue]]] , delete_fields: Map[String , Set[FieldValue]] , add_fields : Map[String , Set[FieldValue]] ) : Option[Map[String,Set[FieldValue]]] = {
    val after_delete = delete_f(current_fields , delete_fields)
    val after_update = merge_f(after_delete,Some(add_fields))
    after_update
  }

  private def delete_merge(prev_infoton: Infoton, fields: Map[String, Set[FieldValue]], lastModified: DateTime): Infoton = {
    prev_infoton match {
  ...
  private def update_merge(current_infoton: Infoton, delete_fields: Map[String, Set[FieldValue]], add_fields: Map[String, Set[FieldValue]], lastModified: DateTime): Infoton = {
  ...
  def merge(baseInfoton : Option[Infoton], cmds : Seq[SingleCommand]): MergeResponse = {
    ...
    def ensurePrevUUID(base: Infoton, prevUUID: Option[String])(getMerged: Infoton => Option[Infoton]): (Option[Infoton],Option[String]) =  {
    ...
    def ensurePrevNone(prevUUID: Option[String])(getApplied: =>Option[Infoton]): (Option[Infoton],Option[String]) = prevUUID match {
    ...
    def do_merge(base: Option[Infoton], cmd: SingleCommand): (Option[Infoton],Option[String]) = {
      cmd match {
        ...
        case DeleteAttributesCommand(_, fields, lastModified,_,prevUUID) =>
          base match {
            case Some(last_infoton) => ensurePrevUUID(last_infoton,prevUUID)(i => Some(delete_merge(i, fields, lastModified)))
            case None => ensurePrevNone(prevUUID)(None)
          }
        case UpdatePathCommand(path, deleteFields, updateFields, lastModified,_,prevUUID) =>
          base match {
            case Some(last_infoton) => ensurePrevUUID(last_infoton,prevUUID)(i => Some(update_merge(i, deleteFields, updateFields, lastModified)))
            case None => ensurePrevNone(prevUUID)(Some(ObjectInfoton(path, defaultDC, None, lastModified, updateFields)))
          }
    ...
    /**
      ...
      */
    @tailrec
    def merge_recurse(prev:Option[Infoton],
