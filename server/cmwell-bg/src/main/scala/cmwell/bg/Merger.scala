...
final case class NullUpdate(path: String,
                            tids: Seq[String],
                            evictions: Seq[(String, Option[String])])
    extends MergeResponse {
  override def merged = None
}
final case class RealUpdate(infoton: Infoton,
                            tids: Seq[String],
                            evictions: Seq[(String, Option[String])])
    extends MergeResponse {
...
class Merger(config: Config) extends LazyLogging {
  ...
  private def merge_f(
    last_fields: Option[Map[String, Set[FieldValue]]],
    current_fields: Option[Map[String, Set[FieldValue]]]
  ): Option[Map[String, Set[FieldValue]]] = {

    val previousFields =
      if (last_fields.exists(_.isEmpty)) None else last_fields
    val currentFields =
      if (current_fields.exists(_.isEmpty)) None else current_fields
  ...
  private def delete_f(
    current_fields: Option[Map[String, Set[FieldValue]]],
    deleteFields: Map[String, Set[FieldValue]]
  ): Option[Map[String, Set[FieldValue]]] = {
    val (asterix, filterQuad): (Boolean, Option[String] => Boolean) =
      deleteFields.get("*") match {
    ...
    val delete_fields: Map[String, Set[FieldValue]] =
      deleteFields.filterKeys(_ != "*")
    ...
    current_fields.map(_.map {
      case (k, vs) =>
        ...
        if (delete_fields.keySet(k) || asterix) {
          if (delete_fields
                .get(k)
                .collect { case xs => xs.isEmpty }
                .getOrElse(false))
            (k, Set.empty[FieldValue])
          else {
            val v = vs.filterNot(
              fv =>
                filterQuad(fv.quad) || (delete_fields
                  .contains(k) && delete_fields(k).exists {
  ...
  private def delete_merge(prev_infoton: Infoton,
    ...
    prev_infoton match {
      ...
      case i: DeletedInfoton =>
        i // if we got a delete on delete we need ignore the create of delete
      case j =>
        throw new NotImplementedError(
          s"kind [${j.kind}] uuid [${prev_infoton.uuid}] info [$j]"
        )
    }
  }
  ...
  private def update_merge(current_infoton: Infoton,
    ...
    current_infoton match {
      ...
      case _ =>
        ...
        logger.warn(
          s"kind [${current_infoton.kind}] uuid [${current_infoton.uuid}] info [$current_infoton]"
        )
        current_infoton
    }
  }

  private def write_merge(prev_infoton: Infoton,
                          current_infoton: Infoton): Infoton = {
    ...
    current_infoton match {
        ...
        prev_infoton match {
          case ObjectInfoton(_, _, _, _, prev_fields, _) =>
            ObjectInfoton(
              ...
              None,
              lastModified,
              merge_f(prev_fields, current_fields)
            )
          ...
          case _ =>
            throw new NotImplementedError(
              s"was trying to write_merge o[ $current_infoton ] on top of[ $prev_infoton ]"
            )
        }
        ...
        prev_infoton match {
          ...
          case FileInfoton(_, _, _, _, prev_fields, prev_fc, _) =>
            FileInfoton(
              ...
              lastModified,
              merge_f(
                if (prev_fields.exists(_.nonEmpty)) prev_fields else None,
                current_fields
              ),
              c_fc.orElse(prev_fc)
            )
          case LinkInfoton(_, _, _, _, prev_fields, _, _, _)
              if prev_fields.exists(_.nonEmpty) =>
            FileInfoton(
              path,
              dc,
              None,
          ...
          case _ =>
            throw new NotImplementedError(
              s"was trying to write_merge f[ $current_infoton ] on top of[ $prev_infoton ]"
            )
      ...
      case LinkInfoton(
        ...
        prev_infoton match {
          ...
          case FileInfoton(_, _, _, _, prev_fields, _, _)
              if prev_fields.exists(_.nonEmpty) =>
            LinkInfoton(
              path,
              dc,
          ...
          case _ =>
            throw new NotImplementedError(
              s"was trying to write_merge l[ $current_infoton ] on top of[ $prev_infoton ]"
            )
        }
      case _ =>
        throw new NotImplementedError(
          s"was trying to write_merge [ $current_infoton ] on top of[ $prev_infoton ]"
        )
    }
  }

  def merge(baseInfoton: Option[Infoton],
            cmds: Seq[SingleCommand]): MergeResponse = {

    def ensurePrevUUID(base: Infoton, prevUUID: Option[String])(
      getMerged: Infoton => Option[Infoton]
    ): (Option[Infoton], Option[String]) = {
      lazy val merged
        : (Option[Infoton], Option[String]) = getMerged(base) -> None
    ...
    def ensurePrevNone(
      prevUUID: Option[String]
    )(getApplied: => Option[Infoton]): (Option[Infoton], Option[String]) =
      prevUUID match {
        case None | Some("") => getApplied -> None
        case Some(someMadeUpUUID) =>
          None -> Some(s"expected:$someMadeUpUUID,actual:none")
      }
    ...
    /**
      ...
      */
    def do_merge(base: Option[Infoton],
                 cmd: SingleCommand): (Option[Infoton], Option[String]) = {
      cmd match {
        case WriteCommand(currInfoton, _, prevUUID) =>
          base match {
            case Some(last_infoton) =>
              ensurePrevUUID(last_infoton, prevUUID)(
                i => Some(write_merge(i, currInfoton))
              )
        ...
        case DeleteAttributesCommand(_, fields, lastModified, _, prevUUID) =>
          base match {
            case Some(last_infoton) =>
              ensurePrevUUID(last_infoton, prevUUID)(
                i => Some(delete_merge(i, fields, lastModified))
              )
          ...
          base match {
            case Some(last_infoton) =>
              ensurePrevUUID(last_infoton, prevUUID)(
                i =>
                  Some(
                    update_merge(i, deleteFields, updateFields, lastModified)
                )
              )
            case None =>
              ensurePrevNone(prevUUID)(
                Some(
                  ObjectInfoton(
                    path,
                    defaultDC,
        ...
        case DeletePathCommand(path, lastModified, _, prevUUID) =>
          base match {
            case sdi @ Some(di: DeletedInfoton) =>
              ensurePrevUUID(di, prevUUID)(_ => sdi)
            case Some(infoton) =>
              ensurePrevUUID(infoton, prevUUID)(
                _ => Some(DeletedInfoton(path, defaultDC, None, lastModified))
              )
            case None => ensurePrevNone(prevUUID)(None)
          }
    ...
    def merge_recurse(
      ...
      if (cmds.isEmpty) None -> evictionsAcc
        ...
        val evAcc =
          eviction.fold(tid.fold(evictionsAcc)(Right(_) :: evictionsAcc))(
            e => Left((e, tid)) :: evictionsAcc
          )
    ...
    val (evictions, trackingIds) =
      cmwell.util.collections.partitionWith(evictionsAndTIDs)(identity)
    ...
    merged match {
      case Some(i) if !baseInfoton.exists(_.isSameAs(i)) =>
        val infoton = baseInfoton.fold(i) { j =>
          if (j.lastModified.getMillis < i.lastModified.getMillis) i
            ...
            i.copyInfoton(
              lastModified = new DateTime(j.lastModified.getMillis + 1L)
            )
        }
      ...
      case Some(i) if baseInfoton.exists { bi =>
        ...
        RealUpdate(
          i.copyInfoton(lastModified = baseInfoton.get.lastModified),
          trackingIds,
          evictions
        )
      case _ =>
        NullUpdate(
          baseInfoton.fold(cmds.head.path)(_.path),
          trackingIds,
          evictions
        )
    }
