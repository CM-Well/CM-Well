...
class CMWellBGActor(partition: Int,
  ...
  val esReporterOpt: Option[ElasticsearchReporter] = if (reportMetricsToES) {
    logger.debug(
      s"available ES nodes: ${ftsService.nodesHttpAddresses().mkString(",")}"
    )
    Some(
      ElasticsearchReporter
        .forRegistry(metricRegistry)
        .hosts(ftsService.nodesHttpAddresses(): _*)
        .build()
    )
  } else None
  ...
  override def supervisorStrategy = OneForOneStrategy() {
    case t: Throwable =>
      logger
        .error("Exception caught in supervisor. resuming children actors", t)
  ...
  override def receive: Receive = {
    ...
    case All503 =>
      ...
      stopAll
      logger.debug(
        s"became state503. scheduling resume in [waitAfter503] seconds"
      )
      context.system.scheduler.scheduleOnce(waitAfter503.seconds, self, Resume)
    case Indexer503 =>
      logger.error(
        "Indexer Stopped with Exception. check indexer log for details. Restarting indexer."
      )
      stopIndexer
      startIndexer
    case Imp503 =>
      logger.error(
        "Imp stopped with exception. check imp log for details. Restarting imp."
      )
      stopImp
      startImp
    case ExitWithError =>
  ...
  private def startImp = {
    if (impOn) {
      if (impStream == null) {
        ...
        logger.warn(
          "requested to start Imp Stream but it is already running. doing nothing."
        )
    }
  }
  ...
  private def startIndexer = {
    if (indexerOn) {
      if (indexerStream == null) {
        ...
        indexerStream = new IndexerStream(
          partition,
          config,
          irwService,
          ftsService,
        ...
        logger.warn(
          "requested to start Indexer Stream but it is already running. doing nothing."
        )
    }
  }
...
class SimpleESIndicesMapping(mapping: Map[String, Iterable[String]])
    extends ESIndicesMapping {

  /**
    * gets relevant indices for given Infoton's UUID
    */
  override def indicesForUuid(uuid: String): Iterable[String] =
    mapping.get(uuid).getOrElse(Iterable.empty)
}
