...
class KafkaMonitorActor(zkServers:String,
                        ...
                        implicit val ec:ExecutionContext = global) extends Actor with LazyLogging {
  ...
  def normal: Receive = {
    case PeriodicCheck =>
      ...
      val decommissionBrokerCandidates = unavailableBrokers.filter{ unavailableBrokerId =>
        ...
        missingBrokers.get(unavailableBrokerId).fold {
          // this is a new missing broker, add it
            logger info s"broker id: $unavailableBrokerId is first to be noticed as missing, adding it with current time"
  ...
  def mitigating: Receive = {
    ...
    case ValidateAssignment(partitionsAssignment) =>
      ...
      if(reassignedPartitionsStatus.forall{
        ...
        context.system.scheduler.scheduleOnce(60.seconds, self, PeriodicCheck)
      } else if (reassignedPartitionsStatus.exists{_._2 match {case ReassignmentInProgress => true; case _ => false}}) {
        ...
        logger error s"some/all partitions failed to reassign. Maybe will succeeded in the next round: $reassignedPartitionsStatus"
        context.become(normal)
        context.system.scheduler.scheduleOnce(60.seconds, self, PeriodicCheck)
      }
  }
...
object KafkaAssigner extends LazyLogging{
  ...
  def generateAssignment(topics:Seq[String], assignToBrokers:Set[Int])(implicit zkUtils:ZkUtils):Map[TopicAndPartition, Seq[Int]] = {
//    val kafkaAssigner = new KafkaTopicAssigner
    val currentAssignment = zkUtils.getPartitionAssignmentForTopics(topics).map{ case (k,v) =>
      (k, v.map{case (key, value) => (key:java.lang.Integer, value.map{i => i:java.lang.Integer}.asJava)}.asJava)
    }.asJava
    logger debug s"current assignment: $currentAssignment"

    val assignToBrokersJ = assignToBrokers.map{ i => i:java.lang.Integer }.asJava
...
//    newAssignment.toMap
  ...
  def checkIfReassignmentSucceeded(zkUtils: ZkUtils, partitionsToBeReassigned: Map[TopicAndPartition, Seq[Int]])
