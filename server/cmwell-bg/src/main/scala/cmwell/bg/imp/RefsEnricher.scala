...
object RefsEnricher extends LazyLogging {
  ...
  ): Flow[BGMessage[Command], BGMessage[SingleCommand], NotUsed] = {
    Flow.fromGraph(GraphDSL.create() { implicit b =>
      import GraphDSL.Implicits._

      // CommandRef goes left, all rest go right
      // update metrics for each type of command
      val commandsPartitioner =
        b.add(
          PartitionWith[BGMessage[Command], BGMessage[CommandRef], BGMessage[
            Command
          ]] {
            case bgm @ BGMessage(_, CommandRef(_)) =>
              Left(bgm.asInstanceOf[BGMessage[CommandRef]])
            case bgm => Right(bgm)
          }
        )

      val commandRefsFetcher =
        Flow[BGMessage[CommandRef]].mapAsync(irwReadConcurrency) {
      ...
      commandsPartitioner.out0 ~> commandRefsFetcher ~> singleCommandsMerge.in(
        0
      )
      ...
      FlowShape(
        commandsPartitioner.in,
        singleCommandsMerge.out.map {
          bgMessage =>
            ...
            bgMessage.message match {
              ...
              case _: DeleteAttributesCommand =>
                bgm.deleteAttributesCommandsCounter += 1
              case unknown => logger.error(s"unknown command [$unknown]")
            }
            bgm.commandMeter.mark()
            bgMessage.copy(
              message = bgMessage.message.asInstanceOf[SingleCommand]
            )
        }.outlet
      )
    })
