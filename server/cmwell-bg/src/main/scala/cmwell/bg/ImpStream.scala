...
class ImpStream(
  ...
  bgActor: ActorRef,
  bGMetrics: BGMetrics
)(implicit actorSystem: ActorSystem, executionContext: ExecutionContext, materializer: ActorMaterializer)
  ...
  val (cmdsSrcFromKafka, sharedKillSwitch) =
    CommandsSource.fromKafka(persistCommandsTopic, bootStrapServers, partition, startingOffset, startingOffsetPriority)

  val heartBitLog =
    Flow[BGMessage[Command]].keepAlive(60.seconds, () => BGMessage(HeartbitCommand.asInstanceOf[Command])).filterNot {
  ...
  val breakOut2 = scala.collection
    .breakOut[Map[String, Seq[(BGMessage[SingleCommand], Int)]], (Int, BGMessage[(String, Seq[SingleCommand])]), collection.immutable.Seq[
    ...
    .mapConcat[BGMessage[(String, Seq[SingleCommand])]] { messages =>
      ...
      groupedByPath
        .map[(Int, BGMessage[(String, Seq[SingleCommand])]),
             collection.immutable.Seq[(Int, BGMessage[(String, Seq[SingleCommand])])]] {
  ...
  val addLatestInfotons = Flow[BGMessage[(String, Seq[SingleCommand])]].mapAsync(irwReadConcurrency) {
  ...
  val breakOut3 = scala.collection.breakOut[Seq[(Boolean, (String, DateTime))], ProducerRecord[Array[Byte], Array[
    Byte
  ]], collection.immutable.Iterable[ProducerRecord[Array[Byte], Array[Byte]]]]

  val createVirtualParents = Flow[BGMessage[Seq[(IndexCommand, Option[DateTime])]]]
    .mapAsync(1) {
        ...
        Future
          ...
          .map(_.collect {
            case (true, (path, childLastModified)) =>
              val infoton = ObjectInfoton(path = path,
                                          dc = defaultDC,
      ...
      case BGMessage(offsets, (previous, (latest, trackingIds))) =>
        ...
        irwService.writeAsync(latestWithIndexName, ConsistencyLevel.QUORUM).map { i =>
          ...
          if (previous.isEmpty || previous.get.isSameAs(latest)) {
            ...
            val indexNewInfoton: (IndexCommand, Option[DateTime]) = IndexNewInfotonCommand(
              i.uuid,
              true,
              i.path,
              Some(i),
            ...
            val indexExistingInfoton: (IndexCommand, Option[DateTime]) = IndexExistingInfotonCommand(
              previous.get.uuid,
              previous.get.weight,
              previous.get.path,
            ...
            List(BGMessage(updatedOffsetsForNew, Seq(indexNewInfoton)),
  ...
  val breakOut = scala.collection
    .breakOut[Iterable[IndexCommand], Message[Array[Byte], Array[Byte], Seq[Offset]], collection.immutable.Seq[
      Message[Array[Byte], Array[Byte], Seq[Offset]]
    ]]
      ...
      commands.map { command =>
        val commandToSerialize =
          if (command.isInstanceOf[IndexNewInfotonCommand] &&
            ...
            command
        Message(
          new ProducerRecord[Array[Byte], Array[Byte]](indexCommandsTopic,
                                                       commandToSerialize.path.getBytes,
                                                       CommandSerializer.encode(commandToSerialize)),
          offset
        )
  ...
  val impGraph = RunnableGraph.fromGraph(
    GraphDSL.create(cmdsSrcFromKafka, commitOffsetsFlow, publishVirtualParentsSink)((_, a, b) => (a, b)) {
      implicit builder => (commandsSource, commitOffsets, publishVirtualParents) =>
            ...
            case BGMessage(offset, (_, NullUpdate(path, tids, evictions))) =>
              ...
              val evictionsReported = {
                Future.traverse(evictions) {
                  case (reason, Some(tid)) =>
                    TrackingUtilImpl.updateEvicted(path, tid, reason).map {
                      case LogicalFailure(reason) =>
        ...
        val partitionOverrideInfotons = builder.add(
          PartitionWith[BGMessage[(String, Seq[(Infoton, Option[String])])],
        ...
        val processOverrideInfotons = builder.add(
          Flow[BGMessage[(String, Seq[(Infoton, Option[String])])]]
            .mapAsync(irwReadConcurrency) {
            ...
            .mapAsync(irwWriteConcurrency) {
              case BGMessage(offsets, (newInfotons, existingInfotonOpt)) =>
                val newInfotonsWithIndexName = newInfotons.map {
                  case (i, tid) => (i.copyInfoton(indexName = currentIndexName) -> tid)
                }
                irwService
                  .writeSeqAsync(newInfotonsWithIndexName.map {
                    _._1
                    ...
                    writtenInfotons =>
                      val bgMessages: List[BGMessage[Seq[IndexCommand]]] = newInfotonsWithIndexName.toList match {
                        case ((headInfoton, headTrackingId) :: tail) =>
                          ...
                          val indexNewInfotonCommands: Seq[IndexCommand] = IndexNewInfotonCommand(
                            headInfoton.uuid,
                            isHeadCurrent,
                            headInfoton.path,
                            Some(headInfoton),
                            headInfoton.indexName,
                            Seq(headTrackingId).flatten.map {
                              StatusTracking(_, numOfParts)
                            }
                          ) :: tail.map {
                            case (i, t) =>
                              IndexNewInfotonCommand(i.uuid, false, i.path, Some(i), i.indexName, Seq(t).flatten.map {
                                StatusTracking(_, numOfParts)
                              })
                          }
                          existingInfotonOpt.fold {
                            ...
                            existingInfoton =>
                              val statusTracking = newInfotonsWithIndexName
                                .map {
                                  _._2
                                }
                                .flatten
                              ...
                              if (isHeadCurrent && !existingInfotonOpt.exists(_.uuid == headInfoton.uuid)) {
                                ...
                                val indexExisting: BGMessage[Seq[IndexCommand]] = BGMessage(
                                  ...
                                  Seq(
                                    IndexExistingInfotonCommand(existingInfoton.uuid,
                                                                existingInfoton.weight,
                                                                existingInfoton.path,
                                                                existingInfoton.indexName,
                                                                statusTracking)
                                  )
                                )
        ...
        val groupEsActions = builder.add(
          ...
          )(_.message._1.map(_._2).foldLeft(0L)(_ + _))
        )
        ...
        val indexCommandsToESActions = builder.add(
          Flow[BGMessage[Seq[IndexCommand]]]
            .map {
              case bgMessage @ BGMessage(_, indexCommands) =>
        ...
        val sendActionsToES = builder.add(
          Flow[Seq[BGMessage[(Seq[(ESIndexRequest, Long)], Seq[IndexCommand])]]].mapAsync(1) {
            case bgMessages =>
              ...
              logger.debug(
                s"sending ${actions.size} actions to elasticsearch: ${actions} from commands: ${bgMessages.map {
                  _.message._2
                }.flatten}"
              )
              bGMetrics.indexBulkSizeHist += actions.size
              bGMetrics.indexingTimer
                .timeFuture(ftsService.executeIndexRequests(actions).recover {
                    ...
                    val filteredResults = bulkIndexResult match {
                      case s @ SuccessfulBulkIndexResult(_, failed) if failed.size > 0 =>
                        s.copy(failed = failed.filterNot { f =>
                          (f.reason.startsWith("DocumentAlreadyExistsException") || f.reason
                            .startsWith("VersionConflictEngineException"))
                        })
                      case other => other
                    }
            ...
            case bgMessage @ BGMessage(_, (bulkRes, commands)) =>
              ...
              if (startingIndexCount / numOfShardPerIndex >= maxDocsPerShard) {
                ...
                cmwell.util.concurrent
                  .retry(3)(ftsService.createIndex(nextIndexName))
                  .flatMap { _ =>
        ...
        val updateIndexInfoInCas = builder.add(
          Flow[BGMessage[(BulkIndexResult, Seq[IndexCommand])]].mapAsync(math.max(numOfCassandraNodes / 3, 2)) {
            case bgMessage @ BGMessage(_, (bulkRes, commands)) =>
              ...
              val indexTimeToUpdate = bulkRes.successful.collect {
        ...
        val partitionIndexResult = builder.add(
          Partition[BGMessage[(BulkIndexResult, Seq[IndexCommand])]](
            2, {
              case BGMessage(_, (bulkIndexResult, commands)) =>
                ...
                if (bulkIndexResult.isInstanceOf[RejectedBulkIndexResult] || bulkIndexResult.failed.size > 0) {
                  logger.debug(s"BulkIndexResult:${bulkIndexResult
                    .isInstanceOf[RejectedBulkIndexResult]} failed size:${bulkIndexResult.failed.size}")
                  1
                } else {
              ...
              Future
                .traverse(indexCommands) { indexCommand =>
                  TrackingUtilImpl.updateSeq(indexCommand.path, indexCommand.trackingIDs).map {
                    case LogicalFailure(reason) =>
                      logger.warn(
                        s"Failed to report tracking for path: ${indexCommand.path} with tids: ${indexCommand.trackingIDs
                          .mkString(",")} due to: $reason"
                      )
                      Future.successful(Unit)
                    case _ => Future.successful(Unit)
                  }
        ...
        val nonOverrideIndexCommandsBroadcast =
          builder.add(Broadcast[BGMessage[Seq[(IndexCommand, Option[DateTime])]]](2))

        val removeLastModified = builder.add(Flow[BGMessage[Seq[(IndexCommand, Option[DateTime])]]].map { msg =>
        ...
        singleCommandsPartitioner
          .out(0) ~> groupCommandsByPath ~> overrideCommandsToInfotons ~> partitionOverrideInfotons.in
        ...
        fusePartition
          .out(0) ~> indexCommandsToESActions ~> groupEsActions ~> filterNonEmpty ~> sendActionsToES ~> manageIndices ~> updateIndexInfoInCas ~> partitionIndexResult.in
