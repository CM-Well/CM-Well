...
//  val maintainIndicesInterval = Duration.fromNanos(config.getDuration("cmwell.bg.maintainIndicesInterval").toNanos)
  ...
  override def receive: Receive = {
    ...
    case UpdateCurrentAlias(previousIndex, newIndex) =>
      val replaceCurrentIndexAliasRes = ftsService.client.admin().indices().prepareAliases().removeAlias(previousIndex, latestIndexAliasName)
        .addAlias(newIndex, latestIndexAliasName).execute().actionGet()
      logger info s"Replace current index alias from: $previousIndex to: $newIndex acknowledged: ${replaceCurrentIndexAliasRes.isAcknowledged}"
  ...
  private def maintainIndices() = {
    ...
    val indicesStats = ftsService.client.admin().indices().prepareStats(allIndicesAliasName).clear().setDocs(true).execute().actionGet()

    // find latest index name
    val currentAliasRes = ftsService.client.admin.indices().prepareGetAliases(latestIndexAliasName).execute().actionGet()
    ...
    if( (numOfDocumentsCurrent/numOfShardsCurrent) > maxDocsPerShard) {
      logger info s"number of docs per shard:${numOfDocumentsCurrent/numOfShardsCurrent} has passed the threshold of: $maxDocsPerShard , shifting gear"
      // create new index
      ...
      val createNextCurrentIndexRes = ftsService.client.admin().indices().prepareCreate(nextCurrentIndexName)
        .addAlias(new Alias(allIndicesAliasName)).execute().actionGet()

      logger info s"Create new current index named:$nextCurrentIndexName acknowledged: ${createNextCurrentIndexRes.isAcknowledged}"
