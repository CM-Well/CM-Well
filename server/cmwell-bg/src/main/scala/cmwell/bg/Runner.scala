  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      logger.info(s"Loaded Configuration:\n ${config
        ...
        .asScala
        .collect {
          case entry if entry.getKey.startsWith("cmwell") =>
      ...
      Grid.setGridConnection(
        GridConnection(memberName = "bg", labels = Set("bg"))
      )
      ...
      case t: Throwable =>
        logger.error(
          s"BG Process failed to start thus exiting. Reason:\n${cmwell.common.exception
            .getStackTrace(t)}"
        )
        sys.exit(1)
    }
    ...
    sys.addShutdownHook {
      logger.info(
        s"shutting down cmwell-bg's Indexer and Imp flows before process is exiting"
      )
      try {
