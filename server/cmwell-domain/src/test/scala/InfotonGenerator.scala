...
object InfotonGenerator {
  ...
  val infotons: Gen[Infoton] = for {
    ...
    numVals <- Gen.resize(
      4,
      Gen.nonEmptyContainerOf[Set, java.math.BigInteger](Gen.choose(-100, 100).map(BigInt(_).underlying))
    )
    ...
    decVals <- Gen.resize(
      4,
      Gen.nonEmptyContainerOf[Set, java.math.BigDecimal](Gen.choose(-100.0, 100.0).map(BigDecimal(_).underlying))
    )
  ...
  val fCmp: Function2[Option[Map[String, Set[FieldValue]]], Option[Map[String, Set[FieldValue]]], Boolean] = (i, j) =>
  ...
  val cCmp: Function2[Option[FileContent], Option[FileContent], Boolean] = (i, j) =>
    List(i, j).flatten.size match {
      case 0 => true
      case 1 => false
      case _ => i.get == j.get
  ...
  val iCmp: Function2[Infoton, Infoton, Boolean] = (i, j) =>
    (i, j) match {
      ...
      case (f1: FileInfoton, f2: FileInfoton) =>
        sCmp(f1, f2) && fCmp(f1.fields, f2.fields) && cCmp(f1.content, f2.content)
