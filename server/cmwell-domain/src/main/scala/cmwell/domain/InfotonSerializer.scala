...
object InfotonSerializer extends LazyLogging {
  ...
  val tuplesToFields =
    breakOut[Map[CField, Iterable[(CQuad, CField, CValue)]],
             (CField, Vector[CValue]),
             Vector[
               (CField, Vector[CValue])
             ]]
  val tuplesToValues =
    breakOut[Iterable[(CQuad, CField, CValue)], CValue, Vector[CValue]]
  ...
  class MutableInfotonBuilder(uuidHint: String) {
    ...
    def setKind(kind: String): Unit = {
      if (this.kind eq null) this.kind = kind
        ...
        throw new IllegalStateException(
          s"kind was already set for uuid [$uuidHint] [${this.kind},$kind]"
        )
    }
    ...
    def setPath(path: String): Unit = {
      if (this.path eq null) this.path = path
        ...
        throw new IllegalStateException(
          s"path was already set for uuid [$uuidHint] [${this.path},$path]"
        )
    }

    def setLastModified(lastModified: String): Unit = {
      if (this.lastModified eq null)
        this.lastModified = fmt.parseDateTime(lastModified)
      else {
        val newLastModified = fmt.parseDateTime(lastModified)
        if (this.lastModified.compareTo(newLastModified) == 0)
          logger.warn(
            s"Been called twice on lastModified with same (parsed) value [${this.lastModified
              .toString(fmt)},$lastModified]"
          )
        else
          throw new IllegalStateException(
            s"lastModified was already set for uuid [$uuidHint] [${this.lastModified
              .toString(fmt)},$lastModified]"
          )
      }
    }
    ...
    def setDc(dc: String): Unit = {
      if (this.dc eq null) this.dc = dc
        ...
        throw new IllegalStateException(
          s"dc was already set for uuid [$uuidHint] [${this.dc},$dc]"
        )
    }
    ...
    def setIndexTime(indexTime: Long): Unit = this.indexTime match {
      ...
      case Some(t) =>
        throw new IllegalStateException(
          s"indexTime was already set for uuid [$uuidHint] [$t,$indexTime]"
        )
    }
    ...
    def setMimeType(mimeType: String): Unit = {
      if (this.mimeType eq null) this.mimeType = mimeType
        ...
        throw new IllegalStateException(
          s"mimeType was already set for uuid [$uuidHint] [${this.mimeType},$mimeType]"
        )
    }
    ...
    def setLinkTo(linkTo: String): Unit = {
      if (this.linkTo eq null) this.linkTo = linkTo
        ...
        throw new IllegalStateException(
          s"linkTo was already set for uuid [$uuidHint] [${this.linkTo},$linkTo]"
        )
    }
    ...
    def setLinkType(linkType: Int): Unit = {
      if (this.linkType == -1) this.linkType = linkType
        ...
        throw new IllegalStateException(
          s"linkType was already set for uuid [$uuidHint] [${this.linkType},$linkType]"
        )
    }
    ...
    def result(): Infoton = {
      if (this.infoton ne null) this.infoton
        ...
        require(
          ...
          s"must have lastModified initialized [$uuidHint]"
        )
        ...
        infoton = kind match {
          ...
          case "d" =>
            new DeletedInfoton(path, dc, indexTime, lastModified, indexName)
          case _ =>
            throw new IllegalStateException(
              s"unrecognized type was inserted [$kind] for uuid [$uuidHint]"
            )
        }
  ...
  def deserialize2(
    uuidHint: String,
    serializedInfoton: Iterator[(CQuad, CField, CValue)]
  ): Infoton = {
    ...
    def applySystemChanges(field: CField, value: CValue): Unit = field match {
      ...
      case f =>
        logger.error(
          s"got a weird system field [$f] from cassandra for uuid [$uuidHint]"
        )
    }

    def appendValues(field: CField, value: String, quad: Option[String]): Unit =
      field.head match {
        case 's' =>
          field.indexOf('$') match {
            case 1 =>
              infotonBuilder.addFieldValue(
                field.drop(2),
                FString(value, None, quad)
              )
            case i =>
              infotonBuilder.addFieldValue(
                field.drop(i + 1),
                FString(value, Some(field.substring(1, i)), quad)
              )
          }
        case 'r' =>
          infotonBuilder.addFieldValue(field.drop(2), FReference(value, quad))
        case 'd' =>
          infotonBuilder.addFieldValue(field.drop(2), FDate(value, quad))
        case 'b' =>
          infotonBuilder.addFieldValue(
            field.drop(2),
            FBoolean(value.toBoolean, quad)
          )
        case 'i' =>
          infotonBuilder.addFieldValue(field.drop(2), FInt(value.toInt, quad))
        case 'l' =>
          infotonBuilder.addFieldValue(field.drop(2), FLong(value.toLong, quad))
        case 'k' =>
          infotonBuilder.addFieldValue(
            field.drop(2),
            FBigInt(BigInt(value).underlying(), quad)
          )
        case 'f' =>
          infotonBuilder.addFieldValue(
            field.drop(2),
            FFloat(value.toFloat, quad)
          )
        case 'g' =>
          infotonBuilder.addFieldValue(
            field.drop(2),
            FDouble(value.toDouble, quad)
          )
        case 'h' =>
          infotonBuilder.addFieldValue(
            field.drop(2),
            FBigDecimal(BigDecimal(value).underlying(), quad)
          )
        case 'x' =>
          field.indexOf('$') match {
            case i =>
              infotonBuilder.addFieldValue(
                field.drop(i + 1),
                FExternal(value, field.substring(1, i), quad)
              )
          }
      }
  ...
  def serialize2(infoton: Infoton): (CUuid, SerializedInfoton) = {
    ...
    def systemAttributes(i: Infoton): Vector[(CField, Vector[CValue])] = {
      ...
      implicit def stringToCValue(s: String): CValue =
        s -> null.asInstanceOf[CData]

      val builder: mutable.Builder[(CField, Vector[CValue]), Vector[
        (CField, Vector[CValue])
      ]] = {
        ...
        i.indexTime.fold(b) { indexTime =>
          b += "indexTime" -> Vector(indexTime.toString)
        }
      }
      ...
      i match {
        case FileInfoton(
            ...
            _,
            _,
            Some(FileContent(data, mime, dl, dp)),
            _
            ) => {
          ...
          data.foreach { d =>
            def padWithZeros(num: String, padToLength: Int): String = {
              ...
              require(
                len <= padToLength,
                s"numeric String [$num] longer than padded length [$padToLength]"
              )
              if (len == padToLength) num
              else
                s"${"0" * (padToLength - len)}$num" // String.join(Array.fill(padToLength-len)('0'),num)
            }
            ...
            for (arr <- d.grouped(chunkSize)) {
              builder += "data" -> Vector(
                padWithZeros(count.toString, indexLength) -> arr
              )
              count += 1
            }
    ...
    def aggregateFieldsByQuad(
      fields: Map[String, Set[FieldValue]]
    ): Iterable[(CQuad, CField, CValue)] =
      for {
        (field, values) <- fields
        value <- values
      } yield
    ...
    val vecBuilder = Vector
      .newBuilder[(CQuad, Vector[(CField, Vector[CValue])])] += sysQuad -> systemAttributes(
      infoton
    )
      ...
      aggregateFieldsByQuad(fields).groupBy(_._1).foreach {
        case (quad, tuplesByQuad) => {
          vecBuilder += (quad -> tuplesByQuad
            .groupBy(_._2)
            .map {
              case (field, tuplesByField) =>
                field -> tuplesByField.map(_._3)(tuplesToValues)
            }(tuplesToFields))
        }
      }
      vecBuilder.result()
  ...
  def serialize(infoton: Infoton): mutable.Buffer[(String, Array[Byte])] = {
    val data: mutable.Buffer[(String, Array[Byte])] =
      new mutable.ListBuffer[(String, Array[Byte])]()
...
//        data += ( ("$type" , "o".getBytes("utf-8") ) )
  ...
  def deserialize(data: mutable.Buffer[(String, Array[Byte])]): Infoton = {
    ...
    val lastModified: DateTime =
      fmt.parseDateTime(new String(v("$lastModified"), "UTF-8"))
...
//    val sysFields = SystemFields(new lang.String(path, "utf-8"), lastModified, new lang.String(uuid, "utf-8"))
    ...
    val reply = infoType match {
      case "o" =>
        val i =
          ObjectInfoton(pathString, dcString, indexTime, lastModified, fields)
        i
