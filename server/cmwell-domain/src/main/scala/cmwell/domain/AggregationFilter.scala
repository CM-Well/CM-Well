...
trait AggregationFilter extends Formattable {
  ...
  def field: Field
  def `type`: String =
    this.getClass.getSimpleName.dropRight(6) // 6 == "Filter".length
...
case class StatsAggregationFilter(name: String = "Statistics Aggregation",
                                  override val field: Field)
    extends AggregationFilter {
  override def copyWithSuffix(suffix: String): AggregationFilter =
    this.copy(name = name + suffix)
}
...
case class TermAggregationFilter(name: String = "Term Aggregation",
  ...
  override def copyWithSuffix(suffix: String): AggregationFilter = {
    var counter = 0
    this.copy(name = name + suffix, subFilters = subFilters.map {
      counter += 1; _.copyWithSuffix(suffix + counter)
    })
  }
}
...
case class HistogramAggregationFilter(name: String = "Histogram Aggregation",
                                      ...
                                      extMin: Option[Long],
                                      extMax: Option[Long],
                                      subFilters: Seq[AggregationFilter] =
                                        Seq.empty)
    extends BucketAggregationFilter {
  override def copyWithSuffix(suffix: String): AggregationFilter = {
    var counter = 0
...
case class SignificantTermsAggregationFilter(
...
case class CardinalityAggregationFilter(name: String,
                                        override val field: Field,
