...
case class StatsAggregationFilter(name:String = "Statistics Aggregation", override val field:Field) extends AggregationFilter {
  override def copyWithSuffix(suffix: String): AggregationFilter = this.copy(name = name + suffix)
}

case class TermAggregationFilter(name:String = "Term Aggregation", override val field:Field, size:Int = 10,
...
case class SignificantTermsAggregationFilter(name:String = "Signigicant Terms Aggregation", override val field:Field,
...
case class CardinalityAggregationFilter(name:String, override val field:Field, precisionThreshold:Option[Long]) extends AggregationFilter {
  override def copyWithSuffix(suffix: String): AggregationFilter = this.copy(name = name + suffix)
}