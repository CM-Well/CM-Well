...
case class SearchResults(fromDate: Option[DateTime],
    ...
    extends Jsonable
    with Formattable {
  def masked(fieldsMask: Set[String]): SearchResults =
    copy(infotons = infotons.map(_.masked(fieldsMask)))
}

case class PaginationInfo(first: String,
...
case class SearchResponse(pagination: PaginationInfo, results: SearchResults)
    extends Jsonable
    with Formattable
...
case class IterationResults(iteratorId: String,
    ...
    extends Jsonable
    with Formattable {
  def masked(fieldsMask: Set[String]): IterationResults =
    copy(infotons = infotons.map(_.map(_.masked(fieldsMask))))
}

case class SimpleResponse(success: Boolean, msg: Option[String])
    extends Formattable
...
object Bucket {
  def apply(k: FieldValue, dc: Long, subAgg: Option[AggregationsResponse]) =
    new Bucket { def key = k; def docCount = dc; def subAggregations = subAgg }
  def unapply(
    b: Bucket
  ): Option[(FieldValue, Long, Option[AggregationsResponse])] =
    Some((b.key, b.docCount, b.subAggregations))
}

case class SignificantTermsBucket(
  override val key: FieldValue,
...
case class AggregationsResponse(responses: Seq[AggregationResponse],
                                debugInfo: Option[String] = None)
    extends Formattable

case class TermsAggregationResponse(filter: TermAggregationFilter,
                                    buckets: Seq[Bucket])
...
case class HistogramAggregationResponse(filter: HistogramAggregationFilter,
                                        buckets: Seq[Bucket])
    extends BucketsAggregationResponse
case class SignificantTermsAggregationResponse(
  filter: SignificantTermsAggregationFilter,
  docCount: Long,
  buckets: Seq[SignificantTermsBucket]
) extends BucketsAggregationResponse
case class CardinalityAggregationResponse(filter: CardinalityAggregationFilter,
                                          count: Long)
    extends AggregationResponse
