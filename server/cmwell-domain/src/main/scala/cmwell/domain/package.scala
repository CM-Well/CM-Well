...
package object domain extends LazyLogging {
  ...
  def addIndexTime(infoton: Infoton,
                   indexTime: Option[Long],
                   force: Boolean = false): Infoton =
    infoton match {
      // format: off
  ...
  def addIndexInfo(infoton: Infoton,
                   indexTime: Option[Long],
                   indexName: String,
                   force: Boolean = false): Infoton = infoton match {
    // format: off
  ...
  def addDc(infoton: Infoton, dc: String, force: Boolean = false): Infoton =
    infoton match {
      // format: off
    ...
    // format: on
      case i if i.dc != "na" => {
        logger.warn(
          s"was asked to add dc, but one is already supplied! uuid=${i.uuid}, path=${i.path}, dc=${i.dc}"
        )
        i
      }
      case _ => ???
  ...
  def addDcAndIndexTimeForced(infoton: Infoton,
                              dc: String,
                              indexTime: Long): Infoton =
    infoton match {
      // format: off
