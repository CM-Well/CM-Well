...
sealed trait Infoton extends Formattable { self =>
  ...
  def overrideUuid(forcedUuid: String) = this match {
    ...
    case di: DeletedInfoton =>
      new DeletedInfoton(di.path, di.dc, di.indexTime, di.lastModified) {
        override def uuid = forcedUuid
      }
    case ci: CompoundInfoton =>
      new CompoundInfoton(
    ...
    case gi: GhostInfoton =>
      new GhostInfoton(gi.path) { override def uuid = forcedUuid }
  }
  ...
  def replaceIndexTime(indextime: Long): Infoton =
    copyInfoton(indexTime = Some(indextime))
  ...
  private val (uuid_, weight_) = {
    ...
    fields.foreach { f =>
      (f.map { case (k, v) => (k, SortedSet(v.map(_.toString).toSeq: _*)) }
        .toSeq
        .sortBy(_._1))
        .foreach {
          case (k, v) =>
  ...
  protected def maskedFields(fieldsMask: Set[String]) =
    fields.map(flds => flds -- flds.keySet.diff(fieldsMask))
}

case class ObjectInfoton(
  path: String,
  dc: String,
...
case class FileInfoton(
  ...
  def withoutData: FileInfoton = {
    require(
      content.isDefined && (content.get.data.isDefined || content.get.dataPointer.isDefined),
      "content must be defined with either data or dataPointer"
    )
    val originalUuid = uuid
    val hash = content
      .flatMap(_.dataPointer)
      .orElse(content.flatMap(_.data).map(cmwell.util.string.Hash.sha1))
    new FileInfoton(
      path,
      dc,
      indexTime,
      ...
      fields,
      content.map(
        c => FileContent(None, c.mimeType, content.get.dataLength, hash)
      )
    ) {
      override val uuid = originalUuid
      override def kind = "FileInfoton"
  ...
  def populateDataFromPointerBy(
    ...
    require(
      content.isDefined && content.get.dataPointer.isDefined,
      "dataPointer must exist"
    )
    val originalUuid = uuid
    ...
    val dataFut = content
      ...
      .getOrElse(fetchFunc(hashOpt.get))
    dataFut.map(
      data =>
        new FileInfoton(
          path,
          ...
          fields,
          content
            .map(c => FileContent(Some(data), c.mimeType, data.length, hashOpt))
        ) {
          override val uuid = originalUuid
          override def kind = "FileInfoton"
      }
...
case class FileContent(data: Option[Array[Byte]],
  ...
  override def equals(other: Any) = other match {
    case fc: FileContent =>
      util.Arrays.equals(this.data.orNull, fc.data.orNull) && this.mimeType
        .equals(fc.mimeType) && this.dataPointer.equals(fc.dataPointer)
    case _ => false
  }
...
object FileContent {
  def apply(data: Array[Byte], mimeType: String) =
    new FileContent(Some(data), mimeType, data.length)
  def apply(mimeType: String, length: Long) =
    new FileContent(None, mimeType, length.toInt)
}

case class VirtualInfoton(infoton: Infoton) {
  require(
    !infoton.isInstanceOf[VirtualInfoton],
    "youtube.com/watch?v=v2FMqtC1x9Y"
  )
...
case class BagOfInfotons(infotons: Seq[Infoton]) extends Formattable {
  def masked(fieldsMask: Set[String]): BagOfInfotons =
    BagOfInfotons(infotons.map(_.masked(fieldsMask)))
}
case class RetrievablePaths(infotons: Seq[Infoton],
                            irretrievablePaths: Seq[String])
    extends Formattable {
  def masked(fieldsMask: Set[String]): RetrievablePaths =
    copy(infotons = infotons.map(_.masked(fieldsMask)))
}
...
object ContentPortion {
  ...
  def unknownNestedContent(infoton: Infoton): ContentPortion =
    UnknownNestedContent(infoton)
}
...
final class ComparisonImpossible private (val valueType: String,
                                          val input: String,
                                          cause: Throwable)
    extends Exception(
      s"can't compare [$input] with values of type [$valueType]",
      cause
    )
object ComparisonImpossible {
  def apply(valueType: String, input: String): ComparisonImpossible =
    new ComparisonImpossible(valueType, input, null)
  def apply(valueType: String,
            input: String,
            cause: Throwable): ComparisonImpossible =
    new ComparisonImpossible(valueType, input, cause)
  def unapply(
    ex: ComparisonImpossible
  ): Option[(String, String, Option[Throwable])] =
...
case class FNull(quad: Option[String]) extends FieldValue {
  ...
  override def compareToString(unparsedValue: String): Try[Int] =
    Failure(ComparisonImpossible("FNull", unparsedValue))
}
...
case class FExtra[T](value: T, quad: Option[String]) extends FieldValue {
  ...
  override def compareToString(unparsedValue: String): Try[Int] =
    Failure(ComparisonImpossible("FExtra", unparsedValue))
}
...
object FieldValue {
  ...
  def apply(value: String, dataTypeURI: String): FieldValue =
    this.apply(value, dataTypeURI, None)
  def apply(value: String,
            dataTypeURI: String,
            quad: Option[String]): FieldValue =
    FExternal(value, dataTypeURI, quad)
  ...
  def apply(num: java.math.BigInteger, quad: Option[String]): FieldValue =
    FBigInt(num, quad)
  ...
  def apply(num: java.math.BigDecimal, quad: Option[String]): FieldValue =
    FBigDecimal(num, quad)
  def apply(bool: Boolean): FieldValue = this.apply(bool, None)
  def apply(bool: Boolean, quad: Option[String]): FieldValue =
    FBoolean(bool, quad)
  def apply(str: String): FieldValue = this.apply(str, None, None)
  def apply(str: String,
            lang: Option[String],
            quad: Option[String]): FieldValue =
    FString(str, lang, quad)
}
...
case class FBigInt(value: java.math.BigInteger, quad: Option[String])
    extends FieldValue {
  override def toString(): String = value.toString
...
case class FBigDecimal(value: java.math.BigDecimal, quad: Option[String])
    extends FieldValue {
  override def toString(): String = value.toString
  ...
  override def compareToString(unparsedValue: String): Try[Int] =
    Try(BigDecimal(unparsedValue).underlying())
      .transform(
        parsedValue => Try(value.compareTo(parsedValue)),
        cause =>
          Failure(ComparisonImpossible("FBigDecimal", unparsedValue, cause))
      )
}
...
case class FExternal(value: String, dataTypeURI: String, quad: Option[String])
    extends FieldValue {
  require(dataTypeURI.forall(_ != '$'))
  override def toString(): String = value
  override def size: Long =
    value.getBytes("UTF-8").length + dataTypeURI.getBytes("UTF-8").length
...
case class FString(value: String, lang: Option[String], quad: Option[String])
    extends FieldValue {
  override def toString(): String = value
...
case class FReference(value: String, quad: Option[String])
    extends FieldValue
    with LazyLogging {
  override def toString(): String = value
  override def size: Long = value.getBytes("UTF-8").size
...
object FReference {
  ...
  def isUriRef(s: String): Boolean =
    scala.util.Try { new java.net.URL(s) }.isSuccess || s.startsWith(
      "cmwell://"
    )
}
...
final class FDate(private val temp: String, val quad: Option[String])
    extends FieldValue {
  ...
  override def equals(that: Any): Boolean = that match {
    case that: FDate =>
      that.canEqual(this) && that.value == this.value && that.quad == this.quad
    case _ => false
  }
...
object FDate extends LazyLogging {
  ...
  def unapply(fDate: FDate): Option[(String, Option[String])] =
    Some(fDate.value -> fDate.quad)

  private val withDotdateParser =
    ISODateTimeFormat.dateTimeParser().withZone(DateTimeZone.UTC)
  private val withoutDotdateParser = DateTimeFormat
    .forPattern("yyyy-MM-dd'T'HH:mm:ss'Z'")
    .withZone(DateTimeZone.UTC)
  private val justDateParser =
    DateTimeFormat.forPattern("yyyy-MM-dd").withZone(DateTimeZone.UTC)
  private val FixableDate =
    """(\d{4}-\d{2}-\d{2})\s*T?\s*(\d{2}:\d{2}:\d{2})(.\d+)?\s*Z?\s*""".r

  def fixFormattingIfNeeded(str: String) = str match {
    case FixableDate(date, time, millis) =>
      date + "T" + time + Option(millis).getOrElse("") + "Z"
