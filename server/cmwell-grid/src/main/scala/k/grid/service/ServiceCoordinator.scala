...
class ServiceCoordinator extends Actor with LazyLogging {
  ...
  private[this] case class UpdateServiceMapping(name: String,
                                                ogm: Option[GridJvm])
  ...
  override def preStart(): Unit = {
    Grid.system.scheduler.schedule(
      0.seconds,
      30.seconds,
      self,
      SendRegistrations
  ...
  override def receive: Receive = {
    ...
    case BroadcastServiceMapping =>
      ...
      jvms.foreach { jvm =>
        Grid.selectActor(LocalServiceManager.name, jvm) ! ServiceMapping(
          mapping
        )
      }
    case SendRegistrations => {
      ...
      val futures = jvms.map { jvm =>
        (Grid.selectActor(LocalServiceManager.name, jvm) ? RegisterServices(
          Grid.thisMember
      ...
      future.foreach { stat =>
        ...
        val candidatesMap =
          candidatesSet.groupBy(_._1.name).map(t => t._1 -> t._2)
        candidatesMap.foreach {
          case (serviceName, ssg)
            ...
            val winner = ssg.find(_._1.preferredJVM.isDefined) match {
              case Some((ServiceStatus(_, _, Some(preferredJvm)), _))
                  if (stat.exists(_.member == preferredJvm)) =>
                logger.info(
                  s"[ServiceInstantiation] choosing preferred JVM as target for service"
                )
                preferredJvm
              case _ =>
                logger.info(
                  s"[ServiceInstantiation] choosing random JVM as target for service "
                )
            ...
            logger.info(
              s"[ServiceInstantiation] will run $serviceName on $winner"
            )
            Grid.selectActor(LocalServiceManager.name, winner) ! RunService(
              serviceName
            )
          ...
          case (serviceName, ssg)
            ...
            ssg.collect {
              case e @ (ServiceStatus(_, true, Some(pJVM)), rJVM)
                  if pJVM == rJVM =>
                e
            } match {
              ...
              case runningOnPref if runningOnPref.size > 0 =>
                ...
                logger.info(
                  s"[ServiceInstantiation] keeping preferred JVM and stopping the rest"
                )
                stop.foreach { m =>
                  Grid.selectActor(LocalServiceManager.name, m._2) ! StopService(
                    serviceName
                  )
                }
              ...
              case _ =>
                logger.info(
                  s"[ServiceInstantiation] keeping the first JVM and stopping the rest"
                )
                ssg
          ...
          case (serviceName, ssg)
              if ssg.count(_._1.isRunning) == 1 => // All is good!
            logger.debug(
              s"[ServiceInstantiation] All is good! There is one instance of $serviceName"
            )
            ...
            if (runner._1.preferredJVM.isEmpty && candidatesMap(serviceName)
                  .exists(_._1.preferredJVM.isDefined)) {
              ...
              Grid.selectActor(LocalServiceManager.name, runner._2) ! StopService(
                serviceName
              )
              // We will update that currently no one is running the service, we will know if it runs only in the next sample.
              self ! UpdateServiceMapping(serviceName, None)
              ...
              if (mapping.get(runner._1.name).flatten != Some(runner._2))
                self ! UpdateServiceMapping(
                  serviceName,
                  ssg.find(_._1.isRunning).map(_._2)
                )
            }
        }
