...
/**
  ...
  */
case class ServiceInitInfo(ServiceType: Class[_ <: Actor],
                           preferredJVM: Option[GridJvm],
                           args: Any*)

case class ServiceTypes(
  m: Map[String, ServiceInitInfo] = Map.empty[String, ServiceInitInfo]
) {

  def addLocal(name: String,
               clazz: Class[_ <: Actor],
               args: Any*): ServiceTypes = {
...
class ServiceRouter(routee: String) extends Actor with LazyLogging {
  ...
  override def receive: Actor.Receive = {
    case KillService => {
      ...
      jvmOpt match {
        case Some(jvm) =>
          Grid.selectActor(s"${LocalServiceManager.name}/${routee}", jvm) ! PoisonPill
    ...
    case msg => {
      ...
      jvmOpt match {
        case Some(jvm) =>
          logger.trace(
            s"[ServiceRouter] routing message($msg) to $routee on $jvm"
          )
          val s = sender()
          (Grid.selectActor(s"${LocalServiceManager.name}/${routee}", jvm) ? msg)
            .foreach { f =>
              logger.trace(
                s"[ServiceRouter] received msg from $routee, piping to sender"
              )
              s ! f
            }
        case None => {
          ...
          if (now - printWarnOnResolvingErrorsTimestamp > 120000) {
            logger.warn(
              s"[ServiceRouter] couldn't find Service $routee instance!"
            )
  ...
  def serviceRef(name: String): ActorRef = scala.concurrent.blocking {
    synchronized {
      routers.get(name) match {
        ...
        case None =>
          val newRouter =
            Grid.create(classOf[ServiceRouter], s"$name-router", name)
...
class LocalServiceManager(st: ServiceTypes) extends Actor with LazyLogging {
  ...
  override def receive: Receive = {
    ...
    case RegisterServices(gm) => {
      ...
      logger.debug(
        s"[RegisterServices] registering [${st.m.keySet.mkString(",")}]"
      )
      val statuses = st.m.map { serviceType =>
        val isRunning = context.child(serviceType._1).isDefined
        logger.debug(
          s"[RegisterServices] the Service ${serviceType._1} running: $isRunning"
        )
    ...
    case RunService(name) => {
      if (context.child(name).isDefined) {
        ...
        logger.info(s"[RunService] will run $name, known Services: ${st.m.keySet
          .mkString("[", ",", "]")}")
