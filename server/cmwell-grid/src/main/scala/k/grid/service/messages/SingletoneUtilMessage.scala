...
case class ServiceStatus(name: String,
                         isRunning: Boolean,
                         preferredJVM: Option[GridJvm])

case class ServiceInstantiationRequest(member: GridJvm, s: Set[ServiceStatus])
    extends ServiceUtilMessage
...
case class ServiceMapping(mapping: Map[String, Option[GridJvm]])
    extends ServiceUtilMessage
