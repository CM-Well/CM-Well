...
class MonitorActor extends Actor with LazyLogging {
  ...
  override def receive: Receive = {
    ...
    case SetNodeLogLevel(lvl, duration) =>
      ...
      duration.foreach { d =>
        logger.info(
          s"Scheduling $editableLogger to be in level $originalLogLevel in $d minutes"
        )
        scheduledLogLevelReset = context.system.scheduler
          .scheduleOnce(
            d.minutes,
            self,
            SetNodeLogLevel(originalLogLevel, None)
          )
      }

      LoggerFactory
        .getLogger(editableLogger)
