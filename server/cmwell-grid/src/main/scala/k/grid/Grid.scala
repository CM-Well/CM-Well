...
case class GridConnection(memberName: String,
...
object GridJvm {
  ...
  def apply(m: Member): GridJvm = {
    GridJvm(
      m.address.host.getOrElse("") + ":" + m.address.port.getOrElse(0).toString
    )
  }
...
case class GridJvm(host: String, identity: Option[JvmIdentity]) {
  def port: Int = {
    if (identity.isEmpty) 0
      ...
      LocalRegistrationManager.jvms
        .find(
          jvm =>
            jvm.host == host && jvm.identity.isDefined && jvm.identity.get.name == idName
        )
  ...
  def hasLabel(label: String) =
    identity.map(_.labels.contains(label)).getOrElse(false)
...
object Grid extends LazyLogging {
  ...
  private def systemMemberIdentity =
    JvmIdentity("ctrl", seeds.head.split(":")(1).toInt)

  def jvmIdentities: Set[JvmIdentity] = {
    val res = LocalRegistrationManager.jvms
      .flatMap(_.identity)
      .map(jid => JvmIdentity(jid.name, 0))
    res + systemMemberIdentity
  }
  private var services: ServiceTypes = ServiceTypes()

  def declareServices(serviceTypes: ServiceTypes): Unit =
    scala.concurrent.blocking {
      synchronized {
        if (services.m.isEmpty) this.services = serviceTypes
        else {
          logger.warn(
            s"declaring services more than once [existing services: ${services.m.keys
              .mkString("[", ",", "]")}] [new services: ${serviceTypes.m.keys
              .mkString("[", ",", "]")}]"
          )
  ...
  def create(clazz: Class[_ <: Actor], name: String): ActorRef =
    g.create(clazz, name)
  def createAnon(clazz: Class[_ <: Actor], args: Any*) =
    g.createAnon(clazz, args: _*)
  def create(clazz: Class[_ <: Actor], name: String, args: Any*) =
    g.create(clazz, name, args: _*)
  def createSingleton(clazz: Class[_ <: Actor],
                      name: String,
                      role: Option[String],
                      args: Any*) =
    g.createSingleton(clazz, name, role, args: _*)
  def selectSingleton(name: String,
                      role: Option[String],
                      proxy: GridJvm = thisMember) =
    g.selectSingleton(name, role, proxy)
  // TODO: change to address
  def create_remote(clazz: Class[_ <: Actor],
                    name: String,
                    hostname: String,
                    port: Int,
                    args: Any*): ActorRef =
  ...
  def createSomewhere(white: Set[GridJvm],
                      ...
                      name: String,
                      args: Any*): ActorRef =
    g.createSomewhere(white, black, clazz, name, args: _*)
  ...
  def getRunningJvmsInfo = {
    ...
    val futures = all.map { jvm =>
      (selectActor(ClientActor.name, jvm) ? GetClientInfo).mapTo[JvmInfo].map {
        ci =>
          jvm -> ci
      }
    }
  ...
  class Resolver(actorSelection: ActorSelection,
                 retries: Int,
    ...
    def retry(retriesLeft: Int,
              client: ActorRef,
              cancellable: akka.actor.Cancellable): Receive = {
      ...
      case Resolve => {
        ...
        context.system.scheduler
          .scheduleOnce(
            timeout * retries,
            self,
            NoResponseAfterGraceTimeTimedOut
          )
      }
      ...
      case NoResponseAfterGraceTimeTimedOut => {
        client ! Status.Failure(
          new RuntimeException(
            s"No response (timeout) for actor selection of $actorSelection Identity request"
          )
        )
        context.stop(self)
      }
    ...
    def receive: Receive = {
      case Resolve => {
        actorSelection ! Identify(None)
        val c =
          context.system.scheduler.schedule(timeout, timeout, self, Resolve)
        context.become(retry(retries - 1, sender(), c))
      }
    }
  }

  def getRefFromSelection(
    ...
    import scala.concurrent.ExecutionContext.Implicits.global
    val ar =
      system.actorOf(Props(new Resolver(actorSelection, retries, timeout)))
  ...
  def actors(name: String,
             role: Option[String] = None): Vector[ActorSelection] =
    g.actors(name, role)
  ...
  def task(name: String, start: Long, interval: Long, role: Option[String])(
    t: => Unit
  ): ActorRef = {
  ...
  def subscribeForGridEvents(subscriber: ActorRef) =
    g.subscribeForGridEvents(subscriber)
...
trait Grid {
  ...
  def joinClient(systemName: String,
                 hostname: String,
                 seeds: Set[String],
                 port: Int,
...
case class JvmIdentity(name: String,
                       systemPort: Int,
                       labels: Set[String] = Set.empty[String])
...
//    }
    ...
    val nodes =
      status.filter(mem => mem.status == MemberStatus.Up).map(_.address)
    nodes
  }
  ...
  def joinClient(clusterName: String,
    ...
    val confTemp =
      ConfigFactory
        ...
        .withFallback(
          ConfigFactory
            .parseString("akka.remote.netty.tcp.hostname=%s".format(hostname))
        )

    system = ActorSystem(
      clusterName,
      confTemp.withFallback(
        config
          .getConfig("cmwell.grid")
          .withOnlyPath("akka")
          .withFallback(config)
      )
    )
    ...
    val monitorActor =
      system.actorOf(Props[MonitorActor], name = MonitorActor.name)
//    IO(Http)(Grid.system) ! Http.Bind(monitorActor, interface = hostname, port = monitorPort)
    ...
    Grid.create(
      classOf[LocalRegistrationManager],
      LocalRegistrationManager.name
    )
  ...
  def join(clusterName: String,
    ...
    roles.foreach(
      rs =>
        confTemp = confTemp.withFallback(
          ConfigFactory
            .parseString("akka.cluster.roles = [%s]".format(rs.mkString(",")))
      )
    )
...
//    settings = new Settings(settingsActor)
  ...
  def createOnJvm(jvm: GridJvm,
    ...
    val address = Address(
      ...
      clusterSystemName,
      jvm.host,
      jvm.identity.get.systemPort
    )
    system.actorOf(
      Props(clazz, args: _*).withDeploy(Deploy(scope = RemoteScope(address))),
      name
    )
  }
  ...
  def createSingleton(clazz: Class[_ <: Actor],
    ...
    val p = ClusterSingletonManager.props(
  ...
  def getSingletons: Map[String, Option[String]] = {
    LocalServiceManager.mapping.map(
      tpl =>
        tpl._1 -> tpl._2
          .map(gj => s"${gj.identity.map(_.name).getOrElse("")}@${gj.host}")
    )
  }
  ...
  def selectSingleton(name: String,
    ...
    if (!singletonsProxies.contains(name)) {
      ...
      val aRefFut =
        system.actorSelection(s"/user/$proxyName").resolveOne().andThen {
          case Failure(_) =>
            system.actorOf(
              ClusterSingletonProxy.props(
                singletonManagerPath = s"/user/$name",
...
//  }
  ...
  def terminate: Future[Unit] =
    system
      .terminate()
      .map(_ => ())(scala.concurrent.ExecutionContext.Implicits.global)
  ...
  def subscribeForGridEvents(subscriber: ActorRef) = {
    Grid.selectActor(ClientActor.name, Grid.thisMember) ! EventSubscription(
      subscriber
    )
  }
}
