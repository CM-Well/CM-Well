...
case class GridJvm(host : String, identity : Option[JvmIdentity]) {
  def port : Int = {
    if(identity.isEmpty) 0
      ...
      LocalRegistrationManager.jvms.find(jvm => jvm.host == host && jvm.identity.isDefined && jvm.identity.get.name == idName).map(_.identity.map(_.systemPort)).flatten.getOrElse(0)
    }
  }
...
object Grid extends LazyLogging {
  ...
  def declareServices(serviceTypes: ServiceTypes) : Unit = scala.concurrent.blocking {
    synchronized {
      if (services.m.isEmpty) this.services = serviceTypes
        ...
        logger.warn(s"declaring services more than once [existing services: ${services.m.keys.mkString("[",",","]")}] [new services: ${serviceTypes.m.keys.mkString("[",",","]")}]")
  ...
  def createSingleton(clazz: Class[_ <: Actor] , name : String, role : Option[String], args: Any*) = g.createSingleton(clazz , name, role, args:_*)
  def selectSingleton(name : String , role : Option[String], proxy : GridJvm = thisMember) = g.selectSingleton(name, role, proxy)
  // TODO: change to address
  def create_remote( clazz: Class[_ <: Actor] , name : String , hostname : String , port : Int, args : Any* ) : ActorRef = g.create_remote(clazz , name , hostname , port , args:_* )

  def createOnJvm(jvm: GridJvm, clazz: Class[_ <: Actor] , name : String , args: Any*) : ActorRef = g.createOnJvm(jvm, clazz, name, args:_*)
  def createSomewhere(white : Set[GridJvm], black : Set[GridJvm], clazz: Class[_ <: Actor] , name : String , args: Any*) : ActorRef = g.createSomewhere(white, black, clazz, name, args:_*)
  ...
  def allActors = {
    val responses = Grid.jvmsAll.map {
      member =>
...
trait Grid {
  def join( clusterName : String, hostname : String , seeds : Set[String] , port : Int , singletons : ServiceTypes, roles : Option[Set[String]]) : Unit
  ...
  def createSomewhere(white: Set[GridJvm], black: Set[GridJvm], clazz: Class[_ <: Actor], name: String, seq: Any*) : ActorRef
...
//    }
  ...
  def joinClient(clusterName : String, hostname : String, seeds : Set[String], port : Int, singletons : ServiceTypes): Unit = {
    logger.info(s"############### Connecting to port $port ###################")
    val confTemp =
      ConfigFactory.parseString("akka.remote.netty.tcp.port=%d".format(port)).
        withFallback(ConfigFactory.parseString("akka.remote.netty.tcp.hostname=%s".format(hostname)))

    system = ActorSystem(clusterName, confTemp.withFallback(config.getConfig("cmwell.grid").withOnlyPath("akka").withFallback(config)))
...
//    IO(Http)(Grid.system) ! Http.Bind(monitorActor, interface = hostname, port = monitorPort)
  ...
  def join(clusterName : String, hostname : String , seeds : Set[String] , port : Int , singletons : ServiceTypes , roles : Option[Set[String]]):Unit = {
    ...
    */
    ...
    roles.foreach(rs => confTemp = confTemp.withFallback(ConfigFactory.parseString("akka.cluster.roles = [%s]".format(rs.mkString(",")))))
...
//    settings = new Settings(settingsActor)
  ...
  def createSomewhere(white: Set[GridJvm], black: Set[GridJvm], clazz: Class[_ <: Actor], name: String, args: Any*): ActorRef = {
  ...
  def getSingletons : Map[String, Option[String]] = {
    LocalServiceManager.mapping.map(tpl => tpl._1 -> tpl._2.map(gj => s"${gj.identity.map(_.name).getOrElse("")}@${gj.host}"))
  }
