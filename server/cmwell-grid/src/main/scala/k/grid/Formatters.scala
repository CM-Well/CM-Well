...
object Formatters {
  ...
  def memberFormatter(member: Member, isLeader: Boolean): String = {
    taple(
      member.address.toString + (if (isLeader) "*" else ""),
      member.roles.mkString(", "),
  ...
  def membersFormatter(members: Map[GridJvm, JvmInfo]): String = {
    taple("Address") +
      members.toSeq
        .sortBy(m => m._1.hostname)
        .map(m => taple(m._1.hostname))
        .mkString +
      s"Total: ${members.size}"
  }

  def singletonsFormatter(singletons: Set[SingletonData]): String = {
    taple("Singleton", "Role", "Location") + singletons
      .map(s => taple(s.name, s.role, s.location))
      .mkString
  }
    ...
    taple("Member", "Actor", "Latency") +
      actors
        .map(
          a =>
            a.actors
              .map(aa => taple(a.host, aa.name, aa.latency.toString))
              .mkString
        )
        .mkString
  }
