...
/**
  ...
  */
case class DMapActorInit(classType: Class[_ <: DMapActor],
                         name: String,
                         args: Any*)
...
object SettingsValue {
  ...
  implicit val stringSettingsValueExtractor =
    new SettingsValueExtractor[String] {
  ...
  implicit val booleanSettingsValueExtractor =
    new SettingsValueExtractor[Boolean] {
  ...
  implicit val stringSetSettingsValueExtractor =
    new SettingsValueExtractor[Set[String]] {
...
case class MapData(m: Map[String, SettingsValue],
                   timestamp: Long = DMapMessage.currentTimestamp)
    extends DMapMessage
case class HandShake(m: Map[String, SettingsValue], timestamp: Long)

case class SetValue(key: String,
                    value: SettingsValue,
                    timestamp: Long = DMapMessage.currentTimestamp)
    extends DMapMessage
case class AggregateValue(key: String,
                          value: String,
                          timestamp: Long = DMapMessage.currentTimestamp)
    extends DMapMessage
case class SubtractValue(key: String,
                         value: String,
                         timestamp: Long = DMapMessage.currentTimestamp)
    extends DMapMessage
...
trait DMapMaster extends DMapActor with LazyLogging {
  ...
  private[this] def slaves: Set[ActorSelection] =
    Grid.jvmsAll.map(jvm => Grid.selectActor(facade.salveName, jvm))
  ...
  override def receive: Receive = {
    ...
    case CurrentClusterState(
        ...
        leader,
        roleLeaderMap
        ) => //this.members = members.map(_.address)
