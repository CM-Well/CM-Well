...
object PersistentDMap extends DMapFacade {
  override def masterType: DMapActorInit =
    DMapActorInit(classOf[PersistentMaster], "PersistentMaster")

  override def slaveType: DMapActorInit =
    DMapActorInit(classOf[PersistentSlave], "PersistentSlave")
}
...
class PersistentSlave extends DMapSlave with LazyLogging {
  ...
  private val dataFile = new File(
    s"${Grid.persistentDmapDir}/${Config.clusterName}"
  )
