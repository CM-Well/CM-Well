...
class GridSpec extends FlatSpec with Matchers with BeforeAndAfterAll {
  ...
  def cleanup: Unit = {
    val filesToRemove =
      Seq("node-data", "client1-data", "client2-data", "sbt-tester-dmap").map(
  ...
  def killProcesses: Unit = {
    Seq("ps", "aux") #| Seq("grep", "TestService") #| Seq("awk", "{print $2}") #| Seq(
      "xargs",
      "kill",
      "-9"
    ) !
  ...
  "PersistentDMap" should "be persistent after jvm restarts" in {
    ...
    val m2 = Map(
      ...
      "previous-occupations" -> SettingsSet(
        Set("Pizza guy", "Astronaut", "Project Manager")
      )
    )
    ...
    Grid.serviceRef("DummyService") ! WriteToPersistentDMap(MapData(m2),
                                                            delay = 10.seconds)
    ...
    info(
      "Data that was written while this node is down should not be written to this node"
    )
    PersistentDMap.sm should equal(m1)
  ...
  "new service" should "be spawned on another jvm" in {
    ...
    Thread.sleep(60000)
    val newServiceJvm = LocalServiceManager
      .getServiceJvm("DummyService")
      .map(_.jvmName)
      .getOrElse("")
