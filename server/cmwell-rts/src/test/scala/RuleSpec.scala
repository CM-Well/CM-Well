...
//    println(r2.path == ObjectInfoton("/mark/test/1/2/").path ) // true
    ...
    true should equal(
      r1.path.check(ObjectInfoton("/mark/test", "dc_test").path)
    )
    true should equal(
      r1.path.check(ObjectInfoton("/mark/test/1", "dc_test").path)
    )
    false should equal(
      r1.path.check(ObjectInfoton("/mark/test/1/2/", "dc_test").path)
    )

    false should equal(r2.path.check(ObjectInfoton("/mark", "dc_test").path))
    true should equal(
      r2.path.check(ObjectInfoton("/mark/test", "dc_test").path)
    )
    true should equal(
      r2.path.check(ObjectInfoton("/mark/test/1", "dc_test").path)
    )
    true should equal(
      r2.path.check(ObjectInfoton("/mark/test/1/2/", "dc_test").path)
    )
  ...
  "match rule" should "be successful" in {
    ...
    val m2 = Rule(
      Map[String, Set[FieldValue]](
        "kids" -> Set(FString("gal"), FString("yoav"), FString("roni")),
        "name" -> Set(),
        "age" -> Set(FInt(34))
      )
    )
