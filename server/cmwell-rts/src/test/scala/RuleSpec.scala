...
class RuleSpec extends FlatSpec with Matchers {
  "path rule" should "be successful" in {
    ...
    false should equal (r1.path check ObjectInfoton("/mark","dc_test",protocol=None, lastModifiedBy = "").path)
    true should equal  (r1.path check ObjectInfoton("/mark/test","dc_test",protocol=None, lastModifiedBy = "").path)
    true should equal  (r1.path check ObjectInfoton("/mark/test/1","dc_test",protocol=None, lastModifiedBy = "").path)
    false should equal (r1.path check ObjectInfoton("/mark/test/1/2/","dc_test",protocol=None, lastModifiedBy = "").path)

    false should equal(r2.path check ObjectInfoton("/mark","dc_test",protocol=None, lastModifiedBy = "").path)
    true should equal (r2.path check  ObjectInfoton("/mark/test","dc_test",protocol=None, lastModifiedBy = "").path)
    true should equal (r2.path check  ObjectInfoton("/mark/test/1","dc_test",protocol=None, lastModifiedBy = "").path)
    true should equal(r2.path check ObjectInfoton("/mark/test/1/2/","dc_test",protocol=None, lastModifiedBy = "").path)
  }
