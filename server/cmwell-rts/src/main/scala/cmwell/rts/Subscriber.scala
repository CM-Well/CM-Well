...
class SubscriberPushActor( sendData : (Seq[String]) => Unit , bulkSize : Int = 100 ) extends Actor with LazyLogging {
  ...
  def receive = {
    ...
    case NotifyAddRule(publisherActorMap : Map[GridJvm , ActorSelection] , subscriber : String , rule : Rule) =>
      logger.info(s"NotifyAddRule($publisherActorMap : Map[Member , ActorSelection] , $subscriber : String , $rule : Rule)")
...
object PullDataContainer {
  def apply(overflowFlag : Boolean , data : Vector[String], format : FormatType) : PullDataContainer = new PullDataContainer(overflowFlag,data,format)
}

class PullDataContainer(val overflowFlag : Boolean , val data : Vector[String], val format : FormatType ) extends Serializable
...
object Subscriber {
  ...
  def init : Unit = {}
  def subscribe(subscriber : String , rule : Rule , transmitType: TransmitType ) = s.subscribe(subscriber,rule,transmitType)
