...
case class NotifyAddRule(publisherActorMap: Map[GridJvm, ActorSelection],
                         subscriber: String,
                         rule: Rule)
case class NotifyRemoveRule(subscriberActorMap: Map[GridJvm, ActorSelection],
                            subscriber: String)

case object PullData

class SubscriberPushActor(sendData: (Seq[String]) => Unit, bulkSize: Int = 100)
    extends Actor
    with LazyLogging {
  ...
  def receive = {
    ...
    case NotifyAddRule(
        publisherActorMap: Map[GridJvm, ActorSelection],
        subscriber: String,
        rule: Rule
        ) =>
      logger.info(
      ...
      publisherActorMap.foreach {
        case (m, a) => a ! AddSubscriber(subscriber, rule)
      }

    case NotifyRemoveRule(
        subscriberActorMap: Map[GridJvm, ActorSelection],
        subscriber: String
        ) =>
      logger.info(
        s"NotifyRemoveRule($subscriberActorMap : Map[Member , ActorSelection] , $subscriber : String)"
      )
      subscriberActorMap.foreach {
        case (m, a) => a ! RemoveSubscriber(subscriber)
      }
...
object PullDataContainer {
  def apply(overflowFlag: Boolean,
            data: Vector[String],
            format: FormatType): PullDataContainer =
    new PullDataContainer(overflowFlag, data, format)
}

class PullDataContainer(val overflowFlag: Boolean,
                        val data: Vector[String],
                        val format: FormatType)
    extends Serializable

class SubscriberPullActor(format: FormatType, bulkSize: Int = 100)
    extends Actor
    with LazyLogging {
  ...
  def receive = {
    ...
    case PublishOne(i: String) =>
      ...
      if (deltaTimestamp <= 1000 && count >= 100)
        // we need to call Unsubscribe.
        logger.info(
          s"call Unsubscribe delta [${deltaTimestamp}] count [${count}}]"
        )
      else {
        if (deltaTimestamp > 1000) {
          // create new timestamp && zero count
...
class SubscriberAgent extends Actor with LazyLogging {
  ...
  var publisherActorMap: Map[GridJvm, ActorSelection] =
    Map.empty[GridJvm, ActorSelection]
  // this the map of all subscribers

  var subscriberActorMap: Map[GridJvm, ActorSelection] =
    Map.empty[GridJvm, ActorSelection]

  var rulesMap: Map[String, Rule] = Map.empty[String, Rule]

  var localSubscriberActorMap: Map[String, ActorRef] =
...
object Subscriber {
  val subscriberAgentActor: ActorRef =
    Grid.create(classOf[SubscriberAgent], "subscriber")
...
class Subscriber(val subscriberAgent: ActorRef) extends LazyLogging {
  ...
  def subscribe(subscriber: String,
                rule: Rule,
                transmitType: TransmitType): Future[String] = {
    val futureSubscribe = subscriberAgent ? Subscribe(
      subscriber,
      rule,
      transmitType
    )
    futureSubscribe.map(
      d =>
        Base64.encodeBase64URLSafeString(
          d.asInstanceOf[String].getBytes(StandardCharsets.UTF_8)
      )
    )
  }
