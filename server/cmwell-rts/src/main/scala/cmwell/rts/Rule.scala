...
import cmwell.domain.{Infoton, FieldValue}
...
object Rule {
  ...
  def apply(path : String , recursive : Boolean ,  fields:Map[String, Set[FieldValue]]) = PMFilter(new Path(path , recursive) , new MatchMap(fields))
}
