...
class PublishAgent extends Actor with LazyLogging {
  ...
  def receive = {
    ...
    case AddSubscriber(subscriber: String, rule: Rule) =>
      ...
      val path =
        s"akka.tcp://${addr.system}@${addr.host.getOrElse("")}:${addr.port
          .getOrElse(0)}/user/$subscriber"
    ...
    case RemoveSubscriber(subscriber: String) =>
      ...
      subMap -= (subscriber)
      logger.debug(
        s"RemoveRule ${subscriber} rules [${rules}] sub map [${subMap}]"
      )
            ...
            rule match {
              ...
              case MatchFilter(f) =>
                if (i.fields.isDefined && f.check(i.fields.get))
                  publish_data(subscriber, i)
              case PMFilter(p, m) =>
                if (p.check(i.path) && i.fields.isDefined && m.check(
                      i.fields.get
                    ))
                  publish_data(subscriber, i)
            }
...
object Publisher {
  val publishAgentActor: ActorRef =
    Grid.create(classOf[PublishAgent], "publisher")
