...
object DownloaderUtils {
  ...
  def createIterator(baseUrl: String,
    ...
    /**
      ...
      */
    class DownloaderIterator(token: String, downloader: Downloader)
      ...
      override def next(): String = {
        val tokenAndDataFuture = downloader
          ...
          .map {
            case (token, data) => TokenAndData(token = token, data = data)
          }
  ...
  def createInputStream(baseUrl: String,
    ...
    dataSource
      ...
      .runWith(
        StreamConverters
          .asInputStream(FiniteDuration(timeout, TimeUnit.MILLISECONDS))
      )
  }
}
