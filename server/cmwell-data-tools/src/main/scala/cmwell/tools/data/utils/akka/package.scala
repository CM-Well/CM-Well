...
package object akka extends DataToolsConfig {
  ...
  def balancer[In, Out](worker: Flow[In, Out, Any],
                        workerCount: Int): Flow[In, Out, _] = {
    import GraphDSL.Implicits._

    Flow.fromGraph(GraphDSL.create() { implicit b =>
      val balancer =
        b.add(Balance[In](workerCount, waitForAllDownstreams = false))
  ...
  def concatByteStrings(lines: Seq[ByteString],
                        start: String,
                        sep: String,
                        end: String): ByteString = {
    concatByteStrings(
      lines,
      ByteString(start),
      ByteString(sep),
  ...
  implicit class ByteStringUtils(b: ByteString) {
    ...
    def split(delimiter: Byte) = {
      @tailrec
      def splitByteString(bytes: ByteString,
                          delimiter: Byte,
                          acc: Seq[ByteString]): Seq[ByteString] = {
        bytes.splitAt(bytes.indexOf(delimiter)) match {
          case (split, rest) if split.isEmpty => acc :+ rest
          case (split, rest) =>
            splitByteString(rest.tail, delimiter, acc :+ split)
        }
      }
