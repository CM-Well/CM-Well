...
object Downloader extends DataToolsLogging with DataToolsConfig {
  ...
  case class TsvData(path: Path, uuid: Uuid, lastModified: ByteString, indexTime: ByteString = ByteString.empty)
      extends Tsv {
    override def toByteString: ByteString = concatByteStrings(Seq(path, lastModified, uuid, indexTime), "", "\t", "")
  }
...
class Downloader(baseUrl: String,
                 ...
                 isBulk: Boolean = false,
                 indexTime: Long = 0L,
                 override val label: Option[String] = None)(implicit system: ActorSystem, mat: Materializer)
              ...
              DataPostProcessor.postProcessByFormat(format, unzippedData).runFold(blank)(_ ++ _).map { receivedData =>
                ...
                getMissingPaths(receivedData, sentPaths.map(_.utf8String)) match {
                  ...
                  case missingPaths if state.retriesLeft > 0 =>
                    ...
                    val newState = state.copy(pathsToRequest = missingPaths.map(ByteString.apply),
                  ...
                  case missingPaths =>
                    ...
                    val newState =
                      state.copy(pathsToRequest = missingPaths.map(ByteString.apply), retrievedData = totalReceivedData)
              ...
              DataPostProcessor.postProcessByFormat(format, unzippedData).runFold(blank)(_ ++ _).map { receivedData =>
                ...
                getMissingUuids(receivedData, sentUuids.map(_.utf8String)) match {
                  ...
                  case missingUuids if state.retriesLeft > 0 =>
                    ...
                    // retry retrieving data from missing uuids
                    val newState = state.copy(uuidsToRequest = missingUuids.map(ByteString.apply),
                                              retrievedData = totalReceivedData,
                                              retriesLeft = state.retriesLeft - 1)
                  ...
                  case missingUuids =>
                    ...
                    val newState =
                      state.copy(uuidsToRequest = missingUuids.map(ByteString.apply), retrievedData = totalReceivedData)
