...
object Downloader extends DataToolsLogging with DataToolsConfig {
  ...
  def getToken(baseUrl: String,
    ...
    val uri =
      s"${formatHost(baseUrl)}$path?op=create-consumer$qpValue$paramsValue$recursiveValue&index-time=$indexTime"
  ...
  def getChunkData(
    ...
    tokenFuture.flatMap { tokenValue =>
      downloader.getChunkData(tokenValue)
    }
  }
          ...
          if (nextToken == tokenValue) {
            ...
            src
              ...
              .fold("" -> Seq.empty[ByteString]) {
                case ((oldToken, paths), (newToken, path)) =>
                  newToken -> (paths :+ path)
              }
              .mapConcat {
                case (token, paths) =>
                  paths
                    .grouped(numUuidsPerRequest)
                    .map(path => (token, path))
                    .toStream
              }
...
class Downloader(
  ...
  private[Downloader] val numDataFetchRetries =
    config.getInt("cmwell.downloader.consumer.data-fetch-retries")

  private val HttpAddress(protocol, host, port, uriPrefix) =
  ...
  private[data] def downloadDataFromPaths()(implicit ec: ExecutionContext) = {
    ...
    def getMissingPaths(receivedData: ByteString, paths: Seq[String]) = {
      def extractPathNtriples(data: ByteString) =
        data.utf8String
          .split("\n")
      ...
      format match {
        case "json" =>
          paths.diff(extractPathJson(receivedData)) //extractMissingPathsJson(receivedData)
        case "ntriples" | "nquads" =>
          paths.diff(extractPathNtriples(receivedData))
    ...
    def sendPathRequest(timeout: FiniteDuration,
      ...
      def retryWith(
        state: State
      ): Option[immutable.Iterable[(Seq[Path], State)]] =
        state match {
        ...
        .mapAsyncUnordered(parallelism) {
          case (Success(res @ HttpResponse(s, h, e, p)), sentPaths, Some(state))
            ...
            e.toStrict(1.minute).flatMap { strict =>
              val unzippedData =
                Source.single(strict.data).via(Compression.gunzip())

              DataPostProcessor
                .postProcessByFormat(format, unzippedData)
                .runFold(blank)(_ ++ _)
                .map { receivedData =>
                  ...
                  getMissingPaths(receivedData, sentPaths.map(_.utf8String)) match {
                    case Seq() =>
                      ...
                      Success(totalReceivedData) -> state.copy(
                        pathsToRequest = Seq(),
                    ...
                    case missingPaths if state.retriesLeft > 0 =>
                      ...
                      logger.debug(
                        s"sent ${sentPaths.map(_.utf8String).mkString("\n")}"
                      )
                      ...
                      logger.error(
                        s"detected missing paths size=${missingPaths.size} retries left=${state.retriesLeft}"
                      )
          ...
          case (Success(HttpResponse(s, h, e, p)), sentPaths, Some(state)) =>
            ...
            Future.successful(
              Failure(new Exception("cannot send request to send paths")) -> state
            )
      ...
      Flow[(Seq[Path], Token)]
        .map {
          case (paths, token) =>
            paths -> State(paths, token, Seq.empty[ByteString], limit)
        }
        .via(GoodRetry.concat(Long.MaxValue, job)(retryWith))
        .collect {
          case (Success(receivedData), state) => receivedData -> state.token
        }
    }
    ...
    Flow[(Token, Path)]
      ...
      .mapConcat {
        _.groupBy { case (token, _) => token }
          .map {
            case (token, tokenAndPath) =>
              tokenAndPath.map { case (token, path) => path } -> token
          }
      }
      .via(
        sendPathRequest(retryTimeout, bufferSize, numDataFetchRetries)(
          createDataRequest
        )
      )
      .flatMapConcat {
        case (responseBytes, token) =>
          Source.fromIterator(() => responseBytes.map(token -> _).iterator)
      }
  }
  ...
  private[data] def downloadDataFromUuids()(implicit ec: ExecutionContext) = {
    ...
    def getMissingUuids(receivedData: ByteString, uuids: Seq[String]) = {
      ...
      format match {
        ...
        case "ntriples" | "nquads" =>
          uuids.diff(extractUuidNtriples(receivedData))
    ...
    def sendUuidRequest(timeout: FiniteDuration,
      ...
      def retryWith(
        state: State
      ): Option[immutable.Iterable[(Seq[Uuid], State)]] =
        state match {
          case State(uuidsToRequest, _, _, _) =>
            Some(immutable.Seq(uuidsToRequest -> state))
        }
                ...
                .map { receivedData =>
                  ...
                  getMissingUuids(receivedData, sentUuids.map(_.utf8String)) match {
                    ...
                    case missingUuids if state.retriesLeft > 0 =>
                      ...
                      logger.debug(
                        s"sent ${sentUuids.map(_.utf8String).mkString("/ii/", "\n/ii/", "\n")}"
                      )
                      ...
                      logger.error(
                        s"detected missing uuids size=${missingUuids.size} retries left=${state.retriesLeft}"
                      )
      ...
      Flow[(Seq[Uuid], Token)]
        .map {
          case (uuids, token) =>
            uuids -> State(uuids, token, Seq.empty[ByteString], limit)
        }
        .via(GoodRetry.concat(Long.MaxValue, job)(retryWith))
        .collect {
          case (Success(receivedData), state) => receivedData -> state.token
    ...
    Flow[(Token, Uuid)]
      ...
      .mapConcat {
        _.groupBy { case (token, _) => token }
          .map {
            case (token, tokenAndUuid) =>
              tokenAndUuid.map { case (token, uuid) => uuid } -> token
          }
      }
      .via(
        sendUuidRequest(retryTimeout, bufferSize, numDataFetchRetries)(
          createDataRequest
        )
      )
      .flatMapConcat {
  ...
  /**
    ...
    */
  private[consumer] def getTsvFromToken(
    token: String
  )(implicit ec: ExecutionContext) = {
    // create HTTP request from token
    val consumeHandler = if (isBulk) "_bulk-consume" else "_consume"
    val req = HttpRequest(
      uri = s"${formatHost(baseUrl)}/$consumeHandler?position=$token&format=tsv"
    )
      ...
      case (nextToken, src) =>
        if (nextToken == token) {
          ...
          src
            ...
            .fold("" -> Seq.empty[ByteString]) {
              case ((_, buffer), (token, line)) =>
                token -> (buffer :+ line :+ endl)
            }
            .map {
              case (token, data) =>
                token -> concatByteStrings(data, endl).utf8String
            }
            .runWith(Sink.head)
        }
    }
