...
//      Future { blocking { fillBuffer(initToken) } }
      ...
      private def fillBuffer(token: String): Future[Unit] = {
        var expectedNumInfotons: Option[Long] = None
        val receivedUuids = scala.collection.mutable.Set.empty[ByteString]
...
//          consumerStatsActor ! NewToken(token)
          ...
          val source = Source.single(token)
            ...
            .map {
              ...
              case (Success(HttpResponse(s, h , e, _)), _) if s == StatusCodes.NoContent =>
                ...
                if (updateFreq.isEmpty) buffer.put(None)
              ...
              case (Success(HttpResponse(s, h, e, _)), _) if s == StatusCodes.OK || s == StatusCodes.PartialContent =>
                ...
                logger.debug(s"expected num uuids = ${expectedNumInfotons.getOrElse("None")} token=$token")

                val dataSource = e.withoutSizeLimit().dataBytes
                  .via(lineSeparatorFrame)
                  .map(extractTsv)
                  .map(nextToken -> _)
            ...
            .alsoToMat(Sink.last)((_,element) => element.map { case (token, _) => token})
            .map { case (token, dataSource) => dataSource}
            .flatMapConcat(identity)
            .map { case (token, tsv) =>
              // if uuid was not emitted before, write it to buffer
              if (receivedUuids.add(tsv.uuid)) {
                consumerStatsActor ! ConsumeEvent
                while (!buffer.offer(Some(token -> tsv))) {} // write to buffer when a free space is available
              }
              token }

          val result = source.toMat(Sink.ignore) { (token, done) => done.flatMap(_ => token) }
            .run()
            .map { case nextToken =>
          ...
          result.onSuccess{ case _ =>
            ...
            logger.debug(s"successfully consumed token: $token point in time: ${decoded.getOrElse("")} buffer-size: ${buffer.size}")
...
//            consumerStatsActor ! Reset
            ...
            after(retryTimeout, system.scheduler)(sendNextChunkRequest(token)) }
        }

        // fill buffer if it is almost empty
        if (buffer.size < prefetchBufferSize * 0.3) {
          sendNextChunkRequest(token).flatMap {
            case Some(nextToken) =>
              fillBuffer(nextToken)
            case None if updateFreq.nonEmpty =>
              logger.info("no more data is available, will check again in {}", updateFreq.get)
              after(updateFreq.get, system.scheduler)( fillBuffer(token) )
            case None =>
              Future.successful(Unit)
          }
        } else {
          logger.info("buffer-size={}", buffer.size)
          after(1.second, system.scheduler)( fillBuffer(token) )
        }
      }
