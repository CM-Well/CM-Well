...
object GroupChunker {
  ...
  /**
    ...
    */
  def extractSubject(data: ByteString): ByteString =
    data.splitAt(data.indexOf(' '))._1
  ...
  def apply(groupExtractor: ByteString => ByteString,
            within: FiniteDuration = 3.seconds) =
    new GroupChunker(groupExtractor, within)
}

class GroupChunker(extractGroup: ByteString => ByteString,
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic =
    new TimerGraphStageLogic(shape) with InHandler with OutHandler {
      ...
      def isNeedToCloseGroup(newElement: ByteString): Boolean =
        extractGroup(newElement) != lastGroup
