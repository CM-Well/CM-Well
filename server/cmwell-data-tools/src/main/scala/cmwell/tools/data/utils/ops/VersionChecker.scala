...
//  printIfNeedUpdate()
  ...
  private def getVersionFromRemote() = {
    ...
    Source
      .single(
        HttpRequest(
          uri = s"http://$remoteVersionHost$remoteVersionPath?format=ntriples"
        ) -> None
      )
      .via(conn)
      .mapAsync(1) {
  ...
  private def printIfNeedUpdate() = if (!wasChecked) {
    wasChecked = true
    val currentVersion =
      System.getProperty("prog.version", ops.getVersionFromManifest()) match {
        case null                                                  => None
      ...
      case Success(Some(data)) =>
        Some(
          data.utf8String.lines
            ...
            .map(
              line => line.substring(line.lastIndexOf('>') + 3, line.size - 3)
            )
            .mkString
        )
      case Success(None) =>
