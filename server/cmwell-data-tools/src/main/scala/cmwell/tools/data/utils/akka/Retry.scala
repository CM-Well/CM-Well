...
object Retry extends DataToolsLogging with DataToolsConfig{
  ...
  def retryHttp[T](delay: FiniteDuration,
    ...
    case class State(data: Seq[ByteString], context: Option[T] = None, response: Option[HttpResponse] = None, count: Option[Int] = limit)

    def stringifyData(data: Seq[ByteString]) = concatByteStrings(data, ByteString(",")).utf8String

    def retryWith(state: State): Option[immutable.Iterable[(Future[Seq[ByteString]], State)]] = state match {
      case State(data, _, Some(HttpResponse(s, h, e, _)), _) if s == StatusCodes.TooManyRequests =>
        // api garden quota error
        e.toStrict(toStrictTimeout).map { strictEntity =>
          logger.error(s"$labelValue retry data: api garden mis-configured, call Yaniv to increase! host=${getHostname(h)} status=$s entity=${strictEntity.data.utf8String} data=${concatByteStrings(data, endl).utf8String}")
        }.onFailure{ case err => logger.error(s"$labelValue api garden mis-configured, call Yaniv to increase! host=${getHostname(h)} status=$s cannot read entity", err) }
...
//        e.discardBytes()
      ...
      case State(data, _, Some(HttpResponse(s: ServerError, h, e, _)), _) =>
        ...
        if (e.toString contains "Fetching") {
          logger.warn(s"$labelValue will not schedule a retry on data ${concatByteStrings(data, endl).utf8String}")
          e.discardBytes()
          None
        } else {
          e.discardBytes()
          ...
          // schedule a retry to http stream
          logger.warn(s"$labelValue server error: will retry again in $delay host=${getHostname(h)} status=$s entity=$e")
          val future = after(delay, system.scheduler)(Future.successful(data))
          Some(immutable.Seq(future -> state))
        }
      ...
      case State(data, context, Some(HttpResponse(s: ClientError, h, e, _)), _) =>
        ...
        if (data.size > 1) {
          ...
          e.withoutSizeLimit().dataBytes.runFold(blank)(_ ++ _)
            .map { entityBytes =>
              logger.warn(s"$labelValue client error: will retry again in $delay to send a single request, host=${getHostnameValue(h)} status=$s, entity=${entityBytes.utf8String}, request data=${stringifyData(data)}")
              redLogger.error(s"$labelValue client error: host=${getHostnameValue(h)} status=$s data=${stringifyData(data)} entity=${entityBytes.utf8String}")
              badDataLogger.info(s"$labelValue data=${concatByteStrings(data, endl).utf8String}")
            }

          // failed to send a chunk of data, split to singles and retry
          Some(
            data.map(dataElement => Future.successful(Seq(dataElement)) -> State(Seq(dataElement), context))
              .to[immutable.Iterable]
          )
        } else {
          e.toStrict(toStrictTimeout).map { strictEntity =>
            logger.warn(s"$labelValue client error: will not retry sending request, host=${getHostnameValue(h)} status=$s, entity=${strictEntity.data.utf8String}")
            badDataLogger.info(s"$labelValue data=${concatByteStrings(data, endl).utf8String}")
          }.onFailure{ case err => logger.warn(s"$labelValue client error: will retry again in $delay to send a single request, host=${getHostnameValue(h)} status=$s, cannot read entity, request data=${stringifyData(data)}", err) }
      ...
      case State(data, _, Some(HttpResponse(s, h, e, _)), _) if s.isSuccess() =>
        // content error
        logger.warn(s"$labelValue received $s but response body is not valid, will retry again in $delay host=${getHostnameValue(h)} data=${stringifyData(data)}")
        val future = after(delay, system.scheduler)(Future.successful(data))
        Some(immutable.Seq(future -> state))

      case State(data, _, Some(HttpResponse(s, h, e, _)), count) =>
        redLogger.error(s"$labelValue error: host=${getHostnameValue(h)} status=$s entity=$e data=${stringifyData(data)}")
        logger.warn(s"$labelValue error: host=${getHostnameValue(h)} status=$s data=${stringifyData(data)}")

        count match {
          case Some(c) if c > 0 =>
            e.discardBytes()
            logger.warn(s"$labelValue received $s, count=$count will retry again in $delay host=${getHostnameValue(h)} data=${stringifyData(data)}")
            val future = after(delay, system.scheduler)(Future.successful(data))
            Some(immutable.Seq(future -> state.copy(count = Some(c - 1))))
          case Some(0) =>
            logger.warn(s"$labelValue received $s, count=$count will not not retry sending request, host=${getHostnameValue(h)} data=${stringifyData(data)}")
            badDataLogger.info(s"$labelValue data=${concatByteStrings(data, endl).utf8String}")
            None
          case None =>
            e.discardBytes()
            logger.warn(s"$labelValue received $s, will retry again in $delay host=${getHostnameValue(h)} data=${stringifyData(data)}")
            val future = after(delay, system.scheduler)(Future.successful(data))
            Some(immutable.Seq(future -> state))
        }
      ...
      case State(data, _, None, count) =>
        count match {
          case Some(c) if c > 0 =>
            ...
            Some(immutable.Seq(future -> state.copy(count = Some(c - 1))))
          case Some(0) =>
            logger.warn(s"$labelValue error: could not send http request, counter=0, will not retry sending request, data=${stringifyData(data)}")
            badDataLogger.info(s"$labelValue data=${concatByteStrings(data, endl).utf8String}")
            None
          case None =>
            logger.warn(s"$labelValue error: could not send http request, will retry again in $delay data=${stringifyData(data)}")
            val future = after(delay, system.scheduler)(Future.successful(data))
            Some(immutable.Seq(future -> state))
        }
      case x =>
...
//    val conn = Http().newHostConnectionPool[State](host = baseUrl, port = port) // http connection flow
    ...
    val conn = HttpConnections.newHostConnectionPool[State](host, port, protocol) // http connection flow
    ...
    val job = Flow[(Future[Seq[ByteString]], State)]
      ...
      .mapAsyncUnordered(httpParallelism) {
          ...
          val responseAndState = e.withoutSizeLimit().dataBytes.runFold(blank)(_ ++ _)
            .map { dataBytes =>
              ...
              if (isValidBody) {
                ...
                Failure(new Exception("response body is not valid")) -> state.copy(response = response.toOption)
              }
            }

          responseAndState.recover { case err =>
            logger.error(s"$labelValue error in consuming data bytes from response, paths=${stringifyData(state.data)}, err=$err", err)
        ...
        case (x, state) => logger.error(s"$labelValue unexpected message: $x")
          Future.successful(Failure(new Exception("unexpected message")) -> state)
      }

    Flow[(Seq[ByteString], Option[T])]
      .map{case (data, context) => Future.successful(data) -> State(data = data, context = context) }
