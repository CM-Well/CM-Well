    ...
    def retryWith(state: State): Option[immutable.Iterable[(Future[Seq[ByteString]], State)]] = state match {
      case State(data, _, Some(HttpResponse(s, h, e, _)), _) if s == StatusCodes.TooManyRequests =>
        ...
        e.toStrict(toStrictTimeout)
          .map { strictEntity =>
            logger
              .error(s"$labelValue retry data: api garden mis-configured, call Yaniv to increase! host=${getHostname(
                h
...
//        e.discardBytes()
      ...
      case State(data, context, Some(HttpResponse(s: ClientError, h, e, _)), _) =>
        ...
        if (data.size > 1) {
          ...
          Some(
            data
              .map(dataElement => Future.successful(Seq(dataElement)) -> State(Seq(dataElement), context))
          ...
          e.toStrict(toStrictTimeout)
            .map { strictEntity =>
              logger.warn(
                s"$labelValue client error: will not retry sending request, host=${getHostnameValue(h)} status=$s, entity=${strictEntity.data.utf8String}"
              )
              badDataLogger.info(s"$labelValue data=${concatByteStrings(data, endl).utf8String}")
            }
            .onFailure {
