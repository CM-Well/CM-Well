...
//    wireMockServer.shutdown()
  ...
  def createTriplesFile(numInfotons: Integer,
                        numTriples: Integer,
                        filename: String,
...
//    val writer = new PrintWriter(testFile)
    ...
    val writer =
      if (zipped) new PrintWriter(new GZIPOutputStream(file))
      else new PrintWriter(file)
  ...
  def createTriplesStream(numInfotons: Integer, numTriples: Integer) = {
    ...
    Iterator
      ...
      .flatMap(
        i =>
          (1 to numTriples)
            .map(t => s"${subject(i)} ${predicate(t)} ${obj(t)}\n")
      )
  }
  ...
  "Ingester" should "ingest all blocks without retries" in {
    ...
    result.flatMap { _ =>
      wireMockServer
        .findAll(postRequestedFor(urlPathMatching("/_in")))
        .size should be(2)
    }
  }
  ...
  it should "split data into single infotons (client error) and succeed ingest" in {
    ...
    result.flatMap { _ =>
      wireMockServer
        .findAll(postRequestedFor(urlPathMatching("/_in")))
        .size should be(numInfotonsToCreate + 1)
    }
  }
  ...
  it should "ingest first block and retry second block (client error)" in {
    ...
    val result = Ingester
      .fromInputStream(baseUrl = s"$host:${wireMockServer.port}",
                       "ntriples",
                       in = in)
      .runWith(Sink.ignore)
    ...
    result.flatMap { _ =>
      val requestsReceived =
        wireMockServer.findAll(postRequestedFor(urlPathEqualTo("/_in")))
  ...
  it should "split packet to single infotons (client error), do retries and fail ingest" in {
    ...
    result.flatMap { _ =>
      wireMockServer
        .findAll(postRequestedFor(urlPathMatching("/_in")))
        .size should be(numInfotonsToIngest + 1)
    }
  }
}
