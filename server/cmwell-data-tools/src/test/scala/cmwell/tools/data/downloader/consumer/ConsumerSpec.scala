...
class ConsumerSpec extends BaseWiremockSpec {
  ...
  it should "download all uuids while getting server error" in {
    ...
    val tsvsBeforeError =
      List("path1\tlastModified1\tuuid1\tindexTime1\n",
           "path2\tlastModified2\tuuid2\tindexTime2\n")
    val tsvsAfterError =
      List("path3\tlastModified3\tuuid3\tindexTime3\n",
    ...
    result
      ...
      .flatMap { _ =>
        val numRequestsToConsume = wireMockServer
          .findAll(getRequestedFor(urlPathMatching("/_consume")))
          .size
