...
.controller('infotonsController', function($scope, $routeParams, $location, infotonService, $http) {
            ...
            $('#app-container').on('click', '.add-user-button', function() {
                var form = $('.add-user-form')
                    ...
                    , fetchPassword = function() {
                        form.find('#password').html('<img src="/meta/app/images/loading2.gif"/>');
        ...
        $.getJSON(src+'?format=jsonl&override-mimetype=text/plain%3Bcharset=utf-8').then(function (fileInfoton) {
            ...
            if(isTextual && !isImage) {
                ...
                if(isImage) {
                    ...
                    addMarkup('<a href="'+fileInfoton.path+'" target="_blank"><img style="margin:6px;" width="20" src="/meta/app/images/download-icon.svg"/>Download</a>');
                }
            }
        });
    }
    ...
    $('.analytical-menu').on('change', function() {
        ...
        var req = getUrl() + '?format=json&op=aggregate&ap='+ap+qp
            ...
            , dialog = $('<div><img class="loading-image" title="loading..." src="/meta/app/images/loading.gif"/></div>').appendTo('body').dialog({ position: { my: "top+42", at: "top" }, show: {effect: 'fade'}, hide: { effect: 'fade' }, width: '50%', close: function() { dialog.remove(); }});
...
.config(function($routeProvider, $locationProvider, $httpProvider) {
    var routeObj = {
       templateUrl: '/meta/app/views/main.html?override-mimetype=text/html',
       controller: 'infotonsController'
    };

    var standardViewWithCtrlr = function(name) {
        return {
            templateUrl: '/meta/app/views/'+name+'.html?override-mimetype=text/html',
    ...
    $routeProvider
        ...
        .when('/_vld/', {templateUrl: '/meta/app/views/vld.html?override-mimetype=text/html' })
        .when('/_vld/:infotonPath*', {templateUrl: '/meta/app/views/vld.html?override-mimetype=text/html' })
        .when('/dashboard', {templateUrl: '/meta/app/views/dashboard.html?override-mimetype=text/html', controller: 'dashboardController' }) // might as well be standardViewWithCtrlr('dashboard')
