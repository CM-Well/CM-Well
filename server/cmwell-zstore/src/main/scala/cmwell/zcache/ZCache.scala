...
class ZCache(zStore: ZStore) {
  ...
  ): K => Future[V] = { (input: K) =>
    {
      ...
      get(key)(deserializer, pollingMaxRetries, pollingInterval)(ec).flatMap {
        ...
        case None => {
          ...
          task(input).flatMap {
            case result if isCachable(result) =>
              put(key, result)(serializer, ttlSeconds)(ec).map(_ => result)
  ...
  def get[T](key: String)(
    ...
    def isActualValue(valueOpt: Option[Array[Byte]]) =
      !valueOpt.exists(_.sameElements(Pending.payload))
    unsafeRetryUntil(isActualValue, pollingMaxRetries, pollingInterval.seconds)(
      zStore.getOpt(key, dontRetry = true)
    ).map {
  ...
  def put(key: String,
          state: State)(ttl: Int)(implicit ec: ExecutionContext): Future[Unit] =
    zStore.put(key, state.payload, secondsToLive = ttl, false)

  def put[T](key: String, value: T)(serializer: T => Array[Byte], ttl: Int)(
    implicit ec: ExecutionContext
  ): Future[T] =
    zStore
      .put(key, serializer(value), secondsToLive = ttl, false)
      .map(_ => value)
