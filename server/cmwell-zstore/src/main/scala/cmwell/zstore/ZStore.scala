...
trait ZStore {
  def put(uzid: String,
          value: Array[Byte],
          batched: Boolean = false): Future[Unit]
  def put(
    uzid: String,
    value: Array[Byte],
    secondsToLive: Int,
    batched: Boolean
  ): Future[Unit] // not using FiniteDuration, since CAS only supports INT values for TTL
  def get(uzid: String): Future[Array[Byte]]
  def get(uzid: String, dontRetry: Boolean): Future[Array[Byte]]
  def getOpt(uzid: String,
             dontRetry: Boolean = false): Future[Option[Array[Byte]]]
  def putString(uzid: String,
                value: String,
                batched: Boolean = false): Future[Unit] = {
    put(uzid, value.getBytes("utf-8"), batched)
  }
  def putLong(uzid: String,
              value: Long,
              batched: Boolean = false): Future[Unit] =
  ...
  def getStringOpt(uzid: String): Future[Option[String]] = getOpt(uzid).map {
    _.map { new String(_) }
  }
  ...
  def getLongOpt(uzid: String): Future[Option[Long]] = getOpt(uzid, true).map {
    bytesOpt =>
      bytesOpt.map { ByteBuffer.wrap(_).getLong }
  }

  def getIntOpt(uzid: String): Future[Option[Int]] = getOpt(uzid, true).map {
    bytesOpt =>
      bytesOpt.map { ByteBuffer.wrap(_).getInt }
  }

  def getBooleanOpt(uzid: String): Future[Option[Boolean]] =
    getOpt(uzid, true).map { bytesOpt =>
      bytesOpt.map { bytes =>
...
class ZStoreImpl(dao: Dao) extends ZStore with DaoExecution with LazyLogging {
  ...
  override def put(uzid: String,
                   value: Array[Byte],
                   batched: Boolean): Future[Unit] =
  ...
  override def put(uzid: String,
    ...
    def serialize(obj: ZStoreObj): Vector[ZStoreRow] = {
      ...
      val bytesRow =
        ZStoreRow(obj.uzid, "bytes", toLongBytes(obj.value.length.toLong))
      val indexLength =
        (((obj.value.length - 1) / chunkSize) + 1).toString.length
      val dataRows = obj.value.grouped(chunkSize).zipWithIndex.map {
        case (chunk, index) =>
          ZStoreRow(obj.uzid, s"data%0${indexLength}d".format(index), chunk)
      }
    ...
    def rowToStatement(row: ZStoreRow) =
      putPStmt
        .bind(row.uzid, row.field, ByteBuffer.wrap(row.chunk), ttl)
    ...
    if (batched) {
      ...
      rows.foreach { row =>
        batch.add(
          putPStmt.bind(row.uzid, row.field, ByteBuffer.wrap(row.chunk), ttl)
        )
      }
      execWithRetry(batch.setConsistencyLevel(ConsistencyLevel.QUORUM))
        .map(_ => ())
    } else
      cmwell.util.concurrent
        .travector(rows)((rowToStatement _).andThen(execWithRetry))
        .map(_ => ())
  }

  override def get(uzid: String): Future[Array[Byte]] = {
    val getStmtOne =
      getPStmt.bind(uzid).setConsistencyLevel(ConsistencyLevel.ONE)
    val getStmtQuorum =
      getPStmt.bind(uzid).setConsistencyLevel(ConsistencyLevel.QUORUM)
    ...
    def fetchAndDeserialize(stmt: Statement) =
      retry(
        exec(stmt)
          .map(
            (deserialize(uzid) _)
              .andThen(_.fold(throw new NoSuchElementException)(_.value))
          )
      )
    ...
    fetchAndDeserialize(getStmtOne).andThen {
      ...
      case Failure(e) =>
        logger.warn(
          s"[zStore] Reading uzid $uzid with cl=ONE threw $e. Trying cl=QUORUM..."
        )
        fetchAndDeserialize(getStmtQuorum).andThen {
          case Success(value) => value
          case Failure(e1) =>
            logger.warn(
              s"[zStore] Reading uzid $uzid with cl=QUORUM threw $e1. Trying last time with cl=ONE..."
            )
  ...
  override def getOpt(uzid: String,
                      dontRetry: Boolean): Future[Option[Array[Byte]]] = {
    require(dontRetry, "getOpt only works in dontRetry mode") // might be implemented, but YAGNI
    val stmt = getPStmt.bind(uzid).setConsistencyLevel(ConsistencyLevel.ONE)
    exec(stmt)
      .map(deserialize(uzid)(_))
      .map(_.map(_.value)) // not throwing an exception, but returning an Option
  }

  override def remove(uzid: String): Future[Unit] = {
    val delStmt =
      delPStmt.bind(uzid).setConsistencyLevel(ConsistencyLevel.QUORUM)
  ...
  private def execWithRetry(stmt: Statement) =
    cmwell.util.concurrent.retry(8, 50.millis, 2)(exec(stmt))

  private def exec(stmt: Statement) = executeAsyncInternal(stmt)

  private def retry[T](task: Future[T]) =
    cmwell.util.concurrent.retry(3, 50.millis, 2)(task)
  ...
  private val putPStmt = prepare(
    "INSERT   INTO data2.zstore (uzid,field,value) VALUES(?,?,?) USING TTL ?"
  ...
  private def deserialize(
    uzid: String
  )(result: ResultSet): Option[ZStoreObj] = {
    if (result.isExhausted) None
    else {
      val adlerRow = result.one()
      require(
        !result.isExhausted,
        "cassandra ResultSet exhausted after adler row"
      )
      val adler =
        ByteBuffer.wrap(Bytes.getArray(adlerRow.getBytes("value"))).getInt
      val bytesRow = result.one()
      val bytes =
        ByteBuffer.wrap(Bytes.getArray(bytesRow.getBytes("value"))).getLong
      if (result.isExhausted) {
        require(
          bytes == 0 && adler == ZStoreImpl.zeroAdler,
...
class ZStoreMem extends ZStore {
  ...
  override def put(uzid: String,
                   value: Array[Byte],
                   secondsToLive: Int,
                   batched: Boolean): Future[Unit] = Future.successful {
    store += uzid -> value
