...
class BinRangeSearchTests
  ...
  describe("binRangeSearch should") {
    val bigGap: Future[Assertion] = {
      ...
      binRangeSearch[Long](83929378534L,
                           1425817765688L,
                           1508943374048L,
                           1000000L,
                           0.5,
    ...
    val earlyCutOffWithTimeToShrink: (Future[Assertion], Future[Assertion]) = {
      ...
      val f0 = expandRange[Long](83929378534L,
                                 1425817765688L,
                                 1508943374048L,
                                 500000,
                                 1500000,
                                 timeout.future)(searchFunction)
      ...
      val f2 = f0.flatMap {
        case Right(_) =>
          Future.successful(fail("Future result should be Left (was Right)"))
        case Left((timePosition, step, nextToOptimization)) =>
          shrinkingStepBinarySearch(83929378534L,
                                    timePosition,
                                    step,
                                    nextToOptimization,
                                    500000,
    ...
    it("handle initial big gaps properly (adapted from actual recorded case)")(
      bigGap
    )
    it("handle initial big gaps properly with nextTo optimization")(
      bigGapNextTo
    )
