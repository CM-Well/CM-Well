...
class ProcessStopWatch(id: String) {
  ...
  def combineTaskList: mutable.Buffer[TaskInfo] = {
    val m: mutable.Map[String, TaskInfo] =
      new mutable.HashMap[String, TaskInfo]()
    taskList.foreach { task =>
      m.get(task.getTaskName) match {
        ...
        case None =>
          m.put(
            task.getTaskName,
            new TaskInfo(task.getTaskName, task.getTimeMillis)
          )
      }
    }
    var taskListTmp: mutable.Buffer[TaskInfo] =
      new mutable.ListBuffer[TaskInfo]()
    m.foreach {
  ...
  def show: String = {
    ...
    taskListTmp.foreach { task =>
      ...
      var percent: Long =
        Math.round((100.0 * task.getTimeSeconds) / getTotalTimeSeconds);
