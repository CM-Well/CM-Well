 ...
 *
 */
class ChildFirstURLClassLoader(urls: Array[URL], parent: ClassLoader, except: Seq[String] = Seq()) extends URLClassLoader(urls, parent) {
  ...
  override def getResource(name: String): URL = findResource(name) match { case null => super.getResource(name) case u => u }

  override def getResources(name: String): java.util.Enumeration[URL] = {
    val parent = getParent
    val localUrls = findResources(name)
    val parentUrls: java.util.Enumeration[URL] = if(parent!=null) parent.getResources(name) else java.util.Collections.emptyEnumeration()
  ...
  override def getResourceAsStream(name: String): InputStream = {
    getResource(name) match {
      case null => null
      case url => Try(url.openStream) match {
        case Success(x) => x
        case Failure(_) => null
      }
    }
...
object  ChildFirstURLClassLoader {
  ...
  case class Loader(jarPath: String, excludes: Seq[String] = Seq()) {
    private val cl = new ChildFirstURLClassLoader(Array(new File(jarPath).toURI.toURL), this.getClass.getClassLoader, excludes)
