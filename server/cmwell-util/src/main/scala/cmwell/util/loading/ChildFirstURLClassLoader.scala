...
/**
  ...
  *
  */
class ChildFirstURLClassLoader(urls: Array[URL],
                               parent: ClassLoader,
                               except: Seq[String] = Seq())
    extends URLClassLoader(urls, parent) {

  protected override def loadClass(name: String, resolve: Boolean): Class[_] = {
    def tryFind(findAction: => Class[_]): Option[Class[_]] =
      Try(findAction) match {
    ...
    def loadLocally =
      if (except.exists(name.startsWith)) None else tryFind(findClass(name))
    def loadFromParent =
      if (getParent == null) None else tryFind(getParent.loadClass(name))
  ...
  override def getResources(name: String): java.util.Enumeration[URL] = {
    ...
    val parentUrls: java.util.Enumeration[URL] =
      if (parent != null) parent.getResources(name)
...
object ChildFirstURLClassLoader {
  def loadClassFromJar[T](className: String,
                          jarPath: String,
                          excludes: Seq[String] = Seq()): T =
    Loader(jarPath, excludes).load(className)

  case class Loader(jarPath: String, excludes: Seq[String] = Seq()) {
    private val cl = new ChildFirstURLClassLoader(
      Array(new File(jarPath).toURI.toURL),
      this.getClass.getClassLoader,
      excludes
    )
    def load[T](className: String) =
