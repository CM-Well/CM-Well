...
object Hash {
    ...
    def digitMap: Map[Long, Char] = {
      ...
      if (r > 62)
        throw new RuntimeException(
          "unable to represent a radix that large. (" + r + ")"
        )
  ...
  def crc32(s: String): String =
    crc32(s.getBytes(StandardCharsets.UTF_8))(Hexadecimal)
  ...
  def hashToBase64(
    s: String
  )(hash: String => Long)(bytes: Long => Array[Byte]): String = {
  ...
  def crc32(bytes: Array[Byte])(implicit radix: Radix = Hexadecimal): String = {
    ...
    radix match {
      ...
      case r: Radix => {
        ...
        if (r.getRadix < 1)
          throw new RuntimeException("radix = " + r.getRadix + " ???")
  ...
  def xxhash32(str: String): Long =
    xxhash32(str.getBytes(StandardCharsets.UTF_8))
  ...
  def xxhash64(str: String): Long =
    xxhash64(str.getBytes(StandardCharsets.UTF_8))
  def xxhash64(arr: Array[Byte]): Long =
    xxhashFactory.hash64().hash(arr, 0, arr.length, 786)
