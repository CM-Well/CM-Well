...
package object string extends LazyLogging {
  ...
  object Base64 {
    ...
    def encodeBase64(binaryData: Array[Byte], isChunked: Boolean, urlSafe: Boolean): Array[Byte] =
      ApacheBase64.encodeBase64(binaryData, isChunked, urlSafe)

    def encodeBase64(stringData: String, isChunked: Boolean, urlSafe: Boolean): Array[Byte] =
      ApacheBase64.encodeBase64(stringData.getBytes("UTF-8"), isChunked, urlSafe)

    def encodeBase64(binaryData: Array[Byte], isChunked: Boolean, urlSafe: Boolean, maxResultSize: Int): Array[Byte] =
      ApacheBase64.encodeBase64(binaryData, isChunked, urlSafe, maxResultSize)

    def encodeBase64(stringData: String, isChunked: Boolean, urlSafe: Boolean, maxResultSize: Int): Array[Byte] =
      ApacheBase64.encodeBase64(stringData.getBytes("UTF-8"), isChunked, urlSafe, maxResultSize)
    ...
    def encodeIntegerString(bigInt: BigInt): String =
      org.apache.commons.codec.binary.StringUtils.newStringUtf8(ApacheBase64.encodeInteger(bigInt.bigInteger))
  }
  ...
  def unixify(path: String): String = {
    ...
    val rv =
      if (isWin) path.replaceAllLiterally(thisOs.toString, thatOs.toString).replaceAllLiterally("%20", " ")
