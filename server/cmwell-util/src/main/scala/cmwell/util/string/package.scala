...
package object string extends LazyLogging {
  ...
  object Base64 {
    ...
    def isBase64(arrayOctet: Array[Byte]): Boolean =
      ApacheBase64.isBase64(arrayOctet)

    def encodeBase64(binaryData: Array[Byte]): Array[Byte] =
      ApacheBase64.encodeBase64(binaryData)

    def encodeBase64(stringData: String): Array[Byte] =
      ApacheBase64.encodeBase64(stringData.getBytes("UTF-8"))

    def encodeBase64String(binaryData: Array[Byte]): String =
      ApacheBase64.encodeBase64String(binaryData)

    def encodeBase64String(stringData: String): String =
      ApacheBase64.encodeBase64String(stringData.getBytes("UTF-8"))

    def encodeBase64URLSafe(binaryData: Array[Byte]): Array[Byte] =
      ApacheBase64.encodeBase64URLSafe(binaryData)

    def encodeBase64URLSafe(stringData: String): Array[Byte] =
      ApacheBase64.encodeBase64URLSafe(stringData.getBytes("UTF-8"))

    def encodeBase64URLSafeString(binaryData: Array[Byte]): String =
      ApacheBase64.encodeBase64URLSafeString(binaryData)

    def encodeBase64URLSafeString(stringData: String): String =
      ApacheBase64.encodeBase64URLSafeString(stringData.getBytes("UTF-8"))

    def encodeBase64Chunked(binaryData: Array[Byte]): Array[Byte] =
      ApacheBase64.encodeBase64Chunked(binaryData)

    def encodeBase64Chunked(stringData: String): Array[Byte] =
      ApacheBase64.encodeBase64Chunked(stringData.getBytes("UTF-8"))

    def encodeBase64(binaryData: Array[Byte], isChunked: Boolean): Array[Byte] =
      ApacheBase64.encodeBase64(binaryData, isChunked)

    def encodeBase64(stringData: String, isChunked: Boolean): Array[Byte] =
      ApacheBase64.encodeBase64(stringData.getBytes("UTF-8"), isChunked)

    def encodeBase64(binaryData: Array[Byte],
                     isChunked: Boolean,
                     urlSafe: Boolean): Array[Byte] =
      ApacheBase64.encodeBase64(binaryData, isChunked, urlSafe)

    def encodeBase64(stringData: String,
                     isChunked: Boolean,
                     urlSafe: Boolean): Array[Byte] =
      ApacheBase64.encodeBase64(
        stringData.getBytes("UTF-8"),
        isChunked,
        urlSafe
      )
    ...
    def encodeBase64(binaryData: Array[Byte],
                     isChunked: Boolean,
                     urlSafe: Boolean,
                     maxResultSize: Int): Array[Byte] =
      ApacheBase64.encodeBase64(binaryData, isChunked, urlSafe, maxResultSize)
    ...
    def decodeBase64(base64String: String): Array[Byte] =
      ApacheBase64.decodeBase64(base64String)

    def decodeBase64(base64Data: Array[Byte]): Array[Byte] =
      ApacheBase64.decodeBase64(base64Data)

    def decodeBase64String(base64String: String, charset: String): String =
      new String(ApacheBase64.decodeBase64(base64String), charset)

    def decodeBase64String(base64Data: Array[Byte], charset: String): String =
      new String(ApacheBase64.decodeBase64(base64Data), charset)

    def decodeInteger(pArray: Array[Byte]): BigInt =
      ApacheBase64.decodeInteger(pArray)

    def encodeInteger(bigInt: BigInt): Array[Byte] =
      ApacheBase64.encodeInteger(bigInt.bigInteger)

    def encodeIntegerString(bigInt: BigInt): String =
      org.apache.commons.codec.binary.StringUtils
        .newStringUtf8(ApacheBase64.encodeInteger(bigInt.bigInteger))
  }
  ...
  object Zip {
    ...
    def compress(input: String,
                 ...
                 strategy: Int = Deflater.DEFAULT_STRATEGY): Array[Byte] = {
      val bytes = input.getBytes(encoding)
      val l = bytes.length
      require(
        l < sizeLimit,
        "compress is allowed only for small input (max 64KB)"
      )
    ...
    def decompress(input: Array[Byte], encoding: String = "UTF-8"): String = {
      ...
      require(
        Hash.adler32long(payload).toByte == checksum,
        "compressed data contains invalid checksum byte"
      )
  ...
  def unixify(path: String): String = {
    ...
    val rv =
      if (isWin)
        path
          .replaceAllLiterally(thisOs.toString, thatOs.toString)
  ...
  val dateStringify =
    ISODateTimeFormat.dateTime().withZone(DateTimeZone.UTC).print(_: DateTime)
  ...
  def parseDate(dateStr: String): Option[DateTime] = {
    ...
    val funcs: Seq[String => Option[DateTime]] =
      Seq(
        fullDateFormatter.parseDateTime(_),
        longDateFormatter.parseDateTime(_),
        shortDateFormatter.parseDateTime(_)
      ).map { f ⇒ (s: String) ⇒
        Try(f(s)).toOption
      }
  ...
  def stringToInputStream(s: String) =
  ...
  def classpathResourceToString(resource: String) = {
    ...
    val rv =
      scala.io.Source.fromInputStream(is, "UTF-8").getLines().mkString(endln)
    is.close()
    rv
  }

  def resourceTemplateToGeneratedString(
    resource: String,
  ...
  def replaceTemplates(text: String, templates: Map[String, String]): String =
    """\{\{([^{}]*)\}\}""".r.replaceSomeIn(text, {
      case Regex.Groups(name) => templates.get(name)
    })

  /**
    * Like splitAt, but discarding the separator. e.g. splitAtNoSep("Hello World", ' ') == "Hello" -> "World"
    */
  def splitAtNoSep(s: String, idx: Int): (String, String) =
    s.take(idx) -> s.drop(idx + 1)

  def spanNoSepBy(s: String, p: Char) =
    spanNoSep(s, _ != p) // this is just to spoil the user for the most common case

  /**
    * Like span, but discarding the separator. e.g. spanNoSep("Hello World", ' '.!=) == "Hello" -> "World"
    */
  def spanNoSep(s: String, p: Char => Boolean) =
    splitAtNoSep(s, s.prefixLength(p))
}
