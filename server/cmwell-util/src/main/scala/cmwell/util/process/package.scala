...
package object process extends LazyLogging {
  ...
  def exec(args: Seq[String],
           environmentVariables: Seq[(String, String)] = Seq(),
  ...
  def injectToStream(outputStream: java.io.OutputStream,
                     injectionString: String) = {
    val writer =
      new java.io.BufferedWriter(new java.io.OutputStreamWriter(outputStream))
    writer.write(injectionString)
    writer.flush()
  }

  private def mapContainsSuffixOf(map: Map[String, String],
                                  text: String): Boolean = {
    map.keySet.exists { k =>
  ...
  private def getKeyThatIsSuffixOf(map: Map[String, String],
                                   text: String): String = {
    map.keySet.filter { k =>
      k.length <= text.length && k == text.drop(text.length - k.length)
    }.toList match {
      case Nil =>
        throw new NoneOfTheKeysMatchThisTextSuffixException(
  ...
  def pipeWithLinesReplacment(src: InputStream,
    ...
    replacementMap.isEmpty match {
      ...
      case false => {
        val sb: StringBuilder = new StringBuilder
        val max = replacementMap.foldLeft(0) {
          case (i, (k, _)) => if (i >= k.length) i else k.length
        }
        while (true) {
  ...
  def killWaitAndKill9(pid: Int, ttl: Int): Unit =
    killWaitAndKill9(pid.toString, ttl)
  ...
  def jps: Set[(Int, List[String])] = {
    val arguments: Arguments = new Arguments(
      Array[String]("-l", "-v", "-V", "-m")
    )
    val hostId: HostIdentifier = arguments.hostId
    val monitoredHost: MonitoredHost = MonitoredHost.getMonitoredHost(hostId)
    val jvms = scala.collection.immutable.Set[Int]() ++ (for {
      j <- monitoredHost.activeVms.asScala
    } yield j.toInt)
    for {
      lvmid <- jvms
      vm: MonitoredVm = monitoredHost.getMonitoredVm(
        new VmIdentifier("//" + lvmid + "?mode=r"),
        0
      )
    } yield
      (lvmid, if (vm == null) {
  ...
  /**
    ...
    */
  def runAndDo(pb: ProcessBuilder)(
    runAgain: (List[String], List[String]) => Boolean,
    doAction: => Any
  ): Int = {
