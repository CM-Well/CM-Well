...
object Props {
  ...
  private[this] val runtimemxBean: RuntimeMXBean =
  ...
  val javaApp = pathWrapper(
    properties.get("java.home") + fileSeparator + "bin" + fileSeparator + {
  ...
  lazy val machineIP: String = {
    ...
    lb.filter(_.isSiteLocalAddress)
      .head
      .getHostAddress //TODO: Choose the ip address according to interface name.
  }
  ...
  def javaLib: String = {
    ...
    else if (suffix.matches("""jre\d+["]"""))
      jh.dropRight(suffix.length) + "\"lib"
    else
      javaHome + {
