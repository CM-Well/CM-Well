...
object Combiner {
  implicit val setStringCombiner = new Combiner[Set[String]] {
    override def combine(t1: Set[String], t2: Set[String]): Set[String] =
      t1.union(t2)
  }
...
object Validator {
  implicit val setStringIsEmptyValidator: Validator[Set[String]] =
...
class SingleElementLazyAsyncCache[T: Combiner: Validator](
  ...
  private[this] var cachedElement: Either[Future[T], (T, Long)] = Right(
    initial -> 0L
  )
  ...
  private[this] val rightHandlingFunctionOnUpdatingRaceCondition
    : ((T, Long)) => Future[T] = {
  ...
  def getAndUpdateIfNeeded: Future[T] = {
    cachedElement match {
      ...
      case right @ Right((elem, timestamp)) => {
        if (System
              .currentTimeMillis() - timestamp <= refreshThresholdInMillis) {
          Future.successful(elem)
        } else {
          if (!isBeingUpdated.compareAndSet(false, true))
            cachedElement.fold(
              identity,
              rightHandlingFunctionOnUpdatingRaceCondition
            )
          else {
            val p = Promise[T]()
            val rv = p.future
              ...
              p.completeWith {
                f.andThen {
                    case Success(newValue) if validator.isValid(newValue) =>
                      cachedElement = Right(
                        combiner.combine(elem, newValue) -> System
                          .currentTimeMillis()
                      )
                    case _ =>
                      cachedElement = right
  ...
  def resetWith(value: T): Try[T] = {
    if (isBeingUpdated.compareAndSet(false, true)) {
      ...
      Failure(new RuntimeException("SELAC cannot be reset while being updated"))
  }
}
