...
class GuavaCacheJMX[K, V](var _cache: Cache[K, V]) extends GuavaCacheJMXMBean {
  ...
  override def getLoadExceptionCount(): Long =
    _cache.stats().loadExceptionCount()

  override def getLoadExceptionRate(): Double =
    _cache.stats().loadExceptionRate()

  override def getTotalLoadTime(): Long = _cache.stats().totalLoadTime()

  override def getAverageLoadPenalty(): Double =
    _cache.stats().averageLoadPenalty()
