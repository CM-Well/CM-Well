...
/**
  ...
  * Time: 1:20 PM
  */
class GlobalVarStateHandler[State](setters: Int, getters: Int)(
  initialAsync: () ⇒ Future[State]
)(implicit ec: ExecutionContext)
    extends GraphStage[SingleTypeAmorphousShape[State, State]] {
  require(
    setters >= 0 && getters >= 0 && setters + getters > 0,
    "must have positive number of either inlets or outlets"
  )

  val inlets: List[Inlet[State]] =
    List.tabulate(setters)(i => Inlet[State]("GlobalVarStateHandler.in" + i))
  val outlets: List[Outlet[State]] =
    List.tabulate(getters)(i => Outlet[State]("GlobalVarStateHandler.in" + i))

  override val shape: SingleTypeAmorphousShape[State, State] =
    SingleTypeAmorphousShape(inlets, outlets)
    ...
    new GraphStageLogic(shape) {
        ...
        setHandler(
          ...
          new OutHandler {
            override def onDownstreamFinish(): Unit = {
              if (inlets.forall(isClosed[State]) && outlets
                    .forall(isClosed[State]))
                completeStage()
            }
      ...
      override def preStart(): Unit = {
        ...
        val acb = getAsyncCallback[Try[State]] {
          case Failure(ex) if state != null && hasActiveSetters ⇒
            replaceHandlers
