...
import java.io.{File, InputStream, OutputStream, FileOutputStream, FileInputStream}
...
package object files extends LazyLogging  {
	...
	private[this] class FileOpsTmpLogger(threshold: Int, name: String) extends org.codehaus.plexus.logging.AbstractLogger(threshold, name) with LazyLogging {
    ...
    def deployDirectoryFromJarToFile(jarPath: String, to: String) {
        ...
        unzipEntries(jarfile, jarfile.entries, to,e => e.getName.startsWith(jarPath), s => if(s.startsWith(jarPath)) s.drop(jarPath.length) else s)
    }
    ...
    private[this] def unzipEntries(zip: java.util.zip.ZipFile, entries: java.util.Enumeration[_ <: java.util.zip.ZipEntry], to: String, filter: java.util.zip.ZipEntry => Boolean, pathModifier: String => String) {
    ...
    def untgz(srcFile: File, dstDir: File) {
        ...
        val logger: org.codehaus.plexus.logging.Logger = new FileOpsTmpLogger(0, srcFile.getName.toLowerCase.filterNot(validLetters))
    ...
    def replaceFileContentFromInputStream(contentInputStream: InputStream, fileToOverride: String){
        ...
        val path = new java.io.File(fileToOverride.dropRight(fileToOverride.length - fileToOverride.lastIndexOf(fileSeparator)))
