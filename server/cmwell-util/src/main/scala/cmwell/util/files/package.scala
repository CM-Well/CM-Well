...
package object files extends LazyLogging {
  ...
  private[this] class FileOpsTmpLogger(threshold: Int, name: String)
    ...
    def getChildLogger(arg0: String): org.codehaus.plexus.logging.Logger = {
      null
    }

    def fatalError(arg0: String, arg1: Throwable): Unit =
  ...
  def fileFromZipToStream(filename: String, zipfile: String): InputStream = {
    ...
    val ze = {
      ...
      if (rv == null) {
        val toggleSlash =
          if (filename.head == '/') filename.drop(1) else s"/$filename"
  ...
  def resourceToFile(resource: String, file: String) {
    resourceToFile(resource, new File(file))
  }
  ...
  private[this] def ensureDirHirerchy(path: String) {
    ...
    new java.io.File(
      nixPath.dropRight(nixPath.length - nixPath.lastIndexOf("/"))
    ).mkdirs
  }
  ...
  /**
    ...
    */
  implicit def stringToZipFile(zip: String): java.util.zip.ZipFile =
    new java.util.zip.ZipFile(zip)
  implicit def fileToZipFile(file: java.io.File): java.util.zip.ZipFile =
  ...
  def untgz(srcFile: File, dstDir: File) {
    ...
    val logger: org.codehaus.plexus.logging.Logger =
      new FileOpsTmpLogger(
        0,
        srcFile.getName.toLowerCase.filterNot(validLetters)
      )
  ...
  def cleanDir(dir: java.io.File, filter: String => Boolean) {
    ...
    if (!dir.exists) {
      throw new IllegalFileException(
        "can't clean a dir which does not exist! (" + dir.getAbsolutePath + ")"
      )
  ...
  def replaceFileContentFromInputStream(contentInputStream: InputStream,
                                        fileToOverride: String) {
    val file = new java.io.File(fileToOverride)
    val path = new java.io.File(
      fileToOverride.dropRight(
        fileToOverride.length - fileToOverride.lastIndexOf(fileSeparator)
      )
    )
