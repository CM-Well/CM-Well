...
class FormatsSpec extends FunSpec with Matchers with Helpers {
  ...
  describe("JSONL tests") {
    ...
    val infotons = Map(
      'simpleObjectInfoton -> ObjectInfoton("simpleObjectInfoton",
                                            "dc_test",
                                            None,
                                            date,
                                            defaultFields),
      'objectInfotonWithQuads -> ObjectInfoton(
        ...
        date,
        Map(
          "field1" -> Set[FieldValue](
            FString("value1", None, Some("spiderman"))
          )
        )
      ),
      'fileInfoton -> FileInfoton("fileInfoton",
                                  ...
                                  date,
                                  defaultFields,
                                  FileContent("CAFEBABE".getBytes("UTF8"),
    ...
    def assertSystemFields(i: Infoton, jsonl: JsValue) = {
      Seq("@id" -> i.path,
        ...
        case (key, expectedValue) =>
          jsonl.getValueOfFirst(s"$key.sys") should be(expectedValue)
      }
    }
    ...
    it("should serialize an object infoton with quads") {
      ...
      val jsonlResult = Json.parse(
        new JsonlFormatter(_ => None, _ => Some("spiderman")).render(i)
      )
    ...
    it("should serialize a FileInfoton") {
      ...
      Seq("field1.nn" -> "value1",
        ...
        case (key, expectedValue) =>
          jsonlResult.getValueOfFirst(key) should be(expectedValue)
      }

      (jsonlResult.getFirst("length.content.sys") \ "value").as[Int] should be(
        8
      )
    }
    ...
    val aggregationsResponse1 = AggregationsResponse(
      Seq(
        TermsAggregationResponse(
          filter =
            TermAggregationFilter(name = "TermAggregation",
                                  field =
                                    Field(NonAnalyzedField, "category.jX2eAA"),
                                  size = 5),
          buckets = Seq(
            Bucket(k = FieldValue(
                     "http://en.wikipedia.org/cat/living_people_d45c539"
                   ),
                   dc = 710585,
                   subAgg = None),
            Bucket(k = FieldValue(
                     "http://en.wikipedia.org/cat/year_of_birth_missing__living_people__170e89c5"
                   ),
                   dc = 56019,
                   subAgg = None),
            Bucket(k = FieldValue(
                     "http://en.wikipedia.org/cat/american_films_1fbf60ae"
                   ),
                   dc = 39532,
                   subAgg = None),
            Bucket(
              k = FieldValue(
                "http://en.wikipedia.org/cat/english_language_films_d8965cab"
              ),
              dc = 33083,
              subAgg = None
            ),
            Bucket(k = FieldValue(
                     "http://en.wikipedia.org/cat/place_of_birth_missing__living_people__3f637be9"
                   ),
                   dc = 26766,
                   subAgg = None)
          )
    ...
    val aggregationsResponse2 = AggregationsResponse(
      Seq(
        SignificantTermsAggregationResponse(
          filter =
            SignificantTermsAggregationFilter(name =
                                                "SignificantTermsAggregation",
                                              field = Field(NonAnalyzedField,
                                                            "referTo.jX2eAA"),
                                              backgroundTerm = None,
                                              minDocCount = 0,
                                              size = 3),
          ...
          buckets = Seq(
            SignificantTermsBucket(
              key = FieldValue(
                "http://en.wikipedia.org/wiki/united_states_b01f3e84"
              ),
              docCount = 282203,
              score = 19307.54729346422,
              bgCount = 282203,
            ...
            SignificantTermsBucket(
              key = FieldValue(
                "http://en.wikipedia.org/wiki/association_football_b70932fc"
              ),
              docCount = 115031,
              score = 3207.989815760563,
              bgCount = 115031,
                  ...
                  Seq(
                    TermsAggregationResponse(
                      filter = TermAggregationFilter(
                        name = "TermAggregation",
                        field = Field(NonAnalyzedField,
                                      "organizationFoundedDay.-Jamjg"),
                        size = 3
                      ),
                      buckets =
                        Seq(Bucket(k = FieldValue(1), dc = 7579, subAgg = None),
                            Bucket(k = FieldValue(28),
                                   dc = 5849,
                                   subAgg = None),
                            Bucket(k = FieldValue(15),
                                   dc = 5699,
                                   subAgg = None))
                    )
                  )
                ...
                AggregationsResponse(
                  Seq(
                    TermsAggregationResponse(
                      ...
                      buckets =
                        Seq(Bucket(k = FieldValue(1), dc = 3011, subAgg = None),
                            Bucket(k = FieldValue(19),
                                   dc = 2933,
                                   subAgg = None),
                            Bucket(k = FieldValue(14),
                                   dc = 2822,
                                   subAgg = None))
                    )
                  )
