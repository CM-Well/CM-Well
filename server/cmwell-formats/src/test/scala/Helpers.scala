...
trait Helpers {
  implicit class JsValueExtensions(v: JsValue) {
    ...
    def getArr(prop: String): Seq[JsValue] =
      withPrettyException(_ => {
        ((v \ prop).get: @unchecked) match { case JsArray(seq) => seq }
      }, "getArr", prop)
    ...
    private def withPrettyException(f: String => Seq[JsValue],
                                    desc: String,
                                    prop: String): Seq[JsValue] =
      Try(f(prop)).getOrElse(
        throw new Exception(s"""Operation $desc("$prop") failed on $v""")
      )
  }
}
