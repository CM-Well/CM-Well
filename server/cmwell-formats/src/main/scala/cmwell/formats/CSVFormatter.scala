...
class CSVFormatter(fieldNameModifier: String => String) extends Formatter {
  ...
  private[this] def renderGeneric(formattable: Formattable): String = {
    ...
    val internalFields = infotons.map(_.fields).collect {
      case Some(m) => m.keySet
    }.flatten.distinct.toVector.sortBy {
      s => s.lastIndexOf('.') match {
    ...
    val specialSystem = distinctBy(infotons)(_.kind).collect {
      ...
      case _: FileInfoton => Seq("mimeType", "length") ++ {
  ...
  def extractInfotonsFromFormattable(formattable: Formattable): Seq[Infoton] = formattable match {
    ...
    case _ => throw new NotImplementedError(s"extractInfotonsFromFormattable implementation not found for ${formattable.getClass.toString}")
  }
  ...
  private[this] def renderAggregationsResponse(aggr: AggregationsResponse): String = {
    ...
    def getSortedKeys(ar: AggregationResponse) = ar match {
      case StatsAggregationResponse(filter, count, min, max, avg, sum) => ("Field-Name", Seq("count", "min", "max", "avg", "sum"))
      ...
      case bar: BucketsAggregationResponse => {
        val keys = aggRespDepth match {
          case  1 => Seq("value")
          case  2 => bar.buckets.flatMap(_.subAggregations.fold(Nil: Seq[Any])(sa =>
            sa.responses.map(_.asInstanceOf[BucketsAggregationResponse]).flatMap(_.buckets.map(_.key.value))))
          case _  => ???
        }
        (s"""${fieldNameModifier(bar.filter.field.value) + bar.buckets.head.subAggregations.flatMap(_.responses.headOption).fold("")(ar => " \\ " + fieldNameModifier(ar.filter.field.value))}"""
          , keys.distinct)
      }
    }
    ...
    val printer = new CSVPrinter(sb, format)
    agr match {
      case StatsAggregationResponse(filter, count, min, max, avg, sum) => printer.printRecord(fieldNameModifier(filter.field.value), count.toString, min.toString, max.toString, avg.toString, sum.toString)
      case CardinalityAggregationResponse(filter, count) => printer.printRecord(fieldNameModifier(filter.field.value), count.toString)
      case bar: BucketsAggregationResponse => aggRespDepth match {
        case 1 => {
          bar.buckets.foreach(b => printer.printRecord(b.key.value.toString, b.docCount.toString))
        }
        case 2 => bar.buckets.foreach(b => {
          val bucketKeysMap = b.subAggregations.fold(Map[Any, Any]())(_.responses.head.asInstanceOf[BucketsAggregationResponse]
            .buckets.map(b => (b.key.value, b.docCount)).toMap.withDefaultValue(""))
