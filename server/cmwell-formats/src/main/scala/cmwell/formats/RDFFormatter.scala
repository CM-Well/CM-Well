...
/**
  ...
  */
abstract class RDFFormatter(hostForNs: String,
  ...
  lazy val host = {
    if (hostForNs.startsWith("http")) hostForNs
      ...
      logger.warn(
        san"host provided for RDF formatter ($hostForNs) does not contain a known protocol ('http://' or 'https://'), assigned with 'http://' as default."
      )
      s"http://$hostForNs"
    }
  }
  ...
  val stringToLtrl: (String, Option[String]) => RDFNode = (v: String, lang: Option[String]) =>
    lang match {
  ...
  val dateToLtrl: DateTime => RDFNode = (dt: DateTime) =>
    ResourceFactory.createTypedLiteral(dateStringify(dt), NodeFactory.getType(XSD.dateTime.getURI))
  val refToLtrl: String => RDFNode = (v: String) => ResourceFactory.createResource(v)
  val intToLtrl: Int => RDFNode = (v: Int) =>
    ResourceFactory.createTypedLiteral(v.toString, NodeFactory.getType(XSD.xint.getURI))
  val longToLtrl: Long => RDFNode = (v: Long) =>
    ResourceFactory.createTypedLiteral(v.toString, NodeFactory.getType(XSD.xlong.getURI))
  val bigIntToLtrl: BigInteger => RDFNode = (v: BigInteger) =>
    ResourceFactory.createTypedLiteral(v.toString, NodeFactory.getType(XSD.integer.getURI))
  val floatToLtrl: Float => RDFNode = (v: Float) =>
    ResourceFactory.createTypedLiteral(v.toString, NodeFactory.getType(XSD.xfloat.getURI))
  val doubleToLtrl: Double => RDFNode = (v: Double) =>
    ResourceFactory.createTypedLiteral(v.toString, NodeFactory.getType(XSD.xdouble.getURI))
  val bigDecimalToLtrl: BigDecimal => RDFNode = (v: BigDecimal) =>
    ResourceFactory.createTypedLiteral(v.toString, NodeFactory.getType(XSD.decimal.getURI))
  val boolToLtrl: Boolean => RDFNode = (v: Boolean) =>
    ResourceFactory.createTypedLiteral(v.toString, NodeFactory.getType(XSD.xboolean.getURI))
  val bytesToLtrl: Array[Byte] => RDFNode = (v: Array[Byte]) =>
    ResourceFactory.createTypedLiteral(base64(v), NodeFactory.getType(XSD.base64Binary.getURI))

  private val emptySeq = Seq[(Property, RDFNode)]()
  private val mEmptySeq = Seq[(Property, RDFNode, Model)]()

  def fields(theFields: Option[Map[String, Set[FieldValue]]])(implicit ds: Dataset): Seq[(Property, RDFNode, Model)] =
    theFields match {
      case None => mEmptySeq
      case Some(m) =>
          ...
          case (seq, (f, set)) => {
            ...
            val modifiedSet: Set[(Property, RDFNode, Model)] = set.map { fv =>
              ...
              fv match {
                ...
                case FDate(v, _) =>
                  (prop, ResourceFactory.createTypedLiteral(v, NodeFactory.getType(XSD.dateTime.getURI)), model)
                case f @ FExternal(v, _, _) =>
                  (prop, ResourceFactory.createTypedLiteral(v, NodeFactory.getType(f.getDataTypeURI)), model)
...
//        logger.info(res.toString)
  ...
  def infotonHistoryVersions(ihv: InfotonHistoryVersions)(implicit ds: Dataset): Dataset = {
    if (withoutMeta) {
      ...
      if (!filterOutBlanks) {
        model.add(
          ResourceFactory
            .createStatement(subject, stringToSysProp("type"), stringToLtrl("InfotonHistoryVersions", None))
        )
      }
  ...
  def retrievablePaths(rp: RetrievablePaths)(implicit ds: Dataset): Dataset = {
    if (withoutMeta) {
      ...
      if (!filterOutBlanks) {
        model
          .add(
            ResourceFactory.createStatement(subject, stringToSysProp("type"), stringToLtrl("RetrievablePaths", None))
          )
  ...
  def searchResults(sr: SearchResults,
                    subject: Resource = ResourceFactory.createResource())(implicit ds: Dataset): Dataset = {
    if (withoutMeta) {
      ...
      if (!filterOutBlanks) {
        model.add(
          ResourceFactory.createStatement(subject, stringToSysProp("type"), stringToLtrl("SearchResults", None))
        )
        sr.fromDate.foreach(
          dt => model.add(ResourceFactory.createStatement(subject, stringToSysProp("fromDate"), dateToLtrl(dt)))
        )
        sr.toDate.foreach(
          dt => model.add(ResourceFactory.createStatement(subject, stringToSysProp("toDate"), dateToLtrl(dt)))
        )
  ...
  def iterationResults(ir: IterationResults)(implicit ds: Dataset): Dataset = {
    if (withoutMeta || filterOutBlanks) {
      ...
      model.add(
        ResourceFactory.createStatement(subject, stringToSysProp("type"), stringToLtrl("IterationResults", None))
      )
      model.add(
        ResourceFactory.createStatement(subject, stringToSysProp("iteratorId"), stringToLtrl(ir.iteratorId, None))
      )
...
abstract class SimpleRDFFormatter(host: String,
                                  hashToPrefixAndUri: String => Option[(String, Option[String])],
                                  withoutMeta: Boolean,
                                  filterOutBlanks: Boolean,
                                  raw: Boolean,
...
class RDFXmlFormatter(host: String,
                      hashToPrefixAndUri: String => Option[(String, Option[String])],
                      withoutMeta: Boolean,
                      filterOutBlanks: Boolean,
                      raw: Boolean,
...
class TurtleFormatter(host: String,
                      hashToPrefixAndUri: String => Option[(String, Option[String])],
                      withoutMeta: Boolean,
                      filterOutBlanks: Boolean,
                      raw: Boolean,
...
class JsonLDFormatter private (host: String,
                               hashToPrefixAndUri: String => Option[(String, Option[String])],
                               withoutMeta: Boolean,
                               filterOutBlanks: Boolean,
                               forceUniqueness: Boolean,
...
abstract class QuadsFormatter(host: String,
                              hashToPrefixAndUri: String => Option[(String, Option[String])],
...
class NQuadsFormatter(host: String,
                      hashToPrefixAndUri: String => Option[(String, Option[String])],
                      withoutMeta: Boolean,
                      filterOutBlanks: Boolean,
                      raw: Boolean,
...
class TriGFormatter(host: String,
                    hashToPrefixAndUri: String => Option[(String, Option[String])],
                    quadToAlias: String => Option[String],
                    withoutMeta: Boolean,
                    filterOutBlanks: Boolean,
...
class TriXFormatter(host: String,
                    hashToPrefixAndUri: String => Option[(String, Option[String])],
                    quadToAlias: String => Option[String],
                    withoutMeta: Boolean,
                    filterOutBlanks: Boolean,
...
class JsonLDQFormatter private (host: String,
                                hashToPrefixAndUri: String => Option[(String, Option[String])],
                                quadToAlias: String => Option[String],
                                withoutMeta: Boolean,
                                filterOutBlanks: Boolean,
