...
abstract class RDFFormatter(
    ...
    formattable match {
      ...
      case _ =>
        throw new NotImplementedError(
          s"formattableToDataset implementation not found for ${formattable.getClass.toString}"
        )
    }
  ...
  val stringToSysProp: String => Property =
    ResourceFactory.createProperty(s"$host/meta/sys#", _)
  val stringToNnProp: String => Property =
    ResourceFactory.createProperty(s"$host/meta/nn#", _)

  val stringToLtrl: (String, Option[String]) => RDFNode =
    (v: String, lang: Option[String]) =>
      lang match {
        case None    => ResourceFactory.createPlainLiteral(v)
        case Some(l) => ResourceFactory.createLangLiteral(v, l)
    }
  val dateToLtrl: DateTime => RDFNode = (dt: DateTime) =>
    ResourceFactory.createTypedLiteral(
      dateStringify(dt),
      NodeFactory.getType(XSD.dateTime.getURI)
  )
  val refToLtrl: String => RDFNode = (v: String) =>
    ResourceFactory.createResource(v)
  val intToLtrl: Int => RDFNode = (v: Int) =>
    ResourceFactory.createTypedLiteral(
      v.toString,
      NodeFactory.getType(XSD.xint.getURI)
  )
  val longToLtrl: Long => RDFNode = (v: Long) =>
    ResourceFactory.createTypedLiteral(
      v.toString,
      NodeFactory.getType(XSD.xlong.getURI)
  )
  val bigIntToLtrl: BigInteger => RDFNode = (v: BigInteger) =>
    ResourceFactory.createTypedLiteral(
      v.toString,
      NodeFactory.getType(XSD.integer.getURI)
  )
  val floatToLtrl: Float => RDFNode = (v: Float) =>
    ResourceFactory.createTypedLiteral(
      v.toString,
      NodeFactory.getType(XSD.xfloat.getURI)
  )
  val doubleToLtrl: Double => RDFNode = (v: Double) =>
    ResourceFactory.createTypedLiteral(
      v.toString,
      NodeFactory.getType(XSD.xdouble.getURI)
  )
  val bigDecimalToLtrl: BigDecimal => RDFNode = (v: BigDecimal) =>
    ResourceFactory.createTypedLiteral(
      v.toString,
      NodeFactory.getType(XSD.decimal.getURI)
  )
  val boolToLtrl: Boolean => RDFNode = (v: Boolean) =>
    ResourceFactory.createTypedLiteral(
      v.toString,
      NodeFactory.getType(XSD.xboolean.getURI)
  )
  val bytesToLtrl: Array[Byte] => RDFNode = (v: Array[Byte]) =>
    ResourceFactory.createTypedLiteral(
      base64(v),
      NodeFactory.getType(XSD.base64Binary.getURI)
  )
  ...
  def fields(
    theFields: Option[Map[String, Set[FieldValue]]]
  )(implicit ds: Dataset): Seq[(Property, RDFNode, Model)] = theFields match {
    case None => mEmptySeq
    case Some(m) =>
      (mEmptySeq /: m) {
        case (seq, (f, set)) => {
          val (prop: Property, nsOpt: Option[(String, String)]) =
            f.lastIndexOf('.') match {
              ...
              case index => {
                ...
                hashToPrefixAndUri(hash) match {
                  case Some((url, prefix)) =>
                    ResourceFactory.createProperty(url, firstName) -> prefix
                      .map(_ -> url)
          ...
          val modifiedSet: Set[(Property, RDFNode, Model)] = set.map { fv =>
            ...
            fv match {
              ...
              case FReference(v, _) =>
                (prop, refToLtrl({
                  if (v.startsWith("cmwell://"))
                    v.replaceFirst("cmwell:/", host)
                  else v
                }), model)
              ...
              case f @ FExternal(v, _, _) =>
                (
                  prop,
                  ResourceFactory.createTypedLiteral(
                    v,
                    NodeFactory.getType(f.getDataTypeURI)
                  ),
                  model
                )
              case FNull(_) =>
                !!! //this is just a marker for IMP, should not index it anywhere...
              case _: FExtra[_] => !!!
            }
          }
          seq ++ modifiedSet
  ...
  def excludeParent(path: String, isKnownAsADomain: Boolean = false): Boolean =
    (isKnownAsADomain || isPathADomain(path)) && path
      .split('/')
      .filterNot(_.isEmpty)
      .size <= 1

  def uriFromPath(path: String,
                  isADomainOpt: Option[Boolean] = None): String = {
  ...
  private val memoizedBreakOut = scala.collection
    .breakOut[Map[String, Set[FieldValue]], (Property, RDFNode), Seq[
      (Property, RDFNode)
    ]]
  def infoton(i: Infoton, distinctPathWithUuid: Boolean = false)(
    implicit ds: Dataset
  ): Dataset = { //(implicit model: Model): Model = {
    val subject = {
      val s =
        if (distinctPathWithUuid || forceUniqueness) s"${i.path}#${i.uuid}"
        else i.path
    ...
    val extras = i.fields.fold(Seq.empty[(Property, RDFNode)])(_.collect {
      case (k, vs) if k.head == '$' => {
        ...
        val rdfNode: RDFNode = t match {
          case "score" =>
            floatToLtrl(vs.head.value.asInstanceOf[Float]) //FIXME: what an ugly hack...
          case _ => ???
        }
        (prop, rdfNode)
      }
    ...
    val systemData
      : Seq[(Property, RDFNode)] = t +: extras ++: system[Property, RDFNode](
      i,
      stringToSysProp,
      stringToLtrl(_, None),
      dateToLtrl,
    ...
    val selfAttributes: Seq[(Property, RDFNode)] = i match {
      ...
      case CompoundInfoton(_, _, _, _, _, children, offset, length, total, _) => {
        ...
        val props = systemData ++:
          Seq(
          stringToSysProp("offset") -> longToLtrl(offset),
...
//        logger.info(res.toString)
      ...
      case LinkInfoton(_, _, _, _, _, linkTo, linkType, _) => {
        systemData ++:
          Seq(
          stringToSysProp("linkTo") -> stringToLtrl(linkTo, None),
        ...
        model
          .add(
            ResourceFactory.createStatement(
              subject,
              stringToSysProp("type"),
              stringToLtrl("BagOfInfotons", None)
            )
          )
          .add(
            ResourceFactory
              .createStatement(
                subject,
                stringToSysProp("size"),
                intToLtrl(bag.infotons.size)
              )
          )
  ...
  def infotonHistoryVersions(
    ihv: InfotonHistoryVersions
  )(implicit ds: Dataset): Dataset = {
    if (withoutMeta) {
      ...
      val subject =
        model.createResource(AnonId.create("infotonHistoryVersions"))
      ...
      if (!filterOutBlanks) {
        model.add(
          ResourceFactory.createStatement(
            subject,
            stringToSysProp("type"),
            stringToLtrl("InfotonHistoryVersions", None)
          )
        )
      }
        ...
        model
          .add(
            ResourceFactory.createStatement(
              subject,
              stringToSysProp("type"),
              stringToLtrl("RetrievablePaths", None)
            )
          )
          .add(
            ResourceFactory
  ...
  def pagination(
    pi: PaginationInfo,
    subject: Resource = ResourceFactory.createResource()
  )(implicit ds: Dataset): Dataset = {
    val model = ds.getDefaultModel
    (model /: super.pagination(
      pi,
      stringToSysProp,
      stringToLtrl(_, None),
      refToLtrl
    ...
    if (withoutMeta) {
      ...
      if (!filterOutBlanks) {
        model.add(
          ResourceFactory
            .createStatement(
              subject,
              stringToSysProp("type"),
              stringToLtrl("SearchResults", None)
            )
        )
        sr.fromDate.foreach(
          dt =>
        ...
        model.add(
          ResourceFactory.createStatement(
            subject,
            stringToSysProp("offset"),
            longToLtrl(sr.offset)
          )
        )
        model.add(
          ResourceFactory.createStatement(
            subject,
            stringToSysProp("length"),
            longToLtrl(sr.length)
          )
        )
      }
      ...
      model.add(
        ResourceFactory
          .createStatement(
            subject,
            stringToSysProp("type"),
            stringToLtrl("SearchResponse", None)
          )
      )
      model.add(
        ResourceFactory
          .createStatement(subject, stringToSysProp("results"), results)
      )
      model.add(
        ResourceFactory
          .createStatement(subject, stringToSysProp("pagination"), pgntion)
      )
      ...
      model.add(
        ResourceFactory
          .createStatement(
            subject,
            stringToSysProp("type"),
            stringToLtrl("IterationResults", None)
          )
      )
      model.add(
        ResourceFactory.createStatement(
          subject,
          stringToSysProp("iteratorId"),
          stringToLtrl(ir.iteratorId, None)
        )
      )
      model.add(
        ResourceFactory.createStatement(
...
abstract class SimpleRDFFormatter(
  host: String,
  hashToPrefixAndUri: String => Option[(String, Option[String])],
  withoutMeta: Boolean,
  filterOutBlanks: Boolean,
  forceUniqueness: Boolean
) extends RDFFormatter(
      host,
      hashToPrefixAndUri,
      withoutMeta,
      filterOutBlanks,
...
class NTriplesFormatter(host: String,
