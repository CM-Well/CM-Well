...
/**
  ...
  */
abstract class AbstractJsonFormatter(
  override val fieldNameModifier: String => String
) extends TreeLikeFormatter {
...
class JsonFormatter(fieldNameModifier: String => String,
                    override val callback: Option[String] = None)
    extends AbstractJsonFormatter(fieldNameModifier)
    with JsonP {
  override def makeFromValues(values: Seq[Inner]): Inner =
    JsArray(cleanDuplicatesPreserveOrder(values))
  override def mkString(inner: Inner) = Json.stringify(inner)
}

class PrettyJsonFormatter(fieldNameModifier: String => String,
                          override val callback: Option[String] = None)
    extends AbstractJsonFormatter(fieldNameModifier)
    with JsonP {
  override def makeFromValues(values: Seq[Inner]): Inner =
...
abstract class AbstractJsonlFormatter(
  hashToUrlAndPrefix: String => Option[(String, String)],
  ...
  protected def mkVal[V](value: V,
    ...
    JsObject(
      Seq("value" -> single(value)) ++ toTupleSeq(
        "quad" -> quad.flatMap(f),
        "lang" -> lang,
        "type" -> dataType
      )
  ...
  override def singleFieldValue(fv: FieldValue): Inner = fv match {
    case FString(value: String, lang, quad) =>
      mkVal(value, quad, lang, Some(XSDDatatype.XSDstring.getURI))
    case FInt(value: Int, quad) =>
      mkVal(value, quad, dataType = Some(XSDDatatype.XSDint.getURI))
    case FLong(value: Long, quad) =>
      mkVal(value, quad, dataType = Some(XSDDatatype.XSDlong.getURI))
    case FBigInt(value: java.math.BigInteger, quad) =>
      mkVal(value, quad, dataType = Some(XSDDatatype.XSDinteger.getURI))
    case FFloat(value: Float, quad) =>
      mkVal(value, quad, dataType = Some(XSDDatatype.XSDfloat.getURI))
    case FDouble(value: Double, quad) =>
      mkVal(value, quad, dataType = Some(XSDDatatype.XSDdouble.getURI))
    case FBigDecimal(value: java.math.BigDecimal, quad) =>
      mkVal(value, quad, dataType = Some(XSDDatatype.XSDdecimal.getURI))
    case FBoolean(value: Boolean, quad) =>
      mkVal(value, quad, dataType = Some(XSDDatatype.XSDboolean.getURI))
    case FDate(value: String, quad) =>
      mkVal(value, quad, dataType = Some(XSDDatatype.XSDdateTime.getURI))
    case FReference(value: String, quad) =>
      mkVal({
        if (value.startsWith("cmwell://")) value.drop("cmwell:/".length)
        else value
      }, quad, dataType = Some(XSDDatatype.XSDanyURI.getURI))
    case FExternal(value: String, dataTypeURI: String, quad) =>
      mkVal(value, quad, None, Some(dataTypeURI))
    case FNull(_) =>
  ...
  override def system(i: Infoton): JsObject =
    JsObject(
      Seq(
        "@id.sys" -> mkValWrappedInArray(i.path),
        "type.sys" -> mkValWrappedInArray(i.kind)
      ) ++ system(
        i,
        (s: String) => s"$s.sys",
      ...
      JsObject(
        xs.filter(_._1.head != '$')
          .flatMap(
            field =>
              Seq(
                convertToCorrectPrefixedForm(field._1) -> JsArray(
                  field._2.map(singleFieldValue).toSeq
                )
            )
          )
          .toSeq
    ...
    (i: @unchecked) match {
      ...
      case LinkInfoton(_, _, _, _, _, linkTo, linkType, _) =>
        iSystem ++ iFields ++ JsObject(
          Seq(
            "linkTo" -> mkValWrappedInArray(linkTo),
            "linkType" -> mkValWrappedInArray(linkType)
          )
        )
