...
class AtomFormatter private (reqHost: String,
  ...
  require(
    withDataFormatter.isEmpty || withDataFormatter.get.getClass != classOf[
      AtomFormatter
    ],
  ...
  private[this] val fillInfotonEntry: (Entry, Infoton) => Entry =
    withDataFormatter match {
      ...
      case None => { (entry, i) =>
        ...
        entry
          .addLink(s"$host/ii/${i.uuid}", "alternate")
...
//      entry.addLink(s"/ii/${i.uuid}/format=yaml", "alternate").setMimeType("text/yaml")
  ...
  def formattableToAtom(formattable: Formattable)(implicit feed: Feed): Feed =
    formattable match {
      ...
      case _ =>
        throw new NotImplementedError(
          s"formatting implementation not found for ${formattable.getClass.toString} as Atom"
        )
    }

  private[this] def infotonsAsEntries(feed: Feed,
                                      infotons: Seq[Infoton]): Feed = {
  ...
  private def infotonHistoryVersions(
    ihv: InfotonHistoryVersions
  )(implicit feed: Feed): Feed = {
    ...
    feed.setId(uri)
    val x: FOMExtensibleElement = feed.addExtension(
  ...
  private def retrievablePaths(
    rp: RetrievablePaths
  )(implicit feed: Feed): Feed = rp match {
    case RetrievablePaths(infotons, na) => {
      feed.setTitle("Retrievable Paths")
      feed.setSubtitle(
        s"managed to find ${infotons.size} infotons, out of ${infotons.size + na.size} requested paths."
      )
  ...
  private def searchResponse(sr: SearchResponse)(implicit feed: Feed): Feed = {
    ...
    def fieldsFiltersToString(fieldsFilter: FieldFilter) = {
      ...
      def appendFilter(filter: FieldFilter): Unit = filter match {
        case SingleFieldFilter(fieldOperator, valueOperator, name, valueOpt) =>
          sb.append(
            s"$fieldOperator + ( $name $valueOperator ${valueOpt.getOrElse("")} )"
          )
    ...
    (sr, offset, length) match {
      case (SearchResponse(pi, searchResult), Some(o), Some(l)) => {
        ...
        val subtitle = searchResult.fromDate
          .map(d => s"From date (UTC): ${dateStringify(d)}, ")
          .getOrElse("") +
          searchResult.toDate
            .map(d => s"To date (UTC): ${dateStringify(d)}")
            .getOrElse("") +
      ...
      case _ =>
        throw new IllegalArgumentException("missing data for atom formatting")
    }
  }
  private def iterationResults(
    ir: IterationResults
  )(implicit feed: Feed): Feed = ir match {
    case IterationResults(id, total, infotons, _, _) => {
      feed.setTitle("Infotons iteration results")
      feed.setSubtitle(
        s"current chunk contains: ${infotons.size} infotons, out of ${total} results."
      )
...
object AtomFormatter {
  ...
  def apply(host: String, uri: String): AtomFormatter =
    new AtomFormatter(host, uri, None, None, None, None)
