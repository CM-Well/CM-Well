...
trait Formatter extends LazyLogging {
  ...
  protected def system[K,V](infoton: Infoton, mkKey: String => K, mkVal: String => V, mkDateVal: DateTime => V, mkLongVal: Long => V, includeParent: Boolean = true): Seq[(K,V)] = {
...
//      mkKey("type") -> mkVal(infoton.kind), //TODO: add type to system instead of outer level
  ...
  protected def thinResult[K,V](result: SearchThinResult, mkKey: String => K, mkVal: String => V, mkLongVal: Long => V, mkFloatVal: Float => V): Seq[(K,V)] = {
    Seq(
  ...
  protected def fileContent[K,V](fileContent: FileContent, mkKey: String => K, mkVal: String => V, mkLongVal: Long => V, mkByteArrayVal: Array[Byte] => V): Seq[(K,V)] = fileContent match {
  ...
  protected def pagination[K,V](pagination: PaginationInfo, mkKey: String => K, mkVal: String => V, mkRefVal: String => V): Seq[(K,V)] = Seq(
  ...
  protected def simpleResponse[K,V](simpleResponse: SimpleResponse, mkKey: String => K, mkVal: String => V, mkBoolVal: Boolean => V): Seq[(K,V)] = Seq(
...
abstract class  SimpleFormater extends Formatter {
  ...
  override def render(formattable: Formattable): String =  formattable match {
    ...
    case _ => throw new NotImplementedError(s"PathFormatter.render implementation not found for ${formattable.getClass.toString}")
  }
}

object TsvFormatter extends SimpleFormater {
  override def format: FormatType = TsvType
  override protected def thinResultToString(r: SearchThinResult): String = s"${r.path}\t${r.lastModified}\t${r.uuid}\t${r.indexTime}" + r.score.fold("")(score => s"\t$score")
  override protected def infotonToString(i: Infoton): String = s"${i.path}\t${cmwell.util.string.dateStringify(i.lastModified)}\t${i.uuid}\t${i.indexTime.fold("")(_.toString)}${i.fields.flatMap(_.get("$score").flatMap(_.headOption.map("\t" + _.value.toString))).getOrElse("")}"
}
...
trait TreeLikeFormatter extends Formatter {
  ...
  def singleFieldValue(fv: FieldValue): Inner = fv match {
    ...
    case FReference(value : String,_) => single({if(value.startsWith("cmwell://")) value.drop("cmwell:/".length) else value})
  ...
  def formattable2Inner(formattable: Formattable): Inner = formattable match {
    ...
    case _ => throw new NotImplementedError(s"formattable2Inner implementation not found for ${formattable.getClass.toString}")
  }
  ...
  private val memoizedBreakOutOut = scala.collection.breakOut[Map[String,Set[FieldValue]],(String,Inner),Seq[(String,Inner)]]

  def fields(i: Infoton): Inner = i.fields match {
    case None => empty
    case Some(xs) => makeFromTuples(xs.collect{
      case (fieldName,set) if fieldName.head != '$' => fieldNameModifier(fieldName) -> makeFromValues(set.toSeq.map(singleFieldValue))
    }(memoizedBreakOutOut))
  }

  def extra(i: Infoton): Seq[(String,Inner)] = i.fields match {
    case Some(xs) if xs.exists(_._1.head == '$') => Seq("extra" -> makeFromTuples(xs.collect {
      case (k,vs) if k.head == '$' => {
  ...
  def infoton(i: Infoton): Inner = {
    ...
    val iFields: Seq[(String,Inner)] = i.fields.flatMap {
  ...
  def bagOfInfotons(bag: BagOfInfotons, withDeleted: Boolean = false) = makeFromTuples(Seq(
    "type" -> single("BagOfInfotons"),
    "infotons" -> infotons(bag.infotons, withDeleted)
  ))

  def infotonHistoryVersions(ihv: InfotonHistoryVersions, withDeleted: Boolean = true) = makeFromTuples(Seq(
    "type" -> single("InfotonHistoryVersions"),
     "versions" -> infotons(ihv.versions, withDeleted)
  ))

  def retrievablePaths(rp: RetrievablePaths, withDeleted: Boolean = false) = makeFromTuples(Seq(
  ...
  def searchResponse(SearchResponse: SearchResponse, withDeleted: Boolean = false) = makeFromTuples(Seq(
  ...
  def aggregationsResponse(ar: AggregationsResponse) = makeFromTuples(Seq(
    "AggregationResponse" -> makeFromValues(ar.responses.map(formattable2Inner)))
    ++ ar.debugInfo.map("searchQueryStr" -> single(_))
  )

  def termsAggregationsResponse(tar: TermsAggregationResponse) = makeFromTuples(Seq(
  ...
  def statsAggregationsResponse(sar: StatsAggregationResponse) = makeFromTuples(Seq(
  ...
  def histogramAggregationResponse(har: HistogramAggregationResponse) = makeFromTuples(Seq(
  ...
  def significantTermsAggregationResponse(star: SignificantTermsAggregationResponse) = makeFromTuples(Seq(
  ...
  def cardinalityAggregationResponse(car: CardinalityAggregationResponse) = makeFromTuples(Seq(
  ...
  def bucketsAggregationResponse(bar: BucketsAggregationResponse) = makeFromTuples(Seq(
  ...
  def aggregationResponse(ar: AggregationResponse) = makeFromTuples(Seq(
    "name" -> single(ar.name),
    "type" -> single(ar.`type`),
    "filter" -> formattable2Inner(ar.filter)
  ))

  def aggregationFilter(af: AggregationFilter) = makeFromTuples(Seq(
    "name" -> single(af.name),
    "type" -> single(af.`type`)
  ))

  def bucketAggregationFilter(baf: BucketAggregationFilter) = makeFromTuples(Seq(
    "name" -> single(baf.name),
    "type" -> single(baf.`type`),
    "subFilters" -> makeFromValues(baf.subFilters.map(formattable2Inner))
  ))

  def statsAggregationFilter(saf: StatsAggregationFilter) = makeFromTuples(Seq(
    "name" -> single(saf.name),
    "type" -> single(saf.`type`),
    "field" -> single(saf.field.value)
  ))

  def termAggregationFilter(taf: TermAggregationFilter) = makeFromTuples(Seq(
  ...
  def histogramAggregationFilter(haf: HistogramAggregationFilter) = makeFromTuples(Seq(
  ...
  def significantTermsAggregationFilter(staf: SignificantTermsAggregationFilter) = makeFromTuples(Seq(
  ...
  def cardinalityAggregationFilter(caf: CardinalityAggregationFilter) = makeFromTuples(Seq(
    "name" -> single(caf.name),
    "type" -> single(caf.`type`),
    "field" -> single(caf.field.value))
    ++ caf.precisionThreshold.map("precision threshold" -> single(_))
  )

  def bucket(b: Bucket) = makeFromTuples(Seq(
    "key" -> single(b.key.value),
    "objects" -> single(b.docCount))
    ++ b.subAggregations.map("subAggregations" -> formattable2Inner(_))
  )

  def significantTermsBucket(b: SignificantTermsBucket) = makeFromTuples(Seq(
    "key" -> single(b.key.value),
    "objects" -> single(b.docCount),
