...
trait Formatter extends LazyLogging {
  ...
  protected def system[K, V](infoton: Infoton,
    ...
    val indexTime = infoton.indexTime
      .map(it => Seq(mkKey("indexTime") -> mkLongVal(it)))
      .getOrElse(Nil)
    val seq = Seq(
//      mkKey("type") -> mkVal(infoton.kind), //TODO: add type to system instead of outer level
  ...
  protected def fileContent[K, V](
    fileContent: FileContent,
    mkKey: String => K,
    mkVal: String => V,
    mkLongVal: Long => V,
    mkByteArrayVal: Array[Byte] => V
  ): Seq[(K, V)] =
    fileContent match {
      case c @ FileContent(data, mime, dl, _) => {
        val xs = Seq(
          mkKey("mimeType") -> mkVal(mime),
          mkKey("length") -> mkLongVal(data.fold(dl)(_.length))
        )
        data match {
          case Some(data) if isTextual(mime) =>
            xs :+ (mkKey("data") -> mkVal(c.asString))
          case Some(data) =>
            xs :+ (mkKey("base64-data") -> mkByteArrayVal(data))
          case None => xs
        }
      }
    }
...
abstract class SimpleFormater extends Formatter {
  ...
  override def render(formattable: Formattable): String = formattable match {
    ...
    case bag: BagOfInfotons =>
      bag.infotons.map(infotonToString).mkString("", "\n", "\n")
    case ihv: InfotonHistoryVersions =>
      ihv.versions.map(infotonToString).mkString("", "\n", "\n")
    case rp: RetrievablePaths =>
      rp.infotons.map(infotonToString).mkString("", "\n", "\n")
    case sr: SearchResults =>
      sr.infotons.map(infotonToString).mkString("", "\n", "\n")
    case sr: SearchResponse =>
      sr.results.infotons.map(infotonToString).mkString("", "\n", "\n")
    case ir: IterationResults =>
      ir.infotons
        .map(_.map(infotonToString).mkString("", "\n", "\n"))
        .getOrElse("")
    case _ =>
...
object TsvFormatter extends SimpleFormater {
  ...
  override protected def thinResultToString(r: SearchThinResult): String =
    s"${r.path}\t${r.lastModified}\t${r.uuid}\t${r.indexTime}" + r.score.fold(
      ""
    )(score => s"\t$score")
  override protected def infotonToString(i: Infoton): String =
    s"${i.path}\t${cmwell.util.string.dateStringify(i.lastModified)}\t${i.uuid}\t${i.indexTime
      .fold("")(_.toString)}${i.fields
...
trait TreeLikeFormatter extends Formatter {
  ...
  def singleFieldValue(fv: FieldValue): Inner = fv match {
    ...
    case FReference(value: String, _) =>
      single({
        if (value.startsWith("cmwell://")) value.drop("cmwell:/".length)
        else value
      })
  ...
  override def render(formattable: Formattable): String =
    mkString(formattable2Inner(formattable))

  def formattable2Inner(formattable: Formattable): Inner = formattable match {
    case r: SearchThinResult =>
      makeFromTuples(thinResult(r, identity, single, single, single))
    ...
    case star: SignificantTermsAggregationResponse =>
      significantTermsAggregationResponse(star)
    case car: CardinalityAggregationResponse =>
      cardinalityAggregationResponse(car)
    ...
    case staf: SignificantTermsAggregationFilter =>
      significantTermsAggregationFilter(staf)
    ...
    case _ =>
      throw new NotImplementedError(
        s"formattable2Inner implementation not found for ${formattable.getClass.toString}"
      )
  }

  def system(i: Infoton): Inner =
    makeFromTuples(system(i, identity, single, single, single))

  private val memoizedBreakOutIn =
    scala.collection.breakOut[Set[FieldValue], Inner, Seq[Inner]]
  private val memoizedBreakOutOut = scala.collection
    .breakOut[Map[String, Set[FieldValue]], (String, Inner), Seq[
      (String, Inner)
    ]]
    ...
    case Some(xs) =>
      makeFromTuples(xs.collect {
        case (fieldName, set) if fieldName.head != '$' =>
          fieldNameModifier(fieldName) -> makeFromValues(
            set.toSeq.map(singleFieldValue)
          )
      }(memoizedBreakOutOut))
  }
  ...
  def fileContent(c: FileContent): Inner =
    makeFromTuples(super.fileContent(c, identity, single, single, single))

  def infotons(is: Seq[Infoton], withDeleted: Boolean = false) =
    makeFromValues(is.map(infoton))
  ...
  def infoton(i: Infoton): Inner = {
    ...
    (i: @unchecked) match {
      ...
      case ObjectInfoton(_, _, _, _, _, _) =>
        makeFromTuples(
          Seq("type" -> single(i.kind), "system" -> iSystem) ++ iExtra ++ iFields
        )
      ...
      case d: DeletedInfoton =>
        makeFromTuples(Seq("type" -> single(i.kind), "system" -> iSystem))
    }
  }
    ...
    makeFromTuples(
      Seq(
        ...
        "irretrievablePaths" -> makeFromValues(
          rp.irretrievablePaths.map(single)
        )
      )
    )
  def pagination(pagination: PaginationInfo) =
    makeFromTuples(super.pagination(pagination, identity, single, single))
  def searchResults(searchResults: SearchResults,
  ...
  def searchResponse(SearchResponse: SearchResponse,
                     withDeleted: Boolean = false) =
    makeFromTuples(
      Seq(
  ...
  def iterationResults(iterationResults: IterationResults,
                       withDeleted: Boolean = false) = {
    val s = Seq(
      ...
      "infotons" -> infotons(
        iterationResults.infotons.getOrElse(Seq.empty[Infoton]),
        withDeleted
      )
    )
    makeFromTuples(
      iterationResults.debugInfo
        .fold(s)(d => s :+ ("searchQueryStr", single(d)))
    )
  }
  def simpleResponse(sr: SimpleResponse) =
    makeFromTuples(super.simpleResponse(sr, identity, single, single))

  def aggregationsResponse(ar: AggregationsResponse) =
    makeFromTuples(
      Seq(
        "AggregationResponse" -> makeFromValues(
          ar.responses.map(formattable2Inner)
        )
      )
  ...
  def significantTermsAggregationResponse(
    star: SignificantTermsAggregationResponse
  ) =
    makeFromTuples(
      Seq(
  ...
  def aggregationFilter(af: AggregationFilter) =
    makeFromTuples(Seq("name" -> single(af.name), "type" -> single(af.`type`)))
  ...
  def histogramAggregationFilter(haf: HistogramAggregationFilter) =
    makeFromTuples(
      Seq(
        ...
        ++ Seq(
          "subFilters" -> makeFromValues(haf.subFilters.map(formattable2Inner))
        )
    )

  def significantTermsAggregationFilter(
    staf: SignificantTermsAggregationFilter
  ) =
    makeFromTuples(
      Seq(
        ...
        ++ Seq(
          "subFilters" -> makeFromValues(staf.subFilters.map(formattable2Inner))
        )
    )
