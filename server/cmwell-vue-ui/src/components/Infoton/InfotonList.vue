<template>
  <div class="infoton-list" v-if="path!='/'">
    <div class="title" v-if="paths.length || method=='horizon'">
      <span v-if="!showTypes">
        ...
        <span class="method-selector">
          ...
          <button
            :disabled="isLoading"
            v-on:click="toggleMethod()"
          >{{ methodSelectors[methodSelector].buttonTitle }}</button>
          {{ methodSelectors[methodSelector].titleSuffix }}
        </span>
      </span>
      <span v-else>Types Distribution</span>
      <span class="types-handle" v-on:click="toggleTypes" title="toggle Types dist.">
        <img src="@/assets/typeHandleIcon.svg" />
      </span>
    </div>
    <msg :show="showMenu" v-on:close="showMenu=false" clazz="download-menu">
      <div v-on:click="showMenu=false; togglePin()">
        <router-link to>{{ pinned ? "Unpin" : "Pin" }} {{ lastPartOf(path) }}</router-link>
      </div>
      <div
        class="hint"
      >When a path is pinned, selecting one of its Children Infotons won't fetch its own children but keep its siblings visible.</div>
      <hr />
      Download entire {{ lastPartOf(path) }} as:
      <br />
      <ul>
        <li v-for="format in streamFormats">
          <a
            :href="path+'?op=stream&recursive&format='+format.value"
            :download="lastPartOf(path)+'.'+format.value"
          >{{ format.displayName }}</a>
        </li>
      </ul>
      <span v-if="gm()" class="gm">
        <router-link to v-on:click.native="streamUuids">Stream uuids</router-link>
        <br />
        <span v-if="purging">PURGING {{ lastPartOf(path) }}/* ... {{ purgeProgress }}</span>
        <router-link
          v-else
          to
          v-on:click.native="recursivePurge(path)"
        >RECURSIVELY PURGE-ALL {{ lastPartOf(path) }}</router-link>
      </span>
    </msg>
    <ul class="infotons types" v-if="showTypes">
      <li
        class="infoton-list-item empty-or-loading"
        v-if="typesAreLoading"
        key="loading"
      >[ Loading types ... ]</li>
      <li
        class="infoton-list-item empty-or-loading"
        v-if="!typesAreLoading && !types.length"
        key="loading"
      >[ types not found ]</li>
      <li class="infoton-list-item" v-for="type in types" :key="type.uri">
        <router-link
          :to="'?op=search&recursive&qp=type.rdf::'+type.uri"
          :title="type.uri"
        >{{ lastPartOf(type.uri) }} ({{ type.count }})</router-link>
      </li>
    </ul>
    <ul class="infotons" v-else>
      <li
        class="infoton-list-item empty-or-loading"
        v-if="!paths.length && !isLoading && method!='horizon'"
        key="empty"
      >[ no child Infotons ]</li>
      <infoton-list-item
        v-for="path in paths"
        :key="path"
        :path="path"
        v-on:click.native="itemSelected"
      />
      <li class="infoton-list-item empty-or-loading" id="observed" key="$$observed">
        <span v-if="isLoading">[ Loading more Infotons... ]</span>&nbsp;
      </li>
...
<script>
  import InfotonListItem from '@/components/Infoton/InfotonListItem.vue'
  ...
  export default {
    data() {
      return {
        ...
        offset: 0,
        showMenu: false,
        streamFormats: AppUtils.streamFormats,
    ...
    components: {
      'infoton-list-item': InfotonListItem,
      Msg
    },
    computed: {
      method() {
        return this.methodSelectors[this.methodSelector].method
      }
    },
    methods: {
      ...
      search(path, qp) {
        ...
        mfetch(url)
          ...
          .then(searchResponse => {
            ...
            this.paths = [
              ...this.paths,
              ...searchResponse.results.infotons.map(i => i.system.path)
            ]
      ...
      createConsumer(path, qp) {
        ...
        if (qp) params.append('qp', qp)
        if (this.indexTime && path != '/')
          params.append('index-time', this.indexTime)
        let url = `${path}?${params.toString()}`
        this.curAjax = url
      ...
      consume() {
        ...
        fetchFunc(url)
          ...
          .then(paths => {
            ...
            this.isLoading = false
            if (this.method == 'horizon')
              this.timeout = setTimeout(this.consume, 5000)
          })
      },
      ...
      toggleMethod() {
        this.methodSelector =
          (this.methodSelector + 1) % this.methodSelectors.length
      ...
      reset(path) {
        ...
        ;(this.purging = false), (this.purgeProgress = '')
      ...
      more() {
        if (this.method == 'consume') this.consume()
      ...
      streamUuids() {
        ...
        fetch(this.$route.path + '?op=stream&recursive&format=tsv').then(chunk =>
      ...
      recursivePurge(path) {
        if (
          !confirm(
            'DATA LOSS WARNING! THIS WILL PURGE ALL VERSIONS OF ALL INFOTONS UNDER THIS PATH, RECURSIVELY. AND CANNOT BE UNDONE. Are you sure you want to continue?'
          )
        )
          return
        this.purging = true
        ...
        let purgedSoFar = 0
        fetch(path + '?op=stream&recursive')
          .then(r => r.text())
          .then(t =>
            t.split`\n`.forEach(p => {
      ...
      toggleTypes() {
        ...
        mfetch(
          this.$route.path +
            '?op=aggregate&recursive&ap=type:term,field::type.rdf,size:1024&format=json'
        )
          .then(resp => resp.clone().json())
          .then(aggrResp => {
            this.typesAreLoading = false
            if (aggrResp.AggregationResponse)
              this.types = aggrResp.AggregationResponse[0].buckets.map(bckt => ({
                uri: bckt.key,
                count: bckt.objects
              }))
          })
    ...
    mounted() {
      this.reset(this.$route.fullPath)
    },
    watch: {
      '$route.fullPath': function(path) {
        this.stay ? (this.stay = false) : this.reset(path)
      }
    },
    updated() {
      this.$nextTick(function() {
        if (!this.paths.length || this.observer) return
