window.AppUtils = {
  ...
  hex(str) {
    return str.split``.map(c => c.charCodeAt().toString(16)).join``
  },
  formats: [
...
class ChunkedResponseHandler {
  ...
  readChunk(reader) {
    reader.read().then(buffer => {
      let decodedChunk = this.decoder.decode(buffer.value || new Uint8Array(), {
        stream: !buffer.done
      })
...
window.mfetch = AppUtils.memoizie(
  fetch,
  resp => ![200, 204].includes(resp.status)
)
...
window.Settings = new Proxy(
  {},
  {
    get: (_, k) => localStorage.getItem(k),
    set: (_, k, v) => {
...
Array.prototype.sortBy = function(predicate) {
  return this.sort((el1, el2) => predicate(el1) - predicate(el2))
} // assuming numerial comparison (not lexical!)
