...
import cmwell.analytics.data.InfotonWithParentFlag
import cmwell.analytics.util.CmwellConnector
import org.apache.log4j.LogManager
import org.rogach.scallop.{ScallopConf, ScallopOption}


object DumpInfotonWithParentFlag {

  def main(args: Array[String]): Unit = {

    val logger = LogManager.getLogger(DumpInfotonWithParentFlag.getClass)
    ...
    try {
      ...
      CmwellConnector(
        cmwellUrl = Opts.url(),
        appName = "Dump infoton table - parent flag",
        sparkShell = Opts.shell()
      ).withSparkSessionDo { spark =>

        val ds = InfotonWithParentFlag()(spark)
